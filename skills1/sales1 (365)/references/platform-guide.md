...
mParticle (now "mParticle by Rokt" — merged with Rokt in a ~$300M deal announced January 2025) is an enterprise hybrid Customer Data Platform that combines real-time event streaming with warehouse-native activation. Originally built as a mobile-first CDP, it has expanded to cover web, server-side, and OTT platforms. Target audience: multi-channel brands in media/entertainment, QSR, retail, travel, and financial services needing real-time identity resolution and audience activation at scale. Typically $40K-$375K+/year, enterprise-only, no self-serve tier.
...
- Supports 32 identity types (email, phone, IDFA, GAID, customer ID, social IDs, device stamps, other1-10 custom)
...
| Aspect | Details |
|---|---|
| **Model** | Value-based pricing (VBP) — "mParticle Credits" consumed by event volume (Connect / Preserve / Personalize event tiers, metered per ~1M event counts), profile/feature usage, storage, real-time audiences & calculated attributes (5 included free), rule invocations |
| **Entry point** | Contact sales (no self-serve) |
| **Typical range** | $40K-$375K+/year |
| **Average** | ~$156K/year |
| **Volume discounts** | More credits pre-purchased upfront = lower unit rates (custom rate card) |
| **Available on AWS Marketplace** | Yes |
| **Events API rate limit** | 250 events/sec (Default Service Limits page frames as "270 batches/sec" default; contact support for higher) |
| **Events API request size** | 256kb total request size; each batch ≤128KB |
| **Profile API rate limit** | 15 RPS default (testing); higher rates provisioned in bands of 50 QPS (premium) |
| **Data Planning / Platform API rate limit** | 3,000 req/min (account), 6,000 req/min (org) |
| **DSR API rate limit** | 1,000 req/min per workspace, 80,000 new requests/day per workspace |
| **Bulk events batch limit** | 100 batches per request, 128KB max per batch |
| **OAuth token expiry** | 8 hours (not revocable) |
| **Historical data** | Timestamps must be >72 hours old; data older than 30 days uses `/v2/bulkevents/historical` |
...
- Rokt merger (~$300M, announced January 2025; mParticle now "mParticle by Rokt") may affect product roadmap — monitor for changes
