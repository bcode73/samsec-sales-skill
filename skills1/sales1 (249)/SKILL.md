...
- **Standard IPUs expire MONTHLY — Flex IPU is the annual model.** Per Informatica's official pricing docs, the standard (Metering 1.0) IPU model uses monthly billing and unused IPUs expire at the end of each contract month. The Flex IPU model uses annual billing — unused IPUs carry across services and expire only at the end of the contract anniversary year. If usage is seasonal or spiky, negotiate Flex IPU so you don't forfeit unused units every month.
