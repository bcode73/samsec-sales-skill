...
1. **Setting up a Google Maps lead scraping job**:
   ...
   7. Credits are consumed per result (cost varies by crawler + any enrichment like email/validation) — monitor usage in the dashboard or via `GET /v1/runs/{id}/credits`
...
1. **Free plan is very limited — 100 credits/mo, 30 rows/export, no scheduling, no Premium scrapers, 7-day data retention.** The free tier is suitable only for testing. Any serious data collection requires a paid plan (named tiers Starter/Basic/Growth; check lobstr.io/pricing for current prices — recent listings show ~$10–$50/mo USD). Credits do not roll over, and the 30-row export cap means you cannot download full result sets on the free plan.
2. **Credit cost is variable per crawler and enrichment function — it is NOT a flat 1-credit-per-result.** Base scraping is billed separately from each optional enrichment function (email collection, validation, etc.), and credits are tracked/reported per function. Credits do not roll over and expire at the end of each billing cycle. Estimate cost before large runs and use `GET /v1/runs/{id}/credits` to see actual consumption.
