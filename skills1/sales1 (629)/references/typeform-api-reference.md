...
- `type` (string) — form type: `quiz`, `classification`, `score`, `branching`, `classification_branching`, `score_branching`
...
- **Supports payload signing**: set a `secret` on the webhook (via the Webhooks API `secret` field). Typeform then signs the raw request body with HMAC SHA-256 and sends the result in the `Typeform-Signature` header, formatted as `sha256={base64-encoded hash}`. Verify by recomputing HMAC SHA-256 over the raw body with your secret, base64-encoding, prefixing `sha256=`, and comparing. If you don't set `secret`, the `Typeform-Signature` header is NOT sent.
