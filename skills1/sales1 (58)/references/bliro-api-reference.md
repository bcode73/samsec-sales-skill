...
2. Construct signed payload: `{timestamp}.{JSON of callData ONLY}` — NOT the full request body. Per the official docs the signed string is `` `${timestamp}.${JSON.stringify(payload.callData)}` `` (only the inner `callData` object is serialized, not the top-level payload).
...
> **IMPORTANT (verified 2026-06-13):** Bliro signs `JSON.stringify(payload.callData)`, not the raw HTTP body. Re-serializing `callData` in Python must match the bytes Bliro signed. Because Python's `json.dumps` and JS's `JSON.stringify` can differ in whitespace/key order/unicode escaping, prefer extracting the `callData` substring from the raw body when possible, or normalize separators (`json.dumps(call_data, separators=(",", ":"), ensure_ascii=False)`) and test against a real "Test Webhook" delivery before trusting the comparison.

Official Node example:
```javascript
const verifySignature = (payload, signature, secret) => {
  const timestamp = payload.timestamp;
  const data = `${timestamp}.${JSON.stringify(payload.callData)}`;
  const hmac = crypto.createHmac('sha256', secret).update(data).digest('hex');
  return hmac === signature; // signature = the v1 value from Bliro-Signature
};
```
...
def verify_bliro_signature(parsed_payload: dict, signature_header: str, secret: str) -> bool:
    # parsed_payload is the decoded JSON body: {"callData": {...}, "timestamp": "..."}
    parts = dict(p.split("=", 1) for p in signature_header.replace(" ", "").split(","))
    timestamp = parts["t"]
    expected = parts["v1"]
    # Bliro signs the timestamp + "." + JSON.stringify(callData) — callData ONLY
    call_data_json = json.dumps(parsed_payload["callData"], separators=(",", ":"), ensure_ascii=False)
    signed = f"{timestamp}.{call_data_json}"
