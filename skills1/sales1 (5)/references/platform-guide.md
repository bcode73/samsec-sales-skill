...
- **Base URL**: `https://{account}.api-us1.com/api/3/` (unique per account — replace `{account}` with your account name; ActiveCampaign warns `api-us1.com` is not guaranteed for every account, so read the actual Base URL from the Developer tab)
- **Authentication**:
  - API key: `Api-Token` HTTP header with your API key — the **only** auth method for the v3 REST API
  - OAuth 2.0 is NOT used to call the v3 API — it appears only in App Studio, where a CX App's config (`auth.type = "oauth2"`) defines how ActiveCampaign authenticates to *your* external service
...
- **Webhooks**: 25+ event types with at-least-once delivery (may fire more than once — implement idempotency)
  ...
  - Deal events: deal_add, deal_update, deal_stage_add, deal_pipeline_add, deal_task_add, deal_task_complete, deal_tasktype_add, deal_note_add
...
- **Key endpoints**:
  ...
  - `POST /api/3/contact/sync` — create or update (upsert) a contact by email
