...
The `{account}` portion is unique per account — find yours in Settings > Developer. All endpoints below are relative to this base URL. Note: ActiveCampaign's docs explicitly warn that `api-us1.com` is **not guaranteed** to be the correct Base URL for all current/future accounts — always read the actual Base URL from the account's Developer tab rather than assuming the `api-us1` region.
...
The official authentication docs document **only** the `Api-Token` header — "All requests to the API are authenticated by providing your API key. The API key should be provided as an HTTP header named `Api-Token`." There is **no OAuth flow for the v3 REST API itself**.

**OAuth 2.0 (App Studio / CX Apps only)** — OAuth 2.0 appears only in **App Studio**, where a CX App's config declares how ActiveCampaign authenticates to *your* external service (`auth.type = "oauth2"` with `authorization_base`, `client_id`, scopes). It is not a way to authenticate calls *to* the ActiveCampaign v3 API — those still use `Api-Token`. (Re-verified 2026-06-13.)
...
- Exceeding the limit returns HTTP `429 Too Many Requests`. The response includes these headers (per current official docs):
  - `Retry-After` — seconds to wait before retrying (e.g. `Retry-After: 30`)
  - `RateLimit-Limit` — total requests allowed in the window (e.g. `5`)
  - `RateLimit-Remaining` — requests remaining in the current window (e.g. `3`)
- Custom rate limits are available for high-volume accounts (contact ActiveCampaign support)
