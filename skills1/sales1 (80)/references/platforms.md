...
- [In Sendr](#in-sendr)
- [In Waalaxy](#in-waalaxy)
- [In Linked Helper](#in-linked-helper)
...
- **Positioning**: LinkedIn-first **multichannel** outbound (LinkedIn + email + X/Twitter, phone steps) — "safely automating LinkedIn since 2017," cloud-based with dedicated proxies + data-driven safety limits. Built for GTM Engineers / Sales Ops / SDRs.
- **Model**: you **enroll leads into an audience**; a **campaign** runs the **sequence** over it (conditions, branches, A/B). Outreach executes through **identities** (connected LinkedIn/email accounts), each with per-day safety limits — spread volume across identities.
- **API/automation**: REST API (`https://apiv2.lagrowthmachine.com/flow`, **`apikey` query param**) — `POST /flow/leads` to enroll, plus Get Campaigns / List Audiences / List Identities and **Create/List Inbox Webhooks** (real-time lead-reply events; e.g. create a CRM contact on reply). Native HubSpot/Clay/Make/Slack + Zapier/n8n + an emerging **MCP server**. **API + webhooks are gated to the Ultimate plan.**
- **Extras**: waterfall enrichment, lookalike, **LinkedIn intent signals** (auto-import ICP), shared multichannel inbox, AI Magic Messages + AI Voice.
- **Limitations**: per-identity pricing (~€50 Basic → €150 Ultimate) scales with accounts; no API "send message" (enroll + sequence only); email channel still needs its own warmup/deliverability.
- **Best for**: LinkedIn-led multichannel motions where cloud LinkedIn safety + a unified inbox matter (vs email-first lemlist). **Platform skill**: `/sales-lagrowthmachine`.

## In Instantly

- **Sequences**: Multi-step cold-email campaigns with **subsequences** (branch follow-ups by reply outcome). Add leads + personalization via the API (`POST /leads` with `custom_variables` that resolve sequence merge tags)
- **Scale model**: Built for **high-volume** sending across unlimited inboxes; daily caps are per-account, so spread a campaign over many low-volume inboxes rather than one. SISR rotates IPs/servers
- **Unibox**: Unified reply inbox across all accounts (Hyper-Growth+) for working replies in one place; reply/status events (interested, meeting booked, not interested) are available via webhooks
- **Best for**: Bulk cold email at scale where deliverability infrastructure matters more than multichannel. For LinkedIn+calls multichannel cadences, pair with another tool. Lead status is API-readable for CRM sync
- **Limitation**: It's a sending engine, not a copy assistant — sequence strategy/copy is the user's job (this skill)
- **Platform skill**: `/sales-instantly`

## In Waalaxy

- **Positioning**: Budget **LinkedIn-first** (LinkedIn + email) outreach that runs as a **Chrome extension** on your real LinkedIn session — solopreneur/founder/recruiter fit, Free tier, 99+ ready-made sequence templates.
- **Model**: import prospects into a **prospect list**, then optionally enroll them into a **campaign** (sequence: connection request → message → follow-ups with human-like timing). There's **no "send message" API** — you import + the campaign executes through the connected LinkedIn account.
- **Channels**: LinkedIn on all paid plans; **cold email / multichannel sequences require the Business plan**. **Email Finder** (credit-metered) on Pro+; enrichment included.
- **API/automation**: REST API (`https://developers.waalaxy.com`, **`Authorization: Bearer`** key from CRM Sync settings) — `GET /prospectLists/getProspectLists`, `GET /campaigns/getAll` (paused/running only), `POST /prospects/addProspectFromIntegration` (each prospect needs a LinkedIn `url`, plus `prospectListId`, optional `campaignId`, **required `origin`**). **API gated to Advanced/Business.** Native HubSpot/Pipedrive + Make/Zapier/n8n; **no public reply webhook** (use native sync or poll).
- **Safety/limits**: monthly **invitation caps** (Free 80 / Pro 300 / Advanced & Business 800) + a daily ramp; because it's an extension on the real session, exceeding volume is the top cause of **restricted LinkedIn accounts**. Warm new accounts slowly.
- **Limitations**: extension-based execution needs the browser running; cold email is Business-only; reply management (LinkedIn Inbox) is a paid add-on; higher account-risk than cloud tools like LGM.
- **Best for**: a cheap, simple LinkedIn-led motion for a solo founder/SMB (vs email-first lemlist or cloud-safe LGM). **Platform skill**: `/sales-waalaxy`.

## In Linked Helper

- **Positioning**: Long-running, **cheap power-user** LinkedIn automation as a **standalone desktop app** (its own bundled browser — *not* a Chrome extension, *not* cloud). Auto connect/message/InMail/endorse/scrape inside drip "funnels," with a built-in mini-CRM and email finder. 9 years in market, 300k+ users.
- **Model**: import prospects from a LinkedIn/Sales Navigator search URL or CSV (there's **no inbound API**), then run a **campaign funnel** (connect → wait → message → follow-ups, spintax + smart reply detection). Execution happens **locally on your machine** through the connected account.
- **Channels**: **LinkedIn only** — no native email *sending* channel. The email finder discovers addresses (metered by **data credits**) that you export to a separate sender.
- **API/automation**: **No public REST API.** Programmatic output is **outbound webhooks** (campaign action steps: *Send person / Send replied / Send organization to webhook*) + native CRM connectors (HubSpot/Salesforce/Pipedrive/Zoho/ActiveCampaign/Close/Capsule/HighLevel/Streak/Instantly) + Zapier/Make/Pabbly. Webhooks are **unsigned, no delivery log, no retry** — dedupe on `member_id`/`profile_url`. Since v1.11.1 webhooks can include last message/last reply/messaging history.
- **Pricing/limits**: Trial 14 days; **Standard $15/mo** (annual $8.25) = 20 advanced actions/day + 20 webhook profiles/day + 620 data credits; **Pro $45/mo** (annual $24.75) = unlimited actions + unlimited webhooks + 3,100 credits. **One license = one LinkedIn account.**
- **Limitations**: **desktop-only** — campaigns stop when the computer sleeps/closes/drops offline (workaround: run on an always-on VPS); **account-restriction risk** is the #1 complaint (~23% within 90 days for aggressive configs per third-party tests); dated/complex UI; LinkedIn-only.
- **Best for**: a budget, hands-on solo seller/recruiter who wants deep control and the lowest price and accepts running a desktop app (vs cloud-safe LGM/HeyReach or extension-based Waalaxy). **Platform skill**: `/sales-linkedhelper`.
