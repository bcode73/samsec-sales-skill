...
For platform-specific cadence implementation (Salesloft, Mailshake, Lemlist, Smartlead, Mixmax, Reply.io, Woodpecker, Hunter.io, Seamless.AI, Closum, Minelead, GetProspect, ZoomInfo, Snov.io, 6sense, Clay, BrandJet, AiSDR, Nooks, Orum, Amplemarket, Sendr, LeadIQ, Waalaxy, Linked Helper), see references/platforms.md.
...
- `/sales-lagrowthmachine` — La Growth Machine platform help (LinkedIn-first multichannel outbound — LinkedIn/email/X sequences, per-identity safety limits, enrichment + intent signals; REST API `apiv2.lagrowthmachine.com/flow` (apikey) enroll-into-audience + inbox webhooks + MCP; API/webhooks on Ultimate)
- `/sales-instantly` — Instantly platform help (high-volume cold-email campaigns/subsequences across unlimited inboxes, Unibox, v2 API + webhooks)
...
- `/sales-waalaxy` — Waalaxy platform help (budget LinkedIn-first outreach via Chrome extension — ready-made sequences, Email Finder, import/enroll prospects via REST API on Advanced/Business)
- `/sales-linkedhelper` — Linked Helper platform help (cheap standalone *desktop* LinkedIn automation app — auto connect/message/InMail funnels, email finder, built-in CRM; no inbound API, outbound webhooks + native CRM sync; desktop-only so campaigns stop when the computer's off; account-restriction risk)
