...
- Community MCP server (email-focused, 7 tools): https://github.com/jmhron/FastMailMCP
- Community MCP server (email + contacts + calendar, ~38 tools, ships a Claude Desktop DXT desktop extension): https://github.com/MadLlama25/fastmail-mcp — calendar tools fall back to CalDAV when JMAP calendar is unavailable. Auth via `FASTMAIL_API_TOKEN` env var (`fmu1-` bearer token).
