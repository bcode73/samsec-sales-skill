...
Two community-built MCP servers exist (no official Fastmail MCP server as of June 2026):

- **`jmhron/FastMailMCP`** — email-focused, 7 tools (below). Python, configured via `claude_desktop_config.json`.
- **`MadLlama25/fastmail-mcp`** — broader coverage: email + contacts + calendar (~38 tools, including send/reply/draft/search/move, bulk operations, contact list/search, calendar list/create events, and identity tools). Calendar tools fall back to CalDAV when JMAP calendar is unavailable. Ships a Claude Desktop **DXT** desktop extension (`npx @anthropic-ai/dxt pack`). Auth via `FASTMAIL_API_TOKEN` env var.

### `jmhron/FastMailMCP` tools
