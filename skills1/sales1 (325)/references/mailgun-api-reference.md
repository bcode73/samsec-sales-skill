...
> **Note**: Core endpoints, auth, webhooks, and pricing re-verified against live official docs on 2026-06-13 (see references/learnings.md). Verify request/response field-level details against https://documentation.mailgun.com/docs/mailgun/api-reference/
...
#### 8. Webhooks API — `/v3/domains/{domain}/webhooks` (v3) and `/v4/domains/{domain}/webhooks` (v4)

Configure webhook endpoints for real-time event notifications. Both a v3 and a newer v4 surface exist.
...
**v4 domain webhooks** (newer — support multiple URLs per event type, up to 3 endpoints):

| Method | Endpoint | Description |
|---|---|---|
| POST | `/v4/domains/{domain}/webhooks` | Create a webhook (multiple URLs supported) |
| PUT | `/v4/domains/{domain}/webhooks` | Update domain webhooks |
| DELETE | `/v4/domains/{domain}/webhooks` | Delete domain webhooks |

**Account-level webhooks** — `/v1/webhooks`:

| Method | Endpoint | Description |
|---|---|---|
| GET | `/v1/webhooks` | List account webhooks |
| POST | `/v1/webhooks` | Create an account webhook |
| GET | `/v1/webhooks/{webhook_id}` | Get an account webhook |
| PUT | `/v1/webhooks/{webhook_id}` | Update an account webhook |
| DELETE | `/v1/webhooks/{webhook_id}` | Delete an account webhook |
...
| `engagement` | object | Engagement data for the address (e.g., behavior, engaging, is_bot) when available |
