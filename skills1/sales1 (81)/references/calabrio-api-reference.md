...
2. Provide an application name and generate a new token (this is the only time the token is shown — it cannot be viewed or copied again later)
3. In the API docs, click "Authorize" and paste the token
4. Click Authorize to establish session

**Machine-to-machine (M2M) auth**: For server-side integrations, set the username to `__token__` and use the generated personal access token as the password (HTTP Basic). Access for any personal access token can be revoked at any time from User Settings.
...
- Remove Intraday Absence

Three different integration approaches are available, each providing varying functionality for specific HR system needs: **Standard SFTP Balance Import / Absence Export**, plus **API Option 1**, **API Option 2**, and **API Option 3** (Swagger-based).
