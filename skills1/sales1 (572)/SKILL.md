...
1. **10% fee compounds with Stripe** — total take is ~13-16% of gross revenue (10% Substack + 2.9% + $0.30 Stripe + 0.7% Stripe Billing, raised from 0.5% in July 2024). At $10K/yr revenue, that's ~$1,300-$1,600 in fees.
2. **No official API for publishers** — you cannot programmatically manage posts, subscribers, or analytics. The "Developer API" only queries public profiles by LinkedIn handle via `GET substack.com/profile/search/linkedin/{handle}` (token generated in Settings > Developer API).
