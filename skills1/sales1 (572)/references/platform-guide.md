...
| Stripe Billing (recurring) | 0.7% per recurring charge (raised from 0.5% in July 2024; legacy 0.5% pricing ended June 30, 2025) |
...
### Official Developer API (public-profile lookup only)

Substack's only official API queries public profile data by LinkedIn handle — it cannot manage posts, subscribers, payments, or analytics. After approval (7-10 business days via a Terms-of-Use form), generate a token under **Settings > Developer API > Create new token** and call:

```
GET https://substack.com/profile/search/linkedin/{linkedin-handle}
```

Returns public fields like `identityHandle`, `profileUrl`, `leaderboardStatus`, `bestsellerTier`, `roughNumFreeSubscribers`, and `followerCount`. Useful for discovery/lead-research on creators, not for automating your own publication. See `substack-api-reference.md` for full details.
