---
name: sales-bazzly
description: "Bazzly platform help — AI-powered Reddit lead generation with intent scoring, contextual DM/reply drafting, high-karma autopilot accounts, a Chrome extension for posting from your own account, Reply Boost (smart upvotes), plus a REST API (api.bazzly.ai) and MCP server. Use when Bazzly AI-drafted messages sound generic and need heavy rewriting before posting, false positive leads are wasting your credits, you want to reduce Reddit ban risk on DMs, you need to optimize the 100 monthly credits across comments/upvotes/drafts, the Chrome extension isn't detecting leads, you're wiring opportunities into a CRM via the API or the Bazzly MCP server in Claude/Cursor, or you're comparing Bazzly vs KeyMentions vs RedShip vs ReplyGuy. Do NOT use for social listening strategy across tools or choosing between Reddit monitoring platforms (use /sales-social-listening)."
...
1. **What do you need help with?**
   ...
   - D) Credit optimization (getting more from the 100 monthly credits + top-ups)
   - E) Account safety and ban prevention
   - F) Chrome extension issues
   - G) API / MCP integration (api.bazzly.ai, MCP server in Claude/Cursor)
   - H) Something else — describe it

2. **What's your setup?**
   - A) Paid "All in One" plan ($99/mo — 100 credits, unlimited leads, high-karma autopilot, API + MCP)
   - B) Free start / evaluating
   - C) Driving Bazzly via the REST API or MCP server
...
**Read `references/platform-guide.md`** for the full platform reference — capabilities, pricing, credit system, Chrome extension setup, message optimization, and account safety guidance. For the REST API and MCP server (endpoints, auth, Claude/Cursor config), read `references/bazzly-api-reference.md`.
...
- **Single plan now ($99/mo, 100 credits).** The old Starter/Growth/Elite tiers (with 4/6/unlimited subreddits and 20/200/unlimited credits) are gone — there is one "All in One" plan with 100 monthly credits plus top-ups. Budget around the 100 credits; comments cost 10 each.
...
**User says**: "I'm burning through my 100 monthly credits in a week. How do I optimize?"
**Skill does**:
...
### Example 3: Feeding Reddit lead data into a CRM
**User says**: "I want to feed Reddit lead data into my CRM. Can Bazzly do this?"
**Skill does**:
1. Confirms Bazzly now has a REST API (`https://api.bazzly.ai`, Bearer `bzly_live_` keys) and an MCP server (`https://api.bazzly.ai/mcp`) — so yes, you can pull opportunities programmatically
2. Notes the egress is poll-based: `GET /public/v1/opportunities` (or the `list_opportunities` MCP tool), since Bazzly has no webhooks/Zapier
3. Suggests RedShip or Syften (REST + webhooks) if push-based delivery is required, and routes to `/sales-social-listening` for a full comparison
**Result**: Clear path to wire Bazzly into a CRM via API/MCP, with webhook alternatives noted
