...
2. **Which plan are you on?**
   - A) Monitor ($29/mo — 5 keywords, 30-day history)
   - B) Radar ($59/mo — 20 keywords, 90-day history)
   - C) Not sure / haven't signed up yet
...
4. **API integration** — the API supports reads and writes: pull processed mentions (filter by keyword, source, sentiment, content_type, contact_found, date range), list/create/update keywords, save relevance feedback (`PATCH /mentions/{id}`), pull a grouped digest, and generate reply drafts
5. **Plan selection** — Monitor ($29/mo, 5 keywords, 30-day history) covers most solo founders; Radar ($59/mo, 20 keywords, 90-day history) for teams monitoring multiple brands/competitors
