...
2. Shows: `GET /v1/workspace/meetings.list` with date filters → paginate via `nextCursor` → for each meeting `GET /v1/workspace/meetings.get` for transcript (Markdown format); base URL `https://beta-api.meetjamie.ai`
