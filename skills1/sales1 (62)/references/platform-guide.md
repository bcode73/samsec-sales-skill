...
> *Boomi does not publish subscription list prices — almost every tier routes to "Talk to sales". The official **edition names** below are confirmed from boomi.com/pricing and the Integration Editions docs; the **dollar estimates** are approximate from third-party sources (Vendr, Integrate.io, G2). Pay-As-You-Go ($99/mo + usage) is the only price Boomi lists officially. Contact Boomi sales for exact quotes.*

**Official Integration editions** (in ascending order): **Base** (a.k.a. Personal/trial), **Professional**, **Professional Plus**, **Enterprise**, **Enterprise Plus** — plus **Pay-As-You-Go**. API Management and Data Hub have parallel edition tiers; Data Integration uses Base / Professional / Professional Plus / Enterprise.

| Edition | Approx. monthly cost (3rd-party est.) | Standard connectors (Prod/Test)* | Notes |
|---|---|---|---|
| Pay-As-You-Go | $99/mo + $0.05/message (official) | Unlimited connections | Full platform (Integration, Flow, API Mgmt, Data Hub, EDI), unlimited users/workflows, free Standard Support; **excludes** Data Catalog & Prep (DCP). Credit card required to upgrade from trial. |
| Base | — | 2 / NA | Entry tier; lacks Advanced Workflow, Services Enablement, Parallel Processing, Advanced Security, Runtime clusters |
| Professional | ~$550/mo | 5 / 5 | Cloud Atoms, standard connectors, standard support |
| Professional Plus | higher | 7 / 7 | Adds enterprise connectors (extra cost) and advanced features |
| Enterprise | ~$1,200/mo | 10 / 20 | Multi-environment, API management, Flow; enterprise connectors 1/2 |
| Enterprise Plus | Custom (higher) | 15 / 30 | Advanced MDM, EDI/B2B, dedicated CSM; enterprise connectors 2/4 |

\* Connector counts per the official Integration Editions table. **Enterprise connectors** (e.g. SAP, certain ERP) are gated to Professional Plus and above and may carry additional cost.

**Per-connection costs**: Each additional endpoint/connection beyond your tier's included count is billed separately (third-party reports cite roughly ~$300/mo, with some accounts citing $9.4K–$18.9K/yr per connector). Even basic connections (local disk, HTTP) count. Verify exact metering with sales.

**Annual escalation**: Contracts typically include 3-7% annual price increases at renewal. Negotiate a cap upfront.

**Free trial**: 30 days, $0, full Enterprise Platform access. A credit card is required only when converting the trial to a Pay-As-You-Go account.
