...
- **Base URL (US)**: `https://api.surveymonkey.com/v3/`
- **EU Base URL**: `https://api.eu.surveymonkey.com/v3/`
- **Canada Base URL**: `https://api.surveymonkey.ca/v3/`
...
|---|---|---|---|
| Draft / Private | 120 | 500 (starting at) | 12:00 AM GMT |
| Public (published in App Directory) | — | up to 500,000 | 12:00 AM GMT |

Public apps still in draft are subject to the same limits as private apps. Enforcement begins after 3 violations exceeding 150% within 30 days.
...
| `collectors_read` | Any |
| `collectors_write` | Paid (except weblink collectors) |
...
**Object types** (`object_type` / `filter_type`): `survey`, `collector`

**Setup**: `subscription_url` must accept a HEAD request and return a 200. SurveyMonkey verifies before registering. The subscription URL must be unique to each webhook — it cannot be reused across multiple webhooks in the same account.

**Callback payload** (POST body when an event fires):
```json
{
  "name": "...",
  "filter_type": "survey",
...
```
The callback carries only IDs — fetch the full response via `/surveys/{id}/responses/{id}/details` after receiving it.

**Webhook signature verification (HMAC)**: SurveyMonkey signs each callback. Two headers are sent:
- `Sm-Apikey` — your app's Client ID / API key
- `Sm-Signature` — base64-encoded **HMAC-SHA1** digest of the raw request body, keyed with the string `api_key&api_secret` (client ID and client secret joined by `&`)

Verify by recomputing the HMAC-SHA1 of the raw payload with that key, base64-encoding it, and constant-time comparing against `Sm-Signature`. Reject on mismatch.
