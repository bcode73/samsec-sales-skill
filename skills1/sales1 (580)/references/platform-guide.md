...
|---|---|---|---|---|
| **Basic (Free)** | $0 | 25/survey | 10/survey | Basic question types, SurveyMonkey branding, no export |
...
### Webhooks
- Subscribe to events: `response_completed`, `response_disqualified`, `response_updated`, `response_created`, `response_deleted`, `response_overquota`, `collector_created`, `collector_updated`, `collector_deleted`, `survey_created`, `survey_updated`, `survey_deleted`
- Webhook callback POSTs a JSON body with `event_type`, `event_id`, `object_type`, `object_id`, `event_datetime`, and a `resources` object of associated IDs (survey, collector, recipient, respondent, user). The payload carries only IDs — fetch full data via the API afterward.
- Each callback is signed: `Sm-Apikey` header (Client ID) plus `Sm-Signature` header — a base64 HMAC-SHA1 of the raw body keyed with `api_key&api_secret`. Verify before trusting.
- Subscription URL must accept a HEAD request and return 200, and must be unique per webhook (no reuse across webhooks in the account)
...
- Canada datacenter: `https://api.surveymonkey.ca/v3/`
- Rate limits: 120/min and 500/day (starting) for private/draft apps; published public apps get up to 500,000/day
