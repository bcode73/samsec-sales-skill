...
2. Explains the `POST /campaigns/{campaign_id}/leads` endpoint — email required, `custom_fields` for merge variables, `lead_list` array (max 400 leads per request)
3. Shows authentication method (api_key query parameter)
4. Recommends batching (≤400/request) for large imports and monitoring rate limits (60 req/min default)
5. Suggests setting up campaign webhooks (`POST /campaigns/{campaign_id}/webhooks`) for `EMAIL_REPLY` and `LEAD_CATEGORY_UPDATED` (filtered on the Interested category) to sync back to CRM
