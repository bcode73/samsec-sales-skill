...
- **API pricing is separate from the meeting app.** Palabra's platform now uses a unified credit subscription (Starter $60/mo → Pro $200/mo → Team $1,000/mo → Business custom, for the Meetings product); API access is credit-billed within those plans, not a standalone tier. Don't assume your $33/mo Talo plan includes Palabra API access. (Verified 2026-06-13.)
...
4. Notes Palabra API access is credit-billed within its platform plans, which start at Starter $60/mo (150 credits); API speech-to-speech burns ~30 credits/hr
...
**Solution**: Use Palabra's Glossary API to create custom term lists. Upload a CSV with source→target term pairs. Glossary types: `translation` (override translations), `asr` (improve recognition), `asr_hot` (boost recognition for rare terms). This requires a Palabra platform plan with API credits (Starter $60/mo and up).
