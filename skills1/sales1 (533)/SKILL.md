...
1. **What area of Skrapp.io do you need help with?**
   ...
   - G) Company Search — find all professionals at a company by domain (40M+ company profiles)
...
1. **Finding a specific person's email**:
   1. Use the Email Finder: `GET /api/v2/find?firstName=Jane&lastName=Doe&domain=example.com` (base `https://api.skrapp.io`) with your `X-Access-Key` header
   2. Skrapp returns the most likely professional email plus a `quality` object (status/status_message) — 92% success rate
   3. If the email returns a `catch-all` status, the domain accepts all addresses so deliverability is uncertain — verify separately or proceed with caution
   4. For batch lookups, use the bulk Email Finder (`POST /api/v2/find_bulk`, max 100 entries per request)
...
1. **API access requires the Enterprise plan ($262/mo annual, $349/mo monthly) — Professional plan does not include API access.** "API Integration" is listed only on the Enterprise tier of the pricing page; the API docs phrase the gate generically as "paid accounts," but the pricing page is the authoritative source. If you need programmatic access to Skrapp's email finder or verifier endpoints, you must be on Enterprise. Professional plan users are limited to the web UI, Chrome extension, and CRM integrations. Budget accordingly if you are planning to build an integration or automated workflow that depends on the API.
...
5. **Multi-page LinkedIn enrichment is only available on Professional plan and above.** Free plan users can extract emails from individual LinkedIn profiles but cannot bulk-extract from LinkedIn search result pages. If you need to enrich entire search result pages or process multiple pages of Sales Navigator results, you must upgrade to Professional ($29/mo annual, $39/mo monthly) or Enterprise ($262/mo annual, $349/mo monthly).
...
**Skill does**:
1. Uses Email Finder for each (base `https://api.skrapp.io`): `GET /api/v2/find?fullName=VP+Sales&domain=acmecorp.com` with `X-Access-Key` header — repeats for each domain
2. If the user has specific names, refines the search: `GET /api/v2/find?firstName=Jane&lastName=Doe&domain=acmecorp.com` for higher accuracy (92% success rate with full name)
3. Alternatively, uses Company Search on each domain to find all contacts and filter by title containing "VP" or "Sales"
4. Verifies each found email: `GET /v3/verify?email=<found-email>` — returns `email_status` (valid/catch-all/invalid)
5. Notes that catch-all domains will return a positive result even for non-existent mailboxes — flags these for extra caution
...
6. After extraction, runs Email Verifier on the list to validate deliverability — flags catch-all and invalid addresses
