...
### Advanced AI (generative + agentic)
CallMiner positions an **Advanced AI** layer across the Eureka platform, included with all pricing packages at no extra cost. It adds generative-AI and agentic-AI capabilities such as automatic category creation, contact (interaction) summarization, and natural-language discovery/search over interactions. Also surfaced: a **Solution Catalogue** of pre-built, customizable analytics. (Confirmed on callminer.com/products/eureka, 2026-06-13.)
...
Authentication: OAuth 2.0 (client-credentials grant). Exchange `client_id`/`client_secret` (emailed by CallMiner) at the Identity Provider Service token endpoint (`POST /connect/token`, `grant_type=client_credentials`, scope `https://callminer.net/auth/platform-ingestion`) for a bearer token. (Not a static API key.)
Data API is versioned under `/api/v2/` — e.g. `GET /api/v2/audiosources` lists configured audio sources.
