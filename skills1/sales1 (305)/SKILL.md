...
- **Channels available**: Email, LinkedIn (connection requests, profile visits, messages), phone (in-app dialer / VoIP integrations), SMS (Multichannel plan), WhatsApp (paid add-on at $20/seat/month)
...
- **What it is**: Built-in B2B lead database with 650M+ contacts and smart filtering
...
- **Plan requirement**: Multichannel plan ($109/user/month monthly, $87/user/month annual) required for LinkedIn steps. The Email plan ($39/user/month monthly, $31 annual) is email-only.
...
- **Plan requirement**: Multichannel plan required (built-in call dialer / VoIP integrations)
...
**Quick reference**: Base URL `https://api.lemlist.com/api`, **Basic auth only** (`:apiKey` base64-encoded — Bearer is explicitly NOT supported), rate limit 20 requests per 2 seconds (with `X-RateLimit-*` and `Retry-After` headers). Key capabilities: campaign CRUD, lead management, sequence steps, enrichment, Lemwarm control, People Database search, inbox/messaging, webhooks (80+ event types, shared-`secret` verification, no HMAC).
...
**Skill does**:
1. Confirms user is on the Multichannel plan (LinkedIn requires it)
