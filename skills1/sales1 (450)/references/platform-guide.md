...
| Tier | Price | Includes |
|---|---|---|
| **Free Account** | $0 | 3 active surveys, 500 total responses (lifetime), 8 question types, single user, 50+ templates |
| **Strategic Research (online)** | $420/mo ($5,040 billed annually) | 1,000 responses shared across all users, unlimited users/surveys/questions, 23 question types incl. video/audio |
| **Strategic Research (enterprise)** | Custom quote | 5,000+ responses, 23 question types incl. video/audio |
...
3. **For OAuth**: Account Settings → Qualtrics IDs → OAuth Client Manager → Create Client. Two grant types are supported — **Client Credentials** (backend automations, acts as the creating user) and **Authorization Code** (acts on behalf of a signed-in user). **Critical**: Client Secret is shown only once — copy it immediately.
