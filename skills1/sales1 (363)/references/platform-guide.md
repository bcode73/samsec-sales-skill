...
Pricing scales by subscriber count on Pro (monthly rates, verified against moosend.com/pricing 2026-06-13):
- 500 subs: ~$9/mo
- 1,000 subs: ~$16/mo
- 2,000 subs: ~$24/mo
- 5,000 subs: ~$48/mo
...
Billing discounts: pay 6 months upfront to save 15%, or annually to save 20% (so the entry tier is ~$7/mo on the annual plan). Moosend also sells one-off **email credits** that never expire (e.g., 350,000 credits for $350, up to 1,000,000 for $1,000) as an alternative to a subscription.

<!-- Re-verified against moosend.com/pricing 2026-06-13; ladder may shift, re-check before quoting -->
...
# Get campaigns page 1, 10 per page (Page/PageSize are PATH segments, not query params)
curl "https://api.moosend.com/v3/campaigns/1/10.json?apikey=YOUR_API_KEY"
...
# Get campaign summary stats after sending
curl "https://api.moosend.com/v3/campaigns/CAMPAIGN_ID/view_summary.json?apikey=YOUR_API_KEY"

# Or get activity by type (Sent, Opened, Clicked, Bounced, ...)
curl "https://api.moosend.com/v3/campaigns/CAMPAIGN_ID/stats/Opened.json?apikey=YOUR_API_KEY"
```
...
# Wait, then fetch summary stats (per-type detail is at /stats/{Type}.json)
time.sleep(60)
stats = requests.get(
    f"{BASE}/campaigns/{campaign_id}/view_summary.json",
    params={"apikey": API_KEY}
)
