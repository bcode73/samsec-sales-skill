...
1. **Finding a specific person's email by name and company**:
   1. Call `POST /v5.1/find-email/person` with `full_name` and `company_name` or `domain` in the request body, with `Authorization: <your-api-key>` header (no Bearer prefix). This endpoint also accepts `linkedin_url` as a fallback
...
2. **Finding decision-maker emails by role at a company**:
   1. Call `POST /v5.1/find-email/decision-maker` with `domain` (or `company_name`) and `decision_maker_category` as a string array (one or more of: ceo, engineering, finance, hr, it, logistics, marketing, operations, buyer, sales)
...
3. **Running a bulk email search for a large prospect list**:
   ...
   2. Submit via `POST /v5.1/bulk/json` (JSON rows) or `POST /v5.1/bulk/multipart` (CSV/Excel file upload); poll status with `GET /v5.1/bulk/{searchId}` and fetch via `GET /v5.1/bulk/{searchId}/download`
...
4. **Verifying an existing email list**:
   1. For individual emails: `POST /v5.1/verify-email` with `email` — returns valid, risky, or invalid status at 0.2 credits each
...
9. **Unused credits roll over with no cap while you stay subscribed (re-verified 2026-06-13).** The current pricing page states credits keep accumulating with no cap as long as the subscription is active; if you cancel, unused credits expire at the end of the current billing cycle. (An older "capped at 2x your plan size" rule is no longer stated — if you relied on it, re-confirm your balance behavior in-app.)
...
**Skill does**:
1. Uses Decision Maker search: `POST /v5.1/find-email/decision-maker` with `domain: "acme.com"` and `decision_maker_category: ["marketing"]` — returns name, title, LinkedIn URL, and email
...
4. If the result is `risky`, recommends verifying separately with `POST /v5.1/verify-email` (0.2 credits) or cross-referencing the LinkedIn URL to confirm the person is still in-role
...
**Skill does**:
1. Uploads the CSV via `POST /v5.1/bulk/multipart` with `Authorization: <api-key>` and `x-webhook-url: https://myapp.com/webhook` headers
...
**Skill does**:
1. For individual verification: `POST /v5.1/verify-email` with `email` — returns valid, risky, or invalid (0.2 credits each)
2. For 200 emails, recommends bulk approach for efficiency — submit as JSON via `POST /v5.1/bulk/json` or upload as CSV via `POST /v5.1/bulk/multipart`
3. Estimates credit cost: 200 x 0.2 = 40 credits for verification — well within the entry tier (400 credits/mo at EUR 26)
