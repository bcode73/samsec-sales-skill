...
- **AI Crew pricing**: Usage-based by runs (each agent execution = 1 run). Starter (1,000 runs/mo) is free and included with every plan; Growth (5,000 runs) €299/mo; Scale (25,000 runs) €999/mo; Enterprise unlimited custom. Start on the free Starter tier to gauge run volume before upgrading.
...
- **AI Crew has a free Starter tier** — 1,000 runs/mo are included free with every plan. Higher capacity is usage-based by runs: Growth 5,000 runs €299/mo, Scale 25,000 runs €999/mo, Enterprise unlimited custom (priced in "runs," not "events").
...
- **Rate limits are per API key, not per user** — 120 req/min global, 60 req/min on the recordings endpoint, 20 req/min on listing endpoints. Batch transcript endpoint handles up to 100 tokens per request.
- **Auth uses Bearer tokens** — current V2 scheme is `Authorization: Bearer YOUR_API_KEY`; the legacy `api-key:` header is deprecated but still accepted.
- **Pricing is in EUR** — Coaching & AI plan is €49/user/mo monthly, €39.20/user/mo annual (20% off); API access included. Free viewer seats; meeting invitees don't need a seat.
