...
- Site storage: 1 TB per site (Standard); 5 TB per site (Enterprise / Tableau+ / Advanced Management add-on). Individual workbook, published data source, or flow suggested limit is 25 GB. A view querying an extract can use up to 20 GB of memory. (No fixed 10 GB extract cap in current docs.)
...
- Tableau Cloud site storage: 1 TB (Standard) / 5 TB (Enterprise, Tableau+, or Advanced Management add-on)
- Tableau Cloud: 25 GB suggested max size for an individual workbook, published data source, or flow; a view querying an extract can use up to 20 GB of memory
- REST API: Tableau Cloud documents concurrency limits — exceeding them returns HTTP 429 and the request should be retried later (no fixed published per-endpoint numbers)
- Extract refresh on Cloud: schedule as hourly, daily (every 2/4/6/8/12 hours or once a day), weekly, or monthly
...
- REST API v3.29 (current — Cloud June 2026 / Server 2026.2; 300+ endpoints — see tableau-api-reference.md)
