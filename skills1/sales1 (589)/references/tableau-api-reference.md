...
<!-- Source: https://help.tableau.com/current/api/rest_api/en-us/REST/rest_api_whats_new.htm -->
<!-- Re-verified 2026-06-13: current REST API version is 3.29 (Cloud June 2026 / Server 2026.2). -->
<!-- See references/learnings.md for the dated verification entry. -->

# Tableau REST API Reference

## Overview

The Tableau Server REST API enables programmatic management of Tableau Server and Tableau Cloud resources via HTTP. Current version: **REST API 3.29** (Tableau Cloud June 2026 / Tableau Server 2026.2). REST API 3.28 corresponds to Tableau Cloud February 2026 / Server 2026.1.
...
```
Example: `https://10ay.online.tableau.com/api/3.29/sites/{site-id}/workbooks`
...
JWT-based tokens for Tableau Cloud only, configured through Tableau Cloud Manager REST API. Introduced in Tableau 2025.3; support granular access scopes and concurrent user sessions. Also usable as an Embedding API v3 authentication method.
...
### Recycle Bin Methods (added API 3.29)
- Get/List recycled content
- Restore content from recycle bin
- Permanently delete some or all recycled content

### Bridge Methods (added API 3.29)
- Bridge client management: list clients, get client details, delete client, move client between pools
- Bridge pool operations: list, create, update, delete pools
- Bridge domain mapping and management

### Tableau MCP Configuration Methods (added API 3.29)
- Per-site configuration of Tableau MCP server settings via the REST API (site-level admin control)
...
|---|---|
| Cloud June 2026 / Server 2026.2 | 3.29 |
| Cloud Feb 2026 / Server 2026.1 | 3.28 |
| 2025.3 | 3.27 |
| 2025.2 (Cloud only) | 3.26 |
| 2025.1 | 3.25 |
| 2024.2 | 3.23 |
| 2023.1 | 3.19 |
| 2020.1 | 3.7 |
...
- Tableau Cloud documents concurrency limits on REST API calls: exceeding them returns HTTP 429 (Too Many Requests) and the request should be retried later. Specific per-endpoint concurrency thresholds are not published as fixed numbers — design clients to back off on 429 rather than fire large parallel batches.
