...
- **Tableau Cloud has content-size and storage limits (not a 10GB extract cap).** Current docs: site storage is 1 TB (Standard) or 5 TB (Enterprise/Tableau+/Advanced Management); an individual workbook, published data source, or flow has a suggested 25 GB limit; a view querying an extract can use up to 20 GB of memory. For very large datasets or to avoid these limits, consider Tableau Server (bounded only by your hardware).
...
**Common causes**: Credentials expired (especially OAuth tokens), content exceeds Cloud size limits (a workbook/data source/flow over the suggested 25 GB, or a view query needing more than 20 GB of memory), Bridge connection dropped for on-prem sources
**Solution**: Re-authenticate the data source, check extract/content size (consider incremental refresh or filtering to reduce size), verify Bridge client is running and connected if using on-prem data.
