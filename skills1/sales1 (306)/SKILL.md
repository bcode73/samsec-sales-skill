...
6. **The 5% + 50¢ can climb higher** — base transaction fee is 5% + 50¢, but surcharges stack: +1.5% international, +1.5% PayPal, +0.5% subscriptions, +5% abandoned-cart recovery, +3% affiliate-referred orders. Payouts also have fees (non-US bank 1%, non-US PayPal 3% capped at $30). Model your actual all-in fees before committing.
...
- `/sales-merchant-of-record` — Choosing a Merchant of Record / payments provider across tools (Lemon Squeezy vs Paddle vs Polar vs Creem vs Stripe; MoR vs self-managed tax)
- `/sales-paddle` — Paddle platform help (the closest MoR competitor — deepest tax coverage + B2B invoicing)
