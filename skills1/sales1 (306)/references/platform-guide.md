...
- **Fees**: +3% on affiliate-referred orders for merchants (added to base 5% + 50¢) + 2% deducted from affiliate payouts (charged to the affiliate)
...
### Transaction Fees
- **Base platform fee**: 5% + 50¢ per transaction (no monthly fee, no setup fee), calculated on the total order value (including tax) and collected when the order is placed
- **Additional processing surcharges** (stack on top of the base):
  - +1.5% for international (outside-US) transactions
  - +1.5% for PayPal transactions
  - +0.5% for subscription payments
- **Marketing fees**:
  - +5% on payments recovered through abandoned-cart emails
  - +3% on affiliate-referred orders (merchant) — note the affiliate is also charged 2% on their payouts
- **Payout fees**:
  - Stripe: free for US bank accounts; 1% per payout for non-US bank accounts
  - PayPal: flat $0.50 per payout for US accounts; 3% (capped at $30) per payout for non-US accounts
- **Custom pricing**: For high-volume/large-scale businesses or products priced under $10 — contact sales

### Email Marketing Pricing
Free for up to 500 subscribers (confirmed on the live pricing page); above that, charges scale by total subscriber count via an interactive slider. The specific dollar tiers below are from the 2026-04 capture and were NOT re-confirmed (the live slider values don't render in static HTML) — verify on the pricing page before quoting.
...
4. Set up webhooks for: `subscription_created`, `subscription_updated`, `subscription_cancelled`, `subscription_resumed`, `subscription_expired`, `subscription_payment_success`, `subscription_payment_failed`, `subscription_payment_recovered` (also available: `customer_updated`, `affiliate_activated`)
...
4. Copy the signing secret (6–40 chars) for signature verification
5. Your endpoint must return 200; any other response counts as failed
6. A failed delivery is re-sent up to 3 more times (4 attempts total); after that, re-send recent webhooks manually from the dashboard (retry interval/backoff not documented)
