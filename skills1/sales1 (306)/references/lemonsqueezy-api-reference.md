...
- `POST /v1/orders/{id}/refund` — Issue an order refund (body: `data.attributes.amount`)
- `POST /v1/orders/{id}/generate-invoice` — Generate an order invoice (query params: `name`, `address`, `city`, `state`, `zip_code`, `country`, `notes`)
...
- `GET /v1/subscription-invoices/{id}` — Get subscription invoice
- `POST /v1/subscription-invoices/{id}/refund` — Issue a subscription invoice refund
...
- `GET /v1/subscription-items/{id}` — Get subscription item
- `PATCH /v1/subscription-items/{id}` — Update subscription item (usage)
- `GET /v1/subscription-items/{id}/current-usage` — Retrieve item's current usage
...
### Affiliates (read-only; added Jan 2025)
- `GET /v1/affiliates` — List all affiliates
- `GET /v1/affiliates/{id}` — Get affiliate
...
- `customer_updated` (added Feb 2026)
- `subscription_created`, `subscription_updated`, `subscription_cancelled`, `subscription_resumed`, `subscription_expired`, `subscription_paused`, `subscription_unpaused`, `subscription_payment_success`, `subscription_payment_failed`, `subscription_payment_recovered`
- `license_key_created`, `license_key_updated`
- `affiliate_activated` (added Jan 2025)

Webhook requests are POST with JSON body and an `X-Signature` header (HMAC-SHA256 of the raw request body using the webhook's signing secret, which must be 6–40 chars). Each payload carries a `meta.event_name` and an object of type `subscriptions`, `subscription-invoices`, `orders`, `customers`, or `license-keys`. Custom checkout data is surfaced in `meta.custom_data`.

A non-200 response causes Lemon Squeezy to resend the event up to three more times (four attempts total); after that it is not retried automatically, but recent webhooks can be re-sent manually from the dashboard. (Retry interval/backoff specifics are not documented.)
