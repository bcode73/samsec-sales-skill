...
### Developer tools
- **MCP Server (official, beta)**: Hosted Pendo MCP server at `https://app.pendo.io/mcp/v0/shttp` (regional; EU `https://app.eu.pendo.io/mcp/v0/shttp`). OAuth sign-in or service-account (client-credentials) auth; in Claude's Connectors Directory. Works with Claude, Claude Code, Cursor, VS Code Copilot, ChatGPT. Claude Code: `claude mcp add --transport http pendo https://app.pendo.io/mcp/v0/shttp`. See API reference for details.
- **Webhooks**: Real-time event notifications for page loads, feature clicks, guide displays, Track Events, NPS/poll displays and submissions, new visitors/accounts, and Orchestrate email unsubscribes. See API reference.
- **REST API**: Full programmatic access (see API reference). Regions: US, US1, EU, Japan, Australia.
- **Feedback API**: Separate regional API (`api.feedback.us.pendo.io` / `api.feedback.eu.pendo.io`) with `auth-token` header auth for feedback requests, product areas, and roadmap data.
