...
- **Human transcription**: priced per language, from ~€1.75/min for English/Spanish; more for other languages (French ~€2.50/min, German ~€3.00/min) — ordered separately from the subscription

**AI vs Human decision:**
- AI (85% accuracy) → drafts, internal use, content with clear audio and common accents
- Human (99% accuracy, priced per language from ~€1.75/min English) → legal, broadcast, publication, heavy accents, poor audio quality
...
- **API rate limit is 200 req/hour** (on the transcription-creation endpoint, per user). Lower than most transcription APIs — queue requests and cache responses for high-volume pipelines. Contact sales to raise it.
- **Editor can lose unsaved work.** Some users report page refresh bugs that wipe edits. Save frequently and export drafts.
- **Human transcription is billed separately and priced per language.** The cost (from ~€1.75/min for English, higher for other languages) is on top of your subscription fee, not instead of it. Human subtitles/translation cost more again.
...
**Skill does**:
1. Explains 85% AI accuracy is typical — for legal-grade work, recommends human transcription (99% accuracy; priced per language, from ~€1.75/min for English)
...
**Solution**: Upload the highest quality audio available. Create a custom glossary (Business+) with proper nouns, technical terms, and acronyms. For critical content, use human transcription (priced per language, from ~€1.75/min for English). If accuracy is consistently below 70%, the audio quality may be the limiting factor — consider using a dedicated recording tool with noise cancellation.
