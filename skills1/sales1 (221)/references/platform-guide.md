...
- Price is **per-language, not flat** — starts from €1.75/min for English (Global) and Spanish (Global) at the most cost-efficient service level; higher for other languages (e.g. French €2.50/min, German €3.00/min, and much higher for low-resource languages). Final price varies by language, turnaround, and project size. Billed separately from the subscription. (The old flat "$2.00/min" figure is now only the English-equivalent starting rate.)
- Human captions/subtitles and human translation are priced separately and higher (subtitles from ~€2.00/min English; translation €6.25–€29.60/min by language pair).
- Turnaround depends on length and language
...
**Human transcription**: per-language pricing, **from €1.75/min** (English/Spanish Global, most cost-efficient level); other languages cost more (French €2.50/min, German €3.00/min, etc.). Billed separately, available on all paid plans. Human subtitles/translation are priced separately and higher.
...
| AVID | Avid Media Composer | Import as subtitle/marker track (also avid_script, avid_ds_subtitles, avid_edl variants) |
| EDL | Edit decision list | Video editorial interchange |
| MAXQDA | Qualitative research | Import for coding and analysis |
| JSON | API/pipeline processing | Structured data with timestamps |
| CSV / XLSX | Spreadsheet/data analysis | Tabular transcript export |
| MP3 / MP4 | Media re-export | Audio/video output from the editor |
| HTML | Web publishing | Formatted transcript for embedding |

Full API `format` enum: `txt, docx, pdf, srt, vtt, stl, avid, avid_script, avid_ds_subtitles, avid_edl, facebook_srt, html, premiere, maxqda, json, fcp, edl, csv, xlsx, mp3, mp4`.
