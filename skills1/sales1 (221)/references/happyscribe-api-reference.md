...
**Parameters:**
- `format` (required): txt, docx, pdf, srt, vtt, stl, avid, avid_script, avid_ds_subtitles, avid_edl, facebook_srt, html, premiere, maxqda, json, fcp, edl, csv, xlsx, mp3, mp4
...
- `free_trial_submitted`: Free-trial order submitted for processing
- `free_trial_fulfilled`: Free-trial order complete

### Order Webhooks

Pass `webhook_url` when creating an order (`POST /orders` or `POST /orders/translation`). A webhook is sent to that URL **every time the order changes state**. The payload is a JSON object with the order `id`, current `state`, and — when `state` is `failed` — a `failure_reason`.
...
```json
{ "id": "10b1edd9381d4a00a3d0db1ca1b3d0cf", "state": "failed", "failure_reason": "ingestion_failed" }
```

`failure_reason` values: `ingestion_failed`, `transcription_failed`, `unknown`. For full failure context, fetch the order via `GET /orders/<ID>` (per-ingestion `failure_reason` fields narrow ingestion failures further). No HMAC signing of webhook payloads is documented — validate by re-fetching the order rather than trusting the payload alone.
