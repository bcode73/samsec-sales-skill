...
<!-- Source: https://developer.constantcontact.com/api_guide/scopes.html -->
<!-- Source: https://developer.constantcontact.com/api_guide/partner_webhook_overview.html -->
...
| Scope | Grants |
|---|---|
| `account_read` | Read account data (`/account` endpoints) |
| `account_update` | Update account data (`/account` endpoints) |
| `contact_data` | Read/write contact data and read contact reports — covers `/contacts`, `/contact_lists`, `/contact_custom_fields`, `/segments`, `/activities` (bulk ops), `/reports/contact_reports` |
| `campaign_data` | Read/write campaign data and read campaign reports — covers `/emails` and `/reports/email_reports` |
| `offline_access` | Required for refresh tokens (Authorization Code + PKCE only) |

Scope names must be space-delimited in the authorization request.
...
| POST | `/activities/contacts_json_import` | Import contacts via JSON (max 40,000 contacts; JSON payload must be < 4 MB) |
...
The payload is a notification pointer (no full event data, no HMAC signing) — call the `url` for full event data. Note the `url` host is `api.constantcontact.com` (partner billing host), not the main `api.cc.email` API host. Constant Contact retries all failed webhook notification requests every minute for up to 1 hour (capped at 3600 seconds).
...
Official V3 API client libraries (BSD 3-Clause, under the `constantcontact` GitHub org, repo pattern `constant-contact-v3api-client-{lang}`):
...
- PHP: [github.com/constantcontact/constant-contact-v3api-client-php](https://github.com/constantcontact/constant-contact-v3api-client-php)
- Python: [github.com/constantcontact/constant-contact-v3api-client-python](https://github.com/constantcontact/constant-contact-v3api-client-python)
- TypeScript: [github.com/constantcontact/constant-contact-v3api-client-typescript](https://github.com/constantcontact/constant-contact-v3api-client-typescript)
