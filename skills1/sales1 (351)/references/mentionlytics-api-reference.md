...
- **Rate limit:** 100 requests/minute (documented for v1; the v2 docs state only that "all endpoints are subject to rate limiting" without publishing a specific number — treat 100/min as a working assumption and back off on errors)
- **Plan requirement:** Pro ($499/mo), Business ($749/mo), or Enterprise ($1,299+/mo). (Per official v2 docs, API functionality is available on Pro, Business, and Enterprise plans. The former "Agency" tier is now "Business.")
...
**Two token types (v2):**
- **User API Tokens** — tied to an individual user account. Note: the token changes if you change your Mentionlytics password, so you must regenerate it.
- **Account Access Tokens** — account-level tokens with a configurable expiration period (**default is 30 days**), better suited for long-lived integrations.
...
| 5 | Instagram |
| 7 | Reviews |
| 99 | Others (LinkedIn, Reddit, TikTok, Bluesky, forums, etc.) |

## Rate Limits

- **100 requests per minute** across all endpoints (this figure is documented in the v1 help article; the official v2 article only says all endpoints are "subject to rate limiting" without a published number)
