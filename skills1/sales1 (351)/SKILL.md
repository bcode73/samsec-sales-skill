...
2. **Which Mentionlytics plan are you on?**
   ...
   - B) Essential ($169/mo)
   - C) Advanced ($299/mo)
   - D) Pro ($499/mo — API access)
   - E) Business ($749/mo)
   - F) Enterprise ($1,299+/mo)
...
- **API access requires Pro plan ($499/mo) or higher.** API is available on Pro, Business, and Enterprise plans only — Basic through Advanced have no API access. Verify your plan before building integrations.
