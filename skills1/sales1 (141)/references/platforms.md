...
- **Warmup**: Free auto-warmup on every plan via a shared warmup pool; no separate warmup fee. Warmup status is readable via the API (`/accounts`)
- **Infrastructure**: **SISR** (Server & IP Sharding and Rotation) auto-assigns/rotates servers and IP blocks to protect reputation; supports unlimited connected inboxes for spreading volume across secondary domains
- **Testing**: Built-in **inbox-placement (seed-list) tests** (`/inbox-placement-tests`) — use these, not the warmup heat score, to judge real placement
- **The core gotcha**: Instantly's #1 user complaint is **a green warmup score while real campaigns still land in spam** — the warmup pool generates *synthetic* engagement that doesn't equal real-prospect placement. Validate with seed tests, verify SPF/DKIM/DMARC + a custom tracking domain, keep per-inbox volume low, and warm new domains longer
- **DFY caution**: Done-for-you pre-warmed domains/inboxes sometimes arrive with poor reputation (or get provider-disabled) — run your own placement test before sending at volume
