...
**Per-plan rate limits** (from prospeo.io/api-docs/rate-limits, verified 2026-06-13). Enrich endpoints (`/enrich-person`, `/enrich-company`, `/bulk-enrich-person`, `/bulk-enrich-company`) and Search endpoints (`/search-person`, `/search-company`) have separate, lower limits for search because it is more resource-intensive:
...
Exceeding a limit returns HTTP 429. Monitor remaining quota via the response headers above.
...
**Credits**: 1 credit per email found (standard), 10 credits per mobile found (email included free). Free if no match, or if the same record is re-enriched within 90 days (after 90 days it uses credits again).
...
**Credits**: 1 credit per matched company. Free for no match, or for re-enriching the same record within 90 days (after 90 days it uses credits again).
...
**Credits**: 1 credit per search request that returns at least one person (up to 25 results covered per credit). Duplicate search requests within 30 days are free.
...
- Webhook support — no native webhook/callback events are documented in the REST API (as of 2026-06-13). Push-style automation is via Zapier/Make, not first-party webhooks.
- Detailed field-level documentation for all 50+ company enrichment fields
- Exact monthly dollar prices per plan (pricing page is JS-rendered and was not machine-verifiable; plan names and per-user credit allotments are confirmed via help.prospeo.io)

*Per-plan rate limit thresholds are now documented — see the Rate limits section above (verified 2026-06-13).*
