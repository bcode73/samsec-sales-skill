...
- **Rate limits are per-organization, not per-stack.** CMA read/write: 10 req/s; CDA origin (uncached): 80 req/s by default. If multiple stacks share an org, they compete for the same rate limit pool.
