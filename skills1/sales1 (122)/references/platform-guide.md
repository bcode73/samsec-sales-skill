...
- CDA origin (uncached): 80 req/s per organization by default (CDN-cached responses are not rate-limited but count toward quota; docs also cite 100 req/s for uncached — treat 80/s as the default, request increases via support)
...
- Security: payloads are signed in the `X-Contentstack-Request-Signature` header (SHA-256 + RSA private key, `v1=` prefix, 256-char signature); verify with the public key from `https://<DOMAIN>/.well-known/public-keys.json`. Custom headers can also be attached for endpoint authentication.
