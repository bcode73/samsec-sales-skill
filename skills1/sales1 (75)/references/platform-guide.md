...
|---|---|---|
| **Signal monitoring** | Track "signals" (keywords/topics) across 30+ platforms with deduplication. On the live pricing page the per-tier quota is labeled "signals monitored" (5 / 15 / 30), not "keywords" | All plans (16-33+ sources by tier) |
...
| **REST API** | Programmatic access to leads, keywords, and actions. Two paths: (a) bundled in plans — Growth 500 req/mo, Scale 2,500 req/mo; (b) standalone **API Credit Plans** (pay-as-you-go) where 1 credit = 1 successful API call and credits never expire | Growth (500 req/mo), Scale full (2,500 req/mo), or API Credit Plans à la carte |
| **MCP server** | AI agent integration — query leads, trigger scans, act on signals autonomously. Marketed on the homepage under "API, WEBHOOKS & MCP" as "plug Buska into Claude, GPT, or any AI agent" | Scale / via API access (homepage notes "Full REST API access (Scale plan)") |
...
|---|---|---|---|---|
| Signals monitored* | 5 | 15 | 30 | Unlimited |
| ICP profiles | 2 | 5 | 10 | Custom |
| Platform sources | 16+ | 28+ | 33+ | All |
| Update frequency | Daily | Every 3 hours | Hourly | Real-time |
| Team members | 1 | 5 | Unlimited | Custom |
| Reply Studio | No | Yes | Yes | Yes |
| Prospect Finder | No | 20 searches | 250/month | Custom |
| Webhooks | No | Yes | Yes | Yes |
| REST API | No | 500 req/mo | 2,500 req/mo (full) | Custom |
| MCP server | No | No | Yes (via API) | Yes |
| CRM connectors | No | Yes | Yes | Yes |
| CSV export | No | Yes | Yes | Yes |
| Reports & analytics | No | No | Yes | Yes |
| Automations | 1 | 5 | Unlimited | Unlimited |

\* The live pricing page labels the per-tier monitoring quota "signals monitored" (5 / 15 / 30), replacing the older "keywords" framing. Treat a signal as a keyword/topic you track. Leads/week limits are no longer published on the pricing page; the page now leads with signals + update frequency + sources as the per-tier gates.

All plans include 7-day free trial (no credit card required). Annual billing carries a ~17% discount.

### API Credit Plans (standalone, pay-as-you-go)

Separate from the subscription tiers, Buska sells à-la-carte **API Credit Plans** for developers who want API access without committing to a Growth/Scale subscription. **1 credit = 1 successful API call** and **credits never expire**:

| API Credit Plan | Price | Credits |
|---|---|---|
| Developer | $50 | 1,000 |
| Production | $500 | 50,000 |
| Business | $3,000 | 500,000 |

These cover: search mentions by keyword + platform, AI-score any content, query detected buying signals, across 30+ platforms.

**Plan gate warning:** Programmatic access has two doors. (1) Bundled — API + webhooks require a Growth ($99/mo) subscription or higher; Starter is monitoring + notifications only. (2) À la carte — buy an API Credit Plan (from $50 for 1,000 calls) for pay-as-you-go API access without a Growth subscription. Webhooks remain Growth+ only; the credit plans cover the REST API/MCP query surface.
