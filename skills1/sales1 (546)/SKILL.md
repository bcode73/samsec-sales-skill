...
3. **Current tools?**
   - A) Zendesk / Intercom
   - B) Gladly / Gorgias / Help Scout
   - C) ServiceNow
   - D) Genesys / Five9 / NICE CXone / other CCaaS (no native connector — custom integration)
...
- **No public API.** No public REST API, webhooks, or developer docs appear on solidroad.com (re-verified 2026-06-13). All integrations go through Solidroad's native connectors (Zendesk, Intercom, Gladly, Gorgias, ServiceNow, Help Scout) or a custom integration via their team.
- **Native connector set changed.** The current native connectors are **Zendesk, Intercom, Gladly, Gorgias, ServiceNow, Help Scout** (plus broader integrations with Gong, Guru, Front). **Salesforce Service Cloud is no longer listed** on the official site — treat a Salesforce connector as unconfirmed and verify with Solidroad before relying on it.
- **Custom pricing only — no published tiers.** Solidroad does not publish per-user pricing or named tiers; pricing is quote-based via sales (re-verified 2026-06-13). Older "$10-50/user/mo" figures came from third-party aggregators, not Solidroad, and are no longer reflected anywhere official — don't quote them.
...
**Solution**: Verify API permissions in your helpdesk/CCaaS. Solidroad has native connectors for Zendesk, Intercom, Gladly, Gorgias, ServiceNow, and Help Scout (re-verified 2026-06-13). Salesforce Service Cloud is not currently listed as a native connector — confirm availability with Solidroad before relying on it. For other platforms, contact Solidroad support for custom integration help.
