...
- [In TexAu (waterfall enrichment + API)](#in-texau-waterfall-enrichment--api)
- [In Surfe (LinkedIn-to-CRM + waterfall enrichment)](#in-surfe-linkedin-to-crm--waterfall-enrichment)
...
### In TexAu (waterfall enrichment + API)

- **Waterfall across 32+ providers**: right-click enrichment in the V3 table or programmatic calls cascade through providers (Apollo, Hunter, Snov, RocketReach, Clearbit, etc.) until data is found. Responses include the winning provider, per-provider execution logs, and latency
- **LinkedIn enrichment**: `POST /enrich_profile` and `/enrich_company` (by URL), plus bulk `/enrich_profiles_bulk` and `/enrich_companies_bulk` (by URN/company id, ≤50/call, billed per result)
- **Email + phone**: email finding (`/email_finding`) and verification (`/email_verification`) are **async jobs** — submit with a `webhook` URL and `data` array, then receive results at the webhook or poll `GET /email_finding_inquiry/{id}`. Find = 2 credits/person found, verify = 0.5/email
- **Company + web signals**: `/web_tech_stack`, `/website_intelligence`, `/web_emails`, `/web_sitemap` enrich an account from its domain (technographics, contacts, structure)
- **Pay-on-match economics**: failed lookups return `billed:false` and refund the credit — so a low fill rate costs nothing, but budget per *result* on bulk/search endpoints. `useCache` defaults to `true`; set `false` for fresh data. Check `/usage?month=YYYY-MM` for remaining credits
- **Auth & access**: REST at `https://v3-api.texau.com/api/v1` with the `x-api-key` header; MCP server at `mcp.texau.com/mcp` for Claude/Cursor; n8n community nodes available. API + webhooks are plan-gated (Starter+ on the V3 platform)
- **Waterfall position**: a budget, API+MCP-first alternative to Clay's waterfall — fewer providers than Clay's 150+, but pay-on-match credits and an MCP server make it agent-friendly for GTM engineers
- **Platform skill**: `/sales-texau`

### In Surfe (LinkedIn-to-CRM + waterfall enrichment)

- **What it is**: Surfe (formerly Leadjet) is a Chrome extension that pushes LinkedIn/Sales Navigator contacts into HubSpot, Salesforce, Pipedrive, or Copper, plus a waterfall enrichment layer cascading 15+ providers for verified (triple-verified) professional emails and mobile numbers. Best fit: individual SDRs and small teams who live on LinkedIn
- **Two surfaces**: the **extension** enriches in-session only (you must be viewing a LinkedIn profile/list — no background bulk). True bulk/background enrichment lives in the **REST API**: `POST /v2/people/enrich` (≤10,000 people/job) and `POST /v2/companies/enrich` (≤500 companies/job). Don't tell users "Surfe can't bulk-enrich" — the extension can't, the API can
- **Async model**: a start call returns an `enrichmentID`; fetch results from `GET /v2/people/enrich/{id}` or via webhook. Auth is `Authorization: Bearer <key>`, base URL `https://api.surfe.com`
- **Credit economics**: credits are billed **per YEAR**, not per month — the #1 cost gotcha (Essential ~150 email + 50 phone/year; Pro ~1,000 email + 100 phone/year). Three **separate** pools (email/mobile/search). A credit is spent only on a successful find. Set `skipMobileEnrichmentIfNoEmailFound: true` to protect the mobile pool; check `GET /v1/credits` before big jobs
- **API does NOT write to the CRM**: it returns enriched data only. CRM writes happen through the extension/native connector (HubSpot/Salesforce/Pipedrive/Copper); or pipe API results into the CRM's own API, mapping on `externalID`. Niche CRMs are unsupported
- **Webhooks**: `person.enrichment.completed`, `person.batch-enrichment.completed`, `company.enrichment.completed`. HTTPS-only; verify the `x-surfe-signature` (HMAC-SHA256 of `t.body`) before trusting payloads
- **Waterfall position**: a LinkedIn-first, in-CRM enrichment tool rather than a broad standalone data engine — fewer providers than Clay (150+) or even TexAu (32+), and per-seat annual credits cap volume. Great as the "enrich while I prospect on LinkedIn" layer; pair with Clay/Apollo for high-volume background enrichment
- **Platform skill**: `/sales-surfe`
