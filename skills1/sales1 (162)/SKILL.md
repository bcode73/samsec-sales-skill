...
- `/sales-texau` — TexAu platform help (waterfall enrichment across 32+ providers, async email finding/verification, web/company signals, REST API + MCP server)
- `/sales-surfe` — Surfe platform help (LinkedIn-to-CRM Chrome extension + waterfall enrichment API, HubSpot/Salesforce/Pipedrive/Copper sync, async enrich endpoints + webhooks)
