---
name: sales-sozai
description: "Soz AI platform help — mobile-first AI transcription and note-taking with 100+ languages, speaker diarization, YouTube URL transcription, and subtitles. Premium is $3.99/week (300 min/month). Use when setting up Soz AI for meeting recording on mobile, transcription accuracy issues with accents or noisy environments, comparing Soz AI to Otter or Notta or Transkriptor for multilingual transcription, YouTube video transcription not working or transcript quality poor, subtitle generation in SRT or VTT format, or understanding Soz AI free tier (30 min/month) vs Premium limits. Do NOT use for picking between note-takers generally (use /sales-note-taker) or building a transcript API pipeline (Soz AI advertises a REST API but publishes no developer docs — use /sales-note-taker for API-first platforms)."
...
| Building a transcript pipeline or API integration | `/sales-note-taker {user's question}` — Soz AI advertises a REST API but publishes no developer docs (no endpoints/auth/webhooks) |
...
**Setup priority**: Download app (iOS/Android; Chrome extension is "Coming Soon" as of 2026-06-13) → test with a short recording → verify language detection → try YouTube URL paste if relevant → evaluate subtitle export formats.

**Key decision point**: Soz AI excels at mobile transcription and YouTube URL paste. It advertises a REST API but publishes no developer docs (no documented endpoints, auth, or webhooks), and it has no CRM integration and no team features. If you need a documented programmatic surface, CRM sync, or team controls, look at Fathom, Fireflies, or Otter instead.
...
- **REST API is advertised but undocumented** (re-verified 2026-06-13) — sozai.app/transcribe-youtube-videos states "Connect SozAI to your workflow with our REST API," but there is NO published base URL, auth scheme, endpoint list, rate limit, or webhook docs anywhere on the site. You cannot wire against documented endpoints today; for a real pipeline use an API-first platform via `/sales-note-taker`, or contact Soz AI (Telegram) for access.
- **No CRM integration** — no Salesforce, HubSpot, Pipedrive, Zapier, or Make connectors. The site advertises "auto-export to Google Drive, Dropbox, or CMS" but with no setup docs. Transcript data stays in the app.
- **No team features yet** — no shared workspaces, no admin controls, no team analytics. FAQ: team plans are "not yet" available (contact via Telegram for volume pricing). Designed for individual users.
- **Free tier is 30 min/month; Premium is $3.99/week** (re-verified 2026-06-13 on /pricing) — the Premium card says "unlimited" but the pricing-page FAQ caps it at **300 minutes/month**. Treat it as 300 min/mo, not truly unlimited. (Older comparison/marketing pages still cite the legacy $9.99/mo "unlimited" figure — the /pricing page is authoritative.) Billing is weekly only, via App Store / Google Play.
...
- **Speaker diarization** — auto speaker labels; current pages no longer state a speaker count (an earlier capture cited "up to 10" — treat the cap as unconfirmed). Larger/overlapping meetings may not get accurate attribution.
- **Chrome extension is "Coming Soon"** (as of 2026-06-13) — available on iOS and Android; older macOS/Chrome availability is not confirmed on the current download pages.
...
2. Compares pricing (Soz AI Premium $3.99/week / 300 min per month vs Transkriptor $9.99/mo tiered minutes)
...
**Solution**: Place the recording device centrally among speakers. Avoid crosstalk. For critical recordings where attribution matters, manually note speaker transitions. Soz AI auto-labels speakers (current pages no longer state a count; an earlier capture cited up to 10) and accuracy degrades above ~5 speakers.
