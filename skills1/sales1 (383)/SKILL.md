...
- **MCP v2 uses OAuth, not a pasted key.** The current MCP server is HTTP transport at `https://app.octolens.com/api/mcp/v2` and authenticates via browser OAuth — there's no token to paste. The legacy v1 server (SSE, `?token=` in the URL) still works for existing connections but gets no new features. Don't paste an API key into a v2 config.
...
2. Provides the v2 `claude mcp add --transport http` command (OAuth — no token to paste)
3. Notes v1 SSE/token URL is legacy and only for existing connections
...
**Cause**: Using the legacy v1 URL/token, OAuth not completed, or firewall blocking app.octolens.com
**Solution**: Prefer MCP v2 — `claude mcp add --transport http octolens "https://app.octolens.com/api/mcp/v2"` and complete the browser OAuth sign-in on first use (no token to paste). If on legacy v1 (SSE `?token=`), confirm the token isn't expired. Ensure outbound connections to app.octolens.com are allowed. Restart Claude Code completely after adding the MCP config.

### Webhook not delivering mentions
**Symptom**: Configured webhook endpoint but no POST requests arriving
**Cause**: Webhook not attached to a feed, frequency set to Daily/Weekly, endpoint not reachable, or wrong URL
**Solution**: Webhooks are configured **per feed** (open feed → Set up alert → Webhook), not globally — confirm it's enabled on the right feed. Check the delivery **frequency** (Instantly / Hourly / Daily / Weekly) — a Daily/Weekly setting batches sends. Test endpoint reachability from outside your network and verify the URL is HTTPS. Payload arrives as `{ "action": "mention_created", "data": {...} }`.
