...
2. **Which plan are you on?** (prices are annual-billed effective monthly; monthly billing ~20% higher)
   - A) Starter ($79/mo — 1 Reddit account, 150 comments, 50 AI prompts, 1 domain)
   - B) Growth ($159/mo — 3 accounts, 450 comments, 200 AI prompts, 3 domains, Slack)
   - C) Pro ($319/mo — unlimited accounts, 750 comments, 300 AI prompts, 5 domains, AEO API)
   - D) Agency (from $1,000/mo) / Enterprise (custom)
...
- **Slack integration is Growth+ only.** Starter plan users get email alerts only. Upgrade to Growth ($159/mo) for Slack notifications.
- **AI Visibility prompts are limited per plan.** Starter gets 50 prompts/mo, Growth 200, Pro 300. Running out means no AI Visibility Scanning until the next billing cycle.
- **The AEO API is Pro-only.** Programmatic AI-visibility data (`/v1/aeo/*` — visibility, citations, sources, brand ranking, score transparency) requires the Pro plan; it returns HTTP 402 below Pro. The Agent API (`/agent/*`) is available on all plans.
- **No webhooks or Zapier/Make.** Both APIs are read-only (GET-only) — integration is limited to the REST API and CLI. No push notifications — poll the API or use email/Slack alerts.
...
2. Shows the domain mentions endpoint: `GET /agent/domains/{domain}/mentions` with `x-api-key` auth (returns a flat array of post objects, `?limit=` max 100)
3. Walks through getting an API key from Settings → API Keys (keys prefixed `rg_`)
4. Notes rate limits: 60 req/min, 1K req/hr; domain mentions is a heavy call (5 credits); credit headers `X-Credits-Remaining` returned on each response
