...
<!-- Re-verified against live official docs: 2026-06-13 (see references/learnings.md) -->
...
| Success Plan | 100 calls/min | 50,000/day |
...
### Check Async Record Request Status

**GET** `https://personapi.gainsightcloud.com/v1/peoplemgmt/v1.0/people/async/status/{statusId}`

Poll with the `statusId` returned by the async upsert/delete call to retrieve completion state.
...
| GU_2404 | Lookup field is null/empty | 400 |
| GU_2405 | Invalid target object name | 400 |
| GU_2406 | Empty/null matching criteria | 400 |
| GU_2407 | Fields missing from user record | 400 |
...
**PUT** `v1/ant/es/job/abort/<job_id>`

Aborts an in-progress Timeline bulk job by job ID.
...
## Events API (inbound event publishing)

The Events API lets external systems publish events INTO Gainsight (it is inbound only — Gainsight does NOT provide outbound webhooks or HMAC-signed callbacks). Published events can drive Rules Engine / Journey Orchestrator automation.

### Publish an Event

**POST** `version/eventManager/event`

Auth is via a shared secret, NOT the standard `Accesskey` header.
...
Supports system asset events with operation metadata (INSERT, UPDATE, UPSERT, DELETE).
...
PX SDKs available: JavaScript (web), iOS, Android, React Native, Flutter, NativeScript, Xamarin, Java (server-side), C# (desktop). SDK source repos live at https://github.com/gainsight (px-android, px-ios, px-react-native-sdk, px-flutter, px-nativescript-sdk). The Java SDK requires a PX REST API key.
