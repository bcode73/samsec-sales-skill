...
3. **Flow webhooks are NOT the same as system webhooks — flow webhooks fire as a flow action step, system webhooks (Advanced KDP) fire on any event.** Most accounts only have access to flow webhooks (a POST action within a flow step). System webhooks that fire on email events (`event:klaviyo.opened_email`, `event:klaviyo.clicked_email`, `event:klaviyo.bounced_email`), review events, and consent events with HMAC-SHA256 signing (signature in the `Klaviyo-Signature` header — NOT `Klaviyo-Webhook-Signature`; max 10 webhooks per account, secret key ≥16 chars) are an Advanced KDP feature. Don't confuse the two when planning integrations.
