...
**OAuth** — Available for technology partners building integrations. Uses the OAuth 2.0 **authorization code flow with PKCE required** (`code_challenge_method=S256`); the resulting access token is sent as a standard `Authorization: Bearer <token>` header. OAuth apps receive a separate rate-limit quota per app instance.
...
- **Algorithm**: Fixed-window rate limiting with two windows per endpoint — a **burst** limit (1-second window) and a **steady** limit (1-minute window).
- Rate limits are applied **per-account**, but the specific tier depends on **which endpoint** you call — each endpoint is assigned one of five tiers (XS / S / M / L / XL):
...
- Exceeding either window returns HTTP `429 Too Many Requests`.
- On a 429, the standard rate-limit headers are replaced with a **`Retry-After`** header giving the number of seconds to wait before retrying. Retry with exponential backoff.
- OAuth apps get a separate rate-limit quota per app instance.
...
Subscribe to real-time event notifications via webhooks. Only **Advanced KDP customers and Klaviyo app partners** can access and manage webhooks via the Webhooks API. Each Klaviyo account can have a **maximum of 10 webhooks** total, whether created through the UI or the API.

| Method | Endpoint | Description |
|---|---|---|
| GET | `/api/webhook-topics/` | List available webhook topics |
| GET | `/api/webhook-topics/{id}/` | Get a single webhook topic |
...
**Webhook topics** use the ID format `event:klaviyo.<event_name>`. Use the Get Webhook Topics endpoint (`GET /api/webhook-topics/`) to retrieve the full, account-specific list. Common topics include:
- **Email events**: `event:klaviyo.opened_email`, `event:klaviyo.clicked_email`, `event:klaviyo.bounced_email`, `event:klaviyo.marked_email_as_spam`
- **SMS events**: `event:klaviyo.sent_sms`, `event:klaviyo.received_sms`
- **Push events**: `event:klaviyo.received_push`, `event:klaviyo.bounced_push`
- **Review events**: `event:klaviyo.ready_to_review`, `event:klaviyo.submitted_review`
- **Consent events**: `event:klaviyo.subscribed_to_email_marketing`, `event:klaviyo.unsubscribed_from_email_marketing`, `event:klaviyo.subscribed_to_sms_marketing`

(The list above is not exhaustive — there is no limit on how many topics a single webhook can subscribe to.)
  ...
  --data '{
    "data": {
      ...
      "attributes": {
        ...
        "enabled": true,
        "secret_key": "your-16-char-min-secret",
        "topics": ["event:klaviyo.opened_email", "event:klaviyo.clicked_email"]
      }
    }
  }'
```

**Webhook signing**: Klaviyo signs each webhook request body using HMAC-SHA256 with the webhook's `secret_key` (which **must be at least 16 characters**). The signature is sent in the **`Klaviyo-Signature`** header. Klaviyo also includes a **`Klaviyo-Timestamp`** header (when the request was sent) and a **`Klaviyo-Webhook-Id`** header (the webhook's unique ID). Verify by computing HMAC-SHA256 of the raw request body using your secret key and comparing it to the `Klaviyo-Signature` value. Reject payloads that do not match.
  ...
  --data '{
    "data": {
      ...
      "attributes": {
        ...
        "endpoint_url": "https://example.com/webhooks/klaviyo",
        "secret_key": "your-16-char-min-secret",
        "topics": ["event:klaviyo.opened_email", "event:klaviyo.clicked_email", "event:klaviyo.marked_email_as_spam"]
      }
    }
  }'

# Step 2: Verify webhook signature in your application (pseudocode):
# secret = YOUR_WEBHOOK_SECRET   # must be >= 16 chars
# expected = hmac_sha256(secret, raw_request_body)
# if expected != request.headers["Klaviyo-Signature"]: reject
# (Klaviyo also sends Klaviyo-Timestamp and Klaviyo-Webhook-Id headers)
```
