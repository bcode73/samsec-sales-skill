...
- **Advanced webhooks**: System-level event-driven webhooks (max 10 per account) with HMAC-SHA256 signing — fire on any subscribed topic (e.g. `event:klaviyo.opened_email`, `event:klaviyo.clicked_email`, `event:klaviyo.bounced_email`, review and consent events). Signature is sent in the `Klaviyo-Signature` header (plus `Klaviyo-Timestamp` and `Klaviyo-Webhook-Id`); secret key must be ≥16 chars. Distinct from flow webhooks
...
- **Rate limits**: Fixed-window burst limit (per second) + steady limit (per minute), assigned per-endpoint across five tiers — XS (1/s, 15/m), S (3/s, 60/m), M (10/s, 150/m), L (75/s, 700/m), XL (350/s, 3,500/m). Per-account. OAuth apps get separate quota. Exceeding limits returns HTTP 429 with a `Retry-After` header
- **Key endpoints**:
