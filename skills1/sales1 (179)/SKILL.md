...
- **Rate-limit sizing**: Free = 50 req/day, Pro = 500 req/day (raised from 50 in API v2.24.1) — Pro now handles modest backfills but is still per-day, not per-minute. Business/Enterprise = 60 req/min — the only realistic tier for a production pipeline. Anything beyond that needs queue-and-retry.
...
- **Free is capped at 50 API requests per day; Pro at 500 req/day** (Pro was raised from 50 to 500 in API v2.24.1). A single backfill of 500 historical meetings will exhaust Pro's daily budget in one shot and break Free. Upgrade to Business or Enterprise for a 60 req/min ceiling.
...
**Skill does**:
1. Rules out Free ($0, **50 req/day**) and Pro ($10/seat, **500 req/day** since API v2.24.1) — both per-day caps, so a ~50-rep backfill would exhaust Pro's daily budget and break Free on day one
...
### Hitting the daily request cap on Free/Pro during a backfill
