...
- **Job** — the core object. Represents a transcription request.
  ...
  - `status` (string) — `in_progress`, `transcribed`, `failed`
  ...
  - `duration_seconds` (integer) — audio duration; `null` when not available
  - `name` (string) — file name; `null` when not available
  - `metadata` (string, 256 char limit) — user-provided metadata
  - `failure` (string) — failure reason on failed jobs: `unspecified`, `download_failure`, `insufficient_balance`, `invalid_media`
  - `failure_detail` (string) — human-readable failure reason
