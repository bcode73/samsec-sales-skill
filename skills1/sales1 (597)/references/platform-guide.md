...
- **iQ Profiles API (V3)** — programmatically read/update iQ config (variables, tags, load rules, extensions, events, tag templates) via `GET`/`PATCH` at `/v3/tiq/accounts/{account}/profiles/{profile}` (see `tealium-api-reference.md`)
