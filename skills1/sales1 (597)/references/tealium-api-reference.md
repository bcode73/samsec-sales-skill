...
- `/v3/collect/regional/integration/event/accounts/{account}/profiles/{profile}/datasources/{datasourceId}` — region-specific integration/webhook event

Regional endpoints send events to the server in the profile's region; global endpoints route to the nearest (lowest-latency) server.

---

## iQ Profiles API v3

Programmatically read and update iQ Tag Management profile configurations (variables, tags, load rules, JavaScript extensions, events, tag templates). Base path: `/v3/tiq/accounts/{account}/profiles/{profile}`. Bearer token auth (same V3 auth flow).

The API exposes two operation types:
- **GET** — returns specific content about iQ Tag Management account profiles.
- **PATCH** — performs create, update, and delete operations on a limited subset of configuration components.

### GET /v3/tiq/accounts/{account}/profiles/{profile}

Retrieve an iQ profile's configuration and version history.

**Query parameters:**
- `includes` — array of components to return: `loadRules`, `tags`, `extensions`, `variables`, `events`, `versionIDs`
- `publishVersion` — version ID in format `YYYYMMDDhhmm` (optional)

**Auth:** Bearer token required.

### Sub-APIs (all under the `/v3/tiq/...` base path)

| Sub-API | Manages |
|---|---|
| iQ GET Profile API | Whole-profile read |
| iQ Variables API | Data layer variables |
| iQ Load Rules API | Load rules |
| iQ JavaScript Extension API | JavaScript code extensions |
| iQ Events API | Event listeners |
| iQ Tags API | Tag configurations |
| iQ Tag Template API | Tag templates (PATCH for create/update) |
...
Note: programmatic management of iQ profile *configuration* (variables, tags, load rules, extensions, events, tag templates) is now available in **V3** via the iQ Profiles API (see above) — it is no longer V2-only.
