...
| **Bounce processing** | Inbound webhook receivers for SES (SNS), SendGrid/Twilio, Postmark, Forward Email, Lettermint (v6.1.0+), plus a generic `/webhooks/bounce` endpoint | **Webhook-accessible** (inbound only) |
...
| **Authentication** | OIDC SSO (Google/Microsoft/Auth0/GitHub), role-based permissions (user roles + list roles), granular API tokens with per-scope permissions (e.g. `campaigns:send` is separate from `campaigns:manage` as of v6.1.0) | **Admin UI** |
| **Privacy/tracking** | Global Privacy setting to disable view + click tracking; optional S3 media proxying through Listmonk (v6.1.0+) | **Admin UI** |
...
- **Amazon SES** — cheapest at scale, requires bounce/complaint SNS setup (`/webhooks/service/ses`)
- **Mailgun** — easy setup (no native bounce webhook endpoint — use the generic `/webhooks/bounce`)
- **Postmark** — best deliverability, native bounce webhook (`/webhooks/service/postmark`)
- **SendGrid / Twilio** — common, native event webhook for bounces (`/webhooks/service/sendgrid`)
- **Forward Email** — native bounce webhook (`/webhooks/service/forwardemail`)
- **Lettermint** — native bounce webhook added in v6.1.0 (`/webhooks/service/lettermint`)
- **Any SMTP server** — including self-hosted Postal, Haraka, etc. (use the generic `/webhooks/bounce` endpoint for custom bounce ingestion)
...
**Gotcha:** Campaign status transitions are strict (per official docs): only `paused` and `draft` campaigns can start (`running`); only `scheduled` can revert to `draft`; only `draft` can become `scheduled`; only a `running` campaign can be `paused` or `cancelled`. So you cannot cancel a `paused` campaign directly — resume it to `running` first, then cancel. Also note: starting a send via `PUT /api/campaigns/{id}/status` requires the `campaigns:send` permission (v6.1.0+), which is separate from `campaigns:manage` — a token without it returns 403.
