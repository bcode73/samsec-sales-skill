...
### Permission scopes (RBAC)

API tokens and user roles carry granular permission scopes. Notable scopes:

- **Lists:** `lists:get_all`, `lists:manage_all`
- **Subscribers:** `subscribers:get`, `subscribers:get_all`, `subscribers:manage`, `subscribers:import`, `subscribers:sql_query`
- **Transactional:** `tx:send`
- **Campaigns:** `campaigns:get`, `campaigns:get_all`, `campaigns:get_analytics`, `campaigns:manage`, `campaigns:manage_all`, `campaigns:send`
- **Bounces:** `bounces:get`, `bounces:manage`, `webhooks:post_bounce`
- **Media:** `media:get`, `media:manage`
- **Templates:** `templates:get`, `templates:manage`
- **Users/roles:** `users:get`, `users:manage`, `roles:get`, `roles:manage`
- **Settings:** `settings:get`, `settings:manage`, `settings:maintain`

**Gotcha (v6.1.0+):** `campaigns:send` is a SEPARATE permission from `campaigns:manage` — a token with `campaigns:manage_all` can create/update campaigns but CANNOT start sending unless it also has `campaigns:send`. If a `PUT /api/campaigns/{id}/status` to `running` returns 403, the token is missing `campaigns:send`.
...
Valid statuses: `draft`, `scheduled`, `running`, `paused`, `cancelled`.

Valid transitions (per official docs):
- Only `scheduled` campaigns can change to `draft`.
- Only `draft` campaigns can change to `scheduled`.
- Only `paused` and `draft` campaigns can start (`running`).
- Only `running` campaigns can change to `cancelled` and `paused`.

So a `paused` campaign must be resumed to `running` before it can be `cancelled`. Sending requires the `campaigns:send` permission (v6.1.0+), separate from `campaigns:manage`.
...
These INBOUND endpoints receive bounce notifications from SMTP providers (require the `webhooks:post_bounce` permission / bounce webhooks enabled in Settings → Bounces):

- `POST /webhooks/service/ses` — Amazon SES (via SNS)
- `POST /webhooks/service/sendgrid` — SendGrid / Twilio Event Webhook
- `POST /webhooks/service/postmark` — Postmark bounce webhook
- `POST /webhooks/service/forwardemail` — Forward Email
- `POST /webhooks/service/lettermint` — Lettermint (added v6.1.0)
- `POST /webhooks/bounce` — generic/custom bounce endpoint (post your own bounce events, e.g. from reading a mailbox, DB, or mail server logs)

These are INBOUND-only (providers POST to Listmonk). Listmonk does NOT push outbound event webhooks (no "new subscriber" or "campaign sent" callbacks) — for outbound, poll the API or use HTTP messengers for messaging delivery.
