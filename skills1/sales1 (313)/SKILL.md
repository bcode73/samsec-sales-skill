...
5. **Bounce processing requires SMTP provider setup** — Configure SES SNS topics, SendGrid/Twilio Event Webhooks, Postmark, Forward Email, or Lettermint (v6.1.0+) webhooks to POST bounces to Listmonk's `/webhooks/service/{provider}` endpoint, or post custom bounces to the generic `/webhooks/bounce`. Without this, your sender reputation degrades silently.
...
8. **`campaigns:send` is a separate API permission (v6.1.0+)** — A granular API token with `campaigns:manage`/`campaigns:manage_all` can create and edit campaigns but CANNOT start a send. Starting a campaign (`PUT /api/campaigns/{id}/status` → `running`) requires the distinct `campaigns:send` scope, otherwise you get a 403. Grant both scopes for an automated send pipeline.
