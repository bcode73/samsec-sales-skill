...
2. Explains the `/recipients/add` endpoint — emailAddress required, fields for merge vars (accepts `addresses`, `listOfEmails`, or `csvData`; max 5,000 per campaign)
3. Warns about the quota-units-per-hour limit (`/recipients/add` costs 20 + N units) and the separate monthly recipient-add cap, and recommends batching/pacing
4. Shows how to check import status with `/recipients/addStatus`
**Result**: User knows the endpoint, required fields, and how to batch large imports within the quota-unit and monthly-recipient limits
