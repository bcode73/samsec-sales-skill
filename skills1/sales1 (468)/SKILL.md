...
2. **Which plan are you on?**
   - A) Founder ($29/mo — 1 website, 10 keywords, 3 competitors, 30 DMs/day, 1 seat — Email & Slack alerts, no webhooks)
   - B) Company ($79/mo — 3 websites, 30 keywords, 10 competitors, 100 DMs/day, 3 seats — adds Webhook notifications)
   - C) 7-Day Pass ($15 one-time, no subscription — everything in Founder for 7 days)
...
- **Keyword limits are hard caps per plan.** Founder gets 10 keywords only (Company gets 30). No overage option — upgrade or consolidate.
- **Webhooks require the Company plan ($79/mo).** The Founder plan ($29/mo) only has Email & Slack alerts. Webhook notifications unlock on Company. The API itself is documented separately from notification channels.
...
**Solution**: Be selective — only DM users from posts scored 70+. Focus DMs on users who explicitly asked for recommendations. Upgrade to Company ($79/mo, 100/day) if volume is justified. Consider that replies in threads are often more effective than DMs.
