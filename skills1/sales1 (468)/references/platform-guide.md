...
| Webhook alerts | Company plan only ($79/mo — Founder is Email & Slack only) |
| Competitor tracking | All plans (Founder 3, Company 10) |
| REST API (inbox read/update) | Bearer auth (pricing page does not list API per-tier; auth + endpoints documented) |
...
*Re-verified against https://redship.io/pricing on 2026-06-13. The earlier 3-tier structure (Starter $19 / Growth $39 / Professional $89) has been replaced by 2 recurring tiers plus a one-time pass.*

| Feature | Founder ($29/mo) | Company ($79/mo) | 7-Day Pass ($15 one-time) |
|---|---|---|---|
| Websites monitored | 1 | 3 | Everything in Founder, 7 days |
| Keywords | 10 | 30 | (Founder limits) |
| Competitors tracked | 3 | 10 | (Founder limits) |
| Auto DMs per day | 30 | 100 | (Founder limits) |
| Team seats | 1 | 3 | 1 |
...
| Webhook notifications | No | Yes | No |

- No free plan. A **$15 7-Day Pass** (one-time, no subscription) unlocks everything in Founder for 7 days.
- **Webhooks are Company-tier only** ($79/mo) — Founder is Email & Slack alerts only.
- The pricing page does not break out REST API access per tier; the API (Bearer auth, inbox read/update endpoints) is documented in the API reference. Treat per-tier API availability as unverified.
- Keyword limits are hard caps — no overage.
- Auto DM limits reset daily.

*Note: RedShip's docs nav references a "Creator plan" (`/docs/create` — Drafts, Calendar, Knowledge). This plan is not surfaced on the public pricing page, so its price and gating are unverified here.*
...
### Webhooks
- **Company plan only ($79/mo)** — Founder plan ($29/mo) does not include webhook notifications
...
|---|---|---|---|---|---|---|
| **RedShip** | $29/mo | Yes (0-100) | Yes (drafts) | No (DMs only) | Yes (REST) | Reddit |
...
- You need **API access** for custom integrations at an affordable price (from $29/mo)
