...
1. **API paths are version- and namespace-specific — use the exact paths, not intuitive guesses.** As re-verified 2026-06-13, the live OpenAPI uses `/v2/email-finder` and `/v2/email-verifier` (V2), `/public/v1/email/find|verify|lookup` (V1), `/public/v1/insights/contacts|companies|contact` for B2B database search, and the workspace CRUD lives under `/api/v1/contacts/contact/...`, `/api/v1/companies/company/...`, and `/api/v1/contacts/lists/...`. Auth is the `apiKey` request header. Create-contact bodies use a `properties` array of `{property,value}` items, not flat fields, and add-to-list uses `contactsIds`. The docs no longer carry an "under development" banner, but still build defensive error handling and pin to the versioned paths. See `references/getprospect-api-reference.md` for the full current surface.
