...
**Skill does**:
1. Confirms the gap (Step 1): checks the routing table in `skills/sales-do/SKILL.md` and `~/.claude/skills/`, finds no QBR skill, and summarizes the need, the closest existing skill, and the category (Strategy & Content).
2. Offers the three paths (Step 2); the user picks Build.
3. Delegates to `/skill-creator` if available, passing the repo conventions (naming, description with negative triggers, question-first body, gotchas section).
4. Writes `skills/qbr-prep/SKILL.md` plus an `evals/evals.json` with 2-3 realistic test cases and runs them.
5. Updates `skills/sales-do/SKILL.md` and `README.md`, branches, commits, pushes, and runs `gh pr create` against `sales-skills/sales`.
...
**Skill does**:
1. Confirms the gap and verifies it fits the sales/marketing/GTM domain.
2. The user chooses Request (Path B).
3. Runs `gh issue create` on `sales-skills/sales` with a filled-in problem statement, category, example use case, and 2-3 suggested trigger phrases.
...
**Skill does**:
1. Scans `~/.claude/skills/*/references/learnings.md`, skipping empty stubs and entries already marked `<!-- shared -->` or `<!-- declined -->`.
2. Presents each learning in a Generalizable? table and asks the user to confirm the classification.
3. Scrubs PII (company names, emails, account IDs, negotiated pricing) and shows before/after for approval.
4. Builds a URL-encoded GitHub new-issue link per skill and opens it in the browser for review — never auto-submitting via `gh`.
5. After the user confirms submission, appends `<!-- shared:YYYY-MM-DD -->` or `<!-- declined:YYYY-MM-DD -->` to each processed learning.
