...
- `startDate` — ISO 8601, earliest date to schedule (trimmed to start of day)
- `deadlineType` — `"HARD"` (must complete by due date), `"SOFT"` (flexible, **default**), or `"NONE"`
- `schedule` — Schedule name, defaults to `"Work Hours"` (must be `"Work Hours"` when scheduling for another user)
...
`customFieldValues` is a keyed record where each value carries a type discriminator. Documented types: `text`, `number`, `url`, `date`, `select`, `multiSelect`, `person`, `multiPerson`, `email`, `phone`, `checkbox`, `relatedTo`.
