...
- **Auth**: OAuth 2.0 (scopes: `public`, `private`, `write` — `write` requires PH approval) or Developer Token (create at producthunt.com/v2/oauth/applications). Pass as `Authorization: Bearer {token}`
- **Rate limit**: 6,250 complexity points per 15 minutes on GraphQL; 450 requests per 15 minutes on other `/v2/*` endpoints. Headers: `X-Rate-Limit-Limit`, `X-Rate-Limit-Remaining`, `X-Rate-Limit-Reset`; 429 when exceeded
