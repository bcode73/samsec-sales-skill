<!-- Source: https://github.com/engagebay/restapi (REST API) and https://github.com/engagebay/webhooks (Webhooks), plus https://www.engagebay.com/api -->
<!-- Captured verbatim 2026-06-28. EngageBay REST API + Webhooks reference. -->

# EngageBay API Reference

This file reproduces EngageBay's official REST API and Webhooks documentation verbatim
from the `engagebay/restapi` and `engagebay/webhooks` GitHub repositories, preceded by a
developer quick-start. Re-verify specifics against the live docs before relying on them.

## Developer quick-start (enrichment)

**Base URL:** `https://app.engagebay.com/`
**Auth:** Send your REST API Key in the `Authorization` HTTP header (no `Bearer` prefix).
Find it at **Account → Admin Settings → API → REST API Key**.
**Response format:** Defaults to **XML**. Add `Accept: application/json` on every request to get JSON.

Minimal authenticated GET (fetch one contact by id):
...
**Pagination:** List endpoints accept `page_size` (max 100) and `cursor`. The `cursor`
for the next page is returned inside the **last record** of the current result set; when no
`cursor` is present you have reached the end of the list. List contacts is a `POST` (not GET):
`POST dev/api/panel/subscribers`.

**Rate limits / quota:** A `429` means "too many requests in a given amount of time" —
EngageBay does not publish a per-second number; monthly API-call quota is plan-gated
(e.g. ~750,000 calls/month on the Pro All-in-One tier). Back off and retry on `429`.

**Error response shapes:**

```json
{ "error": "UnAuthenticated User", "status": "401" }
```
```json
{ "error": "Invalid input. Please provide all the required fields." }
```

**Webhooks are OUTBOUND only.** EngageBay does **not** accept incoming webhooks — to write
data INTO EngageBay you must call the REST API. Outbound webhooks POST a JSON body to your URL
on events like `contact.created`, `deal.created`, `email.open`. No HMAC signature is documented;
secure the receiver by other means (secret path, IP allowlist). See the Webhooks section below.

---

# Verbatim: EngageBay REST API (engagebay/restapi)
...
[
    {
        ...
        "recording_url": "https://api.twilio.com/2010-04-01/Accounts/ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx/Recordings/RE75bfd472d35dbafac8c98a8447c4fed0",
        "isManualEntry": false,
    ...
    },
    {
        ...
        "recording_url": "https://api.twilio.com/2010-04-01/Accounts/ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx/Recordings/RE7f36bd88fd1b096750f41aa3de71bcbf",
        "isManualEntry": false,
        "owner_id": 5675802922319872,
        "notes": [],
        "note": {
...
# Verbatim: EngageBay Webhooks (engagebay/webhooks)
