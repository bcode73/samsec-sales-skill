...
2. **How are you using ReplyAgent?**
   ...
   - B) Active — $79/mo subscription + pay-as-you-go ($4/comment, $8/post)
   - C) Enterprise plan
...
- **Moderators can still remove posts.** Managed accounts reduce ban risk to YOUR account, but subreddit moderators can remove any comment. A single removal check (24h for comments, 48h for posts) refunds 70% of the cost if the content was removed by then.
- **No webhooks or real-time notifications via API.** The API only supports importing posts, approving comments, and listing comments. No push notifications — rely on email digests for monitoring.
- **Costs add up.** A $79/mo (or $699/yr) subscription unlocks AI features, then it's $4/comment and $8/post pay-as-you-go. At 10 comments/day across subreddits expect ~$79 + ~$1,200/month. Budget carefully.
...
2. Shows Import Comment endpoint: POST /api/products/{productId}/ai-comments/manual-import with Bearer (sk_) auth and an `imports` array of `{url}` objects
...
4. Notes the removal-check refund returns 70% of credits if the comment is removed by the 24h check (48h for posts)
...
**Solution**: Review which subreddits remove comments most. Drop subreddits with strict self-promotion rules. Ensure comments lead with genuine help before any product mention. Reduce to 1-2 comments per subreddit per day. ReplyAgent's removal check (24h comments / 48h posts) refunds 70% of the cost for content removed by then.
