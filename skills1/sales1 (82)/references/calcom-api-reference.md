<!-- Source: https://cal.com/docs/api-reference/v2/introduction, https://cal.com/docs/developing/guides/automation/webhooks
     Re-verified 2026-06-13 against live docs (see references/learnings.md). -->
<!-- Sources: https://cal.com/docs/api-reference/v2/introduction,
     https://cal.com/docs/api-reference/v2/bookings/create-a-booking,
     https://cal.com/docs/api-reference/v2/v1-v2-differences,
...
### Required `cal-api-version` header (date-based versioning)
**Every v2 request requires a `cal-api-version` header.** Omitting it makes the
request default to an older endpoint version (and on some endpoints fails / 404s).
The value is **per-endpoint** and date-based:
- `POST /v2/bookings` (create a booking): `cal-api-version: 2026-02-25` (required)
- Most other v2 endpoints (slots, get bookings, reschedule, etc.): `cal-api-version: 2024-08-13`

Always check the per-endpoint docs for the exact date — Cal.com bumps these values
(e.g. bookings moved from `2024-08-13` to `2026-02-25`) and existing integrations
break silently when the default version changes. Migration guidance instructs adding
`cal-api-version: 2024-08-13` to all requests as a baseline.
<!-- Verified 2026-06-13 against live docs -->
...
```
GET    /v2/slots                 # Get available slots for an event type
```

Query params: identify the event by either `eventTypeId` **or** `username` +
`eventSlug`, plus `startTime`, `endTime`, and optionally `timeZone`.
Requires `cal-api-version: 2024-08-13`.

Example:
```
GET https://api.cal.com/v2/slots?username=bailey&eventSlug=15min&startTime=2026-04-28T00:00:00Z&endTime=2026-05-02T00:00:00Z
```
<!-- Path corrected 2026-06-13: live docs use GET /v2/slots, not /v2/slots/available -->
Also available: `/v2/slots/reserve` (reserve a slot) and reserved-slot lookups.
...
| `BOOKING_PAYMENT_INITIATED` | Payment started but not yet completed |
...
| `RECORDING_TRANSCRIPTION_GENERATED` | Cal Video transcription generated |
...
| `DELEGATION_CREDENTIAL_ERROR` | Domain-wide delegation credential error (structured payload for partner platforms) |

<!-- Webhook event list re-verified 2026-06-13 against live docs. PAYMENT_INITIATED is
     named BOOKING_PAYMENT_INITIATED in current docs. RECORDING_TRANSCRIPTION_GENERATED
     and DELEGATION_CREDENTIAL_ERROR are current events not in the original baseline. -->

**Seated event types:** for multi-seat bookings, the `attendees` array in the
webhook payload contains only the attendee for the specific seat that triggered
the webhook (not all seat-holders).
...
- Per-endpoint `cal-api-version` dates beyond bookings (`2026-02-25`) and the
  `2024-08-13` baseline were not exhaustively enumerated — always check the
  individual endpoint page for its required version date.
