...
# NOTE: every v2 request needs a date-based `cal-api-version` header. Creating a
# booking currently requires `cal-api-version: 2026-02-25`; most other endpoints
# use `2024-08-13`. Omitting it defaults to an older endpoint version.
...
# Create a booking (note the required version header)
...
headers = {
    ...
    "cal-api-version": "2026-02-25",  # required for POST /v2/bookings
}
...
# Endpoint is GET /v2/slots (not /v2/slots/available). Identify the event by
# eventTypeId OR username+eventSlug. Requires cal-api-version: 2024-08-13.

```bash
curl -s -H "Authorization: Bearer cal_your_api_key" \
  -H "cal-api-version: 2024-08-13" \
  "https://api.cal.com/v2/slots?eventTypeId=123456&startTime=2026-04-28T00:00:00Z&endTime=2026-05-02T00:00:00Z&timeZone=America/New_York" \
  | jq '.data'
```
