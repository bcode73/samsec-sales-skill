...
- Header: `payhip-api-key: YOUR_API_KEY_HERE` (used for the Coupon endpoints)
- Obtain from: https://payhip.com/settings/developer
- For license operations: `product-secret-key: YOUR_PRODUCT_SECRET_KEY_HERE` — found on the product's edit page in the section where license keys are enabled (not the developer settings page)
...
Payload includes: transaction_id, email, currency, price (in cents), items array (with product details), payment_type, VAT status, customer IP address, fees, timestamp.
...
The official API reference documents no rate limits, request throttling, or quotas (re-verified 2026-06-13). Don't assume an unlimited budget — implement backoff/retry defensively.

## API Limitations

The current API only supports coupons and license keys ("At the moment we only have support for interacting with the Coupon and License Key resources, but more will be added in future"). No endpoints exist for products, sales/transactions, or customers. For other automation, use Zapier triggers (New Sale, Sale Refund, New Membership Subscription) or webhooks.
