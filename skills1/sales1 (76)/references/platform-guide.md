...
> **Pricing note (verified 2026-06-13):** The official pricing page (buttondown.com/pricing) now presents pricing as **pay-as-you-grow by active subscriber count** — "Absolutely nothing for your first 100 subscribers," then a quote based on your subscriber count — rather than fixed named tiers. The specific per-tier prices above (Basic $9 / Standard $29 / Professional $79 / Advanced $139) are NOT individually confirmed on the current public page (it requires entering a subscriber count) and may be stale; the **free-for-first-100, à-la-carte add-ons, active-subscribers-only, and 50% non-profit discount** policies ARE confirmed current.
...
| Sponsorships | $9/mo | Sell/manage sponsor placements in emails |
...
### Webhooks
Custom webhook endpoints for programmatic integrations, managed via `POST /v1/webhooks` or the UI (Settings → Create webhook). A single webhook can subscribe to **multiple event types** (since 2024-10-29). Event types include `subscriber.created`, `subscriber.confirmed`, `subscriber.unsubscribed`, `subscriber.opened`, `subscriber.clicked`, `subscriber.deleted`, `email.created`, `email.sent`, `comment.created`. Payloads are JSON with top-level `event_type` and a nested `data` object. With a signing key set, requests carry an `X-Buttondown-Signature: sha256=<hmac>` header (HMAC-SHA256 over the raw body) — verify before trusting; do not IP-allowlist (no stable IPs). See `references/buttondown-api-reference.md`.

### API
Full REST API at `https://api.buttondown.com/v1/` — date-versioned via `X-API-Version` (current `2026-04-01`); rate limits 600 req/min (429 + `Retry-After`) and 100 subscriber-creates/day (400 → use bulk import). See `references/buttondown-api-reference.md` for details.

### CLI
Buttondown CLI for managing newsletters from the command line (offline edit in Markdown, Git version control, bidirectional sync). Install via npm: `npm install -g @buttondown/cli` (or `npx @buttondown/cli`). Commands: `buttondown login` / `pull` / `create` / `push`.
...
| Subscriber | id, email_address, tags[], metadata{}, type, creation_date | API field is `email_address` (not `email`) and `type` (not `subscriber_type`) on current API version. Types: regular, premium, gifted, unactivated |
...
- **Auth**: `Authorization: Token {your_api_key}` — manage keys at https://buttondown.com/keys (API → Keys); multiple granular keys supported since 2026-01-24
- **Context header** (multi-newsletter): `Buttondown-Context: {newsletter_name}`
- **Versioning**: `X-API-Version` header (current `2026-04-01`)
...
- **Rate limits**: 600 req/min (429 + `Retry-After`); subscriber creation 100/day (400 → use bulk import)
