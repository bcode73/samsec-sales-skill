...
4. **API is available on all plans including free** — unlike many competitors, even free-tier users can use the full REST API. Auth is `Authorization: Token {key}` with base URL `https://api.buttondown.com/v1/`. Manage keys at buttondown.com/keys (API → Keys; granular multiple keys supported). Rate limits: 600 req/min globally; subscriber creation is capped at 100/day (returns 400 — bulk-import larger lists). The create-subscriber body uses `email_address` (not `email`).
