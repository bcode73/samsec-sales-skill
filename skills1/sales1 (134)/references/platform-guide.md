...
- **Track API**: `https://track.customer.io/api/v1/` — Basic Auth (Site ID as username, API key as password), 1000 req/sec fair-use rate limit (applies to both v1 and v2)
  ...
  - v2: `POST /api/v2/entity` and `POST /api/v2/batch` — entity/batch operations
- **App API**: `https://api.customer.io/v1/` — Bearer token authentication, 10 req/sec rate limit (endpoints use the `/v1/` prefix, not `/v1/api/`)
...
- **API-triggered broadcasts**: `POST https://api.customer.io/v1/campaigns/{broadcast_id}/triggers` — 1 req/10sec
- **Data Pipelines (Pipelines/CDP) API**: `https://cdp.customer.io/v1/` — Basic Auth (write key as username, blank password), recommended for new setups, compatible with Segment tracking spec
- **EU region**: Use `track-eu.customer.io`, `api-eu.customer.io`, and `cdp-eu.customer.io` subdomains for EU data residency
