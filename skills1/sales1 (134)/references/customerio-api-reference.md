...
|---|---|---|---|---|
| Track API | Ingest customer data & events | `https://track.customer.io/api/v1/` | Basic Auth | 1000 req/sec |
| App API | Read campaigns, segments, customers | `https://api.customer.io/v1/` | Bearer token | 10 req/sec |
| Transactional API | Send transactional emails & push | `https://api.customer.io/v1/` | Bearer token | 100 req/sec |
| Data Pipelines (Pipelines/CDP) API | Structured data ingestion (newer) | `https://cdp.customer.io/v1/` | Basic Auth (write key as username, blank password) | — |

> **App API path note**: App API endpoints use the `/v1/` prefix directly (e.g. `https://api.customer.io/v1/campaigns`). The older `/v1/api/` path form is deprecated — current docs show `/v1/`.

**EU data center** — substitute the base URLs:
- Track API: `https://track-eu.customer.io/api/v1/`
- App API: `https://api-eu.customer.io/v1/`
- Pipelines/CDP API: `https://cdp-eu.customer.io/v1/`

> **Track API v2**: In addition to the v1 endpoints below, Customer.io offers v2 endpoints `POST /api/v2/entity` (identify/update people and objects, manage relationships) and `POST /api/v2/batch` (batched operations). The 1000 req/sec limit applies to both v1 and v2.
...
|---|---|---|
| Track API | 1000 req/sec | Fair-use; not strictly enforced (applies to both v1 and v2) |
| App API | 10 req/sec | Strictly enforced (most endpoints) |
| Transactional API (App API send endpoints) | 100 req/sec | Strictly enforced |
...
```
GET /v1/campaigns?limit=10&start=0
```
...
```bash
curl --request GET \
  --url "https://api.customer.io/v1/customers/user_123/attributes" \
  --header "Authorization: Bearer YOUR_APP_API_KEY"
```
...
#### 19. Trigger a Broadcast — `POST /v1/campaigns/{broadcast_id}/triggers`

Programmatically trigger a broadcast campaign (formerly called API-triggered broadcasts). This is an App API endpoint (Bearer token).

| Method | Endpoint | Description |
|---|---|---|
| POST | `/v1/campaigns/{broadcast_id}/triggers` | Trigger a broadcast |
| GET | `/v1/campaigns/{broadcast_id}/triggers/{trigger_id}` | Get the status of a triggered broadcast |
...
```bash
curl --request POST \
  --url "https://api.customer.io/v1/campaigns/5/triggers" \
  --header "Authorization: Bearer YOUR_APP_API_KEY" \
  --header "Content-Type: application/json" \
  --data '{
    "data": {
...
The Data Pipelines API (also called the Pipelines / CDP API) is Customer.io's newer data ingestion layer, similar in design to Segment's API. It provides a structured approach to sending customer data through configured sources and destinations.

- **Base URL**: `https://cdp.customer.io/v1/` (US), `https://cdp-eu.customer.io/v1/` (EU)
- **Auth**: HTTP Basic Auth — your source's **API/write key** as the username and a **blank password**.
- **Methods**: POST only — there are no DELETE operations; deletions and suppressions are performed via semantic events instead.

Key operations:
- **Identify** — create or update a person and assign `traits`
- **Track** — record an event / custom behavior
- **Page** — record a pageview
- **Screen** — record a mobile screenview
- **Batch** — send multiple operations in a single call
- **Objects & Relationships** — create objects and relate them to people
- **Merge** — consolidate duplicate person records

> **Note**: The Data Pipelines API is actively evolving. Refer to https://docs.customer.io/integrations/api/cdp/ for the latest endpoints and schema.

---

## Webhook Events (Reporting Webhooks)

Customer.io sends webhook events to report on message delivery status. Configure reporting webhooks in Settings > Webhooks. Reporting webhooks cover these top-level categories: **Customer events**, **Email events**, **SMS events**, **Push notification events**, **In-app message events**, **Slack message events**, **Webhook action events**, and **Subscription events**.

### Webhook Signature Verification

Each webhook request includes two headers you use to verify authenticity:

| Header | Purpose |
|---|---|
| `X-CIO-Signature` | HMAC-SHA256 hash of the request |
| `X-CIO-Timestamp` | Request timestamp |

To verify: build the string `v0:<X-CIO-Timestamp>:<raw request body>`, compute an HMAC-SHA256 of that string using your **webhook signing secret** (found on the webhook/Email Activity Webhook integration page in account settings), and compare the result to `X-CIO-Signature`. Failed deliveries retry with exponential backoff for up to 7 days; requests time out after 4 seconds.

### Customer Events

| Event | Description |
|---|---|
| `customer_subscribed` | Customer's subscription/attribute state changed (subscribed) |
...
| `email_converted` | Recipient completed the configured conversion after the email |
| `email_bounced` | Email bounced (hard or soft) |
| `email_dropped` | Email was dropped before sending |
| `email_spammed` | Recipient reported the email as spam (spam/complaint) |
...
| Event | Description |
|---|---|
| `in_app_sent` | In-app message was sent |
...
| Event | Description |
|---|---|
| `slack_sent` | Slack message was sent |
| `slack_clicked` | A link in the Slack message was clicked |
| `slack_failed` | Slack message failed |

### Webhook Action & Subscription Events

Webhook-action events report on the outcome of a journey **Send Webhook** action (`webhook_sent`, `webhook_clicked`, `webhook_failed`). Subscription events report changes to a person's subscription state via the Subscription Center. Refer to https://docs.customer.io/api/webhooks/ for the exact event names and payload schemas, which are versioned.
