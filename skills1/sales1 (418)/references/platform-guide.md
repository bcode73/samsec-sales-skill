...
- **Deal**: sales opportunity or contract tied to a customer (distinct API model from Opportunity; drives the revenue pipeline)
- **Product**: catalog of sellable products/services (referenced by Line Items and Licenses)
- **Line Item**: individual product line on a Deal/License (quantity, price, product reference)
...
### ID system
- Every object has a Planhat internal `_id`, plus optional `sourceId` (a connector/CRM-assigned ID) and `externalId` (your own system's ID)
- "Keyables" determine uniqueness with match priority **`_id` > `sourceId` > `externalId`** (Enduser also accepts `email`) — if a keyable matches, update; if not, create new
- Get a record by external identifier using URL prefixes: `extid-{externalId}` or `srcid-{sourceId}`
