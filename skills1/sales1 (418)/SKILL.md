...
- **Check rate limits.** 200 calls/min soft limit. If you hit `429`, implement exponential backoff. Use bulk upsert (`PUT /{model}` with an array body, up to 5,000 items) to reduce call volume.
- **Check object limits.** GET list `limit` defaults to 100; max is 5,000 for companies, 2,000 for most other models. Use `limit`/`offset` pagination for larger datasets.
