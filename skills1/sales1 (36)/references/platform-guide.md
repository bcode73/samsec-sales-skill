...
- **MCP Server**: mcp.attio.com/mcp — hosted server; connect `https://mcp.attio.com/mcp` as a remote MCP server with OAuth; reads auto-approved, writes require confirmation
- **App SDK**: Official SDK to embed custom React/TypeScript apps within Attio (not a REST client)
...
| Webhooks | Record, list, list-entry, list/object-attribute, note, note-content, comment, task, workspace-member, call-recording events; signed with `Attio-Signature` (SHA256 HMAC) |
| SDKs | Official App SDK (TypeScript/React, for embedded apps); no official REST client SDKs — call REST directly or use community `attio-js` |
| OpenAPI spec | Available (see docs.attio.com/rest-api/endpoint-reference/openapi) |
