...
- FeatureOS / Hellonext feedback boards + roadmap + changelog → `/sales-featureos {your question}`
- Frill feedback boards + roadmap + changelog → `/sales-frill {your question}`
- Sleekplan feedback boards + roadmap + changelog (AI auto-categorization, MCP server) → `/sales-sleekplan {your question}`
- FeedBear budget feedback boards + roadmap + changelog (no pull API — webhooks/JWT SSO/Zapier) → `/sales-feedbear {your question}`
- UserJot AI-first feedback boards + roadmap + AI changelog (no per-seat pricing; REST API, webhooks, MCP server) → `/sales-userjot {your question}`
- Rapidr feedback boards + roadmap + changelog (Canny/Productboard alternative; NO REST API — JWT SSO/Zapier/native Slack/Jira/HubSpot) → `/sales-rapidr {your question}`
- Quickhunt all-in-one feedback boards + roadmap + changelog + in-app messages + knowledge base + AI assistant + live chat (Canny/Frill alternative; free lifetime tier; API Premium-only & undocumented, integrations Growth-tier — Zapier/viaSocket/Slack/GitHub/Jira/HubSpot) → `/sales-quickhunt {your question}`
- Productlane feedback portal + public roadmap + changelog **built exclusively on Linear** (AI-native support inbox + AI Agent; Canny/Cycle/Productboard alternative for Linear teams; documented REST API at `productlane.com/api/v1` Bearer, public no-auth `/feedback`, Zapier Create-Note, no MCP; per-user, no free tier) → `/sales-productlane {your question}`
- Userback visual feedback + bug-reporting (annotated screenshots/video, auto-captured console/network logs, session replay) + feature boards/roadmap/Feature Portal; REST API + webhooks + MCP server **gated to Business Plus**, JS/React/Vue/mobile SDKs, unsigned webhooks → `/sales-userback {your question}`
...
- `/sales-noisely` — Noisely platform help — AI feedback aggregation across 23+ sources (reviews/social/support), AI categorization + sentiment + urgency, clustered action items, webhooks/Linear/Jira/Sheets; affordable alternative to enterprise VoC tools. Install: `npx skills add sales-skills/sales --skill sales-noisely`
- `/sales-frill` — Frill platform help — affordable Canny/Nolt/UserVoice alternative: feedback boards (voting/comments), public roadmap, changelog announcements, surveys; REST API (cursor pagination) + HMAC-SHA256-signed webhooks. Install: `npx skills add sales-skills/sales --skill sales-frill`
- `/sales-featureos` — FeatureOS (formerly Hellonext) platform help — feedback boards (feature_requests + voting), roadmap, changelog, knowledge base; REST API v3 (API-KEY header, page/per_page), OAuth 2.0 apps, webhooks (post/changelog events, auto-disable on failure), HellonextWidget embed with SSO. Install: `npx skills add sales-skills/sales --skill sales-featureos`
- `/sales-feedbear` — FeedBear platform help — budget feedback boards + roadmap + changelog + AI replies (Canny/Nolt/Frill alternative, $19–$299/mo, no free tier). No pull/REST API — automation is outbound webhooks (no documented HMAC), JWT SSO (`/sso/auth?payload=`), Zapier, and native Intercom/Slack/Trello/Jira. Pick it for cheap push-only feedback; pick Frill/Sleekplan/FeatureOS when you need to read data via an API. Install: `npx skills add sales-skills/sales --skill sales-feedbear`
- `/sales-sleekplan` — Sleekplan platform help — AI customer-feedback suite (Canny/Frill alternative): feedback boards + roadmap + changelog + NPS/CSAT, AI auto-categorization/duplicate merging, and an MCP server for Claude/Cursor; REST API (Bearer, in-app spec), webhooks (secret GET-param, NOT HMAC-signed), JWT SSO (HS256), $sleek widget SDK. Install: `npx skills add sales-skills/sales --skill sales-sleekplan`
- `/sales-userjot` — UserJot platform help — AI-first feedback boards + roadmap + AI changelog (Canny/Productboard alternative, no per-seat pricing): unlimited posts/users on every tier, AI duplicate detection + auto-categorization; REST API (`api.userjot.com/v1`, Bearer; posts via `/requests`, idempotency on `/requests`+`/changelogs`), webhooks, an MCP server (Claude Code/Cursor/Codex/ChatGPT/Windsurf), Widget SDK (`identify`) + Swift SDK. Integrations plan-gated (Free none / Starter 1 / Pro unlimited). Install: `npx skills add sales-skills/sales --skill sales-userjot`
- `/sales-rapidr` — Rapidr platform help — product-feedback & feature-request suite (Canny/Productboard/Frill alternative): public/private boards (voting, proxy voting, duplicate merging, segmentation), public roadmap, changelog, embeddable widget, custom-domain portal. **No public REST/pull API** — automation is JWT SSO (Business+; mint token server-side, redirect to SSO endpoint), Zapier (realtime triggers + create/update actions), native Slack (`/rapidr`)/Intercom/Jira/HubSpot, and voter CSV export. Pricing Startup $49 / Business $199 (SSO + segmentation cliff) / Enterprise; no free tier. Pick it for polished no-code intake; pick Frill/UserJot when you need an API. Install: `npx skills add sales-skills/sales --skill sales-rapidr`
- `/sales-quickhunt` — Quickhunt platform help — affordable all-in-one feedback suite (Canny/Frill/Featurebase/Upvoty alternative): feedback boards + roadmap + changelog + in-app messages (surveys/checklists/banners) + knowledge base + AI assistant + live chat. **Genuinely free lifetime tier.** Developer surface is the most restrictive of the affordable boards: **REST API is Premium-only ($99/mo) and undocumented**, integrations (Slack/Zapier/viaSocket/ClickUp/GitHub/HubSpot/Jira) are **Growth-tier ($49)** — Free/Starter have none — and there's no MCP server; webhooks exist but have no published schema. Pricing Free / Starter $29 / Growth $49 / Premium $99; +$10/mo per extra seat. Pick it for a free start + no-code all-in-one; pick Frill/UserJot when you need a documented API at a lower price. Install: `npx skills add sales-skills/sales --skill sales-quickhunt`
- `/sales-productlane` — Productlane platform help — AI-native customer support + product-feedback suite **built exclusively on Linear**: omnichannel inbox (email/live chat/Slack/Teams/Discord), AI Agent, self-updating help center, feedback portal, public roadmap (= Linear projects), and a changelog whose Release Intelligence auto-drafts release notes from completed Linear issues. A Canny/Cycle/Productboard alternative **only for teams that run engineering in Linear**. Unlike Rapidr/Quickhunt it ships a **documented REST API** (`productlane.com/api/v1`, `Authorization: Bearer`) for Companies/Customers/Insights/Portal — with a **public no-auth `POST /feedback`** for widgets — plus a Zapier "Create Note" action; **no MCP server**, and its own webhooks aren't documented. Per-user pricing, **no free tier** (Starter ~$15 / Pro ~$29 / Scale ~$79). Pick it when feedback→engineering lives in Linear; pick Frill/UserJot when you need a standalone board + API without Linear. Install: `npx skills add sales-skills/sales --skill sales-productlane`
- `/sales-fourfour` — Four/Four platform help — AI voice-of-customer copilot that mines conversations (calls/tickets/emails/chat) into Insights/Topics with an AI Analyst; OData REST API + CRM Importer API + HMAC-SHA256 webhooks + MCP server. Use it when you already have rich conversation data and want it queryable, vs soliciting structured surveys. Install: `npx skills add sales-skills/sales --skill sales-fourfour`
- `/sales-userback` — Userback platform help — visual customer-feedback + bug-reporting (in-app widget with annotated screenshots/video, auto-captured console/network logs, session replay) plus feature boards + roadmap + Feature Portal + surveys. REST API (`rest.userback.io/1.0`, Bearer) + webhooks + a ~12-tool MCP server (OAuth) all **gated to Business Plus**; JS/React/Vue/Next/mobile SDKs; webhooks are **unsigned** (no HMAC). Pick it when bug-reproduction context (logs + replay) matters and you want voting in the same tool; pick a board-first tool (Frill/UserJot) for pure feature voting. Install: `npx skills add sales-skills/sales --skill sales-userback`
