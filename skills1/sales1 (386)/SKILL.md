...
**For API integration**: Map endpoints to the user's goal. Memories for AI-generated insights, Conversations for raw transcripts, Action Items for to-dos, Folders for organization. Use `omi_dev_` keys for REST; `omi_mcp_` keys are for the MCP server only.

**For MCP / Claude or Cursor access**: Point to the hosted MCP server at `https://api.omi.me/v1/mcp/sse` (SSE) authed with an `omi_mcp_` key from Settings → Developer → MCP. Tools: get_memories, create_memory, edit_memory, delete_memory, get_conversations, get_conversation_by_id.
