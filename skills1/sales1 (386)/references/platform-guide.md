...
Two key types: **Developer API keys** (`omi_dev_...`) authenticate REST calls; **MCP keys** (`omi_mcp_...`) authenticate MCP clients against `/v1/mcp/sse` only and do NOT work for REST.
...
### MCP Server
Omi ships an official hosted MCP (Model Context Protocol) server so AI assistants can read/write Omi data directly:
- **URL**: `https://api.omi.me/v1/mcp/sse` (SSE transport)
- **Auth**: `omi_mcp_...` key from Omi app → Settings → Developer → MCP (MCP keys do NOT work for REST calls — those need `omi_dev_...`)
- **Clients**: Claude Desktop, Cursor, Poke, or any MCP-compatible tool
- **Tools**: `get_memories`, `create_memory`, `edit_memory`, `delete_memory`, `get_conversations`, `get_conversation_by_id`
