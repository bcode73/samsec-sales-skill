...
| GET | `/v1/meetings/{id}/insights` | Get conversation-intelligence KPIs (talk time, themes, improvement tips); 204 for pre-Dec-2024 meetings |
| POST | `/v1/meetings/{id}/download` | Get temporary download URL |
| POST | `/v1/upload` | Submit a recording for analysis (body: `download_url` to a public file) |
| GET | `/v1/teams` | List teams |
| GET | `/v1/teams/{id}/meetings` | List team meetings |

`GET /v1/meetings` paginates with `limit` (max 500) + `cursor`.

### Rate limits
Free & Pro: 100 requests/day, 10 uploads/day. Business & Enterprise: 100 requests/minute, 10 uploads/minute.

### Webhooks
Configure a webhook URL at Integrations → Public API. Two events fire as POST: success (`{"message":"File analyzed successfully","meeting_id":"<uuid>"}`) and failure (`{"message":"File analyzed failed"}`, no meeting_id). Respond 200 OK with empty body; up to 3 attempts (initial + 2 retries). Each request carries an `X-MG-Signature` header = HMAC SHA-256 of the raw body using your configured secret — verify with a constant-time comparison.

### MCP Server
- Cloud MCP: hosted at `https://mcp.meetgeek.ai/mcp`; connect via Claude Connectors / ChatGPT Apps, OAuth at `auth.meetgeek.ai`
- Local MCP: `npm install -g @meetgeek/mcp-server` (command `meetgeek-mcp`), uses your API key
