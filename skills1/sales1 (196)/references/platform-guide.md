...
> **Lead-delivery webhooks (different from intent sync)**: G2 also pushes *leads captured on your G2 product profile page* into your CRM via lead-form webhooks — HubSpot via the Forms API (`forms.hubspot.com/uploads/form/v2/:portal_id/:form_guid`) and Salesforce via Web-to-Lead (`webto.salesforce.com/servlet/servlet.WebToLead`, using your 15-digit `oid`). These carry lead-form fields, not API intent/review payloads, and the docs do not document HMAC signing for them.
...
### G2 MCP Server
G2 offers an official MCP (Model Context Protocol) server for connecting G2 data to AI assistants (Claude, Cursor, etc.).
- **Auth**: OAuth — connecting opens a browser to sign in to your G2 account and authorize; requires a valid G2 account with permissions and products registered on G2.
- **Datasets**: Buyer Intent Signals, Product Reviews, Product Catalog, Category Taxonomy (plus competitive signals, buying stage, activity level, sentiment).
- **Setup in Claude**: `+` below the chat box → Connectors → Manage Connectors → search/select **G2 MCP** → complete OAuth → new chat with G2 MCP enabled.
- **Rate limit**: shares the API global limit (100 req/s, then 60-second block).
- Partner AI integrations: HubSpot Breeze Agents, Gong AI workflows, AirOps Playbooks (active G2 subscription required).
...
- **Buyer Intent signals**: company-level only (no contacts/emails). Data-dictionary fields include `product_id`, `product_name`, `category_id`, `company_id`, `company_name`, `company_domain`, `visit_url`, `visit_title`, `visit_type`, `visit_date`, `visitor_region`, `visitor_country`, `company_state`, `company_country`, `company_employees`, `org_employees_range`, `company_sector`, `company_industry`, `total_page_views`, `last_seen`, `activity_level`, `buying_stage` (Source: documentation.g2.com/docs/buyer-intent-data-reference)
...
### v2 API (current — docs front this version)
- OLAP-style query language with dimensions, measures, and filters
- Supports cursor-based pagination
- OAuth2 scopes (current docs example): `openid profile products.read products.reviews.read` (scopes must match the token's enabled permissions). Earlier docs cited `buyer_intent.read` / `screenshots.read`; confirm the exact scope string per endpoint in the live reference.
- Test endpoint: `GET /api/v2/products`
- Endpoints: Buyer Intent (sandbox + production), Screenshots, Product Features, Categories, Market Signals, Credit Account
- Free tier: ~1,000 API calls/month + sandbox (per partner.g2.com/developer)
