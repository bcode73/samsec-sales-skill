...
**Important (re-verified 2026-06-13)**: Clearbit was acquired by HubSpot (Dec 2023). The standalone API still works **for legacy accounts**, but it is in wind-down: SDKs are deprecated, product investment has shifted to Breeze Intelligence (HubSpot), and **free Clearbit accounts are no longer being created**. New customers (2024+) are directed to Breeze Intelligence inside HubSpot rather than standalone API keys; only legacy accounts retain standalone API access. Recent sunsets: **Logo API (Dec 8, 2025)**, free tools incl. free Risk/Name-to-Domain access and Clearbit Connect (Apr 30, 2025), and the Autocomplete `logo` field now returns null (Sep 9, 2025). The endpoints and auth documented here are for the standalone API as used by legacy accounts.
...
**Access (2026-06)**: Now a **customer-only API requiring a valid API key** — the free implementation was discontinued in the April 30, 2025 free-tools sunset.
...
> Note: the `logo` URL points at `logo.clearbit.com`, which was **sunset Dec 8, 2025** — that URL no longer resolves.
...
**Access (2026-06)**: Now a **customer-only API requiring a valid API key** — the previously free implementation was discontinued in the April 30, 2025 free-tools sunset.
...
### Logo API — SUNSET (no longer available)

```
GET https://logo.clearbit.com/{domain}   ← DISCONTINUED
```

**The public Logo API was sunset on December 8, 2025.** Per the official HubSpot developer changelog: "The API will shut down on December 8, 2025. After the service is shut down, requests to logo.clearbit.com will fail to connect and no logos will be returned." HubSpot recommends migrating to **logo.dev** as a replacement. Do not build new integrations against `logo.clearbit.com` — it returns connection failures.
...
Returns company name suggestions with domain. Still free, no API key required. **However, since September 9, 2025 the `logo` field in Autocomplete responses returns `null`** (the underlying Logo service was sunset). Use Autocomplete for name/domain typeahead only — do not rely on the logo field.
...
- **Logo API sunset (Dec 8, 2025)**: `logo.clearbit.com` no longer resolves — migrate to logo.dev. The Autocomplete `logo` field has returned null since Sep 9, 2025.
- **Free access removed**: The April 30, 2025 free-tools sunset ended free implementations of the Risk API and Name to Domain API (now customer-only, API-key required) and removed the free Clearbit Platform, Clearbit Connect, Weekly Visitor Report, TAM Calculator, and free Slack integration. Free Clearbit accounts are no longer being created.
- **Standalone API in wind-down**: Post-HubSpot acquisition, new accounts (2024+) are steered to Breeze Intelligence inside HubSpot rather than standalone API keys; only legacy accounts retain standalone API access. Breeze Intelligence is the go-forward product. (Rate limit of 600 req/min and HTTP Basic / Bearer auth remain as documented for legacy standalone usage — not contradicted by current official statements.)
