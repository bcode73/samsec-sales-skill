...
| paymentProvider | string | Payment provider used for revenue verification (stripe, lemonsqueezy, polar, revenuecat, dodopayment, paddle, superwall, creem, whop). Whop is the newest provider shown on /acquire; the API enum string is unverified but follows the lowercase pattern. |
