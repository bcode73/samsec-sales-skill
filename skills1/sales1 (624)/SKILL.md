...
3. **What's your payment provider?**
   - Stripe / LemonSqueezy / Polar / Paddle / DodoPayment / RevenueCat / Superwall / Creem / Whop / Other
...
- **Revenue verification is mandatory — no exceptions.** Every listing requires a read-only API key from one of the 9 supported payment providers (Stripe, LemonSqueezy, Polar, Paddle, DodoPayment, RevenueCat, Superwall, Creem, Whop). If you're on a payment provider not supported (Square, Braintree, custom Stripe Connect), you can't list until that changes.
