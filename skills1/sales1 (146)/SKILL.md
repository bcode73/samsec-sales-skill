...
**API development**: API base URL is `https://dialpad.com/api/v2/` (sandbox: `https://sandbox.dialpad.com/api/v2/`); auth via `Authorization: Bearer {token}`. Use the Python SDK (`pip install python-dialpad`, import `from dialpad import DialpadClient`, pass `token=`) for quick prototyping. Stats API is async — POST to start a report, wait 15-20s, then GET the result. Rate limit is 20 req/sec per company. Webhook events are JWTs signed with a shared secret (HS256) — verify the signature. Download the OpenAPI spec from `developers.dialpad.com/reference/download-api-specification` for Postman.
