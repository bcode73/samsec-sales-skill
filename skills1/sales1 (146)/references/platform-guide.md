...
**Base URL**: `https://dialpad.com/api/v2/` (production) / `https://sandbox.dialpad.com/api/v2/` (sandbox)
**Auth**: Manual API Key (Admin Settings > My Company > Authentication > API Keys) or OAuth 2.0. Pass as `Authorization: Bearer {token}` (recommended) or `apikey=<token>` query param (discouraged).
**OpenAPI spec**: download from `developers.dialpad.com/reference/download-api-specification`; AI-agent index at `developers.dialpad.com/llms.txt`
**Python SDK**: `pip install python-dialpad` — import `from dialpad import DialpadClient`, instantiate with `token=` (and `sandbox=True` for sandbox) (github.com/dialpad/dialpad-python-sdk)
...
- **Event Subscriptions**: Real-time webhooks for call, SMS, contact, agent-status, and change-log events. Events are sent as JWTs signed with a shared secret (HS256) — the secret is separate from the API key; verify the signature to confirm origin. Plain JSON is sent if no secret is set. Limits: 100 subscriptions/type per company (10/type when targeting the company).
...
**Rate limits** (published at developers.dialpad.com/docs/rate-limits): Global 20 requests/second per company. Per-endpoint: create contacts 100/min, initiate calls 5/min per user target, send SMS 100/min, callbacks 5/min (number) / 100/min (call center) / 1000/min (company), update callrouter 1 per 5 min. Retry on 429 with exponential backoff. Don't repeat identical POST requests.
