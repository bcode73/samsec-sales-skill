...
- CRUD via API: `GET/POST/PATCH/DELETE /contexts` (also `GET /contexts/{id}`)
- Contexts persist across sessions
- Free tier is limited to 1 session context; Pro/Lifetime unlimited
...
### Wearables
- **OMI** — wearable that streams live transcripts into Hedy sessions; works in the iOS, Android, macOS, and Windows apps.
...
- **Auth**: OAuth 2.1 with PKCE (browser sign-in, no static API key)
- **Compatible clients**: Claude Desktop, Claude Code, Cursor, Windsurf, Zed, Cline, Perplexity (paid), plus web-based platforms; `mcp-remote` bridge for clients needing a local server
...
### Data export
- REST API (on-demand pull) — US host `https://api.hedy.bot`, EU host `https://eu-api.hedy.bot` for EU data residency; cursor pagination via `limit` (default 50, max 100) + `after`
