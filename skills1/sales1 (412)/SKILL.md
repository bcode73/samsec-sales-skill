...
- **Pay Per Call (PPC)** — pay per qualified inbound call (Ringba call-tracking integration)
...
- **Minimum payout**: $50 (verified in the partner quick-start guide)
- **Payment methods**: ACH, PayPal, Check, Wire, **Payoneer**, **Capitalist** (W9/W8 tax forms required; contact your Account Manager for alternatives)
- **Payment schedule**: Net-30 standard; weekly available for qualifying partners *(not confirmed on a current official page — verify with your Account Manager)*
- **Commission split**: Affiliates typically retain 70-80% depending on offer terms and traffic volume *(not confirmed on a current official page — verify with your Account Manager)*
- **Referral program**: Partners earn **up to an additional 5%** in commissions once a referred partner starts generating revenue; marketers earn **up to an additional 3%** on referred advertiser spend for an entire year
...
Also integrates with **Forensiq** (fraud), **Optizmo** (suppression/compliance), and **Ringba** (call tracking / Pay Per Call).
...
1. Get your API Key and Account ID from the platform — click the three vertical dots next to your name (top-right)
2. Call the single base endpoint `https://login.performcb.com/api/v7/performcb_api` with `get=findMyApprovedCampaigns` to discover available offers. Auth params are `api_key` and **`user_id`** (the `user_id` param carries your Account ID value — NOT a param literally named `account_id`)
3. Filter campaigns with `status`, `campaign_type` (e.g. `cpi,cpe`), `trackinglink_traffic_type`, and shape the response with `contain[]`
4. Generate tracking links programmatically; append sub-IDs (`subid1`–`subid5`, `creative_id`) or app params (`source_id`, `google_aid`, `ios_ifa`)
5. Pull reports and suppression lists via the same API; the API can also pause campaigns and change rates
6. Set up postback URLs for real-time conversion notifications and whitelist Perform[cb]'s postback IP `52.4.177.89`
