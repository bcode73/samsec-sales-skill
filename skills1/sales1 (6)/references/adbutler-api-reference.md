...
Verified 2026-06-13: data rows can include `responses`, `eCPC`, and `eCPA` in addition to the impressions/clicks/eCPM/revenue metrics; `meta` carries report `type`, `period`, date range, and `timezone`.

### Pagination

Offset-based: use `limit` and `offset` parameters. Default `limit` is **10** records per request. The response includes a top-level `has_more` boolean that is `true` when more filtered records exist beyond the current page (verified 2026-06-13).
...
| `_abdk[]` | No | Data keys for targeting (GET, named keys, e.g. `_abdk[bird]=duck;_abdk[bug]=ant`) |
| `_abdk_json` | No | Data keys for targeting (POST, JSON object, e.g. `"_abdk_json": {"bird": "duck"}`) |
| `click_mode` | No | Set to `click_mode=callback` for callback-based click tracking (verified 2026-06-13: the parameter name uses an underscore — `click_mode`, not `clickmode`) |
...
- `accupixel_url` — **only present when AccuPixel is enabled** in your AdButler settings. When this field is present it **must** be rendered/called as a pixel for AdButler to track the impression. If AccuPixel is disabled, AdButler tracks the impression when it sends the JSON response and this field is omitted. (Verified 2026-06-13.)
...
**GitHub:** https://github.com/adbutler/mcp-server (the org repo is described as "MCP server for the AdButler ad management API — 36 tools for AI assistants", verified 2026-06-13)
...
|---|---|---|
| PHP | `github.com/adbutler/adbutler-php` | Management API bindings (repo **archived** as of 2026-06-13) |
