---
name: sales-syften
description: "Syften platform help — AI-filtered keyword monitoring across Reddit, Hacker News, Bluesky, Mastodon, GitHub, Stack Exchange, Dev.to, Lobste.rs, Indie Hackers, Product Hunt, Slack communities, newsletters, podcasts (X/Twitter and YouTube are paid add-ons). Sub-minute Reddit alerts, Boolean search, AI noise filtering, tag-based routing, Slack integration, POST REST API at /api/0.0 (Standard+), webhooks + MCP server (PRO). Use when Syften keyword alerts return too much noise and you need to tune filters or exclusions, you're not getting alerts fast enough and need to understand monitoring latency, AI filtering is suppressing matches you actually want, you want to push Syften matches to your CRM or dashboard via API or webhooks, tag routing isn't sending results to the right Slack channel, or you need a GummySearch replacement covering Reddit plus other developer communities. Do NOT use for social listening strategy across tools or choosing between social listening platforms (use /sales-social-listening)."
...
2. **Which plan are you on?**
   - A) Entry ($19.95/mo — 3 filters, 100/day, 7-day archive)
   - B) Standard ($39.95/mo — 20 filters, 200/day, API, Slack, AI filtering, 1-month archive)
   - C) PRO ($99.95/mo — 100 filters, 500/day, webhooks, MCP server, unlimited archive)
   - D) Tailor Made / not sure
...
- **Integration**: API on Standard+ (`POST https://syften.com/api/0.0/items/get`), webhooks + MCP server on PRO only. For Standard users needing push delivery, poll `items/get` on a schedule or wire Zapier via the token under Setup → API and Zapier
...
- **X/Twitter and YouTube are paid add-ons.** They are NOT in the base plan — they're paid add-ons available on Standard and PRO. Exact add-on prices aren't published (listed only as "Paid addon").
- **API requires Standard plan ($19.95/$39.95/$99.95 USD).** Entry plan has no API access. API base URL is `https://syften.com/api/0.0` and every endpoint is POST (`/items/get`, `/filters/get`, `/filters/set`, `/settings/get`, `/info/get`). Webhooks and the MCP server (`https://syften.com/mcp`) require PRO ($99.95/mo).
- **Webhook payload is a JSON ARRAY of item objects** — not a single wrapped event. Return any 2xx to ack. Failed deliveries retry for up to 48h (5-min min backoff, up to 12h between attempts); invalid URLs are skipped, not retried. No documented HMAC signature.
- **AI verdict is `item.analysis.accept` (bool).** There is no `ai_verdict`/`ai_confidence`; the source is `item.backend`, the link is `item.item_url`.
- **`filters/set` replaces ALL filters.** There's no per-filter CRUD endpoint — `filters/get`, edit the array, `filters/set` the whole list.
...
- **Archive retention is plan-gated.** Entry: 7 days, Standard: 1 month, PRO: unlimited.
