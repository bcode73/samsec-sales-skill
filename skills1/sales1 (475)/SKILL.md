...
7. **API docs are public, but credentials are not** — the Reporting API and Event Tracking / Event Data Forwarding articles at help.remerge.io are publicly readable (no login to view the docs). However, obtaining a Reporting API Key + Customer ID, or an Event Tracking partner `key`, still requires an active Remerge account — ask your Remerge Account Manager. The Reporting API base is `api.remerge.io` (sign in to get an auth token), retains data for 6 months, and must be queried one day at a time.
