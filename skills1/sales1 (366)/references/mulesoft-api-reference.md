...
## Runtime Manager API (on-prem / hybrid servers only)

**Base URL**: `https://anypoint.mulesoft.com/hybrid/api/v1`

> **IMPORTANT (verified 2026-06-13)**: Per the official docs, "The Runtime Manager REST API manages only on-prem applications deployed to your servers." It is for registering servers/server groups/clusters and deploying apps to **your own on-prem Mule runtimes**. It does **not** manage CloudHub or CloudHub 2.0 apps. Use the **CloudHub API** (CloudHub 1.0) or the **Application Manager API** (CloudHub 2.0) below for cloud-deployed apps.
...
## CloudHub API (CloudHub 1.0 apps)

**Base URL**: `https://anypoint.mulesoft.com/cloudhub/api`

Per the official docs, "The CloudHub API manages only applications deployed to CloudHub, the cloud-based version of Runtime Manager." Use this — not the Runtime Manager (`hybrid`) API — to create, deploy, start, stop, list, and delete CloudHub 1.0 apps. Same `Authorization: Bearer`, `X-ANYPNT-ENV-ID`, and `X-ANYPNT-ORG-ID` headers as above.

| Operation | Method | Endpoint |
|-----------|--------|----------|
| List applications | GET | `/applications` |
| Get application | GET | `/applications/{domain}` |
| Create / deploy application | POST | `/applications` |
| Update application | PUT | `/applications/{domain}` |
| Delete application | DELETE | `/applications/{domain}` |
| Start / stop / restart | POST | `/applications/{domain}/status` |

Source: https://docs.mulesoft.com/cloudhub/cloudhub-api

---

## Application Manager API (CloudHub 2.0)

**Base URL**: `https://anypoint.mulesoft.com/amc/application-manager/api/v2`

For CloudHub 2.0 (and Runtime Fabric) deployments managed through the Anypoint Monitoring control plane (`amc`). Same bearer-token auth.

| Operation | Method | Endpoint |
|-----------|--------|----------|
| Deploy a new app | POST | `/organizations/{orgId}/environments/{envId}/deployments` |
| List deployments | GET | `/organizations/{orgId}/environments/{envId}/deployments` |
| Get a deployment | GET | `/organizations/{orgId}/environments/{envId}/deployments/{deploymentId}` |
| Update a deployment | PATCH | `/organizations/{orgId}/environments/{envId}/deployments/{deploymentId}` |
| Delete a deployment | DELETE | `/organizations/{orgId}/environments/{envId}/deployments/{deploymentId}` |

Deployment request bodies use a `deploymentSettings` section (replicas, vCore size, `tcp` port mappings, etc.).

Source: https://docs.mulesoft.com/cloudhub-2/ch2-deploy-api
...
(e.g. `https://mq-us-east-1.anypoint.mulesoft.com/api/v1/...` — the host is region-specific; verified 2026-06-13.)
