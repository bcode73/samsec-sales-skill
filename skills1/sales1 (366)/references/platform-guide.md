...
- **REST APIs**: Note three distinct management APIs (verified 2026-06-13): the **Runtime Manager API** (`https://anypoint.mulesoft.com/hybrid/api/v1`) manages **on-prem/hybrid servers only**; the **CloudHub API** (`https://anypoint.mulesoft.com/cloudhub/api`) manages CloudHub 1.0 apps; the **Application Manager API** (`https://anypoint.mulesoft.com/amc/application-manager/api/v2`) manages CloudHub 2.0 deployments. See `mulesoft-api-reference.md`.
...
### AI / agentic features (updated 2026-06-13)
- **MuleSoft Agent Fabric**: MuleSoft's "unified control plane for the agentic era" — a network/governance layer for AI agents across platforms (GA). Four pillars: **Discover** (Agent Registry + scanners cataloging agents and MCP servers), **Govern** (Omni Gateway runtime + governance strategies), **Orchestrate** (Agent Broker + Agent Network for multi-agent coordination), **Observe** (Agent Visualizer for topology + monitoring). Docs: https://docs.mulesoft.com/general/agent-fabric-overview
- **Omni Gateway**: The high-performance runtime governance layer that sits between agents and the systems they call. Supports **MCP (Model Context Protocol)** and the **A2A (Agent2Agent)** protocol, plus LLM traffic.
- **AI Gateway / LLM Governance**: Single governed control plane for AI interactions (LLM traffic, MCP tool access, agent-to-agent comms), built on Omni Gateway. **LLM Governance** (centralized visibility of token usage, costs, and data flows for third-party models) is **GA**. Platform page: https://www.mulesoft.com/platform/ai-gateway
- **MCP support**: Anypoint **Connector for MCP (MCP Connector)** lets Mule apps act as MCP servers and clients. Exchange now supports publishing MCP servers via URL (not just schema upload), and API Manager can create/manage instances for public MCP servers. An **A2A Connector** enables standardized agent-to-agent communication.
- **AI coding assistant**: MuleSoft's in-IDE AI assistant is now branded **MuleSoft Vibes** (in Anypoint Code Builder), which generates integration code, suggests configurations, and explains errors. (The earlier "MuleSoft Dev Agent" name no longer appears in current docs.)
- **Agentforce**: Salesforce's AI agent platform; MuleSoft (via Agent Fabric / Topic Center / connectors) exposes enterprise data and actions to Agentforce and other agents.

## Pricing and limits

MuleSoft does NOT publish simple tier pricing. Everything is custom-quoted.

### Pricing model shift — usage-based, NOT vCore-based (verified 2026-06-13)
MuleSoft has moved to **usage-based pricing** for new packages. Per the official docs: *"Accounts provisioned with a usage-based pricing package show flows, messages, and throughput entitlements in Access Management instead of vCores."* The three metrics are:
- **Mule Flows** — breadth of the deployment. A Mule flow only counts if it contains an event source or is generated by APIkit to route API requests (idle/sub-flows don't count). Counted on a **Max Concurrent model**: monthly usage = the highest number of flows running in any single hour that month (a month-end batch spike sets the bill).
- **Mule Messages** — intensity of usage. Only messages triggered by an event source count; downstream processing of the same message does not accumulate charges.
- **Data throughput (GB/year)** — the third entitlement on the package.

Two named editions (capacity figures below are from third-party reseller summaries, not the official page — treat as indicative):
- **Integration Starter** — entry edition; reportedly ~50 flows / 5M messages / 10,000 GB throughput per year.
- **Integration Advanced** — full Anypoint Platform capabilities; reportedly ~200 flows / 20M messages / 40,000 GB throughput per year. Required for the Signature Success plan.

### Legacy vCore-based pricing (still in use on older contracts)
Older contracts price by **vCores**, the per-app compute allocation. CloudHub vCores cost ~$50K-70K per vCore per year:
...
### Other components
- **Platform subscription**: $15K-50K+/year for base Anypoint Platform access (legacy framing)
- **Premium connectors**: SAP, Oracle, Workday, Siebel — $5K-50K+ per connector per year
- **Support tiers**: Gold (standard), Platinum (24/7, faster response), Titanium (mission-critical, dedicated TAM)
