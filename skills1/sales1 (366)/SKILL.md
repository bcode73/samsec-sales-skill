...
1. **What do you need help with?**
   ...
   - G) AI — Agent Fabric, Omni Gateway, AI Gateway / LLM Governance, MCP Connector + A2A, MuleSoft Vibes (in Code Builder)
...
- **Pricing has shifted from vCores to usage-based (flows/messages/throughput).** New Anypoint packages (Integration Starter / Advanced) bill on **Mule Flows**, **Mule Messages**, and **data throughput (GB/year)** — official docs: accounts on a usage-based package "show flows, messages, and throughput entitlements in Access Management instead of vCores." Flows are counted **Max Concurrent** (the peak number running in any single hour that month sets the bill — watch month-end batch spikes). Older contracts are still vCore-based. Confirm which model your contract uses before sizing.
- **vCore pricing spirals with scale (legacy contracts).** On vCore-based contracts each integration flow needs vCores; enterprise contracts commonly exceed $50K/year before headcount. Model your vCore needs before signing — add 30% buffer for non-production environments.
