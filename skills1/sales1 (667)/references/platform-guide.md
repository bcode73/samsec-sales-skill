...
- **Recipes**: The fundamental automation unit. Has triggers, actions, connections, versions, health status. (Note: the Developer API `POST /recipes`, `POST /recipes/:id/copy`, and `POST /connections` endpoints require a `folder_id` parameter from May 7, 2026.)
