...
Bearer token authentication via API clients. Include `Authorization: Bearer <api_token>` header in all requests. Legacy API keys (the old `x-user-token` + `x-user-email` headers) were fully deprecated as of July 14, 2025 — requests authenticated with legacy keys are rejected — with final removal from Oct 14, 2025 onwards.
...
Source: https://docs.workato.com/workato-api/limits.html

| Scope | Limit |
|---|---|
| **Default (most endpoints)** | 60 requests per minute |
| **Request timeout** | 40 seconds |
| Agent Studio — list/get genies, knowledge bases, skills | 1,000 requests per minute |
| Connections — list connections | 60 requests per minute |
| Connections — all other endpoints | 1 request per second |
| Custom connectors — all other endpoints | 1 request per second |
| Recipes — create recipe | 1 request per second |
| Recipes — start recipe health analysis | 20 requests per minute |
| Jobs — repeat jobs | 25 jobs per request, 1 request per second |
| Lookup tables — batch delete | 1 request per second |
| Recipe lifecycle — import package | 500 requests per hour per user ID |
| Workspace collaborators — invite collaborator | 60 requests per minute, plus one request per user/workspace every 20 minutes |
| Event streams — max payload size | 1 MB |

Exceeding a limit returns an error response (the docs do not publish a specific status code or rate-limit headers for the Developer API). Note: these Developer API limits are separate from API Platform endpoint limits (~10 req/sec) and the webhook gateway (20 events/sec steady + 9,000-event burst, 429 on overflow).
...
| POST | `/api/recipes` | Create recipe (**`folder_id` required from May 7, 2026**) |
| POST | `/api/recipes/:recipe_id/copy` | Copy recipe (**`folder_id` required from May 7, 2026**) |
...
| POST | `/api/connections` | Create connection (**`folder_id` required from May 7, 2026**) |
...
| POST | `/api/lookup_tables` | Create lookup table |
