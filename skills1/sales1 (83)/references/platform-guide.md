...
|---|---|---|
| **Event types** | 1:1, group, collective, round-robin scheduling | API read + create/update for **1:1 only** (GET/POST/PATCH `/event_types`, POST `/one_off_event_types`); group/collective/round-robin create still UI-only |
| **Bookings** | Create a scheduled event programmatically | **API-creatable** via Scheduling API (POST `/invitees`, paid plan) — was UI/embed-only until the 2025 Scheduling API |
| **Booking pages** | Customizable scheduling pages with branding | UI-only (create/edit) |
| **Routing forms** | Lead qualification → route to correct rep/event type | UI-only (Teams+ plan); submissions readable via API |
...
| **Availability** | Available time slots and busy times | API read + **update** schedules (GET/PATCH `/event_type_availability_schedules`) |
...
**Gotcha:** Reading availability and booking are now two separate API capabilities. You can both read available slots (above) **and** create the booking via the Scheduling API (`POST /invitees`, paid plan) — see the API reference. Redirecting to the `scheduling_url` or using embeds is still a valid option, especially on the Free plan where the Scheduling API is not available.
