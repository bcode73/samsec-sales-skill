...
- **API builders**: Calendly now has a Scheduling API (`POST /invitees`) that creates bookings programmatically — paid plan required. The API can also create/update 1:1 event types and update availability schedules. Embeds and scheduling links are still useful (and the only path on Free).
...
- **API CAN create bookings (since the 2025 Scheduling API).** `POST /invitees` (Create Event Invitee) books a meeting programmatically — paid plan required, PAT or OAuth. The API can also create/update 1:1 event types and update availability. Scheduling links/embeds remain the Free-plan path.
- **Webhooks require a paid plan.** Free plan has no webhook access. Standard unlocks `invitee.created` and `invitee.canceled`.
- **Rate limits vary by plan.** Roughly 60 req/min on Free/Standard/Teams, 120 req/min on Enterprise (exact numbers not cleanly re-verifiable on the live docs — treat as approximate). HTTP 429 includes a `Retry-After` header; honor it.
