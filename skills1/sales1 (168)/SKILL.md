...
2. **Which plan are you on?**
   ...
   - B) Power ($16.99/mo or $14.17/mo annual — 350 keywords, RSS/JSON, scheduled delivery)
   - C) Ultra ($69.99/mo or $58.33/mo annual — 1000 keywords, AI alerts, API, Slack/Discord)
...
3. **Integration** — API and webhooks require Ultra ($69.99/mo or $58.33/mo annual). For cheaper programmatic access, consider Syften (API at €39.95/mo) or RedShip (API at $19/mo)
4. **AI alerts** — Ultra includes 2 semantic alerts that match on meaning, not just keywords. Write natural-language descriptions (50–1,000 chars, up to 5 subreddits each). LLM token usage is billed separately on top of the subscription; the 2-alert limit can be raised on request
...
- **API and webhooks are Ultra-only ($69.99/mo or $58.33/mo annual).** Free and Power users have no programmatic access. If you need API access at a lower price, Syften starts at €39.95/mo with API.
...
**Solution**: Use multi-word phrases with the `whole` flag. Add `exclude=` for recurring false positives. Use `only-url=` to restrict to 3-5 relevant subreddits. If legitimate volume exceeds 50/day, upgrade to Power ($16.99/mo or $14.17/mo annual, 1,000 hits/day) or Ultra ($69.99/mo or $58.33/mo annual, 1,000 hits/day).
