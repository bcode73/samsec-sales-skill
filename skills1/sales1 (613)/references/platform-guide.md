...
- Enterprise / Premier: 10,000 emails/month
- Higher/custom tiers: custom limits
...
| Events | `/api/v2/events/search` | POST (search; `event_type` note/campaign_touch/alert/task) |
| Touchpoints | `/api/v3/touchpoints/` | POST (create), GET, PUT, DELETE |
| Activity Types | `/api/v3/activity-types` | GET (list Flow IDs for `activity_type_id`) |
| Team Members | `/api/v2/team_members` | GET (list/detail), POST (create/update), DELETE (deactivate) |
| Tasks | `/api/v3/tasks` | POST (create), PUT (update by id), DELETE; `/api/v3/tasks/export/csv` GET (CSV) |
| SuccessPlays | `/api/v2/automations/run-manual-successplay` | POST (run manual play); stats via `/api/v3/tasks/export/csv` GET |
| Objectives | `/api/v3/objectives-collection` | POST (create); `/create-by-template` POST |
| Accounts Plan | `/api/v3/accounts/{account_id}/plan_summary` | GET (plan summary) |
| Audit Log | `/api/v2/audit?startDate=&endDate=` | GET (by date range, ≤1 yr, same year) |
...
- Search / events: max 1,000 results per call (use `offset` for pagination)
- The HTTP API (activity ingest / collection gateway) and the JavaScript collector are **not** rate-limited
- Over-limit returns `429 Too Many Requests` with `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, and `Retry-After` (seconds) headers — honor `Retry-After`
