...
- **Credit cost**: Hunter uses a unified credit pool — 1 credit per email found (Domain Search / Email Finder), 0.5 credit per email verified. (Older docs said "1 credit per 10 results"; the current model charges per email found.)
...
**Quick reference**: Base URL `https://api.hunter.io/v2/`, auth via `api_key` query parameter, `X-API-KEY` header, or `Authorization: Bearer` header. Rate limits are **per endpoint** (not per plan): Domain Search & Email Finder 15 req/sec & 500 req/min, Email Verifier 10 req/sec & 300 req/min, Discover 5 req/sec & 50 req/min. Email Verifier can return HTTP 202 (still processing) — poll the same endpoint. Discover is a **POST** (`POST /discover`). Key capabilities: Domain Search, Email Finder, Email Verifier, Email Count, Enrichment (`/people/find`, `/companies/find`, `/combined/find`), Leads/Lists management, Campaigns (Sequences API), Discover.

**MCP Server**: Hunter.io provides an official **Remote MCP Server** at `https://mcp.hunter.io/mcp` (Streamable HTTP) or `https://mcp.hunter.io/sse` (SSE), authenticated with your API key via `Authorization: Bearer` or `X-API-KEY` header. The older local `hunter-mcp` pip package is **deprecated and unmaintained** — use the remote server. See `hunter.io/api-documentation#mcp`.
...
2. Explains the credit cost (1 credit per email found) and how to paginate for more
...
4. Warns about per-endpoint rate limits (Domain Search 15 req/sec & 500 req/min; Email Verifier 10 req/sec & 300 req/min) and recommends batching within those limits
...
**Cause**: Domain Search on large companies consuming many credits (1 credit per email found), or verifying large lists
