...
| **Optimize** | Programmatic publisher solution (header bidding, 25+ demand sources) | **JS API** (`window.optimize` — events, advanced functions, identity hashing) |
...
**Payout (per official Cashing Out support doc):**
- PayPal: **$20 minimum**, processed every 2-3 days, up to 2 cashouts/month
- Bank check: **$50 minimum**, 1 cashout/month
- Wire transfer: **$500 minimum**, **$35 fee** per transfer, processed weekly
- Funds become available to cash out **30 days after** an ad is purchased
...
**Key fields:**
- `statlink` — click tracking URL. **Always check this exists before rendering.** If missing, no ad is available. (When empty, the response carries a top-level `zonekey` field instead of a populated ad.)
...
### Optimize integration (programmatic, JS)

Optimize (header bidding / 25+ demand sources) also has a client-side JS surface via `window.optimize`:
- Install a single async loader before `</head>`; place `<div id="bsa-zone_[YOUR_ZONE_ID]"></div>` where ads should render.
- SPAs: refresh on route change with `window.optimize.queue.push(() => { window.optimize.pushAll() })`.
- Rewarded-ad events via `window.optimize.addEventListener(type, fn)`: `rewardedSlotReady`, `rewardedSlotGranted` (payload: reward amount/type), `rewardedSlotClosed`.
- Identity: hashed emails passed to the frontend as `window.optimizeIdentityHashes` (client-side Web Crypto hashing).
- See `references/buysellads-api-reference.md` → Optimize section.
...
**Multi-zone batching:** request several zones at once with the email endpoint, semicolon-separated: `GET https://srv.buysellads.com/ads/get/ids/ZONEID1;ZONEID2.json?ignore=yes`.

**Caveat:** The email/`get/ids` endpoint is **not** per-email-sent — the ad is fetched once at send/runtime and the same creative is shown to every subscriber in that send. There is no per-recipient rotation through this path; rotate by re-fetching per send.
