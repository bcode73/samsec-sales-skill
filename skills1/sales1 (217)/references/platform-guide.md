...
### API
REST API v2 at `https://api.gumroad.com/v2/`. OAuth 2.0 authentication. Full CRUD for products, sales, subscribers, licenses, offer codes, custom fields, variants, and resource subscriptions, plus read-only Payouts (`GET /payouts`, `/payouts/upcoming`; needs the `view_payouts` scope) and an account-wide Refund Policy (`GET`/`PUT /refund_policy`). Note: create a resource subscription with **PUT** (not POST), and the Sales `mark_as_shipped`/`refund` and Products `enable`/`disable` member actions are **PUT**. See `references/gumroad-api-reference.md` for complete endpoint documentation.
...
5. Pass `increment_uses_count: false` to verify WITHOUT bumping the counter — it **increments by default** when the param is omitted
