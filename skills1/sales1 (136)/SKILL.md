...
- e) Super CAPI — server-side Meta conversions tracking
- f) Consumer Data Enrichment API — enrich contacts programmatically
- g) Klaviyo (or SendGrid/Sendlane/HighLevel/Salesforce) integration setup
- h) Evaluating — comparing Customers.ai vs Retention.com vs others
- i) Something else
...
- **Klaviyo-led, but not Klaviyo-only (updated 2026-06-13).** Klaviyo is the lead integration and the basis for pricing, but Customers.ai now publishes native integrations for SendGrid, Sendlane, HighLevel, Salesforce, and Mailchimp. Omnisend/ActiveCampaign/Drip were not confirmed. If you're on Klaviyo it's the smoothest path; other supported ESPs work but pricing still keys off your Klaviyo profile count.
- **Shopify-primary.** Built primarily for Shopify stores. Confirm WooCommerce, BigCommerce, or custom-platform support directly — no native non-Shopify commerce integration was confirmed at verification.
- **Pricing is now profile-based, quoted by sales (updated 2026-06-13).** The live pricing page is a custom calculator keyed to your *total Klaviyo profile count* (including suppressed/unsubscribed) plus product selection (X-Ray / Alfred / Super CAPI) and contract terms — not the old fixed $600/$900/$1,500 Starter/Grow/Scale tiers. Get a calculator quote; economics still need meaningful traffic ($500K+ GMV).
