...
**Quick reference** (re-verified 2026-06-13): Developer portal at developer.seismic.com/seismicsoftware/. Three API families — Authentication, Integration (real-time, `https://api.seismic.com/integration/v2/...`), and Reporting (ETL/bulk, ≥24h refresh). OAuth 2.0 / OIDC, token endpoint `https://auth.seismic.com/tenants/{tenant}/connect/token`. **Tiered rate limits per 60s**: LiveDoc gen 10, LiveSend creation 30, library/workspace listing 60, everything else 600 — track `X-Seismic-Remaining-Calls` (no `Retry-After`). Sandbox host `https://api-sandbox.seismic.com`. Webhooks use PascalCase `...V1`/`...V2` event names, `x-seismic-signature` HMAC-SHA256, 10s/2xx response, 26-attempt retry. Key capabilities: content management, engagement tracking, reporting, user/group management (SCIM), LiveDocs/Document Generator, search, webhooks.
...
## Pricing (re-verified 2026-06-13 — still custom-quote/enterprise-only; figures corroborated by current third-party sources, treat as directional)

- Enterprise-only, custom quotes — no public plans, no fixed tiers, no self-serve checkout
- Negotiated ~$30-60/user/month; published list rates run higher (e.g. Content Professional ~$630/user/yr list, Enterprise Premier ~$494/user/yr list)
- Common total spend $20K-$120K+/year; mid-market $20-60K/yr, enterprise often $100K+/yr
- Multi-year contracts unlock significant discounts
- ~4-month average onboarding; professional services range from a few thousand (small) to six figures (enterprise)
...
- Note: Seismic and Highspot announced an intent-to-merge on 2026-02-12 (combined company keeps the Seismic name); packaging/pricing may shift post-close
