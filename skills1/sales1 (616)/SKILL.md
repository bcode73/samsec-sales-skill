...
- **API requires a paid subscription, and the exact gating tier is unverified.** Older notes said "Enterprise-only," but the official developer docs and pricing page do not state which tier unlocks the API. Confirm API availability for your plan in-app before committing. If you need a clearly API-included budget tool, Sonix Premium or Temi are safe bets.
...
4. Flags: Transkriptor API requires a paid plan and the exact gating tier is unverified — confirm API access for their plan before relying on automation; Sonix Premium clearly includes API
...
**Skill does**:
1. Confirms API requires a paid subscription — asks user to verify their plan includes API access (exact gating tier is not stated in official docs)
2. Reads the API reference for endpoint details
3. Sketches pipeline: S3 event → Lambda → `POST /transcription/local_file/get_upload_url` (with `file_name`) → `PUT` binary to the returned upload URL → `POST /transcription/local_file/initiate_transcription` (with `public_url`, ISO `language`, `service`) → webhook on completion (`POST /integrations/webhooks`) → `GET /files/{order_id}/content` → normalize → warehouse
4. Notes: Bearer token auth, 1K req/min rate limit, webhooks fire on transcription completion (no documented HMAC signature)
5. If the plan/pricing is prohibitive, suggests Sonix Premium API as alternative
