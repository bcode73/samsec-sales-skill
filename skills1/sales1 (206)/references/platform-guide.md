...
- **Webhooks**: Configurable per integration — 29 events across site/post/page/tag/member (e.g. `post.published`, `member.added`, `member.edited`, `member.deleted`, `post.tag.attached`). Note the member-update event is `member.edited`, not `member.updated`. No HMAC/signature verification — see `ghost-api-reference.md`.
...
*Prices shown are annual billing. Monthly billing is higher (e.g. Starter is $15/mo billed yearly vs $18/mo billed monthly — re-verified 2026-06-13 against ghost.org/pricing). All plans include: SSL, CDN, automated backups, device verification.*
...
- **Posts**: CRUD + copy. Content is **Lexical** by default (mobiledoc is legacy); use `?formats=html,lexical` to get HTML back and `?source=html` to create from raw HTML
- **Pages**: CRUD + copy
- **Tags**: CRUD
...
- **Labels**: Browse, Read, Edit, Add, Delete (member tags)
