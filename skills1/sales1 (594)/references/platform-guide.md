...
| **Document Modification** | Create, update, upsert, delete, undelete documents | API-accessible (Documents API v2 — `/api/v2/docs/p/<project_id>/...`) |
...
| Streaming (create stream / project data access) | 5 calls/min |

**API credit costs:**
- Search: 1 credit per result, minimum 10 per call
- Histogram: 10 credits per call
- Streaming: 1 credit per new result (updated documents are free)
- Document create/update/delete: no credits
- Summaries: 20 credits each (requires hpp > 100)
- X (Twitter) export: limited to ~1.5M docs/month/account
- Credits reset monthly, on the day of the subscription at 03:00 UTC
...
# Create stream (rate-limited to 5 calls/min)
requests.put(
    f"{BASE}/stream/s/{STREAM_ID}",
    params={"access_token": TOKEN},
    json={"rules": [{"rule_id": "rule-1", "query": '"Your Brand"'}]},
)
...
5. Watch for export restrictions: FB/IG not exportable (aggregated only), X (Twitter) capped at ~1.5M docs/mo/account, online news + TV Eyes truncated, LinkedIn / raw Reddit / raw TikTok not exportable
...
5. Tag processed mentions via the Documents API v2 (`POST /api/v2/docs/p/<project_id>/update`) to avoid duplicates
