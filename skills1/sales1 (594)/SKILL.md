...
- **Data export restrictions are strict.** Facebook/Instagram content is not exportable (aggregated-only via Histogram API). X (Twitter) exports are limited to tweet ID, author ID, and sentiment, capped at ~1.5M docs/month/account. Full news article and TV Eyes content is truncated. Many source types cannot be exported at all (LinkedIn, raw Reddit, raw TikTok, Chinese sources).
