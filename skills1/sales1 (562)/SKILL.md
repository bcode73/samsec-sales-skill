...
2. **Which Sprout plan are you on?**
   - A) Essentials ($79/seat/mo, $99 month-to-month)
   - B) Standard ($199/seat/mo)
   - C) Professional ($299/seat/mo)
   - D) Advanced ($399/seat/mo)
   - E) Enterprise (custom)
...
- **Per-seat pricing adds up fast.** Every user needs their own seat. The entry-level Essentials plan is $79/seat/mo (publishing-focused, 5 profiles, no API); Standard/Professional/Advanced are $199/$299/$399. A 5-person team on Professional = $1,500/month before add-ons. Many features (approval workflows, chatbot, API) require Advanced ($399/seat).

- **API is documented at 60 req/min, 250K req/month.** Sprout's public API now publishes rate limits of 60 requests/minute and 250,000 requests/month. Auth accepts either OAuth 2.0 (recommended — client_credentials machine-to-machine) or an account-scoped API token, both via `Authorization: Bearer`. API access requires the Advanced plan.
