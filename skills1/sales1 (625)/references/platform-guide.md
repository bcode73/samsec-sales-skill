...
- **Native webhooks**: `review.created` / `review.replied` events pushed to your endpoint with an `x-trustpilot-signature` (HMAC-SHA256) header — set up in Trustpilot Labs → Webhooks. Preferred over Zapier/polling for real-time CRM/Slack sync.
...
Rate limit: **833 calls / 5 minutes** or **10,000 calls / hour**. Reuse access tokens (don't mint one per call), use webhooks instead of polling, and cache reviews server-side rather than calling the API on every page view. (developers.trustpilot.com/rate-limiting)

Webhooks: real-time events via Trustpilot Labs → Webhooks app (the official docs recommend webhooks over polling). Events `review.created` and `review.replied`; payloads carry an `x-trustpilot-signature` HMAC-SHA256 header for verification. See `trustpilot-api-reference.md` for details.
