...
Source: https://developers.trustpilot.com/rate-limiting/ (re-verified 2026-06-13)

- Limit: **833 calls per 5 minutes** OR **10,000 calls per hour** (whichever is hit first).
- Best practices to stay under the limit:
  - Use **webhooks instead of polling** — avoid pulling data on a schedule if a webhook event can notify you.
  - **Reuse a valid OAuth 2.0 access token** rather than generating a new one per call (re-generating tokens too often returns HTTP 429 — see Authentication).
  - Do not pull review data directly into your website frontend (each page view would call the API); cache server-side instead.

---

## Webhooks

Trustpilot supports webhooks for real-time event notifications, and the official developer docs recommend webhooks over polling (see Rate limiting). Set up via Trustpilot Business → Trustpilot Labs → Webhooks app (also registrable via API by POSTing a target URL + event list with a business OAuth token).

- **Event types**: `review.created`, `review.replied`
- **Delivery**: HTTP POST with a JSON body shaped `{ "event": "<event-name>", "data": { ... } }`; your server must respond 2xx.
- **Signature**: requests include an `x-trustpilot-signature` header — HMAC-SHA256 of the raw request body using your webhook secret (`HMAC_SHA256(secret, rawBody)` hex-encoded). Verify before trusting the payload.

> Event names, signature header, and HMAC algorithm corroborated by Trustpilot integration guides; the official developer docs confirm webhooks exist and the "Manage your webhooks notifications" management surface. Retry policy is not documented on a current official page — older sources cite up to 5 retries over ~8 minutes; treat as indicative, not authoritative.
