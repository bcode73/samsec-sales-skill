...
- **Auth**: Access token from my.broadstreetads.com/access-token, sent as a `?access_token=` query parameter (NOT an Authorization header)
- **Base URL**: `https://api.broadstreetads.com/api/0/` (PHP SDK) or `/api/1/` (Ruby SDK) — version is a digit, not `v1`
- **SDKs**: PHP (broadstreet-api-php), Ruby (broadstreet-ruby) on GitHub — both expose full CRUD for advertisements, advertisers, campaigns, placements, networks, and zones, plus reporting (`/records`)
