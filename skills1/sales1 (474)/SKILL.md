...
5. **MCP server requires API key** — `https://mcp.reletter.com` needs authentication. Set `RELETTER_API_KEY` before connecting. REST API auth uses the `x-reletter-api-key` header (not `Authorization: Bearer`), base URL `https://api.reletter.com`, paths under `/api/...`.
