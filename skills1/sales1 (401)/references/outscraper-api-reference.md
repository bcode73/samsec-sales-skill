...
> **Note**: Re-verified 2026-06-13 against the official Python SDK source and the live pricing page. The interactive API docs are at https://app.outscraper.com/api-docs (JS-rendered; backend host is app.outscraper.cloud) and the reference index is at https://docs.outscraper.com. Verify edge-case params against the live docs.
>
> **Official Python SDK**: `pip install outscraper`, then `from outscraper import OutscraperClient; client = OutscraperClient(api_key='YOUR_KEY')`. The legacy `ApiClient` name is kept as an alias (`ApiClient = OutscraperClient`) for backward compatibility. Email validation method is `client.validate_emails([...])` (→ `GET /email-validator`); contacts enrichment is `client.contacts_and_leads([...])`. SDKs also exist for Node.js, PHP, Ruby, Go, and Java.
...
The raw REST query parameter that toggles this is `async` (default `true`). Set `async=false` on supported endpoints to get synchronous (blocking) results for small queries. The Python SDK exposes this as the `async_request` kwarg (which it sends as `async` on the wire); `ui=true` forces async mode regardless. Completed async results stay retrievable via `GET /requests/{id}` for ~2 hours.
...
|---|---|---|
| `async` | bool | Wire-level param (default `true`): return immediately with request ID instead of waiting. Python SDK kwarg is `async_request`, sent as `async`. |
| `webhook` | string | URL to POST results to when async task completes (overrides any webhook set in integrations) |
| `fields` | string[] | Specify which fields to return in the response |
| `ui` | bool | Run as a UI/platform task; forces async mode (overrides `async`) |
