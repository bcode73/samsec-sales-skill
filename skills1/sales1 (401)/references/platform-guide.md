...
- **SDKs**: Python (`pip install outscraper` → `from outscraper import OutscraperClient`; `ApiClient` is a legacy alias), PHP, Node.js, Ruby, Go, Java. Python email-validation method is `validate_emails()`.
- **Docs**: docs.outscraper.com (reference index) and app.outscraper.com/api-docs (interactive)
...
## Pricing (pay-as-you-go, no subscriptions — verified 2026-06-13 at outscraper.com/pricing)

Per-service three-tier model. Most scraping services share the same shape (free starter quota, then $3/1,000 up to 100K, then $1/1,000 above 100K). Verified examples:

| Service | Free | Medium tier | Business tier |
|---------|------|-------------|---------------|
| **Google Maps Scraper** | first 500 businesses | $3/1,000 records (501–100K) | $1/1,000 records (100K+) |
| **Google Maps Reviews** | first 500 reviews | $3/1,000 reviews (501–100K) | $1/1,000 reviews (100K+) |
| **Emails & Contacts** | first 500 domains | $3/1,000 domains (501–100K) | $1/1,000 domains (100K+) |
| **Email Validator** | first 25 emails | $3/1,000 emails (26–100K) | $1/1,000 emails (100K+) |
| **Google Search Scraper** | first 10 searches (pages) | $5/1,000 searches (11–100K) | $3/1,000 searches (100K+) |

**Key pricing notes**:
- Pay-as-you-go model — no monthly subscriptions, pay only for what you use. Choose prepaid credits or postpaid card billing.
- Each module is priced independently — Google Maps, reviews, email validation, Google Search etc. each have their own free quota and per-1,000 rate (Google Search is more expensive: $5/1,000 medium).
- The volume discount kicks in at **100,000 units** per service (not 50K), dropping the rate to $1/1,000 for most services.
- Tiers/quotas reset every 30 days.
- Free tier is generous enough for testing and small projects. A 3-day refund guarantee applies if a service fails to retrieve data.
