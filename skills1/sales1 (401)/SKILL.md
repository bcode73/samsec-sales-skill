...
6. **Using the Python SDK**:
   ...
   2. Initialize: `from outscraper import OutscraperClient; client = OutscraperClient(api_key='YOUR_KEY')` (the old `ApiClient` name still works as a backward-compatible alias, but `OutscraperClient` is current)
   ...
   5. Email validation: `results = client.validate_emails(['email@example.com'])`
