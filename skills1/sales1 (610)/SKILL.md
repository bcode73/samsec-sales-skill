...
- **Don't assume all plans include the same features.** Plans differ by monthly credit allowance; pricing is now credit-based (~$8.90 per 1,000 credits, verification bundled free). Full domain-search result downloads may be plan-gated. Note: webhook/`webhook_url` support is NOT documented in current official docs (docs.tomba.io) — confirm directly with Tomba before building webhook-dependent workflows rather than assuming a Pro-plan gate. Mind per-endpoint rate limits (e.g. Domain Search 15/sec, LinkedIn 1.67/sec) and honor `X-RateLimit-Reset` on 429s.
...
**Cause**: Exceeding the per-endpoint rate limit (e.g. Domain Search/Email Finder 15/sec · 900/min; Enrichment 5/sec; Email Verifier/Author Finder 2.5/sec; LinkedIn 1.67/sec · 100/min) — making too many requests too quickly
**Solution**: On a 429, read the `X-RateLimit-Reset` header (seconds until reset) and wait that long before retrying; `X-RateLimit-Limit` / `X-RateLimit-Remaining` show your budget. Use exponential backoff as a fallback. Batch requests and use bulk endpoints instead of looping individual calls. If consistently hitting limits, slow down or upgrade.
