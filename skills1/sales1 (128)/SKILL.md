...
- **No public API documentation.** Docs moved to docs.cresta.com (the legacy developers.cresta.com site stays live until June 30, 2026). The full API Reference and webhook docs are gated behind a Cresta Director login, so you can't evaluate API scope before buying. An OpenAPI spec with Postman/Insomnia collections exists, but concrete endpoints/auth/rate limits aren't public — ask for API docs during evaluation.
