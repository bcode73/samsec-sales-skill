...
- **Build** — create AI agent workflows using the no-code interface
- **Test & Deploy** — staged rollout with governed rollouts, change visibility, and rollback guardrails
...
- **Cresta Conductor** *(launched June 11, 2026)* — "a developer-first agentic engine for building and optimizing AI agents on the Cresta platform"; a natural-language engine spanning the entire agent lifecycle (Discovery & planning → Build with prompts/orchestration/custom code → pre-launch testing with synthetic customers → post-launch optimization). Cresta claims it ships production-grade AI agents ~2x faster.
...
### Knowledge Management (also surfaced as the platform-level "Unified Knowledge" module)
API, Web (URL/site crawl), PDF ingestion, Salesforce Knowledge, Google Drive, SharePoint, Zendesk Guide, Bloomfire, Shelf, Contentful
...
**Developer portal**: docs.cresta.com (new docs experience as of 2026 — faster search, improved navigation). The legacy developers.cresta.com docs site remains active until **June 30, 2026**, then redirects to docs.cresta.com.
...
- **No public endpoint documentation** — the full API Reference / webhook docs are gated behind a Cresta Director login (verified 2026-06-13). You must sign in with valid Cresta credentials or request a demo to see base URLs, auth, endpoints, and webhook payloads. Cannot evaluate the full API scope before buying.
- OpenAPI/Swagger spec exists with Postman and Insomnia collections (per API Tracker), but the concrete endpoint/auth/rate-limit details are not publicly published.
- REST + webhooks with API key auth (reported; not independently verifiable from public pages)
- Future plans include public SDK and exposed public API for historical data ingestion and exports

**GitHub**: github.com/cresta (78 public repos, all internal/infra tooling — e.g. allow-cresta-s3-data-transfer and allow-cresta-aws-connect CloudFormation templates for data/Connect integration; no consumer-facing product SDK as of 2026-06-13)
...
2. Build AI agent workflows in the no-code builder (older docs called this "Opera"; 2026 builds increasingly go through **Cresta Conductor**, the natural-language agent-development engine)
3. Test with simulated/synthetic conversations
