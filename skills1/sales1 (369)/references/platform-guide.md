...
- **Database**: 3M+ indexed influencer profiles across Instagram, TikTok, YouTube, and other platforms (NeoReach surfaces data for YouTube, Twitter, Instagram, Facebook, Twitch, and TikTok)
...
NeoReach offers a "standard REST API which accepts standard parameters to return JSON data" (official wording, neoreach.com/api/). Supported platforms for API insights: YouTube, Twitter, Instagram, Facebook, Twitch, and TikTok. The API surface is grouped into four endpoint categories — Social Profile Information, Social Profile Analytics, Social Post, and Social Audience. Key capabilities:
...
*Verified 2026-06-13 against neoreach.com/pricing/ — NeoReach no longer publishes any list prices. The pricing page now shows three custom-quote offerings, each with a "Let's Chat!" CTA (no dollar figures):*

| Offering | Price | What's included (per official pricing page) |
|---|---|---|
| **Influencer Campaigns** (full service) | Custom — "Let's Chat!" | Campaign strategy & direction, creative brief development, influencer sourcing & scheduling, contracts & negotiation, content licensing & compliance, influencer payments, detailed reporting & insights |
| **Platform & API** | Custom — "Let's Chat!" | Unlimited searches, unlimited lists, unlimited "In Network" influencers, unlimited campaigns, unlimited post tracking links, dedicated account manager & support, API Access* (conditional) |
| **Enterprise** | Custom — contact a product specialist | Fully customizable enterprise platform |

> **Note on historical figures:** Older third-party listings (GetApp, ITQlick, Techjockey) and prior research cited tiers around ~$399/mo to ~$1,500/mo for SaaS, $50K-$500K/yr enterprise annual, and $25K+ per managed campaign. These are NOT confirmed on the current official pricing page (which is now custom-quote only) and should be treated as indicative/dated. Verify any number directly with NeoReach (team@neoreach.com).

Plan-gated features (custom enterprise quote required):
...
| Pricing | Custom quote (no list price) | ~$30K/yr | $199/mo | $299/mo |
| Public API docs | No (capabilities page only) | Yes (ExchangeIQ) | Yes | Yes (Enterprise) |
