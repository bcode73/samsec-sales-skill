...
- **Custom-quote pricing only (verified 2026-06-13).** NeoReach no longer publishes any list prices — neoreach.com/pricing/ now shows three custom-quote offerings (Influencer Campaigns, Platform & API, Enterprise), each with a "Let's Chat!" CTA. Older third-party listings cite ~$399-$1,500/mo SaaS and $50K+/yr enterprise, but these are dated and unconfirmed. Still positioned at the enterprise end — not viable for small budgets. If budget is under $5K/mo, look at Modash ($199/mo), Heepsy ($49/mo), or Afluencer (free tier).
- **No public/self-serve API documentation.** Unlike platforms like Meltwater or CreatorIQ that publish API docs, NeoReach's official API page (neoreach.com/api/) lists capabilities and four endpoint categories (Social Profile Information, Social Profile Analytics, Social Post, Social Audience) but no auth, base URL, or rate limits — those are shared only after a sales conversation. You can't fully evaluate the API before committing.
