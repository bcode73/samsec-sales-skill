<!-- Source: https://adserver.online/site/openapi (OpenAPI v2.4.1 spec) -->

# Adserver.Online API v2.4.1 Reference
...
- **Plan requirement**: REST API (management + reporting) requires Premium ($199/mo) or higher. The Decisioning / programmatic (OpenRTB) API requires the Ultimate plan ($599/mo).

## Authentication

Create API tokens in Account > API tokens menu. Three token types:

- **Owner token**: Access to all endpoints (`/user`, `/campaign`, `/group`, `/ad`, `/site`, `/zone`, `/stats`, `/payment`, `/payout`, `/transaction`, `/referral`, `/dict`)
...
| Method | Path | Description |
|---|---|---|
| GET | `/group` | List campaign groups (paginated; supports `page`, `per-page`, `sort`, `filter`) |
| POST | `/group` | Create campaign group (body: `name`) |

Added in API v2.4.1. The "Campaign grouping" feature must first be enabled in Settings / Common. The Campaign object then exposes a `group` property, and stats can be filtered/grouped by `idgroup`. No DELETE method exists for `/group`.
...
|---|---|---|
| 2.4.1 | 2025-08-01 | Added `GET|POST /group` for campaign group management |
