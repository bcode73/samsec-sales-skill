...
Adserver.Online is a cloud-hosted ad tech platform for building and running ad networks, ad serving, and ad tracking. Targets publishers, advertisers, and ad network operators. Key differentiator: competitive pricing ($199/mo for 10M requests on Premium with email, video, and REST API access) and ad network builder features. Programmatic OpenRTB serving, Prebid adapters, Google RTB, and multicurrency bidding are gated to the Ultimate plan ($599/mo).
...
| Programmatic (RTB) | OpenRTB decisioning endpoints, Prebid adapter, XML/JSON feed support, multicurrency bidding | Ultimate plan, UI + API |
...
| Programmatic (OpenRTB) | No | No | Yes |
| XML/JSON feeds | No | No | Yes |
| Multicurrency bidding | No | No | Yes |
...
- **Programmatic**: OpenRTB endpoints, Prebid adapter, XML/JSON feed support, multicurrency bidding — all Ultimate plan only. OpenRTB v2.5 protocol (Banner, Native, VAST, Direct Link, Push formats).
...
|---|---|---|
| Owner | `/user`, `/campaign`, `/group`, `/ad`, `/site`, `/zone`, `/stats`, `/payment`, `/payout`, `/transaction`, `/referral`, `/dict` | Full platform management |
...
| RTB/Programmatic | Ultimate only | Yes | Yes (free for pubs) | Via Prebid |
