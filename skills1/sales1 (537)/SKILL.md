...
- **API Management 3.0 is evolving fast.** It now exposes pipelines as MCP services for AI agents (MCP Server with OAuth2 Client Credentials + JWT Validator, Trusted Agent Identity, MCP Metrics in Monitor) and integrates with AgentCreator governance. Earlier gaps (e.g., auto-key-management, admin UX) are being closed release by release — check current release notes for the latest APIM 3.0 features before planning a full API governance rollout.
