...
No-code platform for building, deploying, and managing AI agents. Now branded **AgentCreator 3.0** ("infinite AI workforce for the agentic enterprise").
...
- **LLM Snap Packs**: Connectors for OpenAI, Amazon Bedrock, Azure OpenAI, and Google GenAI (Gemini). Support chat completions, embeddings, and prompt generation.
...
- **AI agent governance**: Integrated governance for AI agents accessing APIs — control which agents can call which APIs, audit agent API usage. Integrates directly with AgentCreator.
- **MCP services**: Expose SnapLogic workflows/pipelines as APIs and make them available to GenAI applications and agents as **Model Context Protocol (MCP) services**. The MCP Server supports built-in OAuth2 (Client Credentials and JWT Validator), Trusted Agent Identity with token propagation, and a dedicated MCP Metrics page in Monitor (May 2026 release).
...
- **MCP**: AgentCreator supports Model Context Protocol for AI agent integration. API Management 3.0 can also expose SnapLogic workflows/pipelines as MCP services consumable by any AI agent, with built-in OAuth2 (Client Credentials, JWT Validator) on the MCP Server.
...
### API authentication
- **Basic Auth**: Email (user or service account) + password, base64-encoded — `Authorization: Basic {base64 <email>:<password>}`.
- **JWT**: JSON Web Token authentication (recommended). `Authorization: Bearer Token {token}`. Supports Okta and Microsoft Entra ID as identity providers. Required claims: `iat`, `exp`, `sub`, `aud`, `iss`, `org`; `aud` must equal the POD base URL (custom `snaplogic_aud` claim also accepted).

### API base URL
- Public APIs use the `/api/1/rest/public` prefix: `https://{pod}.snaplogic.com/api/1/rest/public/...`
- POD varies by region/instance (e.g., `elastic.snaplogic.com`, `uat.elastic.snaplogic.com`)
- Example: `https://elastic.snaplogic.com/api/1/rest/public`
...
- **Groundplex requirements**: JDK 11+ (JRE 17 supported as of the May 2026 release), 8GB RAM minimum (16GB recommended), 4 CPU cores minimum. Linux (RHEL 7+, Ubuntu 18+) or Windows Server 2019+. Requires outbound HTTPS (443) to SnapLogic control plane.
