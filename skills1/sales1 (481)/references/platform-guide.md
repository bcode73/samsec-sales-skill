...
- Yearly billing available — annual plans are billed as "6 months free" vs monthly (per replymer.com/pricing toggle, 2026-06-13)
...
4. Respect rate limits — read the `RateLimit-Limit`, `RateLimit-Remaining`, and `RateLimit-Reset` response headers and add exponential backoff on 429 responses
5. Page through results with `page`/`limit` (default 20, max 100); read `meta.pages` to know when to stop
