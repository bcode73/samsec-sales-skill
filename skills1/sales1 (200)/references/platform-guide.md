...
Key differentiators vs competitors: composable architecture, deep Salesforce joint product (CX Cloud), AppFoundry marketplace (~693 apps as of mid-2026), 15+ regional deployments (incl. newer Mexico Central), and phased migration support from on-prem (PureBridge program).
...
~693 curated apps, integrations, and services (mid-2026, up from ~450). Three categories: Developer Apps (free, community-built), Genesys Apps (free or paid, Genesys-built), and ISV Applications (premium, purchasable). Tiered partnership (Bronze entry, progression based on cloud recurring revenue KPIs). Source: https://appmarketplace.com/marketplaces/genesys-appfoundry/ , https://help.genesys.cloud/articles/appfoundry-overview/
...
- **Notifications API**: WebSocket subscribe/topic real-time event streaming (`GET /api/v2/notifications/availabletopics`, `POST /api/v2/notifications/channels`; 20 channels/user/app, 1000 topics/connection)
- **Amazon EventBridge integration**: push-model event stream into AWS for backend integrations (no HMAC webhooks — see api-reference)
- **Developer Center**: https://developer.genesys.cloud/

### GitHub
Organization: MyPureCloud (~103 public repos as of mid-2026)
Notable repos:
- `terraform-provider-genesyscloud` (54 stars) — Terraform provider for infrastructure-as-code
- `platform-client-sdk-javascript` / `-python` / `-java` — official Platform Client SDKs
- `genesys-cloud-webrtc-sdk` — WebRTC SDK for browser softphone
- `developercenter-tutorials` — Sample code and tutorials
Source: https://github.com/MyPureCloud
...
- Data residency options across 15+ regions (incl. newer Mexico Central)
- MFA enforcement for administrators: username/password-only admin login is deprecated **no sooner than March 9, 2026** — from the effective date, admins (except those on SSO, which already requires MFA) must use multi-factor authentication. Source: https://help.genesys.cloud/announcements/deprecation-ability-for-administrators-to-log-in-using-user-name-and-password-only/
