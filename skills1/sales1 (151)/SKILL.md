...
**Exception — named platform + platform-specific problem**: when the user NAMES a specific tool (Fathom, Printify, SamCart, …) and describes a problem specific to using it (broken sync, API/webhook/auth, bot behavior, plan mechanics), skip the strategy table and route to that platform's skill from the catalog. Strategy skills win only for choosing between tools or tool-agnostic method questions (see `references/disambiguation-rules.md`).
...
| `/sales-idea-validation` | Validating a startup/product idea before building it — the evidence ladder (problem → demand → willingness-to-pay), reading an AI idea-score critically, smoke/fake-door tests, pre-sales, and choosing between AI validators (Validator AI, DimeADozen, IdeaProof, Trend Seeker) |
...
| `/sales-multichannel-selling` | Multichannel & marketplace selling — sell one catalog on Amazon/eBay/Etsy/Walmart/TikTok Shop/Google/Meta from one store backend, oversell prevention, channel-manager selection (LitCommerce, Sellbrite, Codisto, CedCommerce, Channable) |
| `/sales-merchant-of-record` | Merchant of Record / payments-provider selection (Paddle vs Lemon Squeezy vs Polar vs Creem vs Stripe; MoR vs self-managed tax) |
| `/sales-subscription-billing` | Recurring-billing engine selection & operation (Chargebee, Recurly, Maxio, Zuora, Stripe Billing, Orb, Lago, Zoho Billing); usage-based/metered pricing, dunning, proration, revenue recognition — stay seller of record, keep tax in-house |
| `/sales-digital-products` | Digital product sales (Gumroad, Podia, Payhip, Lemon Squeezy, etc.) |
| `/sales-print-on-demand` | Print-on-demand provider selection (Printful vs Printify vs CustomCat vs Gelato vs Gooten vs SPOD — fulfillment model, margins, shipping geography, branding, sample vetting) |
| `/sales-store-migration` | Ecommerce store migration / replatforming (Shopify ↔ WooCommerce ↔ Wix ↔ BigCommerce — tool selection, 301 redirect maps, delta sync at cutover, integration re-wiring, verification) |
