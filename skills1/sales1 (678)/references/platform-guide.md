...
## Email Finder
- **What it is**: B2B email discovery — provide a first/last name plus a company domain or company name, and ZeroBounce returns the most likely corporate email
- **Endpoint**: `GET /v2/guessformat` (params: `api_key`, `domain` OR `company_name`, optional `first_name`/`middle_name`/`last_name`); returns `email`, `email_confidence` (HIGH/MEDIUM/LOW), `did_you_mean`, `failure_reason`
- **Bulk variant**: Domain Search API on the bulk host (`domain-search/sendfile|filestatus|getfile`) for many domains at once
- **Credit cost**: 20 credits per successful find (or 1 query on a Deliverability Toolkit subscription); undetermined results cost 0 credits
...
## Email Scoring (AI)
- **What it is**: AI-powered catch-all validation and quality scoring — goes beyond traditional validation to score addresses on a 0–10 scale
- **Why it matters**: Catch-all domains accept all emails at the SMTP level, so traditional validation cannot confirm individual mailbox existence. AI Scoring uses behavioral/statistical analysis to predict whether a specific address is real and likely to engage.
- **Score range**: 0–10 quality score (0/1 = lowest quality, 10 = high probability of interaction). Primarily a bulk CSV workflow (upload file → download CSV with an added Quality Score column); a single-email real-time scoring endpoint also exists.
...
| **Rate limit** | 80,000 validation requests per 10 seconds (100,000 for ZeroBounce ONE™); 1-minute temp block on burst; 200 bad-key requests/hour → 1-hour block |
| **SDKs** | 13 official SDKs/wrappers (Python, JavaScript, PHP, Java, C#/.NET, Ruby, Go, Rust, C, C++, Delphi, iOS, Android) |
| **MCP** | Official MCP server `@zerobounce/mcp` (GitHub `zerobounce/zerobounce-mcp`) |
| **API version** | REST v2 |
...
| `/v2/guessformat` | GET | Email Finder — find email by name + domain/company (20 credits/find) |
| `domain-search/sendfile` (bulk host) | POST | Bulk Domain Search — find emails/formats across a list of domains |
| `/v2/scoring/sendFile` (bulk host) | POST | A.I. Scoring — upload CSV for 0–10 quality scoring |
...
## Pricing (re-verified 2026-06-13 — confirm exact volume tiers on the live pricing page)

| Plan | Price | Credits | Notes |
|---|---|---|---|
| **Free** | $0/mo | 100 validations/month | Requires a business/premium domain to sign up; no credit card |
| **Pay-As-You-Go** | From $20 (2,000 emails) | 2,000+ credits | One-time purchase, credits never expire; per-credit cost drops with volume (~$0.008/credit at 2K, lower at 100K+) |
| **ZeroBounce ONE™** | $99/mo (≈$79/mo billed annually, ~20% off) | 25,000 credits/month | Subscription including the full platform toolkit; scales 25K → 1M emails/mo; 15% discount on extra PAYG credits |
| **Enterprise** | Custom | 1M+ credits | Custom pricing for high-volume users |

**Credit economics**:
- Email validation: 1 credit = 1 email validated (bulk or real-time); Unknown results are free (never charged)
- Email Finder / Domain Search: 20 credits per successful result (1 query on a Deliverability Toolkit subscription)
