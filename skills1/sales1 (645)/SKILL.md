...
**API selection**: Use Post-Production API for batch upload workflows. Use Live Booking API for scheduled live captioning events. Use Insights API (Gen.V) for AI-generated summaries, keywords, and quizzes from completed transcripts — note this API lives on a separate host, `https://ai.verbit.co/api/v2/insights`, not the main `api.verbit.co`. For real-time streaming transcription over WebSocket, use the official Python SDK `verbit-streaming-sdk` (`pip install --upgrade verbit-streaming-sdk`). Base URLs: US `api.verbit.co`, EU `eu.api.verbit.co`, sandbox `sandbox-api.verbit.co`.
