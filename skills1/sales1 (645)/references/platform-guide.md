...
- Available via Insights API after transcription completes (`GET https://ai.verbit.co/api/v2/insights`)
...
### API base URLs
- Main API (US): `https://api.verbit.co/api/`
- Main API (EU): `https://eu.api.verbit.co/api/`
- Sandbox (testing): `https://sandbox-api.verbit.co/api/`
- Insights API (Gen V) — separate host: `https://ai.verbit.co/api/v2/insights`
...
6. Optionally generate insights via `POST https://ai.verbit.co/api/v2/insights/generate` (Insights API is on the `ai.verbit.co` host, not the main API host)
