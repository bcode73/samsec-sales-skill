...
- **Rate limits**: Per-endpoint, enforced per account. Exceeding a limit returns `429 Too Many Requests`. Indicated via response headers: `x-sib-ratelimit-limit` (max requests in the current window), `x-sib-ratelimit-remaining` (requests left), `x-sib-ratelimit-reset` (time until reset, typically in seconds). There is no `retry-after` header — read `x-sib-ratelimit-reset` to know how long to wait. (verified 2026-06-13)
...
Limits are per account and vary by endpoint group. Standard (default) limits (verified 2026-06-13):

| Endpoint group | RPS | RPH |
|---|---|---|
| Send transactional email — `POST /smtp/email` | 1,000 | 3,600,000 |
| Send transactional SMS — `POST /transactionalSMS/send` | 150 | 540,000 |
| Events — `POST /events` | 10 | 36,000 |
| Contacts endpoints | 10 | 36,000 |
| All other endpoints | — | 100 |

**Advanced** limits (Professional / Enterprise) roughly double the above: email 2,000 RPS / 7,200,000 RPH, SMS 200 RPS / 720,000 RPH, events 20 RPS / 72,000 RPH, contacts 20 RPS / 72,000 RPH, all other 200 RPH.

**Extended** limits (Enterprise only): email 6,000 RPS, SMS 250 RPS, contacts 60 RPS, all other 600 RPH.

On `429`, back off using the `x-sib-ratelimit-reset` value. Batch where possible (`POST /contacts/import`, `/ecommerce/products/batch`, etc.) to stay under contact/other-endpoint limits.
...
### Send SMS
`POST /transactionalSMS/send` (verified 2026-06-13)
...
### SMS statistics (verified 2026-06-13)
`GET /transactionalSMS/statistics/aggregatedReport` — aggregated over a period
`GET /transactionalSMS/statistics/reports` — per-day report
`GET /transactionalSMS/statistics/events` — unaggregated events

Statistics endpoints accept `startDate`, `endDate`, `days`, `tag`, `sort`, `limit`, `offset`.
...
`POST /whatsapp/sendMessage` (verified 2026-06-13)

### WhatsApp statistics
`GET /whatsapp/statistics/events` — unaggregated WhatsApp events
...
### Webhook event types (verified 2026-06-13)

**Transactional events**: Sent, Delivered, Opened, Clicked, Soft Bounce, Hard Bounce, **Invalid Email**, **Deferred**, Complaint, Unsubscribed, Blocked, **Error**. (The example above omits `sent`, `invalidEmail`, `deferred`, and `error` — add them if you need full coverage.)

**Marketing events** (set `"type": "marketing"`) use snake_case keys: `delivered`, `opened` / `proxy_open`, `click`, `hard_bounce`, `soft_bounce`, `spam`, `unsubscribe`, `list_addition`, `contact_updated`, `contact_deleted`, `unique_coupon_sent`. Marketing SMS events: `sent`, `delivered`, `replied`, `soft_bounce`, `hard_bounce`, `subscribe`, `unsubscribed`, `skip`.

**Security**: Brevo does NOT sign webhooks with an HMAC signature header. To restrict access, IP-allowlist Brevo's webhook IP ranges rather than relying on a signature.
