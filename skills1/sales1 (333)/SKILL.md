...
- **Plan decision**: Free (basic transcription + summaries) → Pro ($4.99/mo, E2E encryption, advanced summaries, priority support) → Enterprise (custom, API access, custom analysis, dedicated support). Note: the official FAQ says paid/"premium features will be available through paid subscriptions in the future" — confirm Pro is actually purchasable before recommending the upgrade.
...
- **No published transcription-accuracy number** — the official site publishes no word-level accuracy %; it markets a "98% meeting-analysis success rate" (a throughput stat, not transcription accuracy). Don't quote a specific accuracy figure as fact. Accuracy varies by language and audio quality.
...
**Fix**: MBox uses Gemini Pro for transcription; the company publishes no official word-level accuracy figure (it advertises a "98% meeting-analysis success rate"). Ensure your microphone is high quality and positioned close. Reduce background noise. Check if the language being spoken matches the transcription language setting. Note MBox analyzes Google Meet's live text captions, so caption quality affects results. For consistently poor results, consider Tactiq or Fathom which use different STT engines.
