...
3. **Which ChurnZero edition?** (Essentials, Growth, Enterprise, unsure)
...
- **REST API requires Growth or Enterprise.** ChurnZero's current editions are Essentials / Growth / Enterprise; the OData REST API (and custom reporting) unlock at Growth+ — the entry-level Essentials tier has no API access. Earlier docs that called the API "Enterprise-only" are out of date. The Push/Analytics write API is available on all editions.
- **Custom dashboards/reporting need Growth+.** Essentials users get limited default dashboards (41 G2 mentions). Workaround: export data to Excel/Google Sheets via the live spreadsheet integration, or use the data warehouse connectors with Tableau/Power BI.
...
**Approach**: Read platform-guide.md reporting section. On the entry-level Essentials edition, custom dashboards aren't available — workaround: build a segment filtering by health score range + renewal date window, then export to Google Sheets via the live spreadsheet integration. On Growth or Enterprise, use the custom dashboard/reporting builder. For BI-level analysis, connect Snowflake/BigQuery via the data warehouse integration.
