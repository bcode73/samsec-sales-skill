...
2. **REST API v1** — OData v4 compliant, for reading data out of ChurnZero (Growth and Enterprise editions — see Pricing note in platform-guide.md)
...
## API 2: REST API v1 (OData — Growth & Enterprise)
...
| `GET /UserAccount` | ChurnZero internal user (CSM) accounts |
...
| `GET /JourneyMilestoneAchievement` | Milestone achievement records |
| `GET /JourneyMilestoneTask` | Tasks tied to journey milestones |
| `GET /JourneyMilestoneProgress` | Per-milestone progress detail |
| `GET /JourneyMilestoneStepProgress` | Per-step-within-milestone progress |
...
| `GET /ChurnScoreFactorCalculation` | Per-factor calculation breakdown |
| `GET /Task` | CSM tasks |
...
| `GET /SegmentColumnSet` | Segment column-set (custom field set) definitions |
| `GET /Note` | Account/contact notes |
| `GET /NoteType` | Note type definitions |
| `GET /Message` | Messages (emails/in-app) sent or logged |
| `GET /Meeting` | Meetings logged in ChurnZero |
| `GET /MeetingType` | Meeting type definitions |
...
**Filter operators:** `eq`, `ne`, `gt`, `lt`, `ge`, `le`, `in` (set membership), combined with logical `and`/`or`; string functions `startswith()`, `endswith()`, `contains()`; lambda expressions `any()`/`all()` over collection properties.
...
- Default page size: **100 results** (fixed for all collection requests)
- Use the `@odata.nextLink` URL from the response body for server-side paging — it contains either a `$skip` or a `$skiptoken` parameter that fetches the next page
- Or manually increment `$skip` by the page size for client-side paging
...
| 409 | Conflict — the underlying model/schema changed (e.g. a synced entity was reconfigured); refetch metadata and retry |
...
| Edition | All editions | Growth & Enterprise |
