...
| Enterprise+ | $39.75/user | $29.75/user | Unlimited | 8 hours | 300/mo | HIPAA, SSO/SAML/SCIM, domain capture, custom data retention, workspace onboarding. Requires 5+ licenses |
...
- Status: Open beta
- Auth: Bearer token in the `Authorization` header (`Authorization: Bearer ACCESS_TOKEN`); tokens issued via OAuth 2.1 with dynamic client registration (Authorization Code + refresh tokens)
- Rate limit: 100 requests/minute per user → HTTP `429` when exceeded
- Pagination: cursor-based (`cursor` = `id` of the last meeting in the previous page), newest-first
- Endpoints: List Meetings (`GET /v1/meetings`, filters `limit`/`cursor`/`start_time_ms.gte`, `expand=summary,metrics`), Retrieve a Meeting (`GET /v1/meetings/{meeting_id}`), Retrieve a Live Meeting (real-time transcript + chapter summaries for `live_enabled` meetings)
- Planned: Additional endpoints/tools, webhook/event support, expanded docs
...
- **Security**: HMAC SHA-256 signature in the `X-Read-Signature` header. Compute HMAC SHA-256 over the raw request body with your webhook signing key and compare (timing-safe) to the header; reject if missing/invalid.
