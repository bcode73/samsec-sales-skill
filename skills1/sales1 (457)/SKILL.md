...
- Need HIPAA/SSO? → Enterprise+ required ($39.75/mo, 5+ licenses)
...
- **API is in open beta.** Auth is now Bearer token (`Authorization: Bearer …`, OAuth 2.1 dynamic client registration); endpoints are `GET /v1/meetings` (List), `GET /v1/meetings/{meeting_id}` (Retrieve), and Retrieve a Live Meeting. Rate limit 100 req/min/user → HTTP 429; cursor-based pagination. Still beta — expect breaking changes and watch for added endpoints/webhook-event support.
...
**Solution**: Add a delay or re-fetch via the REST API after receiving the webhook. Use the `session_id` from the payload to poll for the complete report — call `GET /v1/meetings/{meeting_id}` with `expand=summary,metrics` (Bearer auth), respecting the 100 req/min/user limit.
