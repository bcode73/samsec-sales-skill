...
- **Max 10 API keys per org.** Plan one key per integration, not per user. Keys are scoped at creation (User-limited, User-full, or Org-limited) and the scope is permanent. Deleting a key immediately revokes all integrations using it; legacy pre-scoped keys go read-only and migrate to Org-limited by May 31, 2026.
