...
**Auth**: Two headers on every request — `X-Api-Key` (API key) and `X-Api-Password` (API secret). Find both in Copilot Web App → Workspace Settings → Integrations → Copilot API.
**Content-Type**: `application/json`
**Rate limit**: 10 requests/second and 100,000 requests/week (the week resets Sunday 00:00 GMT). Short bursts up to ~100 requests are allowed if recent activity is low.
**Plan gate**: Enterprise plan for API access (credentials surface in Workspace Settings).

**Endpoints:**

| Method | Path | Purpose |
|---|---|---|
| GET | `/calls` | Paginated list of all recorded calls. Add `includeVideo=true` to include a `video_url` field. |
| GET | `/call-details` | Transcript, summary, and analytics for one call (requires `id`). Excludes calls without transcripts. |
| GET | `/users` | All users in the Copilot workspace |
| GET | `/accounts` | CRM-related objects (accounts) linked to Copilot calls |
| GET | `/opportunities` | Opportunity data |
| GET | `/topics` | Basic keyword-only topic results |
| GET | `/v2/topics` | Contextual, AI-extracted topic themes |
| GET | `/scorecards` | Coaching scorecard data |
| POST | `/create-call` | Submit call metadata + audio URL from external tools (Twilio, Zoom, etc.) for transcription/analysis |
| PUT | `/update-account` | Write back account fields to a Copilot CRM object |
| PUT | `/update-contact` | Write back contact fields |
| PUT | `/update-deal` | Write back deal/opportunity fields |

**Common query params**: `limit` (records per page, max 100), `skip` (pagination offset), `includeVideo=true`, `filterModifiedGt` / `filterModifiedLt` (modification-time range).

**`POST /create-call` required fields**: `source_id` (unique external id), `type` (`"RECORDING"`), `call_time` (ISO timestamp), `audio_url` (publicly accessible), `user_emails` (participant rep emails). Optional: `stereo_to_mono`, `external_transcript`, `header_key`/`header_value`.

**Webhooks**: No outbound real-time webhooks are documented for the Copilot API — integrations poll the `/calls` and `/call-details` endpoints (filter with `filterModifiedGt`/`filterModifiedLt`) rather than subscribe to events.

For full Copilot API documentation, see api-doc.copilot.clari.com.
...
| **API** | Core API (100 req/sec) + documented Copilot REST API (10 req/sec, 100K/week) with call list/details, CRM write-back, and call upload — no webhooks (Enterprise only) | Richest API in category (REST + webhooks, 3 req/sec, 10K/day) |
