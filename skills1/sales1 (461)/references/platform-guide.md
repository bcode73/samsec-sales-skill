...
Webhook headers: `x-reclaim-webhook-type`, `x-reclaim-api-version` (one of `v2024-10-02`, `v2025-01-15`, `v2025-09-26`), `x-reclaim-signature-256` (HMAC-SHA256). Webhook subscriptions are managed via the API at `/api/team/current/webhooks` (see api-reference).
...
**Gotcha**: Webhooks require Business plan ($15/seat/mo). Delivery is "at most once" with exponential backoff retries (return a `Retry-After` header to override the backoff). Endpoints suspended after more than 24hr of continuous failures.
...
1. Admin creates webhook configuration in Team Settings → Webhook Settings (or via the API: `POST /api/team/current/webhooks`; generate a secret with `GET /api/team/webhook/generate-secret`)
2. Specify: Name, HTTPS endpoint URL, auto-generated Secret
3. Attach webhook config to specific Scheduling Links (API: `GET /api/team/current/webhooks/{id}/associations` to inspect)
