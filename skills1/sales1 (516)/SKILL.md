...
1. **Webhooks are paid-only and signed.** Account webhooks require a Standard or Professional plan; free plan users can only use Zapier for event-driven integrations. There are 8 topics (subscribers/new, subscribers/updated, subscribers/unsubscribed, groups/new, groups/new-subscriber, groups/unsubscribed, campaigns/new, bounces/new). Each webhook has a **Signing Secret** — verify the HMAC over the raw body before trusting a payload.
...
- Rate limits are per-account per-minute — read `X-RateLimit-Limit/-Remaining/-Reset` on every response; on a `429` honor the `Retry-After` header (seconds to wait) before retrying
