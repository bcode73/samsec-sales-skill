...
| Transactional email | API | Template-based or raw HTML; one-off sends via `POST /v2/message/send` (from/to/subject/html). Requires a verified transactional sender identity/domain |
...
| Webhooks | API (paid plans only) | 8 topics: subscribers/new, subscribers/updated, subscribers/unsubscribed, groups/new, groups/new-subscriber, groups/unsubscribed, campaigns/new, bounces/new. Payloads are HMAC-signed with a per-webhook Signing Secret |
...
- **Webhooks**: Account webhooks (paid plans) with 8 topics across subscriber, group, campaign, and bounce events; payloads HMAC-signed via a per-webhook Signing Secret
...
app = Flask(__name__)
SIGNING_SECRET = "YOUR_WEBHOOK_SIGNING_SECRET"  # from Account settings > Webhooks

@app.route("/webhooks/sender", methods=["POST"])
def handle_sender_webhook():
    # Verify the HMAC signature over the raw body. Sender provides a per-webhook
    # Signing Secret; confirm the exact signature header name from the delivered
    # request headers (not published verbatim in the help docs).
...
**Gotchas**: Webhooks require a paid plan (Standard+). There are 8 topics: `subscribers/new`, `subscribers/updated`, `subscribers/unsubscribed`, `groups/new`, `groups/new-subscriber`, `groups/unsubscribed`, `campaigns/new`, `bounces/new`. Group-scoped topics (`groups/new-subscriber`, `groups/unsubscribed`) require `relation_id` (the group ID). Verify payloads with the per-webhook **Signing Secret** (HMAC); rotating the secret immediately invalidates the old one.
