...
### AI Scorecards (Business)
Methodology-based scorecards (MEDDPICC, BANT, custom criteria) applied to recorded calls. Business-only plan feature (coaching metrics + AI scorecards) as of 2026-06-13 — see Pricing.
...
| Premium | $20/mo | $16/mo | 1 | No | No | For individual power users |
| Team | $19/user/mo | $15/user/mo | 2 | No | No | Shared meeting library |
| Business | $34/user/mo | $25/user/mo | 2 | Yes | Yes | Salesforce + HubSpot sync, coaching metrics + AI scorecards |

**Plan-gated features:**
- **CRM sync (Salesforce, HubSpot)**: Business only
- **AI Scorecards / coaching metrics**: Business only (verified 2026-06-13 against the official pricing page — "Coaching metrics and AI scorecards" is listed under Business; the Team plan does NOT include them)
...
- `highlights[]` — Highlight (type = bookmark label, summary, text, start_time, end_time in seconds from recording start) — toggled by `include_highlights=true` (added/verified 2026-06-13)

You can also filter `GET /meetings` by `meeting_type={name}`. Discover valid names via the **`GET /meeting_types`** endpoint (returns `name`, `status` active/inactive, `created_at`) — see `fathom-api-reference.md`.
...
- **Heavy requests: 30 per 60-second window** (down to **5 per 60s** in high-activity periods). Heavy = the recordings endpoints (`GET /recordings/{id}/transcript`, `GET /recordings/{id}/summary`) and any `GET /meetings` call with `include_summary` or `include_transcript`. This is stricter than the global 60/min and is the binding limit for any transcript/summary backfill — verified 2026-06-13.
- Headers on every response:
  - `RateLimit-Limit` — the max for the applicable limit
