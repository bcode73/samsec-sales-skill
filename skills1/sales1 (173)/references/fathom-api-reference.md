...
### Heavy request rate limits

Heavy requests — the recordings endpoints (`GET /recordings/{recording_id}/transcript`, `GET /recordings/{recording_id}/summary`) and `GET /meetings` calls made with `include_summary` or `include_transcript` — are subject to a stricter limit: a maximum of **30 calls in a 60 second window**. During high-activity periods this may be reduced to **5 every 60 seconds**. (Verified live 2026-06-13 against https://developers.fathom.ai/api-overview.md — this heavy-request tier is in addition to the global 60/min limit and applies to any pipeline that pulls transcripts or summaries in bulk.)
> ## Documentation Index
> Fetch the complete documentation index at: https://developers.fathom.ai/llms.txt
> Use this file to discover all available pages before exploring further.
    ...
    get:
      ...
      parameters:
        ...
        - in: query
          ...
          description: >-
            Include the highlights for each meeting. Each highlight object has
            `type` (bookmark label), `summary`, `text` (when it differs from
            summary), `start_time`, and `end_time` (seconds from recording
            start). (Added per live docs verified 2026-06-13.)
          schema:
            type: boolean
            default: false
        - in: query
          name: meeting_type
          description: >-
            Filter by meeting type name. Returns only meetings assigned the
            meeting type with this name. Discover valid names via the
            `GET /meeting_types` endpoint. (Documented filter verified
            2026-06-13.)
          schema:
            type: string
...
<!-- Added 2026-06-13 from https://developers.fathom.ai/api-reference/meetings/list-meeting-types.md (page is new since the 2026-04 capture). -->

> List the meeting types defined for your account. Use the `name` values to filter `GET /meetings?meeting_type=...`.
...
paths:
  /meeting_types:
    get:
      ...
      description: >-
        Returns a paginated list of the meeting types configured for your
        account. Authenticate with the `X-Api-Key` header or a Bearer token.
      parameters:
        - in: query
          name: cursor
          description: Cursor for pagination.
  ...
  schemas:
    ...
    MeetingType:
      ...
      properties:
        name:
          type: string
          description: The meeting type identifier.
          example: Quarterly Business Review
        status:
          type: string
          enum:
            - active
            - inactive
          description: Whether the type is currently assignable to new meetings.
        created_at:
          type: string
          format: date-time
