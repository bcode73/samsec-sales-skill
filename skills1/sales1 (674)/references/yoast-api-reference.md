...
> **Gotcha (per current Schema API docs):** `wpseo_json_ld_output` disables the schema rendered into the page `<head>`, but it does NOT strip schema from the `yoast_head_json` REST API attribute — that data is produced via a separate code path. Headless consumers reading `yoast_head_json` will still receive schema even after this filter is applied; filter/strip it client-side if you don't want it.
