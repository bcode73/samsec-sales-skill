...
Base URL: `https://api.attention.tech/v2` (OpenAPI 3.0.1, spec version 1.8.0 as of 2026-06-13)
Auth: API key in the `Authorization` header as a Bearer token (`Authorization: Bearer YOUR_API_KEY`); OpenAPI security scheme `APIKeyHeader`
...
| GET | `/conversations/by-external-id/{externalId}` | Look up a conversation by external system ID |
| POST | `/conversations/{id}/crm/confirm` | Confirm AI-extracted CRM intelligence before push |
| PUT | `/conversations/{id}/linked-crm-records` | Update CRM records linked to a conversation |
| POST | `/crm/query` | Query the connected CRM (Salesforce/HubSpot) |
| POST | `/snippets` | Create shareable snippet |
| POST | `/createScorecardResult` | Score a conversation |
| POST | `/scorecards/summary` | Scorecard usage stats (POST, body filters) |
| GET/POST/PATCH | `/field-configurations` (+ `/items`) | CRM auto-update field-mapping schema |
| POST | `/super-agent/messages` | Send Super Agent message (async, optional `callback_url`) |
...
### MCP Server
- Official **hosted/remote** MCP server at `https://api.attention.tech/mcp` (MCP over HTTP Streamable transport)
- Auth: OAuth 2.1 + Dynamic Client Registration for interactive clients (Claude.ai, Cursor); API Key JWTs as Bearer tokens for programmatic use
- Search calls, get transcripts, analyze deals/forecasting, review scorecards, query CRM, surface coaching insights via natural language
- Docs: https://docs.attention.com/mcp/authentication
