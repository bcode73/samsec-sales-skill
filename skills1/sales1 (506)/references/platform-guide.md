...
|---|---|---|
| **Basic** | $10/user/mo | Unlimited links/calendars/teams, collective/round-robin/group, workflows, reminders, CRM integrations (HubSpot, Close), Zapier, API, webhooks, website embedding, Chrome extension, conferencing integrations |
| **Premium** | $17/user/mo | All Basic + ranked availability, custom domains, custom sending domains, delegated access, paid bookings (Stripe), remove branding |
...
def handle_webhook():
    # Header is formatted "sha256=<hex>" — strip the prefix before comparing.
    signature = request.headers.get("x-savvycal-signature", "").split("=", 1)[-1]
    expected = hmac.new(
...
| **Ranked availability** | Yes — preferred times shown first (Premium) | No | No |
