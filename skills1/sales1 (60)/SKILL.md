---
name: sales-bluedot
description: "Bluedot platform help — bot-free AI note-taker with video recording, Chrome extension + desktop/mobile/Apple Watch apps, Svix webhooks, MCP server (app.bluedothq.com/api/v1/mcp) for reading transcripts/summaries, screen recording with webcam overlay. Use when setting up Bluedot for a sales team, configuring Bluedot webhook or MCP integrations, syncing Bluedot meeting notes to HubSpot or Salesforce, troubleshooting Bluedot desktop app crashes or recording drops, choosing between Bluedot plans (Free/Basic/Pro/Business/Unlimited), comparing Bluedot vs Fathom or Fireflies for bot-free recording, or debugging Bluedot Chrome extension issues in managed IT environments. Do NOT use for comparing all note-takers (use /sales-note-taker) or reviewing a specific call for coaching (use /sales-call-review)."
...
- Sales team needing CRM auto-sync → Business ($32/mo) — this is the lowest plan with CRM (HubSpot/Salesforce) and ATS integration, and it also includes SSO & SCIM
- Enterprise wanting fixed pricing across unlimited paid seats (vs per-member) → Unlimited (custom pricing); SSO/SCIM itself is already on Business
...
- **No public REST API for writes/queries, but there is an MCP server.** Bluedot 2.1 added an MCP server (`https://app.bluedothq.com/api/v1/mcp`, OAuth auth) that lets AI assistants browse meetings, read transcripts (with speaker labels + timestamps), search, and pull structured summaries. MCP access may be gated at the workspace level on Enterprise plans. Ingress/push is still webhook (Svix) + CRM sync + Zapier/Make.
