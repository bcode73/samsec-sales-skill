...
- **Auto-recording** (Bluedot 2.1+) — desktop app can auto-record audio + video; Chrome extension can auto-record audio — no manual "record" press before meetings.
- **Expanded desktop capture** (Bluedot 2.1+) — desktop app captures WhatsApp calls, phone calls, and in-person chats beyond online meetings.
- **Apple Watch app** (Bluedot 2.1+) — record and transcribe in-person conversations one-tap from the watch, no laptop/phone required; can create Notion pages from conversations.
...
| Business | $32/mo | $39/mo | + CRM integration, ATS integration, SSO & SCIM, unlimited imports, unlimited free members |
| Unlimited | Custom | - | Fixed pricing with unlimited paid seats + all features (custom data retention, etc.) |
...
- CRM integration (HubSpot / Salesforce) → Business+
- ATS integration → Business+
- Unlimited free members → Business+
- SSO / SCIM → Business+ (per the live pricing page, "SSO and SCIM" is listed as a Business plan feature, not Unlimited-only; Unlimited differs by offering fixed pricing with unlimited paid seats)
- Custom data retention → Business+ / Unlimited
- Topic trackers → Business+ (coming soon)
- Public webhooks → Basic+
- MCP server access → may be controlled at the workspace level on Enterprise/Unlimited plans (see Webhook/MCP section)
...
### CRM integrations (Business+ — listed on the pricing page as generic "CRM integration" / "ATS integration"; HubSpot & Salesforce named in help center + marketing)
- **Salesforce** — auto-enriches contact deal records with meeting links and AI notes, syncs to right contacts/companies/deals. Bluedot 2.1 adds **Voice to CRM** (turn conversations into CRM-ready notes, fields, and follow-ups) and **custom field support** for Salesforce.
- **HubSpot** — syncs meeting notes and action items to contacts and deals. Bluedot 2.1 adds Voice to CRM and **custom field support** for HubSpot.
- **Pipedrive / Copper / Zoho** — referenced in earlier research but NOT listed on the current help-center Integrations collection (which shows only Salesforce, HubSpot, Notion, Slack, Zapier, Make, Webhook, Google Drive, MCP). Treat availability as unverified; confirm with Bluedot before relying on it.
...
## MCP server (programmatic read access — Bluedot 2.1+)

Bluedot ships an **MCP (Model Context Protocol) server** that gives AI assistants read access to your recordings. This is the closest thing to a queryable API and supersedes the old "webhook-only, no way to programmatically read meetings" assumption.

- **Server URL**: `https://app.bluedothq.com/api/v1/mcp`
- **Auth**: OAuth — in the client you "Connect and complete authentication" against your Bluedot account (no API key shown in docs). Org admins authenticate via Personal Settings → Connectors → Bluedot.
- **Capabilities**: browse/list meetings from your workspace; read full transcripts including speaker labels and timestamps; search across titles, descriptions, and transcripts; retrieve structured meeting summaries by section; look up participants; filter recordings by date, uploader, collection, or participant.
- **Clients**: docs detail Claude (Customize → Connectors → Add custom connector → name "Bluedot" → enter Server URL → authenticate). Also works with other MCP clients; ChatGPT is referenced generally.
- **Plan gate**: "MCP access may be controlled at the workspace level for Enterprise plans" — Enterprise/Unlimited admins may need to enable it. You also need a paid Claude/ChatGPT subscription on the client side.
- **Access scope**: the assistant can only access data the Bluedot account already has permission to view; MCP grants no additional permissions.

Note: there is still **no documented public REST API for writes or arbitrary queries** — the MCP server is read-oriented and ingress is still via webhook/CRM sync/Zapier/Make.

## Data model

There is no documented public REST API for writes; data is read via the MCP server (transcripts/summaries/search) and pushed out via webhook payloads and CRM sync. Key objects:
...
- **Custom data retention** — configurable retention policies for compliance control (Business+/Unlimited; added in Bluedot 2.1)
