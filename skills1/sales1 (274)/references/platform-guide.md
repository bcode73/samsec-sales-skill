...
- **Custom pricing only** — based on features needed + monthly request volume. No public price list, no self-serve signup for paid plans.
- **Free trial available** — test the API before committing.
- **Two named plans exist**: **Business** and **Enterprise** (per Kevel's SLA). They differ by resource limits rather than a published feature matrix:
...
- **Rate limit**: Management + Reporting APIs are capped at **50 requests/second combined**.
- **SLA**: Decision API <175 ms p95 / ≥99.99% uptime; Management API <500 ms p95 / ≥99.9% uptime.
