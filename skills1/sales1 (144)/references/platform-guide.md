...
| **Zapier** | Available | Devi → External | Published Zapier app. **Trigger-only** — one trigger: **New Posts** ("Triggers when lead found"). No actions (you can't write back into Devi). Use the trigger to fan out leads to 8,000+ Zapier apps (HubSpot, Slack, Gmail, Facebook Pages, etc.). Verified 2026-06-13. |
| **Zapier MCP** | Available (via Zapier) | Devi → AI agent | Zapier auto-generates an MCP endpoint that exposes the Devi Zapier integration to MCP-capable assistants. This is Zapier's wrapper, not a native Devi MCP server. |
| **Make** | Not available | — | No native Make integration (no evidence of a published Make app as of 2026-06-13) |
| **REST API** | Not available | — | No public REST API |
| **MCP (native)** | Not available | — | No native Devi MCP server (reachable only via Zapier MCP, above) |

**Workaround for automation:** Two paths now exist. (1) **Zapier** — connect the **New Posts** trigger ("Triggers when lead found") to any of Zapier's 8,000+ apps; trigger-only, so it's egress of leads only (no write-back to Devi). (2) **Webhooks** — use Settings > Webhook with a middleware (n8n, Make webhook trigger, or custom endpoint) to route Devi leads to CRM, Slack, or other tools. The webhook fires when new leads are detected — connect a webhook relay to transform and forward the payload. Prefer Zapier for no-code routing; prefer the raw webhook when you need full control over the payload or a self-hosted relay.
...
**Alternative with Zapier (no-code, fastest):**
1. In Zapier, create a Zap with the **Devi AI → New Posts** trigger ("Triggers when lead found")
2. Add a Filter by Zapier step to keep only high-intent leads
3. Add the action: Slack "Send Channel Message", or HubSpot "Create/Update Contact", etc.
Note: Devi's Zapier app is trigger-only — there are no Devi actions, so this is one-way (Devi → other app).

**Alternative with n8n (for CRM routing, full payload control):**
...
Devi has two egress paths: the native **webhook** (Settings > Webhook) and a **published Zapier app** (trigger-only: **New Posts** / "Triggers when lead found"). For no-code routing, Zapier is now the simplest option. The webhook remains the right choice when you need full payload control or a self-hosted relay. There is still no REST API and no write-back path — both options are one-way (Devi → external). Architecture for the raw-webhook path:
