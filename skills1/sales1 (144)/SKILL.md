...
- **No REST API.** Devi has no public REST API. For external integrations you have two egress paths: the native webhook (Settings > Webhook) and a published **Zapier** app. The Zapier app is **trigger-only** — one trigger, **New Posts** ("Triggers when lead found"), with no actions, so you can fan leads out to 8,000+ apps but can't write back into Devi. No native Make app and no native MCP server (Devi is reachable via Zapier MCP only). If you need true programmatic data access or write-back, this isn't the right tool.
...
**Skill does**:
1. Offers two paths: the published **Zapier** app (trigger-only, "New Posts") for no-code routing, or the native webhook (Settings > Webhook) for full payload control
2. For HubSpot specifically, recommends the Zapier path: Devi New Posts trigger → HubSpot create-contact action; for a custom CRM, shows a webhook relay (webhook.site or n8n) to transform the payload
3. Suggests mapping Devi lead data (platform, keyword match, post content) to CRM contact fields
4. Notes the limitation: no REST API and trigger-only Zapier, so both paths are one-way (Devi → CRM) with no write-back
**Result**: Lead pipeline from Devi → Zapier/middleware → CRM
