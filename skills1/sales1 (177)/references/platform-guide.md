...
- **API**: REST API for custom integrations (Team+). Base URL `https://{subdomain}.fellow.app/api/v1/`, `X-API-KEY` auth. Outbound webhooks (Svix-signed) for `ai_note.generated`, `ai_note.shared_to_channel`, `action_item.assigned`, `action_item.completed`. See `fellow-api-reference.md`.

### MCP Server
- **Official Fellow MCP server**: `https://fellow.app/mcp`, OAuth auth (admin must enable). Works with Claude (Custom Connectors), Cursor, ChatGPT, and any MCP-connector client. Access scoped to the connecting user's visible data.
- A separate community MCP server (github.com/liba2k/fellow-mcp) also exists, wrapping the REST API locally.

## Data model (API)

The API exposes (all list endpoints are `POST` with a JSON body; cursor pagination, `page_size` 1–50):
- **Recordings**: id, title, timestamps, transcript, AI notes, pre-signed `media_url` — `POST /recordings`, `GET /recording/{id}`
- **Notes (AI notes)**: id, title, event details, recording_ids, attendees, markdown content — `POST /notes`, `GET /note/{id}`
- **Action items**: id, text, status, due_date, assignees, AI-detection metadata — `POST /action_items`, `GET /action_item/{id}`, `POST .../complete`, `POST .../archive`
- **User/workspace**: `GET /me`
- **Webhooks**: `GET`/`POST /webhooks` plus get/update/delete

API responses are JSON. Transcripts and AI notes are returned inline (opt in via the `include` flags `transcript` / `ai_notes` / `content_markdown`).
