...
4. For custom builds, points to the REST API at `https://{subdomain}.fellow.app/api/v1/` (auth via `X-API-KEY` header; docs at developers.fellow.ai) — `POST /recordings`, `POST /notes`, `POST /action_items` return JSON with transcript text, AI notes, action items, and metadata; or wire up Svix-signed webhooks (`ai_note.generated`) for push delivery
5. Flags user-scoped keys — for team-wide ingestion, each user needs their own key. Rate limits are 3 req/sec and 10,000 req/day per key
...
**Solution**: From the meeting view, look for a separate transcript tab/section — it's not co-located with the AI notes export button. The REST API (`https://{subdomain}.fellow.app/api/v1/`, docs at `developers.fellow.ai`) provides programmatic access to transcripts in JSON via `POST /recordings` or `POST /notes` with the `include` flags if the UI export is too clunky.
