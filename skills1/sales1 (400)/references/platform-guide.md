...
Pricing is quote-based (no public per-user price). The official pricing page describes a model that combines **seat-based platform access with consumption-based usage for AI-powered workflows** — each tier includes a monthly pool of AI credits.

| Tier | Price (third-party reported) | AI credits | Key features |
|------|------|------------|--------------|
| Amplify Core | ~$100/user/mo | 25,000 | Sequences, call insights, basic account planning, CRM sync |
| Amplify Plus | ~$130/user/mo | 50,000 | + Kaia CI (real-time transcription, sentiment), full deal & pipeline management, coaching |
| Amplify Pro | ~$160/user/mo | 100,000 | + AI Forecast Projection / Scenario Planner, full platform |

> Per-user dollar figures above are third-party-reported, not published by Outreach. The AI-credit allotments and feature splits are from the official pricing page. Forecasting (AI Forecast Projection / Scenario Planner) is Pro-only; Kaia CI starts at Plus.
...
- **Native webhooks**: Event-driven automation. Subscribe per resource+action (e.g. `task`/`completed`, `mailing`/`replied`, `kaiaRecording`/`created`). Deliveries are HMAC-SHA256-signed via the `Outreach-Webhook-Signature` header when a secret is set; payload version 2 adds a `beforeUpdate` object. 5s timeout, up to 4 retries on network failure only. See API reference.
