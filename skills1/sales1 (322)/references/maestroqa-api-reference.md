...
Import batches of CSAT scores. **Max 100 items per request.** Rate limit: 10 req/s, 100 req/min.

> Note: the current CSAT Bulk Ingestion doc documents this endpoint on the `app.maestroqa.com` host (`https://app.maestroqa.com/api/v1/create-customer-feedback`). Post-rebrand `app.maestroqa.com` and `app.rippit.com` are the same company; use the host shown in your account's API docs.
