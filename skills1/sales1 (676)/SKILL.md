...
3. **What plan are you on?** (Support Team $19 / Suite Team $55 / Suite Growth $89 / Suite Professional $115 / Suite Enterprise & Enterprise Plus — quote-only "Talk to Sales") — note AI agents are now billed per Automated Resolution on top of any plan
...
1. **Real costs are 2-3x advertised** — base plan prices don't include Advanced AI ($50/agent), Copilot ($50/agent), QA ($35/agent), or WFM ($25/agent; the Workforce Engagement Bundle pairs QA+WFM at $50). A team on Suite Pro at $115/agent can easily reach $225/agent with add-ons. As of 2026, Suite Enterprise / Enterprise Plus are quote-only ("Talk to Sales") rather than the old fixed $169.
2. **AI agents are now billed per outcome** — Zendesk AI agents are included in every Suite/Support plan but charged per **Automated Resolution** (publicly cited around $1.50 committed / $2.00 pay-as-you-go), so AI cost scales with volume on top of seat prices. The Advanced AI add-on ($50/agent) unlocks AI triage/sentiment/intent on top. Don't assume "AI included" means free.
...
8. **256 API token limit** — accounts can maintain up to 256 active tokens; at capacity you must delete one to create a new one. (Accounts that already exceed 256 are grandfathered to a 2048 limit.)
9. **Webhooks are signed** — verify the `X-Zendesk-Webhook-Signature` header as `base64(HMAC-SHA256(timestamp + body))` using the webhook's signing secret, where the timestamp is the `X-Zendesk-Webhook-Signature-Timestamp` header. Use the `X-Zendesk-Webhook-Invocation-Id` header for idempotent processing. See `references/zendesk-api-reference.md`.
