...
| Suite Enterprise | Talk to Sales | Up to 300 Help Centers, custom roles, advanced data protection, audit logs, dynamic workspaces |
| Suite Enterprise Plus | Talk to Sales | All Enterprise features plus add-on capabilities tailored to your needs |

> As of 2026-06-13, the public zendesk.com/pricing page lists fixed annual prices only for Support Team ($19), Suite Team ($55), and Suite Professional ($115); both Suite Enterprise and Suite Enterprise Plus are now quote-only ("Talk to Sales") — the previously published $169 Enterprise list price is no longer shown. Suite Growth ($89) still exists per Zendesk's official "About the Zendesk Suite plan types" article but its live price is not displayed on the main pricing page; treat $89 as a last-known figure pending confirmation.
...
| Workforce Engagement Bundle (QA + WFM) | $50 | Both QA and WFM combined (saves $10/agent vs buying separately) |
| Advanced Data Privacy | $50 | Enhanced encryption, data masking, access logs |
| Contact Center | $50 | Full CCaaS capabilities |

### AI Agent / Automated Resolution pricing (outcome-based)
As of 2026-06-13, Zendesk's official pricing page states "Zendesk AI agents are included in every Suite and Support plan" and are billed on outcomes via **Automated Resolutions** — the unit of measurement for AI agent usage. You pay only for customer requests the AI agent successfully resolves without escalation to a human. Public guidance cites roughly $1.50 per automated resolution at committed volume and $2.00 pay-as-you-go (corroborated by multiple 2026 sources; exact per-resolution rates are negotiated/quote-based, so confirm with Zendesk). The Advanced AI add-on ($50/agent/month) unlocks advanced automation such as AI triage, sentiment analysis, and intelligent intent detection on top of the included AI agents.

### Employee Service Suite Plans (per agent/month, annual)
- Team: $29 | Growth: $59 | Professional: $99 | Enterprise: Talk to Sales
...
- Webhooks for ticket, article, community post, organization, user, agent availability, omnichannel routing config, live messaging metrics, and messaging events (managed via `/api/v2/webhooks`)
- Webhooks are signed: verify with the `X-Zendesk-Webhook-Signature` header using `base64(HMAC-SHA256(timestamp + body))`, where the timestamp comes from `X-Zendesk-Webhook-Signature-Timestamp`. See `zendesk-api-reference.md` for full signature verification details.
