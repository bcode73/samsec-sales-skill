...
**Import endpoints are stricter**: subscriber import endpoints (e.g. `POST /subscribers/import`) are limited to **5 requests per minute per API token** — separate from the 120 req/min global limit.

## Versioning
Override with `X-Version` header (date format, e.g., `2038-01-19`). Default uses latest.

## Official SDKs
PHP, Go, Node.js, Python, Ruby, Rust
...
- `fields` (optional): custom field data — **additive only** (does not clear existing fields)
- `groups` (optional): array of group IDs — **additive only** (does not remove from unlisted groups)
- `status` (optional): active, unsubscribed, unconfirmed, bounced, junk
- `subscribed_at` (optional): `yyyy-MM-dd HH:mm:ss`
- `ip_address` (optional): valid IP
- `opted_in_at` (optional): `yyyy-MM-dd HH:mm:ss`
- `optin_ip` (optional): valid IP
- `unsubscribed_at` (optional): `yyyy-MM-dd HH:mm:ss`
...
- Imports many subscribers asynchronously; returns an import job to poll.
- **Rate limit: 5 requests/minute per API token** (stricter than the 120 req/min global limit).
