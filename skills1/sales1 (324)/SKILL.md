...
8. **API rate limit is 120 requests/minute globally** — not per endpoint. Batch operations help (up to 50 requests per batch call). Note: subscriber **import** endpoints are stricter at **5 requests/minute per API token**.
