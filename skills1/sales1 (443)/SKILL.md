...
- `/sales-instantly` — Instantly platform help (Lead Finder B2B database as a separate Credits product; load lists via the v2 API)
...
- `/sales-phantombuster` — PhantomBuster platform help (code-free LinkedIn/web automation + scraping "Phantoms", Flows, AI Agents, LinkedIn Leads DB; REST API launch agent→container→result + per-agent webhooks; slots + execution-time limits, session-cookie risk)
...
- `/sales-texau` — TexAu platform help (LinkedIn/web automations + recipes, V3 people/lead search API with Sales Nav URL and job-change filters, MCP server, n8n nodes)
