...
- [Building lists from TexAu (LinkedIn automation + API)](#building-lists-from-texau-linkedin-automation--api)
...
- **Pricing**: Free (50 credits/mo, 1 user). Professional ~$29/mo (2,000 credits/mo, 2 users). Enterprise $262/mo (50,000 credits/mo, 15 users). No charge for duplicates or invalid results. Pricing is in USD ($), not EUR. Verified 2026-06-13 against skrapp.io/pricing.
...
- **Pricing**: Plans are now labeled by monthly credit count (the old named Starter/Standard/Scale/Ultimate tiers are gone). Monthly: 400 credits €26/mo, 1K €39/mo, 2K €69/mo, 5K €129/mo, 10K €179/mo (best value), 25K €259/mo, 50K €449/mo, 100K €719/mo. Free trial = 100 credits. Yearly billing saves ~33%. Pay only for verified emails. Verified 2026-06-13 against anymailfinder.com/pricing.
...
- **Pricing**: Trial Free (50 credits), Starter $39/mo (1K credits), Pro S $99/mo (5K), Pro M $189/mo (20K), Pro L $369/mo (50K), Ultra $738/mo (100K). Verified 2026-06-13 against snov.io/pricing.
...
### Building lists from Instantly (Lead Finder)

- **Lead database**: Instantly's **Lead Finder** is a B2B contact database — search by firmographics, reveal verified emails, and export or push directly into a campaign. It is a **separate paid product (Instantly Credits)**, billed apart from the Outreach seat price
- **Verification**: Built-in email verification helps keep bounce rates down before sending
- **Tight loop**: Its advantage is that found leads drop straight into Instantly campaigns (no export/import) — but if you already have lists, you can `POST /api/v2/leads` to load them instead
- **Watch-outs**: Database depth/quality is generally rated below Apollo/ZoomInfo; budget Credits separately and verify before bulk sending
- **Best for**: Teams already sending in Instantly who want one fewer tool; pair with a deeper database (Apollo/ZoomInfo) when coverage matters
- **Platform skill**: `/sales-instantly`

### Building lists from TexAu (LinkedIn automation + API)

- **Two ways to build**: (1) classic Cloud/Desktop **automations/recipes** that drive LinkedIn, Sales Navigator, Twitter/X, and web scraping no-code; (2) the **V3 API** with `people_search` (700M+ cached index, ~0.1 credit/result — cheapest discovery), `profile_search` (live LinkedIn), and `lead_search` (30+ filters, `salesNavUrl`, `recentlyChangedJobs`, `sessionId` pagination)
- **From a Sales Navigator search**: pass the Sales Nav URL into `lead_search` (`salesNavUrl`) to convert a saved search into a flat lead list, or import the CSV into the V3 table and right-click enrich
- **Reference data first**: call `GET /search_reference_data` (free) to get valid `seniority`/`industries`/`functions`/`companySize` labels before building a `lead_search` body
- **Warm-list angle**: `recentlyChangedJobs: true` surfaces recently-promoted/moved contacts — a job-change signal you can route to `/sales-intent`
- **Account safety**: LinkedIn automations run on your own session cookie; bans are possible regardless of safety settings. Keep daily volumes human-like, use the cloud's dedicated IP/proxy, and don't stack two automation tools on one account
- **Cost model**: pay-on-match credits (failed lookups return `billed:false`, no charge) on the API; classic Cloud bills monthly automation hours that **don't roll over**. Free tier = 50 credits
- **Export/handoff**: native bidirectional CRM sync (HubSpot, Pipedrive, Salesforce, Zoho, GoHighLevel) and one-click pushes into Smartlead/Instantly/Lemlist/HeyReach/Expandi/Reply.io; n8n community nodes + MCP server (`mcp.texau.com/mcp`) for agent-driven runs
- **Best for**: solopreneurs and GTM engineers who want LinkedIn list-building plus programmatic enrichment in one credit pool; a cheaper, API+MCP-first PhantomBuster/Clay alternative
