...
6. Manage lists and prospects via `list`/`lists`/`createList`/`addProspectToList`
7. Submit corrections via the `submitPersonFeedback` GraphQL mutation (POST to `/graphql` — there is no REST `/v1/feedback` endpoint)
8. Note: the API always returns HTTP 200 — detect failures by reading the `errors` array in the JSON response, not the HTTP status code
...
### MCP Server
LeadIQ ships an official MCP (Model Context Protocol) Connector that links AI platforms directly to your LeadIQ account for natural-language prospecting, contact enrichment, and company enrichment without writing code. Supported clients include Claude (Free/Pro/Teams), ChatGPT (Plus/Team/Enterprise), Cursor, and other MCP-compatible tools. Exposed tools include Get Account Details (plans, billing cycle, credit usage), Get Contact Tracking Balance, and Get Usage Statistics. Setup is documented in the LeadIQ help center ("Using the LeadIQ MCP Connector").
