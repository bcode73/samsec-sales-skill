...
### All Webhook Event Types (37 total)
...
### Delivery Headers & Signature Verification

Every webhook POST includes these special headers:

| Header | Description |
|---|---|
| `x-salesloft-event` | Name of the event type that triggered the delivery |
| `x-salesloft-signature` | HMAC hex digest of the request body, generated with the `sha1` hash function using the subscription's `callback_token` as the HMAC key |

**Verify** by computing `HMAC-SHA1(callback_token, raw_request_body)` and comparing the hex digest (constant-time) against `x-salesloft-signature`. The `callback_token` is set/returned when creating the webhook subscription (`POST /webhook_subscriptions`) and is the shared signing secret. Note: the digest is SHA1, not SHA256.

### Retry Policy

A failing webhook delivery is retried **three additional times, spaced 15 seconds apart**, before being marked as failed. Your endpoint should return a 2xx promptly and process asynchronously to avoid retries.
...
- Webhook Introduction (retry policy): https://developers.salesloft.com/docs/platform/webhooks/introduction/
- Webhook Delivery Headers (signature): https://developers.salesloft.com/docs/platform/webhooks/delivery-headers/
