...
2. **Finding a specific person's email**:
   1. Use the Email Finder: `GET /v1/find?firstname=Jane&lastname=Doe&domain=example.com&key=<your-key>` (API param names have no underscore)
...
3. **Building a prospect list from keywords and locations**:
   1. Use Lead Finder / Tags: `POST /v1/tags?key=<your-key>` with JSON body `{ "tags": ["saas marketing", "london"] }` (location and category are both tag strings; there is no `location` query param)
...
7. **The Free plan is throttled and limited to 25 credits/month — expect slower response times and rate limiting.** The Free tier is suitable for evaluation only. The official documented rate limit is "8 calls per second, or 2400 per 5 minutes" (per minelead.io/pricing), but the Free tier is throttled more aggressively in practice. If you are building an integration, test on a paid plan to get realistic performance characteristics.

8. **Premium endpoints cost more than 1 credit — budget accordingly.** Most operations are 1 credit, but `/enrich` costs 2 credits, `/social-email-finder` costs 2 credits (only charged when emails are found — free if none), and `/buying-intent` costs 5 credits per request. Factor these into credit planning when building enrichment or intent pipelines.
...
**Skill does**:
1. Uses Lead Finder / Tags: `POST /v1/tags?key=<key>` with JSON body `{ "tags": ["fintech", "berlin"] }` to discover fintech companies in Berlin (tags is a POST with a `tags` array, not query params)
...
5. Uses Email Finder for specific decision-makers: `GET /v1/find?firstname=&lastname=&domain=<target-domain>&key=<key>` when the user has named contacts in mind
