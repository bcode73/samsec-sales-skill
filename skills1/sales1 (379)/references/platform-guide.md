...
| Export & share | Export notes to PDF; share via the iOS share sheet | UI-only (manual) |
...
- No export API and no programmatic access — manual PDF export and the iOS share sheet are the only ways to get a note out
- Apple Sign-in is the only authentication method

**Workarounds for getting data out:**
- Built-in PDF export: the App Store listing states "Export notes to PDF, share with teams, or save for personal study" (verified 2026-06-13)
- iOS share sheet: share an individual note out to other apps manually
- Manual copy-paste of transcripts/summaries from the app
- Screenshot/screen recording of flashcards
- No bulk/programmatic export — export is one note at a time, UI-only (no API)
