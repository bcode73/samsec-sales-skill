...
**API integration**: API keys require Pro+ plan (free plan gets 401, even though the pricing page lists "Basic API access"). Create keys in Settings → API & Webhooks (admin only). Use webhooks for real-time — `recording_added` fires when transcription + analysis completes; `recording_updated` fires when a recording's folder, labels, deal/company, or insight template changes. Payload is nested under `event` with a shared `eventId` for dedupe (verify via the `x-claap-webhook-secret` header; no HMAC). For MCP, point clients at `https://api.claap.io/mcp` (OAuth, or `Authorization: Bearer cla_xxxxx`).
...
**Skill does**: Shows webhook setup for `recording_added` (and `recording_updated`) events, explains the nested payload (`event.recording` holds the full Recording entity; `eventId` is shared across retries for dedupe), notes URL expiration (24h), recommends fetching and storing transcript immediately on webhook receipt, points to OpenAPI spec for full schema.
