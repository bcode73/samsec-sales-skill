...
### MCP Connector
- Hosted MCP server at `https://api.claap.io/mcp`. Default auth is OAuth (a connecting client prompts an OAuth sign-in flow); API-key auth via `Authorization: Bearer cla_xxxxx` is also supported.
- Supported clients: Claude.ai, Claude Desktop, ChatGPT (beta), n8n.
- Tools: List Workspaces, Search Companies, Search Contacts, Search Deals, Search Meeting Recordings, and Recording Views & AI Columns (smart-table insights).
- Only recordings visible in global search are retrievable; private recordings are not.

## Pricing and limits

*Re-verified 2026-06-13: per-user prices (Pro $24/$30, Business $48/$60) and tier limits confirmed unchanged. The pricing page now also surfaces per-tier AI-credit allowances — Pro 500 AI credits/mo, Business 1,000 AI credits/mo — which gate AI-heavy actions independently of recording limits. Note: the pricing page lists "Basic API access" under the Free tier, but the API authentication docs still state that free-plan API requests are rejected with 401, so treat Pro as the practical floor for live API use.*
...
| AI credits | — | 500/mo | 1,000/mo | Custom |
...
**Workspace** (`GET /v1/workspaces/mine`):
- `id`, `name`, `createdAt`, `membersCount`, `recordingsCount`
- API keys scoped to workspace

**Webhook events**:
- `recording_added` — fires when a new recording becomes available after transcoding, transcription, and analysis
- `recording_updated` — fires when recording properties change (folder, labels, attached deal/company, or insight template)
- Payload is nested: `{ "eventId": ..., "event": { "type": ..., "recording": <full Recording entity> } }`. `eventId` is shared across retries (use it to dedupe). Private recordings trigger neither event.
...
- `x-claap-webhook-secret` header — compare against your stored secret to confirm the request came from Claap (shared-secret check; no HMAC signature scheme)
- Must respond with HTTP 200 within 5 seconds
- Retry: immediately, 1 min, 5 min, then discarded (all retries share the same `eventId`)
