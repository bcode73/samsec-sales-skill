...
## API quick reference
- **Base URL**: `https://rest.{instance}.braze.com` for US/APAC, `https://rest.{instance}.braze.eu` for EU (instance-specific — check Settings → APIs and Identifiers for yours)
- **Auth**: `Authorization: Bearer YOUR_REST_API_KEY`
- **Rate limit**: 250,000 requests/hour default for most APIs, but many endpoints have their own limits (`/users/track` = 3,000 req/3s; broadcast sends = 250 req/min, etc.) — resets on the clock hour
- **Key endpoints**: `/users/track`, `/users/track/bulk`, `/users/export/ids`, `/messages/send`, `/campaigns/trigger/send`, `/canvas/trigger/send`, `/messages/live_activity/start`, `/catalogs`, `/cdi/integrations/{id}/sync`, `/email/status`, `/templates/email/create`, `/segments/list`
- **SDKs**: iOS, Android, Web (JS), React Native, Flutter, Cordova, Unity, Unreal Engine, Roku, .NET MAUI/Xamarin, Expo
