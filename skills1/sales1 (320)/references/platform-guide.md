...
Order limits: Classic = 500 orders/mo ($199/mo published price; existing-customer cap noted as 2,000), Advanced = up to ~4,000 orders/mo, Plus = 10,000+ orders/mo. Only Classic has a published price — Advanced and Plus are custom-priced (contact sales / book a demo). Verified against https://loyaltylion.com/pricing 2026-06-13.
...
- Base URL: `api.loyaltylion.com`, all endpoints under the `/v2` path prefix
- Auth: **API keys** (Bearer token, created at Manage → API keys, scoped) is the current method for single-store integrations; **OAuth** for partners. Legacy **token+secret Basic auth is deprecated** and will be removed.
- Rate limit: 20 requests/second
- Pagination: cursor-based (recommended) or `since_id` + `limit`
...
- Customer events: enrollment, points earned, approaching expiration, reward claimed/available, tier upgrade/downgrade/approaching, segment movements (at-risk/defected/loyal), rule completed, referral complete, recurring reward reminder
- Other events: customer updates, loyalty email unsubscribes, receipt approval/decline/manual-approval, reward store fulfilment
- Signed with `x-loyaltylion-hmac-sha256` (base64 SHA-256 HMAC of raw body using OAuth client secret or site secret); ack with `200` within 5s; retries with backoff, subscription removed after 30 failures
