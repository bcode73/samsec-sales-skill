...
- [In Builderall (MailingBoss)](#in-builderall-mailingboss)
...
## In Builderall (MailingBoss)

MailingBoss is the email/autoresponder module inside the Builderall all-in-one suite — and the only Builderall module with an API. For full platform help (API, webhooks, plan gates, troubleshooting), use `/sales-builderall`.

**What you can do**
- Lists, broadcasts/campaigns, automations, and tags via the MailingBoss UI.
- Subscriber API: base `https://member.mailingboss.com/integration/index.php/`, token-in-URL auth, endpoints to create / search-by-email / update / unsubscribe (params in the body, `list_uid` + `email` + `taginternals`).
- Per-list **inbound webhook URL** to add leads from external forms/apps, plus Zapier/Make/Pabbly/Integrately (MailingBoss is typically an iPaaS *action* target).

**Data model**
- Subscribers are **scoped per list** (`subscriber_uid` is unique within a `list_uid`) — dedupe on `email`, not `subscriber_uid`.
- Tags ride on `taginternals` and drive automations/segmentation. Custom fields use MailWizz-style uppercase tags (`FNAME`, `LNAME`) — create them on the list first.
- MailingBoss is built on a **MailWizz**-style codebase (useful when a field/behavior isn't documented).

**Gotchas**
- Reviews consistently rate MailingBoss **below dedicated ESPs** on deliverability and template quality — authenticate your domain (SPF/DKIM/DMARC), warm up, and keep lists clean, or send critical campaigns through a specialist ESP and use Builderall for pages.
- No broad REST API beyond MailingBoss; no MCP server.
- Periodic platform outages can affect sends — don't depend on it for time-sensitive launch emails without a fallback.

**Best for**: Solopreneurs already paying for Builderall who want email inside the same suite. If deliverability or rich automation matters, pair Builderall pages with a dedicated ESP. For `/sales-builderall` platform help.
