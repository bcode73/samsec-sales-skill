...
- **Validate data format.** REST import APIs are strict about field mapping and data types. The async Import API (`POST /inbound/v2/async/{importer-name}`) returns a **feed file ID** — use it with the Feed File APIs to pull aggregate stats and per-record processing results (first 5,000 records per status) to find specific row-level errors.
