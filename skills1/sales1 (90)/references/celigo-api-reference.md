...
- Configure APIs via the APIM console (API Builder)
...
As of the 31 Mar 2026 Agentic Automation launch, Celigo ships an Enterprise **Model Context Protocol (MCP) Server** — a standard for secure, real-time, auditable AI connectivity between AI agents and enterprise systems. Generally available; free across editions until June 2026.
- Lets AI agents run the full spectrum of automation (rules-based logic → adaptive AI agents) on the Celigo platform.
- Supports **MCP prompt templates**: discover/use Model Context Protocol prompt templates and map required inputs/arguments directly in Celigo, avoiding custom one-off integrations.
- Companion launches: **Celigo Ora** (conversational NL copilot, beta) and **Agent Builder** (low-code AI automation environment, GA).
- See Celigo Help Center "agentic automation 2026 release notes" for current MCP configuration details.
