...
2. **What edition are you on?** (Official pricing page now lists three editions; older guides used Free/Basic/Premium names.)
   - A) Standard (MFA + role-based access, optional SSO add-on)
   - B) Professional (adds API Builder, Lookup Cache, included SSO, sandbox)
   - C) Enterprise (unlimited endpoints, best SLAs, compliance governance)
   - D) Free trial (30-day unlimited)
...
- **Pricing jumps between tiers are significant.** The official pricing page lists three editions — Standard, Professional, Enterprise (older third-party guides use Free/Basic/Premium names). Endpoint/flow counts per tier aren't published (contact sales), and moving up a tier can materially raise annual cost. Count endpoints carefully — each connected app = 1 endpoint, not each flow. Consolidate connections where possible.
...
- **No standing free paid tier — only a trial.** The current pricing page offers a 30-day unlimited free trial, then a paid edition (Standard/Professional/Enterprise). For long-term proof-of-concept, plan for a paid tier; trial access is time-boxed, not a permanent free plan.
...
**Skill does**: Reads platform guide for comparison context. For NetSuite-centric ecommerce: Celigo is the strongest fit — purpose-built for NetSuite, pre-built Shopify-NetSuite Integration App, endpoint+flow pricing means no per-transaction overages. Workato has more connectors (1,200+) but task-based pricing spirals with ecommerce volume. Boomi is enterprise-focused with per-connection pricing that adds up fast with 12+ apps. Recommends a Celigo Professional-tier quote for ~12 endpoints (endpoint counts per tier aren't published — confirm with sales).
