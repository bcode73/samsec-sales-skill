...
### Native webhooks / subscriptions (Enterprise API)
GetAccept exposes a native webhook API via the `/subscriptions` resource (REST hooks) —
no Zapier/Make required. Subscribe a `target_url` to any of 15 document events
(`document.created`, `.sent`, `.viewed`, `.reviewed`, `.signed`, `.approved`, `.expired`,
`.rejected`, `.downloaded`, `.printed`, `.forwarded`, `.partially_signed`, `.commented`,
`.hardbounced`, `.imported`). Payloads are NOT HMAC-signed; verify via IP whitelisting
(fixed sending IPs) and/or custom request headers you configure on the subscription.
See `getaccept-api-reference.md` for the full endpoint and event list.
...
- **Enterprise/API**: register a native subscription (`POST /subscriptions`) pointing a
  `target_url` at your handler, e.g. subscribe to `document.signed` to update CRM deal
  stage, `document.viewed` to create follow-up tasks, `document.created` to log activity.
  Authenticate inbound calls via the fixed sending IPs or a custom header on the
  subscription (no HMAC signature is sent). Test delivery with `POST /subscriptions/test`.
- **Professional (no API)**: use Zapier/Make triggers (Document Signed / Viewed / Created)
  for the same flows.
