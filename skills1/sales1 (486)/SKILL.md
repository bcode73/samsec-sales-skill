...
1. **Monthly cost**: usage-based at ~$0.15/identified email (verified 2026-06-13; no annual contract, 14-day free trial). Estimate as identified emails × $0.15.
...
- **Pricing is now usage-based, not annual tiers (verified 2026-06-13).** The official page (retention.com/pricing) shows "$0.15/email", "No annual contracts", "Pay monthly, cancel anytime", and a "14-day free trial · No credit card required". This replaces the older $500–$2,500/mo annual-contract tiers + $300/mo a la carte that most third-party sites still cite. Forecast cost as (identified emails × $0.15) and confirm the live rate at signup.
...
- **API is extremely limited (upload-only).** Base URL `https://api.retention.com/api/v1/`, auth via `api-id`/`api-key` headers (or `api_id`/`api_key` body/query). Only two POST endpoints: `/suppression` and `/custom_optouts`, both taking a `multipart/form-data` CSV (`file` + optional `is_md5`). Limit: 5 requests/day, ~40K emails each, rolling 24h. No read/query endpoints — all real operations happen through integrations or the dashboard. Grow webhooks (Integrations > Webhook) POST identified-contact JSON but offer no automatic suppression.
