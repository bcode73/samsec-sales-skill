...
### MCP
- Official MCP server (no longer "beta" — "generally available to test"). Enables AI assistants (Claude, ChatGPT, Cursor) to access Granola meeting data. Browser-based OAuth 2.0 with Dynamic Client Registration. Tools: `query_granola_meetings`, `list_meeting_folders`, `list_meetings`, `get_meetings`, `get_meeting_transcript`, `get_account_info`. Some tools (transcript access, folder/shared-notes listing) are paid-plans-only (Business/Enterprise). On Enterprise, MCP is disabled by default for all members — admin enables it in Settings → Workspace > General.
...
- `title` — meeting title
- `owner` — note creator object with `name` and `email` fields
...
- Create keys in app settings: **Settings → Connectors → API keys** → Create new key
- Any workspace member on a **Business** or **Enterprise** plan can create API keys; on Enterprise, admins control API access
- At creation you pick note-access **scopes**: **Personal notes** (notes you own + directly shared with you) and/or **Public notes** (workspace-visible + Team space notes). Keys can be revoked or rescoped from the same screen.
...
### Rate limits
- **Burst**: 25 requests within a 5-second window
...
1. Generate API key in Settings → Connectors → API keys → Create new key (pick the Personal/Public note scopes you need)
