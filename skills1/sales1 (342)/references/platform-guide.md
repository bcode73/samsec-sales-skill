...
**Mira API:**
- **Endpoint**: `POST /v3/mira/responses` — chat-completion endpoint for integrating AI-powered media insights into custom applications (`GET /v3/mira/projects` lists projects)
- **Modes**: Streaming (`streaming: true`, server-sent events ending on `[DONE]`) and non-streaming responses; Thread ID returned in the `Thread-Id` response header, `Project-ID` request header associates a project
- **Citations**: Returns source citations for all AI-generated insights
- **MCP Server**: Remote MCP server at `https://api.meltwater.com/mcp` (auth via `apikey` header) for AI assistants and agents (Claude Desktop, Cursor); 60 req/min, shared with the responses endpoint
- **Access**: Sold as a separate package — contact your account manager for pricing

## API & Integrations

**Meltwater API (`https://api.meltwater.com`, endpoints as `/v3/<feature>/...`):**
- **Versions**: v3 and v4 are both live — v4 is being built out and "in time will support all features"; newer features land in v4 while some remain in v3. Same `apikey` auth across versions.
...
- **Rate limits** (now public): 100 calls/min general, 2000 req/hr per IP, 20/min exports, earned-media analytics 5/sec + 100/min, Mira 60/min
- **Webhooks** (Data Streams): `POST /v3/hooks` to push mentions to your endpoint; HMAC-SHA1 `X-Hub-Signature` signing via `X-Hook-Secret` shared secret; 5 retries / 5-min delay / 30s timeout, hook deleted on persistent failure
