...
- Tracks Page Visit, View Content, Search, Add to Cart, Add to Wishlist, Purchase, Lead, Sign Up, Custom events (these are Reddit's exact event names — note `Page Visit`, not `PageView`)
...
- Sends events from your server directly to Reddit's servers — endpoint `POST https://ads-api.reddit.com/api/v2.0/conversions/events/{PIXEL_ID}` with a Bearer conversion access token
...
- Deduplicate Pixel vs CAPI by passing a unique Conversion ID + event name (Reddit matches on both)
