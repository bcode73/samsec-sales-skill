...
- **API is rules + analytics only.** The public REST API (verified 2026-06-13) exposes only `/rules` CRUD and `/stats/by-period` + `/stats/response-time` — there are no standalone read/send/archive email endpoints. Email actions (SEND_EMAIL, REPLY, FORWARD, ARCHIVE, MARK_SPAM, MOVE_FOLDER, etc.) exist only as rule *actions* run by the rule engine. There is a `CALL_WEBHOOK` rule action that POSTs to a URL you set (outbound only; gated by `NEXT_PUBLIC_WEBHOOK_ACTION_ENABLED` on self-hosted).
- **Rules use a structured `condition` + `actions` shape, not a flat `instructions` string.** Create/replace requires `name`, `condition` (with `aiInstructions`/`static`), and at least one `action`. There is no `automate` field.
