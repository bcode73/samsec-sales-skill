...
- **Authentication**: API key via `x-api-key` header (the "CreatorIQ Premium API") — contact support@creatoriq.com or your CreatorIQ rep to obtain
- **Format**: bi-directional, JSON-based API, encrypted via SSL/TLS — read CreatorIQ data and post external data (attribution, sales, commission, paid media) back against creators, posts, and campaigns
- **CRM Publishers API**: Create, read, update, and delete creator (publisher) records and sync them with your CRM (Salesforce, HubSpot, etc.)
- **CRM Lists API**: Manage curated creator lists/segments programmatically
- **Campaign data endpoints**: Export campaign performance and content data; Post Campaign Activity to push data against campaigns
- **Reporting endpoints**: Pull metrics into BI tools (Tableau, Looker, Google Analytics)
- **Webhooks**: CreatorIQ fires webhooks to notify your app when an event happens to campaign or creator data; subscriptions are managed via a Webhooks management API (event catalog and signing are in the authenticated docs)
