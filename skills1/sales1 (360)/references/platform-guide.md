...
- Base URL (V1, deprecated): `https://api.modjo.ai/v1/`
- **Current recommended version is API V2** — docs at `https://api.modjo.ai/v2/docs`. V1's Swagger UI shows a deprecation banner recommending migration to V2. V2 lets you query Accounts, Opportunities, Users, Tags, and more with advanced filters (launched ~April 2026). Same `X-API-KEY` auth.
...
### Key endpoints (V1 — deprecated but functional)
- `POST /v1/calls` — Upload a call (provide recording URL or get signed URL for direct upload)
- `POST /v1/calls/exports` — Export call data with advanced filters (BETA; returns `ApiV1CallExportPaginationResult`)
...
For new integrations, prefer **API V2** (`https://api.modjo.ai/v2/docs`), which exposes querying across Accounts, Opportunities, Users, Tags, and more. The V2 OpenAPI spec requires authentication, so exact V2 paths are confirmed via the authenticated docs.
...
Delivered HTTP headers: `x-modjo-payload-signature` (the HMAC-SHA256 signature) and `x-modjo-timestamp` (the signed timestamp) — mirror the body's `payloadSignature` and `timestamp`. HTTPS endpoints are required.
...
- **Transcript pipeline**: Set up `call_summarized` webhook → receive notification (verify `x-modjo-payload-signature` header) → call `POST /v1/calls/exports` with `callIds` filter and the `transcript`/`summary` relations → write to data warehouse. (For new builds, consider API V2, which broadens object querying.)
