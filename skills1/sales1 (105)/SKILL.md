...
2. **Which plan are you on?**
   - A) Starter (€79/mo annual / €99 monthly, 7 signals — webhooks + MCP included)
   - B) Pro (€199/mo annual / €249 monthly, 25 signals — adds Company AI qualification)
   - C) Scale (€439/mo annual / €549 monthly, 75 signals — adds native CRM integrations)
   - D) Enterprise
...
- **CRM integrations are Scale-only (€439/mo).** HubSpot, Salesforce, and Pipedrive native connectors require the Scale plan. On Starter/Pro, use webhooks (any plan) or CSV export as workarounds.
- **MCP is self-serve on all plans.** Publicly available since Feb 12, 2026 — no support request needed. Get your MCP URL from Settings > Integrations > Clearcue MCP and paste it into Custom Connectors (OAuth), or use personal access token auth (added Apr 30, 2026 as an OAuth alternative).
- **Webhooks are available on every plan, including Starter.** Push-based programmatic data flow does not require an upgrade.
- **Company AI qualification is Pro+ only.** This is the main Starter→Pro upgrade trigger — Starter plan signals do keyword matching only without company-level AI filtering, which means more noise.
...
- **No public REST API.** MCP is the only programmatic query interface. For push-based workflows, use webhooks (available on every plan).
...
**Skill does**:
1. Notes webhooks are available on every plan (including Starter) — no upgrade needed
