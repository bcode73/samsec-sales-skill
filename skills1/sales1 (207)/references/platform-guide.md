...
| **Email Sequences** | Outbound email sequences with variable personalization | API-accessible (sequences enroll/disenroll endpoints) |
...
**cURL**:
```bash
curl -X POST "https://api.gmelius.com/public/v2/auth/sequences/{sequence_id}/enroll" \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
...
```python
resp = requests.post(
    f"{BASE_URL}/auth/sequences/{sequence_id}/enroll",
    headers=headers,
    json={
...
**OAuth 2.0 + PKCE flow**:
1. Redirect user to: `https://app.gmelius.com/oauth/authorize?client_id={id}&redirect_uri={uri}&code_challenge={challenge}&scope={scopes}`
