...
- **REST API** (Enterprise only): CRUD for feature flags and rules, workspace management.
  ...
  - Auth: API token from `app.vwo.com/#/developers/tokens`; passed in the request header, tokens don't expire.
  - **Rate limit: 1 request/second per token** (developers.wingify.com/reference/api-rate-limits-1).
  - Docs host: **developers.wingify.com** (old developers.vwo.com URLs redirect there).
  - Contact: api@vwo.com
- **FME Webhooks**: outbound POST on a `settings_changed` event when a flag/setting changes (payload: `timestamp`, `event`, `action`, `triggered_by`). Auth is **API-key based, not HMAC** — VWO sends `x-vwo-auth: YOUR_SECRET_KEY`. Configure under Configurations → Website and Apps → Environment webhook. See `vwo-api-reference.md`.
...
As of mid-2026 the official pricing page (vwo.com/pricing) lists **three tiers — Growth, Pro, Enterprise**. The previously popular permanent **free Starter plan (up to 50K MTU) has been discontinued**; new sign-ups now get an "Explore for Free" trial instead of a forever-free tier. (Existing free-plan accounts were notified separately.)

| Plan | Notes | Key features |
|---|---|---|
| (Free trial) | "Explore for Free" — full-suite trial, no permanent free tier | Evaluate Testing/Insights/Personalize |
| Growth | Entry paid tier (annual commitment) | A/B testing, basic insights, limited integrations |
| Pro | Mid-market tier | Full testing, insights, personalization |
| Enterprise | Custom pricing by MTU/modules | API access, advanced analytics, dedicated support |

> Exact published prices are not shown on the public pricing page (quote/demo gated) and the per-tier dollar figures previously listed here could not be re-confirmed against an official page as of 2026-06-13 — treat third-party price figures as approximate.

**Module pricing**: VWO Testing, Insights, Personalize, Web Rollouts, and Feature Experimentation are priced separately or bundled. The pricing page's FME feature table lists "API Access (rate limit of 60,000/month)."
