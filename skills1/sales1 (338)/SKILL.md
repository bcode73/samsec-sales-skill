...
- **No supported external API** — the standalone FastAPI backend (legacy `localhost:5167`) is **deprecated**; the current app is a Tauri desktop app where the UI talks to the Rust core via Tauri commands, with no exposed HTTP API. There's no hosted/cloud API for external integrations. For automation, read the local store / exported files, not an HTTP API.
...
- **Pro features vs Community** — enhanced-accuracy models, advanced exports (PDF/DOCX/MD), custom summary templates, auto-detect meetings, Windows GPU acceleration, and the Custom Server (OpenAI-compatible) connector require Pro. Pro is **$10/user/mo billed annually ($120/yr), or $25/user/mo month-to-month** — the $10 headline is the annual rate, not the monthly price. Speaker identification, calendar integration, and "Chat with meetings" are **marked "Coming Soon"/roadmap — not shipped Pro features yet**; don't promise them as available. Community edition is fully functional for basic transcription. Ollama and Built-in AI summarization are free.
