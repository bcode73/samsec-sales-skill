...
**Billing note**: $43/mo and $96/mo are the **annual-billed** effective rates. Billed **monthly**, Scale is **$49/mo** and Max is **$109/mo**. The table headers below use the annual rates.

| Feature | Launch (Free) | Scale ($43/mo annual) | Max ($96/mo annual) | Enterprise (Custom) |
|---|---|---|---|---|
...
**Annual pricing** (saves ~12% vs monthly): Scale $517/yr ($43/mo effective), Max $1,151/yr ($96/mo effective). Billed month-to-month it's $49/mo (Scale) and $109/mo (Max).
...
**API**: REST API v2 at `https://api.beehiiv.com/v2`. Bearer token auth via API key OR OAuth2 (authorization code flow with PKCE; endpoints under `/oauth`). Available on all plans. The Send API and Create-post endpoint are Enterprise-only. Rate limit: **180 requests/minute per organization** (429 on exceed; `RateLimit-Limit`/`RateLimit-Remaining`/`RateLimit-Reset` headers).

**MCP server**: beehiiv ships an MCP server at `https://developers.beehiiv.com/_mcp/server` for use with Claude Code, Cursor, and other AI clients.

**Webhooks** (Scale+): Real-time notifications for subscription events (created, confirmed, deleted, upgraded, downgraded, paused, resumed), tier events (`subscription.tier.created/.deleted/.paused/.resumed` — note dot notation), post events (sent, updated, scheduled), survey responses, and newsletter list subscription events. Full CRUD via the Webhooks API. No documented HMAC signature header — treat payloads as unsigned.

**TypeScript SDK**: Official SDK at `github.com/beehiiv/typescript-sdk` (not re-verified 2026-06-13).
