...
description: "Socialhose platform help — social listening and media monitoring with real-time and high-frequency monitoring modes, crisis detection, competitor analysis, public REST API (OpenAPI 3.1, Pro+), webhook integrations (Pro+), no annual contracts. Use when Socialhose mentions are full of irrelevant noise and you need to tighten keyword queries, mention quota is filling up before the month ends, smart alerts aren't catching the spikes you care about, you're wiring the Socialhose API or webhooks into a CRM/warehouse and payloads aren't arriving, high-frequency monitoring seems delayed, you want to upgrade from Starter to Pro or Agency and need to understand feature differences, or crisis mode isn't activating when it should. Do NOT use for social listening strategy across tools (use /sales-social-listening) or choosing between social listening platforms (use /sales-social-listening)."
...
2. **Which plan are you on?**
   ...
   - B) Pro ($399/mo — 25K mentions, 100 keywords, public API + webhooks + export)
   - C) Agency ($999/mo — 100K mentions, unlimited keywords, crisis mode, 10 smart alerts)
...
- **Public REST API is Pro+ (added ~2026).** Socialhose now ships a documented public REST API (OpenAPI 3.1 spec at `socialhose.net/api/public/v1/docs/`) with organization-scoped API keys — read AI-enriched mentions, manage campaigns/live searches, and monitor TikTok Live. API access is plan-gated to **Pro and Agency** (not Starter). Webhooks (Pro+) remain a separate push path. The open-source self-hosted PHP version on GitHub (BSD-3-Clause) is still an option for unlimited mentions / full data control.
...
- **No Zapier/Make/MCP support.** The automation surfaces are the public REST API (Pro+) and outbound webhooks (Pro+) — there are no native iPaaS connectors. To reach a CRM, route the API or a webhook through your own middleware (e.g. a Cloudflare Worker) into the CRM's API.
