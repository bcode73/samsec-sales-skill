...
|--------|-------------|-------------------|
| **Brand monitoring** | Track brand mentions, keywords, competitors across social platforms | API + Webhook-accessible (Pro+) |
| **Sentiment analysis** | AI-enriched sentiment, emotions, intent, named entities on every mention | API + Webhook-accessible (Pro+) |
...
| **Campaigns & live searches** | Create and manage campaigns and live searches across all sources | API + UI (Pro+) |
| **TikTok Live monitoring** | Monitor TikTok Live accounts and download stream recordings on demand | API + UI |
...
| **Export** | Download mention data | UI-only (Pro+) |
...
- Smart Lists: $29 per 10 lists/month
- List members: $29 per 50 members/month
...
**Public API (Pro+ only):**
- Socialhose now ships a **public REST API** (launched ~May 2026). API access is a plan-gated feature included on **Pro and Agency** (not Starter).
- Documentation portal: `https://socialhose.net/api/public/v1/docs/` — published with an **OpenAPI 3.1 specification** (well-suited to AI coding tools / Claude Code).
- **Auth:** organization-scoped API keys, generated under profile settings. Keys can be scoped — e.g. a **read-only** key for a reporting script and a separate **write-access** key for a tool that provisions searches.
- **Capabilities (per official launch announcement):**
  - **Read mentions** enriched by AI — including **sentiment, emotions, intent, and named entities** — with filtering by platform, date, keyword, and more.
  - **Create and manage campaigns and live searches** across Facebook, Instagram, TikTok, X, LinkedIn, and Reddit.
  - **Monitor TikTok Live** accounts and **download stream recordings on demand**.
- The exact base-URL path, auth header name, individual endpoint paths/methods, pagination, and rate limits are published only inside the JS-rendered docs portal above and are not captured here — read the live OpenAPI spec for specifics.

**Webhook integrations (Pro+ only):**
- Outbound webhooks deliver mention data to your endpoints ("Real-time alerts via email, custom webhooks, and smart mailing lists" — Pro feature copy)
...
Socialhose now publishes a public API with an OpenAPI 3.1 spec (see Integrations), but the **outbound webhook** payload structure is still not separately documented. The shape below is illustrative, constructed from the open-source PHP repository — verify against live webhook payloads and the OpenAPI spec:
...
This is an option for teams that need unlimited mentions or full data control. Note: the hosted SaaS now offers a public REST API on Pro+ (see Integrations), so self-hosting is no longer the only way to get programmatic access.
