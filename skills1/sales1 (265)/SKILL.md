...
- **Authentication tokens expire — build refresh logic.** The Harmony login response returns `sessionTimeoutInSeconds: 14400` (4 hours) for the `authenticationToken` (older third-party write-ups cite "15 minutes"). Pass the token as the `authToken` header on every call (login is the only call that doesn't need it). For unattended API access, build refresh-on-401 logic or use service accounts. TFA configured as "on each login" breaks unattended API access — disable TFA for service accounts or use bypass rules. Note the TFA confirmation field is named `code` (not `tfaCode`).
...
**Skill does**: Reads platform guide for agent architecture. Explains the hybrid pattern: install a Private Agent on-prem with network access to SQL Server; build an API in API Manager that routes to an operation on that private agent; the cloud API gateway (region domain `*.jitterbit.net` NA / `*.jitterbit.eu` EMEA / `*.jitterbit.cc` APAC) handles external traffic, auth, and rate limiting; the private agent pulls work from the cloud via outbound HTTPS only (no inbound ports). Walks through agent group setup, API Manager security profile (OAuth 2.0 recommended), and IP allowlisting if needed.
...
**Solution**: Verify the agent host can reach `apps.na-east.jitterbit.com` (NA) or `apps.emea-west.jitterbit.com` (EMEA) or `apps.apac-southeast.jitterbit.com` (APAC) on port 443. Check the allowlist documentation for the full set of hosts (MQ messaging like `mq.apps.{region}.jitterbit.com`, cloud datastore, and the region API gateway — `*.jitterbit.net` NA / `*.jitterbit.eu` EMEA / `*.jitterbit.cc` APAC). Verify the agent config points to the correct region. Check proxy settings in `jitterbit-agent.conf` if behind a corporate proxy.
