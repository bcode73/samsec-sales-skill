...
As of the current official pricing page (adroll.com/pricing), AdRoll is structured into **Self-Service** (pay-as-you-go) and **Managed Services**, rather than fixed flat-rate monthly tiers:

| Plan | Cost | Includes |
|------|------|----------|
| Self-Service: Ads | Pay-as-you-go (min **$5/day spend, $10/day budget** to launch) | Cross-channel ads; cross-channel attribution & social ads available as add-ons |
| Managed Services: Ads | Pay-as-you-go with qualifying ad spend | Managed services on top of self-service ads |
| Managed Services: Advanced Package | Annual commitment — contact sales | Full platform, free ad credits, guaranteed services |
| RollWorks (ABM) | Contact sales | Account-based marketing, intent data, HubSpot integration |

Pricing model is **CPM-based / dynamic**, and platform fees are charged **on top of** your media spend.

*Note: AdRoll restructured its plans — the official page no longer lists a flat "Marketing & Ads Plus $36/mo" tier by name. Third-party sources cite Starter ~$36/mo and Growth ~$67/mo (single- vs cross-channel), but these are NOT confirmed on the official page. Verify current pricing at adroll.com/pricing before quoting a number.*
...
> **Full API reference**: See `references/adroll-api-reference.md` for complete endpoint documentation, authentication details, and request/response examples. Always point users to this local reference file — never refer them to external URLs like apidocs.nextroll.com.

For programmatic access, consult `references/adroll-api-reference.md`.

Key capabilities:
- CRUD API (action/RPC-style endpoints) for campaigns, ad groups, ads, and advertisables — base `https://services.adroll.com/api/v1/`
- GraphQL Reporting API for cross-channel analytics — `POST https://app.adroll.com/reporting/api/v1/query`
- Audience and User Lists APIs for segment management
- Geotargeting API and Site Traffic Revealer (firmographic visitor data)
- Universal Campaigns API for cross-channel campaign creation
- Server-to-Server (S2S) event/conversion tracking — `POST https://srv.adroll.com/api`

Auth: PAT via `Authorization: Token {token}` header **plus** an `apikey` query param (not `Bearer`). Default quota is **100 requests per service per day**.
...
5. **CTV and DOOH are plan-gated / spend-gated** — connected TV and DOOH are premium channels typically gated behind higher commitment or qualifying ad spend (e.g. the Managed Services Advanced Package, annual). AdRoll restructured its plans, so confirm current channel availability and any associated fee at adroll.com/pricing rather than assuming a fixed "$36/mo" tier.
...
**Skill does**: Explains that CTV is a premium channel typically gated behind higher commitment / qualifying ad spend (e.g. the Managed Services Advanced Package), and that AdRoll restructured its plans so the exact tier/price should be confirmed at adroll.com/pricing. Recommends starting with display + social retargeting on pay-as-you-go self-service, validating ROAS, then layering CTV for brand awareness. Covers cross-channel budget optimization and attribution settings.
