---
name: sales-karax
description: "KaraX.ai platform help — agentic AI workspace combining Gemini-family AI chat, real-time meeting transcription, and AI workflow automation across 800+ (marketed 1000+) integrations from a single OneChat interface, SOC 2/HIPAA-aligned, no public developer API. Use when setting up KaraX for meeting transcription on Zoom or Google Meet or Teams, KaraX OneChat not triggering workflows or automations, choosing between KaraX and Otter or Fathom or Fireflies for meeting notes, KaraX free plan data disappearing after 7 days, KaraX integration with Salesforce or HubSpot not syncing, KaraX meeting recording hitting 30-minute cap on Starter, or KaraX plan selection (Starter free/Pro $14.99/Business $39.99/Enterprise custom). Do NOT use for comparing all AI note-takers (use /sales-note-taker) or reviewing a call for coaching (use /sales-call-review)."
...
- **No public developer API.** No developer portal, no published REST/GraphQL endpoints, no SDK, no MCP server. Note: the Business plan's workflow builder does include "Advanced + Webhooks" (re-verified 2026-06-13), but KaraX publishes no webhook event/payload/signing docs, so it is a workflow-builder feature rather than a documented developer webhook surface. All other automation goes through the platform's own agentic AI interface.
