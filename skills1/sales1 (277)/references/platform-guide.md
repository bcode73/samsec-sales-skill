...
| Commerce | Yes (3.5% + $0.30/txn) | Yes (3.5% + $0.30/txn) | Yes (3.5% + $0.30/txn) |
| Kit MCP integration | No | Yes | Yes |
| SMS marketing | No | Yes | Yes |
| Polls / surveys | No | Yes | Yes |
| Subscriber Signals (demographics) | No | No | Yes |

*Verified against kit.com/pricing 2026-06-13. The Commerce transaction fee is 3.5% + $0.30 per transaction (≈0.6% Kit platform cut + 2.9% Stripe processing) and applies on every plan, including the free plan. Verify current pricing at kit.com/pricing.*

## API quick reference

- **Base URL**: `https://api.kit.com/v4/`
- **Auth**: OAuth 2.0 (refresh-token or PKCE) for apps, or API key via `X-Kit-Api-Key` header
- **Rate limit**: 120 req/60s for API key; 600 req/60s for OAuth
- **Pagination**: Cursor-based (`before`/`after`, `per_page`)
- **Webhooks** (`POST /v4/webhooks`): subscriber_activate, subscriber_unsubscribe, subscriber_bounce, subscriber_complain, form_subscribe, course_subscribe, course_complete, link_click, product_purchase, tag_add, tag_remove, purchase.purchase_create, custom_field.field_created/field_deleted/field_value_updated
...
- **Transaction fee**: 3.5% + $0.30 per transaction (≈0.6% Kit cut + 2.9% Stripe processing), on every plan including free
