...
2. **Which plan are you on?**
   - A) Free ($0/mo — 1 keyword, Reddit only, daily alerts, basic analytics)
   - B) Starter ($39/mo — 4 keywords, Reddit + LinkedIn + HN + GitHub, daily alerts, 5K mentions, ≤100 warm emails/mo)
   - C) Pro ($99/mo — 10 keywords, all communities, Slack alerts, Share of Voice, REST API)
   - D) Advanced ($199/mo — 20 keywords, full Community Intelligence, REST API, SoV exporter)
...
- **REST API and Slack alerts are Pro-only ($99/mo).** Free and Starter ($39/mo) get daily alerts only — API keys on those plans return `403 PLAN_NO_API_ACCESS`. For cheaper programmatic access, Octolens (from $119/mo) or RedShip ($19/mo) both offer REST APIs.
- **Auth is the `X-API-Key` header, not `Authorization: Bearer`.** Keys are `ct_live_`-prefixed (generated in Settings → API). The endpoints are GET `/api-posts` and GET `/api-intelligence` — the older `/v1/mentions` path is deprecated.
- **Starter's 4-keyword cap is tight.** Most users need brand + competitor + problem phrase. Consolidate with broader terms or upgrade to Pro (10 keywords).
...
- **Pricing changed (2026-06): a Free tier ($0, 1 keyword, Reddit only) now exists and Starter is $39 (was $29) with 4 keywords (was 3).** Some blog/comparison copy still shows the old figures; the live /pricing page is authoritative. Verify current limits in your dashboard.
...
- **Free and Starter get daily alerts only.** Pro/Advanced add Slack alerts. If faster engagement matters, upgrade to Pro — but note the live pricing page describes Pro/Advanced as "Daily Slack alerts," so confirm the actual alert cadence in-dashboard rather than assuming real-time.
...
4. Maps post fields (source, title, body, ai_score, intent, url) to HubSpot contact properties
...
**Symptom**: API calls returning HTTP 429 with code `RATE_LIMIT_EXCEEDED`
**Cause**: The API enforces a single daily allowance of 5,000 requests with an hourly reset; polling-based integrations burn through it quickly
**Solution**: Read the `X-RateLimit-Remaining` and `X-RateLimit-Reset` response headers and throttle as you approach the limit; on 429 wait until the reset time. Use larger page sizes (`limit` up to 100 on `/api-posts`) to cut call count and cache results locally. If the marketing-page webhook push is available on your plan, prefer it over polling.
