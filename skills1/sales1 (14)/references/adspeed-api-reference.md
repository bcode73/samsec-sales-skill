...
| Parameter | Required | Description |
|---|---|---|
| `status` | No (default: empty) | Filter zones by status: `active`, `pending`, `archived` |
...
| `descr` | No | Zone description |
| `type` | No (default: `display`) | `display`, `email`, `video` |
...
| `name` | No | New name |
| `secondary` | No | Secondary zone ID |
| `optimizer` | No (default: `none`) | `none`, `click`, `revenue`, `conversion`, `CTR` |
...
| `zone` | Yes | Zone ID |
| `token` | Yes | MD5 of `[Zone ID][Zone Name]` |
| `existing` | No | Existing rate ID to replace |
| `width` | Yes | Ad width (e.g., `728`) |
| `height` | Yes | Ad height (e.g., `90`) |
| `type` | Yes | `grosscpm`, `uniquecpm`, `grosscpc`, `uniquecpc`, `perday`, `perweek`, `permonth`, `peryear` |
| `from` | Yes | Quantity range start |
| `to` | Yes | Quantity range end |
| `rate` | Yes | Rate value |
| `notes` | No | Rate notes |
| `condition` | No | Rate condition |
...
| `zone` | Yes | Zone ID |
| `token` | Yes | MD5 of `[Zone ID][Zone Name]` |
| `logic` | No (default: `and`) | `and`, `or` |
| `type` | Yes | `continent`, `country`, `region`, `metro`, `city`, `startend`, `keyword`, `totalimp`, `monthimp` |
| `compare` | No (default: `show`) | `show`, `hide` |
| `value` | Yes | Restriction value |
...
Breakdown stats by linked ads. Limit 50 calls/day.
...
| Parameter | Required | Description |
|---|---|---|
| `name` | Yes | Ad name (max 50 chars) |
| `clickurl` | Yes | Destination URL on click |
| `imageurl` | Yes | URL to banner image |
| `weight` | No | Rotation weight |
...
|---|---|---|
| `name` | Yes | Ad name (max 50 chars) |
| `videourl` | Yes | URL to video file |
| `clickurl` | No | Destination URL on click |
| `weight` | No | Rotation weight |
| `skippable` | No | Whether the video is skippable |
...
|---|---|---|
| `name` | Yes | Ad name (max 50 chars) |
| `wrapperurl` | Yes | URL to the VAST wrapper/tag |
| `weight` | No | Rotation weight |
| `skippable` | No | Whether the video is skippable |
| `descr` | No | Description |
| `pixelurl` | No | Tracking pixel URL |
...
|---|---|---|
| `name` | Yes | Ad name (max 50 chars) |
| `htmlbase64` | Yes | Base64-encoded HTML creative |
...
|---|---|---|
| `name` | Yes | Ad name |
| `first` | Yes | First line / headline text |
| `next` | Yes | Body / description text |
| `clickurl` | Yes | Destination URL on click |
| `weight` | No | Rotation weight |
...
| Parameter | Required | Description |
|---|---|---|
| `status` | No | Filter by `active`, `pending`, `archived` |
| `perpage` | No (default: 10) | Items per page (max 100) |
| `page` | No (default: 1) | Page number |
...
| `password` | Yes | Password — min 8 chars, at least one letter and one number |
| `fullname` | Yes | Full name |
| `email` | Yes | Email address |
| `company` | Yes | Company name |
| `companyurl` | Yes | Company website |
| `sendwelcome` | No | Send welcome email to the advertiser |
| `address`, `address2`, `city`, `state`, `country`, `postal`, `phone` | No | Contact details |
