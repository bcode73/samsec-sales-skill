...
**Rate limits**: 60 req/min, 10,000/day per token. Headers: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`.
...
**Positioning**: AI transcription combined with text-based audio/video editing — you edit recorded media by editing its transcript (delete a sentence in the doc, the audio/video is cut to match). Upload-only / record-into-app, not a live meeting bot that auto-joins calls. Differentiators: filler-word ("um/uh") removal, AI summaries, Overdub AI voice cloning, AI green-screen/eye-contact, and Business-tier video translation/dubbing. Targets podcasters, YouTubers, marketers, and teams repurposing recorded sales/webinar content into clips and posts. In a sales context it fits post-call content editing, not pipeline/CRM intelligence.

**Pricing (2026-06)**: Free $0 (60 media minutes/mo, 100 one-time AI credits). Hobbyist $16/user/mo annual ($24 monthly) — 600 min/mo (10 hrs), filler-word removal, basic AI tools. Creator $24/user/mo annual ($35 monthly) — 1,800 min/mo (30 hrs), full AI features, AI summaries. Business $50/user/mo annual ($65 monthly) — 2,400 min/mo (40 hrs), team collaboration, video translation/dubbing, Brand Studio. Enterprise custom. Plans gate by media minutes + AI credits, not by feature beyond translation (Business+).

**API** (beta):
- Docs: `https://docs.descriptapi.com/`
- Auth: Bearer token in the `Authorization` header; tokens are scoped to a specific Drive and inherit user permissions
- Focus is partner import/export, not full edit control:
  - Import media — by public/pre-signed URL (HTTP Range requests) or by requesting a signed upload URL, PUT the bytes, then poll for completion. Import links expire after ~3 hours
  - Export transcript — plain text, Markdown, HTML, RTF, DOCX; speaker labels (off / on speaker changes / every paragraph), timecodes, and markers optional
  - Publish project — returns a public web-share link plus a time-limited signed mp4 download URL; republishing reuses the prior share URL. No direct local-file export endpoint (must publish first)
- Rate limits: `429 Too Many Requests` with `Retry-After` header (honor it; back off)
- Status endpoint documented but not yet live
- MCP: Descript exposes "API + MCP" — "Edit video through code, or from inside your AI assistant" — so the API can be driven from Claude/Cursor via MCP

**Integrations**: One-click import from Zoom, SquadCast, Ecamm, Restream, Captivate. Publishing to YouTube, Wistia, Slack, HubSpot, Google Drive, and podcast hosts (Buzzsprout, Transistor, Podbean, etc.). Timeline export to Premiere, Final Cut Pro, Pro Tools, Audition, DaVinci Resolve, Logic, Reaper. No native webhooks, no Zapier listed, and the HubSpot tie is a publish destination — not CRM field sync.

**Known issues (from reviews/feature requests)**:
- No webhook/event push — automating "transcript ready → downstream" means polling the API
- No direct local export endpoint — you must publish to a Descript web link to get a signed mp4 back
- Not a meeting note-taker — no auto-join bot, no CRM intelligence, no action-item-to-deal mapping; it's a media editor with transcription
- Per-seat media-minute caps can be limiting for heavy batch transcription vs pure transcription services priced per hour

**Selection notes**:
- **Pick Descript when**: You record calls/webinars/content and want to edit them by editing the transcript, strip filler words, generate clips/summaries, clone a voice (Overdub), or translate video (Business), and you may automate transcript export via the beta API/MCP
- **Avoid Descript when**: You need a bot that auto-joins live sales calls and logs to CRM (→ Fathom/Fireflies/Avoma), you need conversation intelligence/coaching/deal scoring (→ Gong/Avoma/tl;dv), you need pure low-cost batch transcription priced per hour (→ Sonix, Temi, HappyScribe), or you need production webhooks for a transcript pipeline (Descript is poll-only today)
