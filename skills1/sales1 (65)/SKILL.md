...
2. **Which plan are you on?**
   ...
   - C) Expert ($499/mo — 50kw, 75K mentions, real-time, Boolean, white-label, API access)
...
5. **API integration** — confirm Expert plan or higher (API access starts at Expert $499/mo), use PostSearch → callback (or GetProcessedMentions polling) → GetMentions pattern
...
- **API access starts at Expert ($499/mo).** Starter and Pro have no API access — CSV export is the only egress there. Expert and Enterprise include the API; contact support to activate your API key once your plan includes it. (Re-verified 2026-06-13: the live pricing page and help center both list API access on Expert and custom plans, not Enterprise-only.)
...
- **No Zapier or Make, but callbacks (webhooks) do exist.** PostSearch and AddProject accept a `callback` URL that BrandMentions POSTs to on completion (`search_hash` / `project_id` in the body) — available with the API (Expert+). There are no iPaaS connectors; for Starter/Pro the only egress is manual CSV export. The callback POST has no documented HMAC signing, so re-fetch results via the API using the supplied id rather than trusting the body.
...
**Skill does**:
1. Confirms the user is on Expert plan or higher (API access starts at Expert $499/mo)
2. Walks through the async search pattern: PostSearch → wait for callback (or poll GetProcessedMentions) → GetMentions
