...
### Webhook signing
- Signing secret found in Settings → Webhook (also provided during webhook configuration)
- Secret prefix: `whsec_`
- Signature header: `X-Signature` (sent lowercase `x-signature`; HTTP headers are case-insensitive)
...
- Auth: OAuth with dynamic client registration (browser-based authorization flow; refreshes automatically)
- Compatible clients: Claude, Claude Code, ChatGPT, Codex, Cursor, Raycast
...
```json
{
  ...
  "insights": {
    "Customer details": [
      {
        "insight": { "Company name": "Initech", "Number of employees": 120 },
...
| `insights` | object | Keyed by user-defined insight name; each value is an **array of objects** `{ insight: string\|object, speaker: string\|null, timestamp: number\|null }` (timestamp in seconds) |
...
- **URL**: `https://circleback.ai/api/mcp`
- **Transport**: Streamable HTTP
- **Auth**: OAuth with dynamic client registration (browser-based; refreshes automatically)
