...
### Developer
- **MCP Server**: `https://circleback.ai/api/mcp` — Streamable HTTP transport, OAuth with dynamic client registration. Compatible with Claude, Claude Code, ChatGPT, Codex, Cursor, Raycast.
...
| `insights` | object | User-created insights keyed by name; each value is an **array of objects** with `insight` (string or object of custom fields), `speaker` (string\|null), and `timestamp` (number seconds\|null) |
...
1. Configure your MCP client (Claude, Claude Code, Cursor, Raycast, ChatGPT, Codex) with:
