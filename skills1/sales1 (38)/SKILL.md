...
- **Notion workflow**: Connect Notion integration → toggle per-workspace auto-sync so new notes push automatically, or send notes individually with the one-tap "Send to Notion" button (fields: Title, Transcript, AI Notes/summary, Created At, Note Type, Speaker Transcript, action items, tags)
...
- **Webhook payload core fields** are `{noteId, createdAt, noteType, title, transcript, content}`, plus an action-items array, tags, source type (voice/upload/YouTube/WhatsApp), and recording duration in seconds. No HMAC signature, but you CAN attach a custom auth header (e.g. `Authorization: Bearer xxx`) in the Webhook config and validate it server-side to verify the sender.
...
**Skill does**: Explains webhook setup (Integrations → Webhook → Connect → paste URL → optionally add custom headers → test ping), shows the JSON payload format (core fields plus action items, tags, source type, duration), recommends building middleware to parse the `content` field and POST to CRM API, and notes there's no HMAC but you can attach a custom auth header to verify the sender.
...
**Fix**: Verify the URL is publicly accessible (not localhost). Only notes created *after* connecting the webhook are sent — historical notes are not retransmitted. Check your server logs for incoming POST requests. The payload is JSON with core fields (`noteId`, `createdAt`, `noteType`, `title`, `transcript`, `content`) plus an action-items array, tags, source type, and recording duration. Send a test ping from the Webhook card to confirm delivery, and if you set custom headers make sure your server isn't rejecting them.
