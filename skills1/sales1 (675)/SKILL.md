...
- **No Zapier, Make, or MCP integration.** Automation paths are REST API (Unlimited), webhooks, and native no-code connectors. YouScan ships native connectors for HubSpot, Zendesk, Freshdesk, UseDesk, HelpDeskEddy, and Creatio (Zendesk/Freshdesk/UseDesk/HelpDeskEddy turn matching mentions into support tickets; Zendesk uses OAuth 2.0) — use these before hand-building an API/webhook pipeline when your destination is one of them. For other destinations, webhooks are the sole integration option on Starter (API is Unlimited-only).
