...
| HubSpot | Native | YouScan → HubSpot | Posts mentions into HubSpot. Article: "How to set up a HubSpot integration" |
| Zendesk | Native | YouScan → Zendesk | Creates a Zendesk ticket per matching mention. OAuth 2.0 auth, no passwords stored |
| Freshdesk | Native | YouScan → Freshdesk | Posts mentions as Freshdesk tickets so brand mentions enter the support workflow |
| UseDesk | Native | YouScan → UseDesk | Posts mentions as UseDesk tickets |
| HelpDeskEddy | Native | YouScan → HelpDeskEddy | Posts mentions as HelpDeskEddy tickets |
| Creatio | Native | YouScan → Creatio | YouScan-Creatio connector |
| BI tools (Tableau, Power BI, Google Data Studio) | Via API | YouScan → BI | Unlimited plan only |
| **Zapier** | Not available | — | — |
| **Make** | Not available | — | — |
| **MCP** | Not available | — | — |

**Native CRM/helpdesk connectors:** YouScan ships native, no-code connectors to HubSpot, Zendesk, Freshdesk, UseDesk, HelpDeskEddy, and Creatio that turn matching mentions into CRM/helpdesk records (Zendesk/Freshdesk/UseDesk/HelpDeskEddy create support tickets; Zendesk uses OAuth 2.0). These do not require the API or a custom webhook handler — configure them in topic Settings → Integrations. Use them instead of a hand-built API/webhook pipeline when your destination is one of these six.
