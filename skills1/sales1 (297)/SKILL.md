...
| **MCP Server** | Model Context Protocol server that exposes the full LeadMagic enrichment toolset (all 20+ endpoints) to AI agents (Claude, Cursor, Windsurf, VS Code). |
...
| **Profile Search** | `POST /v1/people/profile-search` | 1 | Enrich from LinkedIn/B2B profile URL — work history, education, skills. 100 req/min rate limit. |
...
#### V3 Search Endpoints (added 2026 — criteria-based discovery)

| Endpoint | Path | Credits | What it does |
|----------|------|---------|-------------|
| **People Search (V3)** | `POST /v3/people/search` | 1/person (+1 email, +5 mobile with `include_contact_details`) | Single-company or multi-company (`company_filters`) people discovery by titles/roles. Replaces older V3 people variants. |
| **Company Search (V3)** | `POST /v3/companies/search` | 1/company | Single lookup or bulk discovery via `company_filters` (industry, headcount, technographics, funding). Supports `preview: true` (counts, no credits) + pagination. |
| **Job Search (V3)** | `POST /v3/jobs/search` (alias `/v3/jobs-search`) | 1/result | Filtered job/signal search with semantic title matching, facets, `mode: "deep"`. |
...
> **Webhooks**: LeadMagic now supports webhooks for async/batch enrichment completion (configured from the enrichment history dashboard, with Clay/Smartlead/Instantly integrations). Synchronous single-record API calls remain request/response. Webhook payload schema and HMAC signing are not documented at the source pages reviewed.
...
LeadMagic provides an official MCP server exposing the full toolset (20+ endpoints) for AI agents:
...
### Pricing & credits (re-verified 2026-06-13 — leadmagic.io/pricing)

| Plan | Price | Credits/mo | Per credit | Rollover |
|------|-------|-----------|-----------|----------|
| **Trial** | $0 | 100 | Free | No |
| **Basic** | $49/mo ($490/yr) | 2,000 | ~$0.020 | No |
| **Essential** (most popular) | $99/mo ($990/yr) | 5,000 | ~$0.017 | Up to 2 mo |
| **Growth** | $249/mo ($2,490/yr) | 20,000 | ~$0.010 | Up to 2 mo |
| **Professional** | $4,990/yr | 50,000 | — | Up to 2 mo |
| **Ultimate** | $849/mo ($8,490/yr) | 100,000 | ~$0.008 | Up to 2 mo |
| **Enterprise** | Custom (from $2,490/quarter) | Custom | — | Up to 2 mo |

- **Pay only for valid results** — not_found results cost 0 credits
- Credit rollover on Essential plan and above (up to 2 months while subscription is active)
- All plans include the full API, CLI/TUI, MCP server, and all enrichment endpoints
...
4. **Consider MCP for AI workflows** — if the user works in Claude, Cursor, or Windsurf, the MCP server gives them the full toolset (all 20+ endpoints) without writing code.
...
3. **Basic plan credits don't roll over.** Only Essential ($99/mo) and above get rollover (up to 2 months while subscribed). If you're on Basic and don't use all 2,000 credits, they expire at month end.
