...
**Delivery batching**: SendPulse POSTs webhook events as a JSON array. The platform sends a batch every minute or when 100 events accumulate, whichever comes first — so handlers must iterate over an array of events rather than expecting one event per POST (verified 2026-06-13).

**Signing**: SendPulse does not document an HMAC/signature header on email-service webhooks. Validate by IP allowlist or a secret path/token instead of a signature header.
