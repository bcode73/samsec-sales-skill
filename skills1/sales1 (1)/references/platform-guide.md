...
- **Company Identification API**: `GET https://epsilon.6sense.com/v3/company/details?ip=...` — pass IP, get company firmographics + scores (1 credit per matched response)
...
*Re-verified 2026-06-13 against the public 6sense API Portal (api.6sense.com/docs). The detailed enrichment/search endpoints moved to the `api.6sense.com` host.*

| API | Base URL | Method | Key param | Returns |
|-----|----------|--------|-----------|---------|
| Company Identification | `epsilon.6sense.com/v3/company/details` | GET | IP address | Company firmographics + scores |
| People Enrichment | `api.6sense.com/v2/enrichment/people` | POST | peopleId / email / linkedinUrl (≤25/call) | Contact data (email, phone, title, skills, experience) |
| Company Firmographics | `api.6sense.com/v1/enrichment/company` | POST | email or domain | Company firmographics |
| Lead Scoring & Firmographics | `scribe.6sense.com/v2/people/full` | POST | email + country | Contact + company data + predictive scores |
| Lead Scoring | `scribe.6sense.com/v2/people/score` | POST | email + country | Predictive scores only (no credit cost) |
| People Search | `api.6sense.com/v2/search/people` | POST | domain + filters (≤25/call) | Matching profiles (availability flags only) |
| People Search Dictionary | `api.6sense.com/v2/search/dictionary` | GET | — | Valid filter values for People Search |
| Segments | `scribe.6sense.com/v2/company/segments` | GET | Company identifier | Segment membership + scores |

**Auth**: `Authorization: Token {api_token}` — 40-char alphanumeric, org-level, generated in Settings > Integration > API Token Management. Create per-integration tokens; rotate ~every 90 days. Only the Company Identification token is for client-side use.

**Rate limits**: 100 req/min general; People Enrichment & People Search 20 queries/second per customer (adjustable in integration settings).

**Credits**: Company Identification = 1 credit/matched response; Enrichment APIs share a credit pool; People Search and Lead Scoring (`/v2/people/score`) deduct no credits. Overage is billable.
