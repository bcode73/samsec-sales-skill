...
github: https://github.com/adglare/ad-server-api
---
...
2. **Management API is Enterprise/Custom only (€649/mo+)** — the REST Management/Reporting/Decision APIs are not available on Lite (€99) or Professional (€499). (A JavaScript display-ad API/ad tag works on all plans.) Need full API on a budget? Consider AdSpeed (API on all plans) or Epom (API from $250/mo). API keys are created in Settings => API Keys and can be scoped per-endpoint/method and IP-locked.
3. **Native ads & video are Enterprise/Custom only** — native, VAST video, and CTV zones require Enterprise or Custom. Lite/Professional are display, redirect, and catalog only.
...
**Skill does**: Explains Enterprise/Custom plan requirement (€649/mo+) for the Management API, demonstrates Bearer token auth, shows the two-step campaign flow (POST /campaigns with name/folder_id/ad_format, then PUT to set pacing/pricing/delivery/targeting), notes creatives live under /campaigns/{id}/creatives, explains that Reports endpoint exists but docs were partially available, provides working cURL and Python examples
