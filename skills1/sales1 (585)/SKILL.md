...
2. **Which Sybill plan are you on?**
   - A) Free
   - B) Pro ($30/user/mo annual, $36 monthly)
   - C) Business ($79/user/mo annual, $108 monthly)
   - D) Enterprise (custom)
...
**Webhook setup**: Webhook automations are available on Pro and up (Settings → Integrations → Automations). The current event is `meeting.new_recording.v1`. Delivery is via Svix with **HMAC signature verification** — verify the signature + timestamp using the dashboard signing secret before trusting any payload. For Zapier/Make, connect through the webhook URL.
...
- **CRM Autofill requires Business plan ($79/user/mo annual, $108 monthly)** — the most requested feature is completely unavailable on Free and Pro. Free gets 20 team-wide summaries/month; Pro gets unlimited summaries but no CRM.
...
- **REST API and MCP access are gated to Enterprise** — but the developer docs are now public at api.sybill.ai/docs (REST, alpha) with an MCP server at `https://mcp.sybill.ai/mcp` (OAuth). Auth is `Authorization: Bearer sk_live_...`, rate limits 60/min, 1,000/hr, 10,000/day. **Webhooks, by contrast, are available on Pro+** (not Enterprise-only).
...
2. Calculates cost delta on annual billing: 12 × ($79 - $30) = $588/mo or $7,056/yr (more on monthly billing: 12 × ($108 - $36) = $864/mo)
...
**Solution**: Confirm you're on Pro or above (webhooks are gated below Pro). Verify the webhook URL is publicly accessible and returns 2xx on POST. Webhooks are delivered via Svix and are HMAC-signed — if your handler rejects unsigned/invalid requests, make sure you're verifying with the dashboard signing secret (Svix signature + timestamp) and not accidentally dropping valid events. Svix retries failed deliveries with exponential backoff (5s → 5m → 30m → 2h → 5h → 10h). Test with webhook.site first. Note the processing delay — webhooks (event `meeting.new_recording.v1`) fire after summary generation, not immediately after the call ends.
