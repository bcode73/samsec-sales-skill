...
For platform-specific audience growth features (Kit, Beehiiv, SparkLoop, Mailchimp, Substack, Omnisend, ActiveCampaign, GetResponse, Unbounce, Leadpages, Ghost, ZoomInfo FormComplete, Clearbit, SendPulse, AdRoll, Lettergrowth, Collab Match, InboxReads, MutualGro, ReferralKit, ReferralHero, KickoffLabs, UpViral, LaunchList, Waitlister, Waitlistly, Preshiplist, Referlist, Tuemilio, GetWaitlist, Refind, Viral Loops, Referral Factory, RafflePress, ShortStack, SweepWidget, Customers.ai, etc.), see references/platforms.md.
...
- `/sales-viral-loops` — Viral Loops platform help (turnkey referral/waitlist/giveaway/milestone/ambassador campaign templates, embeddable widget, per-campaign publicToken REST API + webhooks + two-way Zapier; Build ~$49/Accelerate ~$229)
- `/sales-referral-factory` — Referral Factory platform help (no-code referral program builder, 100+ branded templates, User-centric REST API with referrer attribution + qualification→due-reward flow + in/outbound webhooks, native HubSpot/Salesforce/Stripe syncs; Basic ~$200/Pro ~$400, scales by users)
...
- `/sales-rafflepress` — RafflePress platform help (WordPress giveaway/contest plugin, 30+ entry actions, refer-a-friend, outbound webhooks (no HMAC, social-action data excluded) + Zapier "New Contestant Created" + 15+ native ESPs; no REST API, free Lite + one-time Pro ~$299)
- `/sales-shortstack` — ShortStack platform help (standalone SaaS contest/giveaway/quiz/refer-a-friend campaign platform — Entries API `entries.shortstack.com` with Token auth + SIGNED webhooks (X-Ss-Signature HMAC) + native Mailchimp/HubSpot/Salesforce/Zapier; free-forever plan + paid tiers)
- `/sales-sweepwidget` — SweepWidget platform help (cheap, high-limit giveaway/contest/sweepstakes builder, 100+ entry methods across 30+ social platforms — read+write REST API `sweepwidgetapi.com/sw_api/` with Bearer auth + 50/page `page_start` + HMAC-SHA256 signed webhooks `X-SweepWidget-Signature`, both Enterprise-gated; free-forever plan, native ESP/Zapier/JS-callback on lower tiers)
...
- `/sales-waitlistly` — Waitlistly platform help (validation-first hosted waitlist landing pages, custom domains, on-signup webhooks → Zapier/Make/Slack/your own API, free to start, no public REST API)
- `/sales-preshiplist` — Preshiplist platform help (no-code pre-launch waitlist builder that also sends the launch emails itself via built-in drip sequences; the most closed member of the family — no API/webhooks/Zapier, CSV export only, free plan is draft-only)
- `/sales-referlist` — Referlist platform help (cheap no-code waitlist with Dropbox/Robinhood-style referrals, npm SDK + JS snippet + REST API on Pro, Mailchimp/Airtable/CSV export, free for first 100 signups)
- `/sales-tuemilio` — Tuemilio platform help (early-access waitlist + viral referral loop with REST API + JS SDK + webhooks on all tiers, leaderboards, white-labeled dashboards, Zapier/Mailchimp, Founder $29/Startup $49)
- `/sales-getwaitlist` — GetWaitlist platform help (developer-friendly pre-launch waitlist + referral widget, REST API from Basic $15/mo with unauthenticated POST /signup, new_signup/offboarded_signup webhooks, no free tier for new accounts)
