...
- [In Waitlistly](#in-waitlistly)
- [In Preshiplist](#in-preshiplist)
- [In Referlist](#in-referlist)
- [In Tuemilio](#in-tuemilio)
- [In GetWaitlist](#in-getwaitlist)
- [In Refind](#in-refind)
- [In SweepWidget](#in-sweepwidget)
...
Waitlistly (waitlistly.live) is a fast, **validation-first** hosted waitlist landing-page builder — "the fastest way to validate your idea." It's the lightweight end of the family: stand up a page, collect emails before you build, decide whether to build. For full platform help, use `/sales-waitlistly`.

- **Pricing**: no public pricing page — only "**free to start**" is confirmed; tiers/limits live behind sign-in (treat any figure as best-effort, verify in-app)
- **Forms/pages**: hosted waitlist landing page (built in-app) with **custom domains** (point your own domain via DNS)
- **Automation surface**: the only documented egress is an **on-signup webhook** — "real-time lead data to Zapier, Make, Slack, or your own API when someone joins." **No public REST API**, no MCP, no native CRM connectors
- **Referrals/analytics**: a `/growth` page exists (implies viral/referral mechanics) but rendered blank during research — unconfirmed; signup/traffic analytics assumed but not documented
- **Webhook caveat**: payload schema is **not published** — log a real delivery and map from it; no documented signing or retries, so use an unguessable endpoint and dedupe on a stable id
- **Not an ESP**: capture-only — send launch/nurture emails through your own ESP (wire signups over via the webhook from day one)
- **Best for**: makers/solopreneurs who want the quickest possible idea-validation page on a custom domain, free to start. For a documented API + built-in broadcasts use Waitlister; for an unauthenticated signup API + leaderboard use GetWaitlist; for one-time pricing use LaunchList

### In Preshiplist

Preshiplist (preshiplist.co) is a no-code pre-launch waitlist builder for SaaS/app makers — "the fastest way to create waitlists for SaaS and Apps." Its differentiator: unlike capture-only tools, it **sends the launch/nurture emails itself** via built-in drip sequences, so it doubles as a lightweight ESP. The most closed member of the family — no programmatic surface at all. For full platform help, use `/sales-preshiplist`.

- **Pricing**: tiers named but exact figures unpublished (launch pricing) — Free (**draft-only, can't publish/collect**), Chill Builder (1 product, publish + unlimited signups/drips, custom domain, CSV), Serious Builder (unlimited products + portfolio metrics), Lifetime Partner (one-time). Refunds 30/60/90-day. Treat prices as best-effort
- **Forms/pages**: hosted waitlist page from 7+ validated templates, custom domains + SSL, AI-written copy, short links, OG images
- **Email**: **built-in auto-branded drip sequences** (confirmation, launch-day, updates) with deliverability monitoring — it's capture **and** send, no separate ESP needed
- **Automation surface**: **none** — no public API, no webhooks, no Zapier/Make, no MCP. The **only egress is a manual CSV export**; getting data into a CRM is a batch job over that file, not real-time
- **Referrals**: no documented referral/leaderboard mechanic — only social-proof signup counts/notifications
- **Best for**: makers who want a fast waitlist page that also handles the launch email in one closed tool. For on-signup webhooks → Zapier/Make use Waitlistly; for a documented API + signed webhooks use Waitlister; for an unauthenticated signup API + leaderboard use GetWaitlist

### In Referlist

Referlist (referlist.co) is a cheap, **landing-page-agnostic** no-code waitlist with gamified Dropbox/Robinhood-style referrals — you bring your own landing page and bolt on the referral waitlist. For full platform help, use `/sales-referlist`.

- **Pricing**: Free $0 (100 signups, watermark), Growth $29/mo (500, customization, no watermark), Pro $59/mo (3,000, **REST API + branded email domain + seeding + multi-language**), Enterprise (unlimited, usage-based ~$0.01/signup/mo) — best-effort, verify live
- **Referral mechanic**: position in line + unique referral link; each referred join moves the referrer up the queue
- **5 install methods**: embedded form, `referlist` npm SDK (React/Next.js), JS snippet + element IDs (`referlistemail`/`referlistbutton`), manual `window.referlist.addToWaitlist(domain, email, referralCode)`, REST API
- **Automation surface**: client-side SDK on all tiers; **REST API gated to Pro** (add signup, get an email's place in line, list the waitlist). REST base URL/auth aren't in the public docs index (JS-rendered) — verify before building. **No confirmed webhooks/Zapier**
- **Export**: CSV, Mailchimp, Airtable (Free caps export at 100)
- **Gotchas**: Next.js needs a client-side-only import (`ssr:false`) or you hit "window is not defined"; manual `addToWaitlist` calls must forward the inbound `?ref` code or no one moves up
- **Best for**: makers who already have a landing page and want an affordable viral referral waitlist with an npm SDK. For built-in broadcasts + documented API use Waitlister; for a censored public leaderboard + unauthenticated signup API use GetWaitlist

### In Tuemilio

Tuemilio (tuemilio.com) is an early-access waitlist + pre-launch validation tool with a viral referral loop, leaderboards, white-labeled subscriber dashboards, and email marketing — notable for a full developer trio (REST API + JS SDK + webhooks) at a low flat price. For full platform help, use `/sales-tuemilio`.

- **Pricing**: Founder $29/mo, Startup $49/mo (+teams + priority support), Enterprise (custom). **API, webhooks, custom domains, email sending, and unbranded forms are on every paid tier** — teams is the main Startup gate. 30-day free trial, no card (best-effort, verify live)
- **Referral mechanic**: position + referral link; referred signups earn the referrer `points` to climb the queue; leaderboard `waiters` shows censored emails; fraud signals `anti_points`/`blocked`/`fraud_id`
- **Developer surface**: REST API (`tuemilio.com/api/v1`, `?api_token=` query auth, server-side only) for email CRUD + add-points + waitlist reads; JavaScript SDK (`Tuemilio('init'|'createSubscriber'|'getDashboard'|…)`, event listeners) for the browser; webhooks `new-subscriber`/`grant-access`/`confirmed-subscriber` (`X-Tuemilio-Event` header, no documented signing)
- **Integrations**: Zapier (3,000+ apps, New Subscriber trigger), Mailchimp auto-sync, Typeform, Google Analytics
- **Gotchas**: never call the REST API from the frontend (token is a secret query param) — use the JS SDK client-side; `add-points` is additive while `PUT` sets points absolutely; forward the inbound `?r=` code or referrals don't credit
- **Best for**: founders who want a viral referral waitlist with real API/SDK/webhook access without a high-tier upsell. For HMAC-signed webhooks + built-in broadcasts use Waitlister; for one-time pricing use LaunchList

### In GetWaitlist

GetWaitlist is a developer-friendly pre-launch waitlist + referral widget — fast to embed, with a clean REST API available from its entry tier. For full platform help, use `/sales-getwaitlist`.

- **Tiers** (no free tier for new accounts since mid-2025; existing grandfathered): Basic $15/mo (**API included**), Advanced $50/mo (**custom domain + viral referrals unlock here**), Pro $250/mo (custom email-sending domain + full email automation). 7-day trial.
- **Referral mechanics**: each signup gets a `referral_token`/`referral_link` (`?ref_id=`); referrals subtract `spots_to_move_upon_referral` from the queue `priority`; gamified leaderboard with censored email/name/phone (safe to display publicly)
- **Forms**: embed widget (HTML/iframe/React) or hosted page; 13+ no-code embeds (Webflow, Wix, Shopify, Squarespace, Framer, Carrd, Typedream, Weebly, WordPress, Notion); custom questions stored in `answers`, arbitrary `metadata` per signup
- **Developer surface**: REST API (`api.getwaitlist.com/api/v1`) — signup create/get are **unauthenticated** (public `waitlist_id`), while list/advance/offboard/delete need an `api-key` header or JWT; `new_signup` + `offboarded_signup` webhooks (unsigned, no documented HMAC/retries, 30s connect / 90s response timeout); native HubSpot/Airtable/Slack/Discord + Zapier
- **Email built in**: automatic welcome/verification/referral emails; sending from your own domain is Pro-gated
- **Fraud note**: signup-create is unauthenticated so a public `waitlist_id` can be scripted; enable email verification (`uses_signup_verification`) and gate rewards on `verified` signups. Email validation/fraud detection are gated to higher tiers
- **Limitations**: no MCP server, no Make/Pabbly, no bulk-create API (one POST per signup), unsigned webhooks; offboard keeps history (`removed_date`) while `DELETE /signup` is permanent (GDPR)
- **Best for**: developer-led teams who want a waitlist live in minutes with an API from $15/mo. For built-in broadcasts + signed webhooks use Waitlister; for one-time pricing use LaunchList; for Zapier-native giveaways use KickoffLabs
...
- **Turnkey campaign templates**: referral program, **pre-launch waitlist** (move-up-the-line), giveaway/sweepstakes, milestone referral (Robinhood-style), ambassador, leaderboard, refer-a-friend — pick a template, embed the widget, run a viral loop without building mechanics. Campaign-scoped.
- **API/automation**: REST API at `https://app.viral-loops.com/api/v3` with a per-campaign **`publicToken`**; core endpoint `POST /campaign/participant` (pass the `referrer` code so the referral is credited), plus leaderboard/convert/reward endpoints. **Webhooks** (new participant / referral-count change / converted / reward redeemed) + **two-way Zapier** (same triggers + a Create-Participant action). No documented HMAC — secret URL + re-verify before paying out; dedupe (count events repeat).
- **Watch-outs**: built-in email/automation is **basic** (email-only — ReferralHero/UpViral are stronger on SMS/automation/A-B testing); pricing scales by **participants** (Build ~$49/mo, Accelerate ~$229/mo); viral giveaways attract **fake/disposable signups** — validate before rewarding. The marketing site blocks automated fetches, so pricing/API specifics are best-effort.
- **Referral ≠ affiliate**: it runs customer referrals (rewards/discounts), not commission-based affiliate tracking.
- **Best for**: founders/marketers who want a fast, turnkey **viral waitlist or refer-a-friend program** with a clean widget and webhook/Zapier hooks. **Platform skill**: `/sales-viral-loops`.

### In Referral Factory

- **No-code program builder**: 100+ branded **referral** templates — single-sided, double-sided (give-get), invite-only/ambassador, refer-to-win. Build the pages/links/codes in the dashboard (campaign creation is UI-only); promote via popups, sticky bars, embeds, branded portals.
- **API/automation**: clean REST API at `https://api.referral-factory.com/api/v2`, **Bearer** auth (**one token per account** — regenerating breaks old integrations), 600 calls/min. Core flow is User-centric: `POST /users` with a `referrer` field (`{field: code|id|email, value}`) to attribute, `PUT /users/qualification` to qualify (which makes a reward **due**), then `POST /rewards/issue/{id}`. **Outbound + inbound webhooks** (new user / qualified referral; qualify-in by code/coupon) with **no documented HMAC** — secret URL, dedupe, re-verify. Native HubSpot/Salesforce/Stripe/Pipedrive/Zoho/Intercom + Zapier/Make/n8n.
- **Watch-outs**: **adding a user ≠ qualifying** (rewards only become due on qualification); **white-label + custom domain are Pro+** (reviewers flag custom-domain fees); pricing scales by **users** and is on the higher side (Basic ~$200 / Pro ~$400 / Enterprise ~$1,000); **no direct bank payouts** (PayPal/gift card/Stripe/commission/custom only).
- **Referral ≠ affiliate**: runs customer referrals (can pay commissions) but isn't a partner-portal affiliate platform — compare via `/sales-affiliate-program`.
- **Best for**: SaaS/ecommerce/service teams that want an **on-brand, well-documented-API referral program** with flexible rewards and CRM syncs, and don't mind the higher price. **Platform skill**: `/sales-referral-factory`.

### In RafflePress

- **WordPress giveaway/contest plugin** (by Awesome Motive) for viral list growth: drag-and-drop builder, **30+ bonus-entry actions** (email signup, social follows, refer-a-friend +10 entries, shares), hosted giveaway landing pages, winner picker, fraud protection. Self-hosted on WordPress.
- **Automation surface**: **no hosted REST API** (it's a WP plugin) — instead **outbound webhooks** (Settings → Webhooks; JSON/FORM; fields `fullname`/`email`/`giveaway_id`/`giveaway_name`/`sign_up_date`; **no HMAC**; third-party **social-action data is NOT sent**), a **Zapier** "New Contestant Created" trigger (API key from the Join-an-Email-Newsletter action; one such action per giveaway), **15+ native ESPs** (Mailchimp/Kit/ActiveCampaign/MailerLite/Brevo…), Uncanny Automator, and WordPress hooks + URL-parameter prefill.
- **Watch-outs**: WordPress-only (need a self-hosted site); webhook omits social-action data; **Pro is a paid one-time/lifetime license** (~$299 single / ~$349 unlimited, best-effort) with free **Lite**; giveaways attract fake/self-referral entries — validate before rewarding.
- **Refer-a-friend ≠ affiliate**: it's a giveaway *entry mechanic*, not commission-based partner tracking.
- **Best for**: **WordPress** site owners/bloggers/ecommerce who want a native, one-time-cost viral giveaway with strong ESP sync. **Platform skill**: `/sales-rafflepress`.

### In ShortStack

- **Standalone SaaS campaign platform** for contests, giveaways, sweepstakes, quizzes, refer-a-friend, instant-win, landing pages, pop-ups, and social promos (hashtag, comment-to-enter), with lead capture, automatic winner selection, and analytics. Used by SMBs/agencies up to big brands (UFC/Netflix/Live Nation); 1B+ entries. **Free-forever plan** + paid tiers.
- **Automation surface**: a real **Entries API** (`GET https://entries.shortstack.com/entries`, **`Authorization: Token token=`** header, `per_page` ≤5000, `sort`/`direction`, `{data:[...]}` response — reads entries; campaigns are UI-built) **and a SIGNED real-time webhook** on each new entry (**`X-Ss-Signature`** = HMAC of body + secret — verify it; a positive vs RafflePress/WPFunnels which have no HMAC), plus a **Webhooks management API**, sandbox, native **Mailchimp/HubSpot/Salesforce**, and Zapier. No MCP.
- **Watch-outs**: campaigns aren't API-created (UI only); help center is Cloudflare-protected so the full endpoint catalog/payload schema needs in-account confirmation; contests attract fake/disposable entries (validate before the auto winner-draw); tiers scale to enterprise.
- **Best for**: marketers who want a **standalone, brand-grade contest/giveaway tool with a proper Entries API + signed webhook** (not tied to WordPress), with a free tier to start. **Platform skill**: `/sales-shortstack`.

### In SweepWidget

- **Standalone giveaway/contest/sweepstakes builder** with **100+ entry methods across 30+ social platforms** (follows, likes, shares, comments, email subscribe, refer-a-friend, custom fields, leaderboards, photo/video, API-verified actions, crypto/NFT airdrops), instant rewards/coupons/locked prizes, and list capture. Unlimited entries + unlimited contests on all plans; pitched as a lower-cost, higher-limit Gleam/ShortStack alternative. **Free-forever** plan up to **Enterprise**.
- **Automation surface**: a **read+write REST API** (base **`https://sweepwidgetapi.com/sw_api/`**, **`Authorization: Bearer`** or `api_key` param/field; **50 rows/page** via `page_start`) — reads `/entries` `/users` `/winners` `/giveaways` `/user-entries`, writes `create-giveaway`/`update-giveaway`/`new-entry`/`add-manual-entries`/`create-entry-method`/`create-prize`/`create-reward` — **and HMAC-SHA256 SIGNED webhooks** (**`X-SweepWidget-Signature`** = `sha256=` HMAC of raw body; events `entry_submitted`/`task_completed`/`all_entries_completed`), plus client-side JavaScript callbacks, 38+ native email/ESP integrations, Zapier, WordPress plugin, Shopify app.
- **Watch-outs**: the **REST API and server-side webhooks are Enterprise-plan-only** (below Enterprise use native ESP/Zapier/JS callbacks); free plan **truncates entrant data past 10 users** and has no verified entries; some entry methods (YouTube likes/comments) are **honor-system / unverifiable**; anti-spam device fingerprinting occasionally **false-positives** (cleared via support, not API); giveaways attract fake/self-referral entries — validate before rewarding.
- **Best for**: marketers/creators who want a **cheap, high-limit giveaway tool** with a real read+write API + signed webhook once on Enterprise, or native ESP/Zapier sync on lower tiers. **Platform skill**: `/sales-sweepwidget`.
