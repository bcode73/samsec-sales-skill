...
description: "CRM comparison, selection, and migration strategy for startups — evaluates Attio, HubSpot, Salesforce, Keap, Ontraport, EngageBay, Salesmate, Insightly, Maximizer, Folk, Pipedrive, Close, Zoho, and spreadsheets. Use when choosing a first CRM, deciding if you've outgrown spreadsheets, comparing CRMs for a small team, weighing an all-in-one CRM + marketing automation tool, picking a CRM for a financial-advisory or regulated team, planning a CRM migration, figuring out when to upgrade from free tier, or evaluating whether you need Salesforce yet. Do NOT use for Attio-specific config or API (use /sales-attio), CRM data cleanup (use /sales-data-hygiene), or outbound sequences (use /sales-cadence)."
...
| Keap-specific setup, Campaign Builder, deliverability, API, or interfaces | `/sales-keap {user's question}` |
| Ontraport-specific setup, object-based API, webhooks, automation, or pricing | `/sales-ontraport {user's question}` |
| EngageBay-specific setup, REST API (XML vs JSON, auth, pagination), outbound webhooks, pricing/plan limits | `/sales-engagebay {user's question}` |
| Pipedrive-specific setup, API v1/v2, webhooks, custom-field hashes, token rate limits | `/sales-pipedrive {user's question}` |
| Salesmate-specific setup, v4 API (accessToken/x-linkname headers, offset pagination, 1500 calls/hour), Sandy AI, sequences, calling/SMS, Smart Flow | `/sales-salesmate {user's question}` |
| Insightly-specific setup, REST API v3.1 (pod base URL, Base64 Basic auth, top/skip/count_total pagination, plan-gated daily quota), workflow-automation webhooks, opportunity→project, custom objects | `/sales-insightly {user's question}` |
| Maximizer-specific setup, Octopus API (POST-RPC /Read /Create /Update /Delete, Scope/Criteria/Configuration.Drivers, base64 Keys, PAT/OAuth2, per-edition rate limits), Webhooks API, Financial Advisor edition, cloud vs on-premise | `/sales-maximizer {user's question}` |
...
- **Financial advisors / wealth / insurance, or needs on-premise**: Maximizer (Financial Advisor edition — households, family connections, compliance)
...
2. **HubSpot's free tier is genuinely useful, but the jump to paid is steep.** The free plan covers unlimited users and contacts. But the moment you need marketing automation, custom reporting, or sequences, you're looking at $20/seat/mo (Starter) up through $890/mo (Marketing Hub Professional) and beyond depending on tier. Plan for this before you're locked in.
...
- `/sales-gohighlevel` — GoHighLevel platform help — all-in-one CRM + funnels + messaging for agencies/SMBs, white-label SaaS resale, v2 API
- `/sales-keap` — Keap platform help — all-in-one CRM + marketing automation for small service businesses, Campaign Builder, invoicing, REST API
- `/sales-ontraport` — Ontraport platform help — all-in-one CRM + marketing automation + payments + dynamic CMS, object-based REST API, webhooks
- `/sales-engagebay` — EngageBay platform help — affordable all-in-one CRM + marketing + service for startups/SMBs, REST API (XML-default, raw API-key auth, cursor pagination), outbound-only webhooks
- `/sales-pipedrive` — Pipedrive platform help — pipeline-first sales CRM, REST API v1+v2 (x-api-token/OAuth, cursor pagination, free webhooks, token-based rate limits, v1 shutdown 2026-07-31)
- `/sales-bitrix24` — Bitrix24 platform help — free-forever all-in-one suite (CRM + tasks + contact center + sites), method-based REST API (`crm.*` via `/rest/{user}/{code}/{method}`), inbound/outbound webhooks, OAuth, batch, MCP; cloud or self-hosted on-prem
- `/sales-salesmate` — Salesmate platform help — AI sales CRM with built-in calling/SMS, sequences, Smart Flow, and Sandy AI copilot; v4 REST API (`{linkname}.salesmate.io/apis/`, `accessToken`+`x-linkname` headers, offset pagination, 1500 calls/hour, no inbound webhooks)
- `/sales-insightly` — Insightly platform help — unified CRM + project management + marketing + service; REST API v3.1 (pod base URL, Base64 Basic auth, top/skip/count_total pagination, plan-gated daily quota 1k–100k, ETag/If-Match), workflow-automation webhooks (Professional+); opportunity→project, no email sequencing
- `/sales-maximizer` — Maximizer platform help — Canadian cloud/on-premise sales + financial-services CRM; Octopus API (POST-RPC /Read /Create /Update /Delete, Scope/Criteria/Configuration.Drivers, base64 Keys, PAT/OAuth2, per-edition rate limits) + Webhooks API (2-second/3-attempt delivery); Financial Advisor edition (households, family connections)
...
**Cause**: HubSpot's paid tiers jump significantly ($0 → $20/seat/mo Starter → $890/mo Marketing Hub Professional)
