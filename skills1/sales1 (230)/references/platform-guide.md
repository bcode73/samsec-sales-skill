...
Prices below are per-seat/month on **annual** billing unless noted (monthly billing is higher — Starter is ~40% more on monthly). Best-effort — verify at hubspot.com/pricing. Pricing re-verified 2026-06-13 against HubSpot's official pricing blogs.

### Marketing Hub
| Tier | Price | Marketing Contacts | Seats | Key additions |
|------|-------|--------------------|-------|---------------|
| Free | $0 | n/a (1,000 contacts total, 2,000 email sends/mo) | Up to 2 users | Forms, email marketing, basic reporting |
| Starter | $9/seat/mo annual ($10 monthly) | 1,000 | per seat | Remove branding, simple automation, email health |
| Professional | $800/mo annual ($890 monthly) | 2,000 | 3 core (+$45/extra seat) | Full automation, A/B testing, custom reporting, SEO, social. $3,000 onboarding fee |
| Enterprise | $3,600/mo flat | 10,000 | 5 core (+$75/extra seat) | Custom objects, adaptive testing, revenue attribution. $7,000 onboarding fee |

Additional marketing contacts are billed in tiers — exceeding the included count bumps you to the next block (e.g. Professional 2,001 contacts jumps to the 5,000 tier, ≈ +$250/mo, i.e. ≈ $50/mo per 1,000). Enterprise blocks are larger (≈ $100/mo per 10,000).

### Sales Hub
| Tier | Price | Seats | Key additions |
|------|-------|-------|---------------|
| Free | $0 | Up to 2 users | Basic CRM, 1 pipeline, meeting scheduling |
| Starter | $9/seat/mo annual ($15 monthly) | per seat | 2 pipelines, email tracking, simple automation |
| Professional | $90/seat/mo annual ($100 monthly) | per seat | Sequences, forecasting, playbooks, custom reporting. $1,500 onboarding |
| Enterprise | $150/seat/mo annual | per seat | Custom objects, predictive lead scoring, advanced permissions. $3,500 onboarding |
...
### API & Webhooks
- REST API with date-versioned endpoints. First date-based API version `2026-03` is GA. Breaking changes ship twice a year (March and September); each version is supported for 18 months and is immutable within its support window.
- Base URL: `https://api.hubapi.com`
- Authentication: OAuth 2.0 (Bearer token, required for multi-account/public apps), private app access tokens / static auth tokens (single account, also Bearer), developer API keys via `hapikey` for a few developer-level endpoints, and OAuth client-credentials tokens for app-config endpoints (e.g. Webhooks Journal API). Legacy hapikey API keys for data access were deprecated Nov 30, 2022 — use private app tokens or OAuth.
- **Rate limits** (verified 2026-06-13 against official usage guidelines):
  - Private apps — burst (per 10 sec, applied **individually per app** on platform versions 2025.2 / 2026.03) and daily (per account):
    - Free & Starter: **100 / 10 sec**, **250,000 / day**
    - Professional: **190 / 10 sec**, **625,000 / day**
    - Enterprise: **190 / 10 sec**, **1,000,000 / day**
  - Public (marketplace) OAuth apps: **110 requests / 10 sec** per installing account (the add-on below does not apply).
  - **API Limit Increase add-on**: raises the burst limit to **250 / 10 sec** and adds 1,000,000/day (max two packs; burst increase does not stack beyond 250).
  - **CRM Search API** is stricter than the general limits: **5 requests/second** shared across the whole account, max **200 records per page**. Search endpoints do not return the standard rate-limit headers.
- **Webhooks**: Subscribe to CRM events (e.g. `contact.creation`, `contact.propertyChange`, `deal.creation`, `deal.propertyChange`, `*.deletion`). HubSpot signs requests with header `X-HubSpot-Signature-v3` plus `X-HubSpot-Request-Timestamp`. Verify: reject if the timestamp is older than 5 minutes, build a UTF-8 string of `requestMethod + requestURI + requestBody + timestamp`, HMAC-SHA256 it with the app secret, base64-encode, and constant-time compare to the header. Older `X-HubSpot-Signature` (v1/v2) headers are still sent for backward compatibility. App lifecycle events (`app_install` / `app_uninstall`) are available via the Webhooks Journal API.
- SDKs: official HubSpot-maintained client libraries for **Node.js, PHP, Python, and Ruby** (GitHub under the HubSpot org; the Node.js client is the JavaScript SDK). No official Go client.
