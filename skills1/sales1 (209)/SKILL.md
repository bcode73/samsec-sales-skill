...
**For API integrations**: Queue all outbound calls — 3 req/sec + 10k/day are the only published limits, and Gong's concurrency ceiling is low (use a semaphore, don't fan out). Cache call metadata. Use webhooks for real-time, polling for backfill. The transcript endpoint is `POST /v2/calls/transcript` with the callId(s) in the body filter — not GET, and not a per-id path (common gotcha). The base URL is org/region-specific — read it from your API authentication page.
...
- **Transcript endpoint is `POST /v2/calls/transcript` with a body filter.** It's POST (every other vendor uses GET) AND the callId goes in the request-body filter, not in the URL path. This is the most common first-integration bug.
...
3. Walks through `POST /v2/calls/transcript` with callIds in the body filter (POST, not GET, not a per-id path — common gotcha)
