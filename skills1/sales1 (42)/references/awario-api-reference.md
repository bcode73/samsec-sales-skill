<!-- Source: https://awario.com/help/api/api-overview/ , https://awario.com/help/api/api-authentication/ , https://awario.com/help/api/get-alerts-list/ , https://awario.com/help/api/get-mentions/ , https://awario.com/help/api/get-insights/ , https://awario.com/help/api/rate-limits/ , https://awario.com/help/api/error-codes/ ; re-verified 2026-06-13 -->
...
Returns list of all alerts (Mention and Leads types) in your account, plus a `limits` array describing plan quota state.
...
```bash
curl "https://api.awario.com/v1.0/alerts/list?access_token=YOUR_API_KEY"
```
...
```json
{
  "limits": [
    {
      "limit_name": "new_mentions",
      "limit_reached": false,
      "limit_renewal_date": 1714003200000
    }
  ],
  "alerts": [
    {
...
**Note:** `alert_type` is `"mentions"` or `"leads"` (plural `mentions`, not `mention`).
...
**Mention fields (per current docs):** `id`, `date`, `reach`, `language`, `url`, `snippet`, `title`, `done`, `starred`, `tags`, `author` (`name`, `avatar`, `audience`, `description`, `url`), `sentiment`, `source`, `entities`.
...
**Available types:** `mentions`, `sentiment`, `reach`, `countries`, `languages`, `sources`, `topic_cloud`, `gender`, `age` (same set as the total endpoint per current docs)
...
- **Get Alerts endpoint confirmed** (2026-06-13) — path is `/alerts/list` (GET); response uses `alert_id`/`alert_type`/`alert_name`/`project_id`/`project_name` plus a `limits` array.
