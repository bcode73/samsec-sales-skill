...
```json
{
  "alert_id": 123456,
  "alert_type": "mentions",
  "alert_name": "My Brand Alert",
  "project_id": 789,
  "project_name": "My Project"
}
```

Alert types: `mentions` (brand monitoring) and `leads` (Awario Leads prospecting). The `GET /alerts/list` response wraps these in an `alerts` array alongside a `limits` array (`limit_name`, `limit_reached`, `limit_renewal_date`).
