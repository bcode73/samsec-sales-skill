...
- **MCP server support**: Claygent can connect *out* to external MCP servers (Salesforce, Gong, Google Docs, Notion) to pull context. Separately, Clay publishes its own **official MCP server** that exposes Clay to LLM clients like Claude — see Webhooks & HTTP API below.
...
- **Inbound webhooks**: Each table has a unique webhook URL (Add → Webhooks → Monitor webhook). POST JSON to it → creates new rows. Use for triggering enrichment from external events (form submissions, Zapier, custom apps). You can optionally generate an **authentication token** to secure the webhook — Clay only lets you view it once, so save it immediately; include it in the request header from your source system.
- **HTTP API action**: Call any external API from within a Clay table. Configure method (GET/POST/PUT/DELETE), headers, body, and field mapping. Auth schemes supported for the external API: Bearer token, `X-API-Key`, Basic, and a JWT mode where Clay fetches and auto-refreshes a token. Use for pushing data to tools that don't have native integrations.
- **Plan gate**: HTTP API actions require Growth plan or above.
- **No traditional CRUD REST API**: Clay doesn't expose a public CRUD REST API for querying/managing tables programmatically. Integration is webhook-based. The Clay API key (Settings → Account) is for Clay-native cross-table lookups, not external CRUD.
- **Official Clay MCP server**: Exposes Clay enrichments and custom Functions to LLM clients (Claude, ChatGPT). Connect from Claude via the Connectors page (`claude.com/connectors/clay`, OAuth; 500 free credits on first connect) or the endpoint `api.clay.com/v3/mcp`. Available on Launch/Growth/Enterprise; company/contact lookups free for free/trial users. (Distinct from Claygent connecting *out* to other tools' MCP servers.)
- **Enterprise People & Company API**: Lightweight lookup via email, LinkedIn URL, or domain. Available only on Enterprise plans.

## Pricing & credits (re-verified 2026-06-13 against clay.com/pricing — unchanged)
