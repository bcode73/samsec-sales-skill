...
2. **No public CRUD REST API — webhook + MCP integration model.** Clay doesn't expose traditional CRUD endpoints for managing tables or rows programmatically. External integration is via inbound webhooks (POST to table URL), outbound HTTP API actions, and the **official Clay MCP server** (connect from Claude via `claude.com/connectors/clay`). A Clay API key exists in Settings but is only for Clay-native cross-table lookups. Inbound webhooks now support an optional auth token header — generate one for sensitive sources instead of relying on URL obscurity.
