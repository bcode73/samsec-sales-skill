...
github: "https://github.com/SafetyMails-Team/safetymails-sdk"
---
...
### Real-Time API (two modes)
SafetyMails offers the real-time validator two ways:

**1. Standard script (no-code):** a JavaScript `<script>` embed you drop into forms/landing pages to validate emails before they enter your database.
- **How it works**: In the panel, Real-time API → Create a new source → Standard installation; set the domain, choose which statuses to allow/block, then paste the generated script into the page footer: `<script src='https://optin.safetymails.com/main/safetyscript/<TICKET>/safetyoptin.js'></script>`
- **Use cases**: Landing page forms, newsletter signups, registration flows, checkout forms; works with WordPress (Forminator/Smart Forms/Form Maker), Google Tag Manager, RD Station, Unbounce, Instapage, Landingi, E-goi

**2. Custom HTTP REST API (developer):** a direct server-side call for full control.
- **Endpoint**: `POST https://<TICKET_ORIGEM>.safetymails.com/api/<CODE_TICKET>`
- **Auth**: HMAC-SHA256 in the `Sf-Hmac` header — `hash = HMAC_SHA256(email, API_Key)`
- **Body**: form-data, field named `email`
- **Response (JSON)**: `{"Success": true, "Email": "...", "Status": "valid", "Score": 97}`; errors return `{"Success": false, "ErrorCode": "401", "Message": "Invalid API Key"}`
- **SDK**: official examples in JS, PHP, Python, Java, Go at github.com/SafetyMails-Team/safetymails-sdk

- **Response time**: Under 1 second
- **Best practices**: Customize error messages to match your brand voice. Use the API to keep invalid emails out of your CRM/ESP in the first place — cheaper than cleaning lists after the fact.

> **API note**: For programmatic BULK verification, SafetyMails has a batch validation API documented via Swagger at panel.safetymails.com/swagger_api2.html — access requires credentials issued by SafetyMails support (contact via the support address on safetymails.com). Rate limits and Email Finder API access are not publicly documented; flag those as needing confirmation from support.
...
| Feature | Details |
|---|---|
| **Real-time API surface** | Standard `<script>` embed OR custom HTTP REST: `POST https://<TICKET_ORIGEM>.safetymails.com/api/<CODE_TICKET>` |
| **Auth** | HMAC-SHA256 in `Sf-Hmac` header — `HMAC_SHA256(email, API_Key)`; key/ticket from panel.safetymails.com |
| **Response** | JSON: `{Success, Email, Status, Score}` (success) / `{Success, ErrorCode, Message}` (error) |
| **Docs / SDK** | docs.safetymails.com + official SDK (JS/PHP/Python/Java/Go) at github.com/SafetyMails-Team/safetymails-sdk |
| **Response time** | <1 second for real-time validation |
| **Bulk verification** | Panel upload OR batch validation API (Swagger: panel.safetymails.com/swagger_api2.html, support-issued credentials) |
| **Rate limits** | Not publicly documented — confirm with support |
| **Trial** | 100 free credits on signup + 1,000 real-time-API credits free for 15 days |
...
### API key not working / "Unauthorized" / `{"Success": false, "ErrorCode": "401"}`
**Symptom**: Script embed or REST API calls return authentication errors
