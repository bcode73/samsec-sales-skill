...
- **Base URL**: `https://api.whatfix.com/v1/` (content/core API); report-analytics downloads use `https://whatfix.com/api/v1/{accountId}/reports/summary/`. Your Account ID is embedded in the path — obtain it from `support@whatfix.com`.
- **Authentication**: User API token ("Integration Key"). Two headers are required on authenticated calls — `x-whatfix-integration-key` (the token) and `x-whatfix-user` (the token owner's email). HTTPS-only ("The APIs are served only via HTTPS"). The token must be enabled for your account by Whatfix support before it can be generated under **Settings → API token**.
- **Format**: JSON output (CSV for report downloads via `?format=csv`), CORS-enabled
- **Key endpoints**: `GET/POST /v1/accounts/{accountId}/content` (retrieve/create content); `GET https://whatfix.com/api/v1/{accountId}/reports/summary/{reportName}?format=csv` (e.g. `mostPopularFlows`)
...
- **Rate limits / error codes / pagination**: not publicly documented — verify against developer.whatfix.com
