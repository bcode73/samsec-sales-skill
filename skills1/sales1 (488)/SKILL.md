...
- **First-time setup**: Download from revive-adserver.com/download (not GitHub ZIP). Latest is v6.0.6 (March 2026). Requires PHP 7.2.5+ (v5.x) or PHP 8.1+ (v6.0.x, tested through PHP 8.5), MySQL 4.1+/PostgreSQL 8.1+ (MariaDB works as MySQL-compatible), and any FastCGI web server (Apache, Nginx, IIS 7+, lighttpd, FrankenPHP) or Apache with mod_php. Run the web installer, create your first advertiser, campaign, banner, and zone.
...
2. **PHP version compatibility is strict** — v6.0.x requires PHP 8.1+ (tested through PHP 8.5); v5.4.0+ supports PHP 7.2.5 up to 8.1.x; v5.3.0+ supports PHP 7.2.5 up to 8.0.x; v4.2–v5.2.1 require PHP 7.0.8+ but do NOT support PHP 8. Upgrading PHP can break your installation.
...
**Skill does**: Walks through server requirements (PHP 8.1+ for v6.0.x, MySQL/MariaDB or PostgreSQL, a FastCGI web server like Apache/Nginx), downloading from official site, running the web installer, creating the admin account, and setting up the first advertiser/campaign/zone
...
**Cause**: PHP version incompatibility — v6.0.x requires PHP 8.1+; v4.2–v5.2.1 don't support PHP 8 at all
**Solution**: Check the compatibility matrix: Revive v5.3.0+ supports PHP 7.2.5–8.0.x, v5.4.0+ supports PHP 7.2.5–8.1.x, and v6.0.x requires PHP 8.1+ (tested through PHP 8.5). Either downgrade PHP or upgrade Revive to a compatible version.
