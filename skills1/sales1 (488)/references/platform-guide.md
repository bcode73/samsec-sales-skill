...
**Latest version**: v6.0.6 (released March 2026). The v6.0.0 line launched October 2025 and requires PHP 8.1+. Notable v6.0 additions: a new **Mailer plugin** (hands off outbound emails — admin/report notifications — to an SMTP service or 3rd-party mail provider), an optional banner upload file-size limit, a mixed-content warning when saving banners, and statistics-query optimizations (some reports ~100x faster). v6.0 did NOT add a REST API, webhooks, or change the XML-RPC API or email/newsletter zones.
...
| **Requirements** | PHP 8.1+ (v6.0.x, tested through PHP 8.5) or PHP 7.2.5+ (v5.x); MySQL 4.1+ or PostgreSQL 8.1+ (MariaDB works as MySQL-compatible); any FastCGI web server (Apache, Nginx, IIS 7+, lighttpd, FrankenPHP) or Apache with mod_php |
...
Available at reviveadserverrestapi.com (paid plugin). Pricing: US$249/year subscription or US$599 lifetime license. Compatible with Revive v4.x, v5.x, v6.x and all supported PHP 7/8 versions.
...
- **Latest version**: v5.2.0 (April 18, 2024)
...
1. **Server requirements**: PHP 8.1+ (for v6.0.x; tested through PHP 8.5), MySQL 4.1+ or PostgreSQL 8.1+ (MariaDB works as a MySQL-compatible drop-in), and any FastCGI-capable web server (Apache, Nginx, IIS 7+, lighttpd, FrankenPHP) or Apache with mod_php
