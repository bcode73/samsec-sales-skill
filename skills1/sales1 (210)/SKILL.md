...
1. **For API questions** — note that API access requires Business plan (since Dec 2025) or Enterprise. REST API base URL is `https://api.grain.com`; all endpoints sit under `/_/public-api/v2/`. Auth header is `Authorization: Bearer TOKEN` using one of three token types: Personal Access Token (user-level access), Workspace Access Token (full-workspace access), or OAuth2 Authorization Code + PKCE. Developer docs are at `developers.grain.com`. Rate limit: **300 requests/minute**, then `429 Too Many Requests`. Webhooks exist via `POST /_/public-api/v2/hooks/create` (events: recording_added/updated/deleted, highlight_added/updated/deleted, story_added/updated/deleted, upload_status) — no documented signature, so secure the receiver with a secret URL.
...
- **API is Business+ only.** Free and Starter plans cannot use the REST API. Developer docs at `developers.grain.com`. The API is rate-limited to 300 requests/minute (returns `429` beyond that) — not the old "1 req/sec" guidance.
...
**Solution**: Use the API transcript endpoint `GET /_/public-api/v2/recordings/:recording_id/transcript` for structured speaker-labeled JSON, or `.txt`/`.vtt`/`.srt` variants for clean plain text / subtitle formats. Alternatively, use the Zapier integration to export transcript text to Notion/Google Docs where formatting is preserved.
