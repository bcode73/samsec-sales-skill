...
| GET | `/v1/core/mobile-events?pageSize=100` | functions.siro.ai | Bearer | List mobile app events (paginated via `pageSize`) |
...
Payload includes `recordingId`, `siroUserId`, boolean `recordingProcessed` / `recordingLinked` flags, and a `crm` block with `integrationConnectionId`, a `users[]` array, and `engagement` / `opportunity` / `account` objects that each carry both Siro's internal `id` (UUID) and the native CRM `externalId`. Only act on the event when both `recordingProcessed` and `recordingLinked` are `true`.
