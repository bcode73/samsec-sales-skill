...
- **API docs ARE public now.** Brand24 publishes an official OpenAPI 3.1 "API Data" REST API at https://api-data.brand24.com/api-data-docs/documentation (base `https://api-data.brand24.com/api-data/v1/`). Auth is the `X-Api-Key` header (key from `app.brand24.com/account/integrations-api-data`), server-side only — browser calls are CORS-blocked. It's a read/analytics API (mentions, sentiment, reach, topics, events, daily metrics, demographics, create-project). Mentions use cursor pagination (`cursor` + `limit`, max 500). No webhooks or rate-limit headers are documented; use polling, Zapier, or the MCP server for AI-assistant access.
