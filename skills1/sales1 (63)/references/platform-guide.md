...
- **30-day money-back guarantee** (excludes custom/Enterprise plans)
- **No auto-overage charges** — mentions cap at plan limit
- All prices in the table above are **annual** billing rates (per month, billed annually). **Monthly (no-commitment) rates are higher:** Individual $249, Team $349, Pro $499, Business $699. Annual billing saves ~20%.
...
1. Install the **Brand24 app** from the ChatGPT App Store
2. Click **Connect** and authorize your Brand24 account
3. Start asking questions (no manual URL/config needed)

Any MCP-compatible client (Cursor, Copilot, custom agents) can connect with the server URL above + OAuth.
...
## API surface — Brand24 API Data (REST)

Brand24 publishes an official, public **API Data** REST API with an OpenAPI 3.1 specification. This is a **read/analytics API** (statistics, insights, mentions, project management) — not an event-stream API.

- **Docs (public):** https://api-data.brand24.com/api-data-docs/documentation
- **OpenAPI spec (machine-readable):** https://api-data.brand24.com/static/js/api-data/apiDocs.json
- **Base URL:** `https://api-data.brand24.com/api-data/v1/`
- **API version:** `1.0.0`
- **Auth:** `X-Api-Key` HTTP header. Get your key at `https://app.brand24.com/account/integrations-api-data`. **Server-side only** — browser requests are blocked by CORS; never embed the key in frontend/mobile code.
- **Pagination:** Cursor-based on the mentions endpoint (`cursor` + `limit`, `limit` max 500).
- **Requires:** Active Brand24 subscription (key is issued from your account).
...
{ "status": "success", "data": { } }
// Client error
{ "status": "fail", "message": "...", "code": 400 }
// Server error
{ "status": "error", "message": "...", "code": 500 }
```

**Documented error codes:** 401 (missing/invalid API key), 400 (bad/missing params), 404 (invalid endpoint), 500 (server error). **No rate-limit headers or 429 are documented** in the spec — throttle conservatively and handle non-2xx defensively.

### Endpoint inventory (GET unless noted)

**Project statistics**
- `GET /project/{project_id}/mentions/count` — mentions count (`date_from`, `date_to`)
- `GET /project/{project_id}/mentions/sentiment` — positive/negative/neutral breakdown (`date_from`, `date_to`)
- `GET /project/{project_id}/mentions/reach` — social vs non-social reach (`date_from`, `date_to`)
- `GET /project/{project_id}/hot-hours` — peak activity hours
- `GET /project/{project_id}/most-followers` — top authors by followers

**Project insights**
- `GET /project/{project_id}/mentions` — paginated mention records (`date_from`, `date_to`, `limit` max 500, `cursor`, `sentiment`, `category`)
- `GET /project/{project_id}/project_events` — anomaly/Storm-style events (`sort_order` asc/desc, `limit`)
- `GET /project/{project_id}/topics` — AI-detected topics with metrics + share of voice
- `GET /project/{project_id}/daily-metrics` — daily mentions/reach/sentiment/engagement (`from`, `to`, `source`, `includeBySource`)
- `GET /project/{project_id}/ai-summary` — AI summary text
- `GET /project/{project_id}/ai-insights` — AI insights array
- `GET /project/{project_id}/trending-hashtags`
- `GET /project/{project_id}/demographics` — ages, education, countries, income, interests, occupation, sex, social presence

**Project configuration**
- `POST /account/{account_id}/create_project` — create project (`project_name`, `language`; body `keywords[]` with `keyword`/`required`/`excluded`); returns `projectId`
- `GET /account/{account_id}/projects_list/` — map of project IDs → names
- `GET /project/{project_id}/keywords` — keyword config

**Project sources**
- `GET /project/{project_id}/domains/` — popular domains
- `GET /project/{project_id}/trending-links`
- `GET /project/{project_id}/most-active-sites`

**Brand metrics & reference**
- `GET /brand-metrics/brand_score_counts` — cross-source counts by `brand_name` (reddit posts/comments, twitter, gdelt)
- `GET /languages` — supported languages
- `GET /mentions/categories` — source category tokens
- `GET /account/mentions-usage-estimation` — projected mentions usage

### Gaps / unverified

- **Webhooks:** No webhook/callback definitions appear in the official OpenAPI spec, and the integrations help collection lists none. Earlier notes referencing "real-time webhook event notifications" and an official SDK/sandbox are **unconfirmed against current docs** — treat the API Data REST endpoints (polling) + Zapier + MCP as the supported automation paths.
- Rate limits and a 429 response are not documented in the spec.

**Recommendation:** For analytics/export pipelines, use the **API Data REST API** (`X-Api-Key`, server-side). For AI-assistant workflows, use the **MCP server** (OAuth, no key management). For no-code, use **Zapier**.

<!-- Source: https://api-data.brand24.com/api-data-docs/documentation, https://api-data.brand24.com/static/js/api-data/apiDocs.json, https://help.brand24.com/en/collections/10259552-integrations, https://help.brand24.com/en/articles/13011375-brand24-mcp -->
...
**Gotcha:** Zapier connection has rate limits. For high-volume monitoring (10K+ mentions/mo), use the API Data REST endpoint `GET /api-data/v1/project/{project_id}/mentions` (cursor pagination, `limit` up to 500) with the `X-Api-Key` header for batch exports instead. For aggregate counts use `/mentions/count`, `/mentions/sentiment`, and `/daily-metrics` rather than paging every mention.
