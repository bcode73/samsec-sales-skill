...
- **Attachments**: Up to 30MB total message size including headers, body, and attachments (older docs recommended ≤20MB to avoid downstream mail-server limits)
- **Categories**: Tag emails with up to 10 categories for filtering in statistics
- **Rate limits**: General API 600 req/min default; mail/send endpoint has no rate limit; Email Activity API (`/messages`) is limited to ~6 req/min
...
## Event Webhooks
- **Delivery events**: processed, dropped, delivered, deferred, bounce (bounce events carry a `bounce_classification`)
- **Engagement events**: open, click, spam report, unsubscribe, group unsubscribe, group resubscribe
- **Account event**: account_status_change (compliance-related account status changes)
- **Multiple webhooks**: SendGrid supports multiple Event Webhooks per account, each with its own `id`, URL, and event selection — managed via full CRUD (`POST/GET .../settings`, `GET .../settings/all`, `GET/PATCH/DELETE .../settings/{id}`)
...
- **Signed webhooks**: Verify authenticity with ECDSA signatures — headers `X-Twilio-Email-Event-Webhook-Signature` and `X-Twilio-Email-Event-Webhook-Timestamp`, verified against the webhook's public key using the raw request body bytes
...
- **Base URL**: `https://api.sendgrid.com/v3/` (global) or `https://api.eu.sendgrid.com/v3/` (EU data residency)
- **Authentication**: Bearer token — `Authorization: Bearer <API_KEY>`
- **Rate limits**: 600 req/min default general; mail/send has no rate limit; Email Activity API ~6 req/min
- **Message size**: 30MB maximum (including headers, body, and attachments)
...
- **Docs**: www.twilio.com/docs/sendgrid (docs.sendgrid.com now redirects here)
...
## Pricing (USD, verified 2026-06-13 against twilio.com/en-us/products/email-api/pricing and .../marketing-campaigns/pricing — verify current pricing before quoting)

**Email API Plans** (transactional email; priced by email volume, not contacts):

| Plan | Price | Volume | Key gated features |
|------|-------|--------|-------------------|
| **Free Trial** | $0 (60 days) | 100/day | Analytics, 1 teammate, 1 event webhook, dynamic templates |
| **Essentials** | From $19.95/mo | 50,000–100,000/mo (tiered) | 1 teammate, 2 event webhooks, no dedicated IP; email validation available for purchase |
| **Pro** | From $89.95/mo | 100,000–2,500,000/mo (tiered) | Dedicated IP, SSO, subuser management, 1,000 teammates, 5 event webhooks, 2,500 email validations/mo |
| **Premier** | Custom pricing | 5,000,000+/mo | All Pro features + dedicated IP pooling, automated IP warm-up, advanced support |

Overage: sending above your plan volume incurs a small per-email overage fee.

**Marketing Campaigns Plans** (email marketing; priced by contact count):

| Plan | Price | Contacts | Key gated features |
|------|-------|----------|-------------------|
| **Free Trial** | $0 (60 days) | 100 contacts | Design/code editors, segmentation, A/B testing, 3 testing credits, 1 signup form |
| **Basic** | From $15/mo | 5,000–100,000 (tiered) | Drag-and-drop editor, segmentation, A/B testing, 10 testing credits, 5 signup forms; NO automation, NO dedicated IP |
| **Advanced** | From $60/mo | 10,000–200,000 (tiered) | Automations, dedicated IP, 60 testing credits, 15 signup forms, 1,000 teammates |
| **Custom** | Custom pricing | Unlimited | Unlimited contacts/email, enterprise support with guaranteed response times |
