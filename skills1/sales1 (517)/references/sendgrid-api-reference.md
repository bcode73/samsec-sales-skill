...
> **Note**: Some endpoint details below are best-effort from research. The Email Activity API requires an additional purchase on non-Premier plans. Always check https://www.twilio.com/docs/sendgrid/api-reference for the most current documentation (the old docs.sendgrid.com URLs now redirect to www.twilio.com/docs/sendgrid).
...
```
https://api.sendgrid.com/v3/        # Global
https://api.eu.sendgrid.com/v3/     # EU regional (data residency)
```

SendGrid offers a global host and an EU regional host. Use the EU host (`api.eu.sendgrid.com`) only if your account is provisioned for EU data residency; otherwise use the global host.
...
- **600 requests/min** default for most endpoints (limits are applied per endpoint over a refresh period)
- **mail/send endpoint**: no rate limit (no rate limit headers returned)
- **Email Activity API** (`/messages`): much lower limit — ~6 requests/min
- Responses include `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset` headers on rate-limited endpoints
- Exceeding the limit returns HTTP `429 Too Many Requests` (often with a `Retry-After` header); you cannot request that endpoint again until the refresh period resets
- **Message size limit**: 30 MB max total per message (including headers, body, and attachments). Note: older docs recommended staying under 20 MB to avoid downstream mail-server limits, but the hard cap enforced by `/mail/send` is 30 MB. Gzip-compressed payloads must also be ≤30 MB.
...
| 413 | Payload too large (exceeds 30 MB) |
...
SendGrid now supports **multiple Event Webhooks per account**, each with its own `id`, URL, and event selection. The API gives full CRUD over individual webhooks (the older single-webhook `GET`/`PATCH /user/webhooks/event/settings` shape was replaced — note the new `/all` list endpoint and `{id}` path params).

##### Webhook CRUD (multiple webhooks)

| Method | Endpoint | Description |
|---|---|---|
| POST | `/user/webhooks/event/settings` | Create an Event Webhook (returns `201` with a unique `id`) |
| GET | `/user/webhooks/event/settings/all` | Get all Event Webhooks |
| GET | `/user/webhooks/event/settings/{id}` | Get a specific Event Webhook |
| PATCH | `/user/webhooks/event/settings/{id}` | Update a specific Event Webhook |
| DELETE | `/user/webhooks/event/settings/{id}` | Delete a specific Event Webhook |
| POST | `/user/webhooks/event/test` | Send a test event to a URL |

Request/response fields include `url` (required), `enabled`, `friendly_name`, and a boolean flag per event type (`delivered`, `bounce`, `deferred`, `processed`, `dropped`, `open`, `click`, `unsubscribe`, `group_unsubscribe`, `group_resubscribe`, `spam_report`). No more than one webhook may point to the same URL. The `friendly_name` is a convenience label only and is not guaranteed unique — do not use it to programmatically differentiate webhooks; use the `id`.

##### Signed Webhooks (per webhook)

| Method | Endpoint | Description |
|---|---|---|
| PATCH | `/user/webhooks/event/settings/signed/{id}` | Toggle signature verification for a webhook |
| GET | `/user/webhooks/event/settings/signed/{id}` | Get the signed webhook's public key |

Signature verification uses **ECDSA** (Elliptic Curve Digital Signature Algorithm). When enabled, every event POST carries two headers: `X-Twilio-Email-Event-Webhook-Signature` (Base64-encoded signature) and `X-Twilio-Email-Event-Webhook-Timestamp` (Unix timestamp). To verify, concatenate the timestamp with the **raw request body bytes** (do not re-serialize the JSON — transformations can strip bytes that were signed) and verify against the public key. Official SDK helpers exist (e.g. `EventWebhook`/`EventWebhookHeader` in sendgrid-python, sendgrid-nodejs, sendgrid-go).
...
| `bounce` | Receiving server permanently rejected the message (includes `bounce_classification`) |
...
| `account_status_change` | Compliance-related account status change (e.g. account under review/suspended) |

**`bounce_classification`** (on `bounce` events) is one of: `Invalid Address`, `Technical`, `Content`, `Reputation`, `Frequency/Volume`, `Mailbox Unavailable`, or `Unclassified`. Use it to decide whether to permanently suppress (`Invalid Address`), retry later (`Frequency/Volume`, `Mailbox Unavailable`), or fix content/auth (`Content`, `Reputation`).
...
**Example — create an event webhook** (multi-webhook API; POST returns a `201` with an `id`):
```bash
curl --request POST \
