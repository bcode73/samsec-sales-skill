...
- **REST API**: `api.wave.co/v1/` — sessions, transcripts, media, action items, folders, semantic search, webhooks, and a pull-based event feed
- **MCP server**: `mcp.wave.co` — Claude Desktop, Claude Code, ChatGPT integration (browser OAuth)
- **CLI**: `npm install -g @waveai/cli` — OAuth login, `--json` output for piping
...
- Structured action items (readable via `GET /v1/sessions/{id}/action-items`, writable via `PATCH /v1/sessions/{id}`)
- Folder membership (non-exclusive — a session can be in many folders; Gmail-labels model)
- Full transcript with speaker segments
- Audio/video media files
- Phone-call sessions (type `phone`) also include direction, numbers, status, and ongoing flag
