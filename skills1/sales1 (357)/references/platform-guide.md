...
```bash
# List conversations for a specific team inbox (mailbox filter is required)
curl "https://public.missiveapp.com/v1/conversations?inbox=true&limit=25" \
  -H "Authorization: Bearer YOUR_API_TOKEN"
```
...
resp = requests.get(
    ...
    params={"all": "true", "limit": 25},
)
for conv in resp.json().get("conversations", []):
    print(f"{conv['subject']} — {conv['messages_count']} messages")
```

**Gotcha**: The conversations endpoint requires at least one mailbox filter — pass exactly one of `inbox`, `all`, `assigned`, `closed`, `snoozed`, `flagged`, `trashed`, `junked`, `drafts`, `shared_label`, `team_inbox`, `team_closed`, or `team_all`. The conversations list defaults to `limit` 25 (max 50), lower than other endpoints.
...
### Batch pipeline
- **Pagination**: Use `limit` and `offset` params. Defaults/maxes are per-endpoint — conversations default 25 (max 50), contacts/contact books default 50 (max 200), messages default 10 (max 10). Some endpoints support an `until` Unix-timestamp cursor.
- **Rate limits** (documented): 5 concurrent requests, 300 req/min (5/sec), 900 req/15 min (1/sec sustained). On `429`, honor `Retry-After`; also read `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset`. Sustain 1 req/sec for continuous querying or burst up to 5/sec.
- **Error recovery**: Check HTTP status; retry on 5xx and 429 with backoff. Log failed IDs for manual review.
