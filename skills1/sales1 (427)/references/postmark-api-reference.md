...
**SMTP relay** (alternative to the REST API): host `smtp.postmarkapp.com` (transactional) or `smtp-broadcasts.postmarkapp.com` (broadcast), ports 25, 2525, or 587. Authenticate with either the Server API Token (used as both username and password) or a dedicated SMTP Token (Access Key = username, Secret Key = password).
...
- No strict published API rate limits; excessive requests return HTTP `429 Too Many Requests`
- **SMTP concurrency**: Postmark recommends limiting to **10 concurrent connections per IP** to the SMTP endpoint (informal guidance — reuse connections and send multiple messages per connection)
...
- **Batch payload limit**: 50 MB, up to 500 messages per batch call
- **Recipient limit**: max 50 addresses each across `To`, `Cc`, and `Bcc`
...
**Webhook security**: Postmark does **not** HMAC-sign webhook requests — there is no signature header to verify. Secure your receiving endpoint by one or more of: Basic HTTP authentication embedded in the webhook URL (`https://user:pass@host/hook`), IP allowlisting to Postmark's published webhook IP ranges, and HTTPS. Verify the request out-of-band (Basic Auth credentials or source IP), not via a payload signature.

**Webhook retries**: Postmark retries on any non-200 response. Bounce and Inbound webhooks retry at 1 min, 5 min, 10 min (×3), 15 min, 30 min, 1 hour, 2 hours, 6 hours. Click, Open, Delivery, and Subscription Change webhooks retry at 1 min, 5 min, 15 min. A **403** response stops all further retries. Make handlers idempotent on `MessageID` since the same payload may be delivered more than once.
