...
## SMTP relay
- **Host**: `smtp.postmarkapp.com` for transactional messages; `smtp-broadcasts.postmarkapp.com` for broadcast messages (use the broadcast host to send through a broadcast stream over SMTP)
- **Port**: 25, 2525, or 587 (STARTTLS / TLS)
- **Authentication**: Two options — (1) use your **Server API Token** as both the username and password, or (2) use a dedicated **SMTP Token** where the Access Key is the username and the Secret Key is the password
...
- **Security**: Postmark does **not** sign webhooks with HMAC. Secure your endpoint with (1) **Basic HTTP authentication** embedded in the webhook URL (`https://user:pass@host/hook`), (2) **IP allowlisting** restricting to Postmark's published IP ranges, and/or (3) **HTTPS** for transport encryption. Do not look for an `X-Postmark-Signature`-style header — there isn't one
- **Retry schedule**: Postmark retries on any non-200 response. Bounce and Inbound webhooks retry at 1 min, 5 mins, 10 mins (×3), 15 mins, 30 mins, 1 hour, 2 hours, 6 hours. Click, Open, Delivery, and Subscription Change webhooks retry at 1 min, 5 mins, 15 mins. **A 403 response stops all retries**
...
- **Reactivation**: Remove suppressions via the API (`POST /message-streams/{stream}/suppressions/delete` — note this is a POST, not a DELETE) or UI when the underlying issue is resolved
...
## Pricing (USD, re-verified 2026-06-13 at postmarkapp.com/pricing)

| Plan | Price | Volume | Key gated features |
|------|-------|--------|-------------------|
| **Free** | $0/mo | 100 emails/mo | Never expires, full REST API/SMTP access, up to 10 custom domains, 45-day retention, no overages allowed (hard cap at 100) |
| **Basic** | $15/mo | Starting at 10,000 emails/mo | $1.80/1K overage, up to 5 custom domains, 45-day message retention, no inbound email |
| **Pro** | $16.50/mo | Starting at 10,000 emails/mo | $1.30/1K overage, 10 custom domains, **inbound email processing**, customizable retention (up to 365 days), dedicated IP eligibility |
| **Platform** | $18/mo | Starting at 10,000 emails/mo | $1.20/1K overage, unlimited custom domains/users/streams, inbound email, customizable retention |

In early 2026 Postmark moved features like inbound email processing, unlimited team collaboration, and additional message streams down to the 10K-email tier so they no longer require a high-volume plan. Inbound email starts at **Pro** ($16.50/mo); the Free and Basic plans do not include it.

**Add-ons:**
- Dedicated IP: starts at $50/mo per IP (requires Pro tier or above **and** 300K+ emails/mo sending volume)
- Custom Activity Retention: starts at $5/mo
- DMARC Monitoring (DMARC Digests): starts at $14/mo per domain (dashboard + extended retention at dmarc.postmarkapp.com; a free weekly digest is available to anyone)
