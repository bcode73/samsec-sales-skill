...
| **Slack integration** | Global or per-alert Slack incoming webhooks | Business+ |
...
| **Bulk keyword import** | Import keywords in bulk (dashboard Bulk Import page; no bulk API endpoint) | Business+ |
| **Team management** | Multi-user team features | Business+ |
...
| Reddit/HN conversation trackers | 0 | 2 | 10 | 50 |
| Filtering & saved results | No | Yes | Yes | Yes |
...
**Plan gate warning:** API, webhooks, and Slack are all bundled into a single feature row on the pricing page and ALL unlock together at Business ($79/mo). Essential ($19/mo) has NO Slack, NO API, and NO webhooks — its only differentiators over Free are AI sentiment analysis and filtering. Team management also starts at Business, not Enterprise.

## Integrations

- **Slack**: Incoming webhook integration (global or per-alert). Business+.
...
Note: the live docs payload prefixes the matched keywords with `Keywords: ` in the `query` field, and `datetime` is a human-readable string like `19 Jan 24 05:52 UTC` (NOT ISO-8601). Parse defensively.
