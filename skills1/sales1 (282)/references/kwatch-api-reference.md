...
(Payload reproduced verbatim from the live docs notification-channels page, 2026-06-13.)

**Fields:**
- `platform`: `reddit`, `hacker_news`, `twitter`, `linkedin`, `youtube`, `facebook`
- `query`: the matched keywords, prefixed with `Keywords: ` in the live payload (e.g. `"Keywords: vllm"`) — NOT the bare keyword string
- `datetime`: human-readable timestamp string like `19 Jan 24 05:52 UTC` (NOT ISO-8601) — parse defensively
...
- No HMAC signature or request signing is documented on the notification-channels page (verify before relying on signature verification)
