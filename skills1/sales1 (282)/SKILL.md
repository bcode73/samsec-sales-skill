...
2. **Which plan are you on?**
   ...
   - C) Business ($79/mo — 100 Reddit/HN, API/webhooks/Slack, bulk import, team management)
   - D) Enterprise ($199/mo — 500 Reddit/HN, advanced support)
...
- **API, webhooks, AND Slack are all Business-only ($79/mo).** They share one feature row on the pricing page and unlock together at Business — Free and Essential users have no programmatic access and no Slack. Essential ($19/mo) only adds AI sentiment analysis and filtering over Free. If you need API access at a lower price, F5Bot Ultra ($58.33/mo), Syften (EUR 39.95/mo), or RedShip ($19/mo) all offer REST APIs.
...
5. Notes that Slack and webhooks are Business-only — on Essential, alerts arrive by email; recommend upgrading to Business ($79/mo) if real-time Slack/team notifications are needed
**Result**: Multi-platform monitoring set up within Essential plan limits (email alerts only)
