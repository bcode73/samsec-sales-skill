...
**API versions**: v1 (legacy, still active) and v2 (current, under active development). Both can be used simultaneously. As of the 2026-06-13 re-verification the API roadmap announces no v1 deprecation — recently shipped items include LinkedIn step support in v2 campaign endpoints, interest-level info in prospect webhooks, mailbox footer management, and an editable `delivery_time` field.
**Developer contact**: developers@woodpecker.co

**Companion surfaces** (all gated behind the same "API, webhooks, MCP Server, CLI & integrations" add-on, $20/mo — verified 2026-06-13):
- **MCP Server**: official Model Context Protocol server (`Woodpeckerco/woodpecker-mcp-server` on GitHub, distributed as a Docker image). Authenticates with a Woodpecker API key via the `WOODPECKER_API_KEY` environment variable. Works with Claude Desktop, Continue.dev, Cursor, and other MCP-compatible clients. Tool categories: Campaign Management, Prospect Operations, Email Composition, Analytics & Reporting, Mailbox Integration, Advanced Configuration. Requires the API & Integration add-on. Docs: https://developers.woodpecker.co/docs/mcp/
- **CLI**: official command-line tool — install with npm package `@woodpecker.co/cli` (command `woodpecker`), Node.js 22 LTS recommended. Inspect/manage campaigns, prospects, inbox messages, mailboxes, reports, webhooks, blacklists, and agency resources; outputs human-readable text or JSON for scripts/CI/AI agents; can issue raw API requests. Requires API access. Docs: https://developers.woodpecker.co/docs/cli/
...
### Reports — v2 (2 endpoints)

Verified 2026-06-13 against https://developers.woodpecker.co/docs/reports/. Reports are generated asynchronously: you POST to start generation and receive a hash, then GET the hash to retrieve results.

| Method | Endpoint | Description |
|---|---|---|
| POST | `/v2/reports/{report-name}` | Start generating a report for a given type and time period. Returns a hash identifier used to fetch the result. |
| GET | `/v2/reports/{hash}` | Retrieve the generated report using the hash returned by the POST request. |

**Report types** (the `{report-name}` path segment): complete statistics across campaign levels, general campaign statistics, message volume per campaign level, and campaign open rates.

---

### Lead Finder — (requires API add-on)

Verified 2026-06-13 against https://developers.woodpecker.co/docs/lead-finder/. The Lead Finder API supports two workflows; specific endpoint paths and request schemas are documented under the Lead Finder section sub-pages and were not captured verbatim here.

| Workflow | Description |
|---|---|
| Lead search and enrichment | Search the B2B database for leads matching criteria, then queue them for enrichment. Consumes Lead Finder credits. |
| Prospect enrichment | Enrich prospects that already exist in your Woodpecker database. |

> Gap: exact Lead Finder endpoint paths, methods, and request/response bodies are not yet captured in this reference. See https://developers.woodpecker.co/docs/lead-finder/ for the live schemas before building against it.
...
#### Supported Webhook Events (25 events — verified 2026-06-13 against developers.woodpecker.co/docs/webhooks/)
...
| `campaign_paused_by_bounce_shield` | Campaign auto-paused by Bounce Shield when the bounce rate crosses the protection threshold (added since 2026-03) |
...
| Event | Description |
|---|---|
| `linkedin_automation_account_connected` | A LinkedIn account was successfully connected for automation (added since 2026-03) |
| `linkedin_automation_account_disconnected` | A connected LinkedIn automation account was disconnected (added since 2026-03) |
| `linkedin_automation_connection_request_accepted` | LinkedIn connection request was accepted |
| `linkedin_automation_direct_message_sent` | A LinkedIn direct message was sent via automation (added since 2026-03) |
...
| Webhooks | **25 event types** (verified 2026-06-13) including AI-detected interest levels and LinkedIn-automation lifecycle events; payloads batched up to 100; no HMAC/signature verification documented |
...
- Reports API: https://developers.woodpecker.co/docs/reports/
- Lead Finder API: https://developers.woodpecker.co/docs/lead-finder/
- Users API: https://developers.woodpecker.co/docs/users/
- MCP Server: https://developers.woodpecker.co/docs/mcp/
- CLI: https://developers.woodpecker.co/docs/cli/
