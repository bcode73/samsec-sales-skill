...
1. **API access is a paid add-on ($20/mo, verified 2026-06-13)** — not included in base plans. Many users expect API access by default, but you need the "API, webhooks, MCP Server, CLI & integrations" add-on or an active trial. The same single add-on now also unlocks the official MCP Server (`Woodpeckerco/woodpecker-mcp-server`, `WOODPECKER_API_KEY` env var, works with Claude/Cursor) and the CLI (`@woodpecker.co/cli`, command `woodpecker`).
2. **LinkedIn automation is a separate add-on ($29/LinkedIn account/mo)** — not included in any plan tier. Each LinkedIn account you want to automate costs extra on top of your base plan.
3. **Woodpecker prices by "contacted prospects" not by user/seat** — pricing is now a calculator at ~$4.8 per 100 contacted prospects/mo (verified 2026-06-13). Every new person added to a campaign counts against your monthly limit, even if they don't receive all steps. Recontacting the same prospect in a different campaign counts again.
4. **The agency panel is per active client ($27/client/mo, plus optional White Label at $5/client/mo)** — costs scale linearly with client count. Compare with Smartlead's flat agency pricing if managing many clients.
