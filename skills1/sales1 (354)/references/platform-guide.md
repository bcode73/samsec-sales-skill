...
Native macOS email client built exclusively for Gmail by a former Apple Mail engineer. Uses the Gmail API directly — not IMAP — for native label, category, alias, and search support. Privacy-focused: tokens stored in macOS Keychain, no intermediary sync servers, built-in tracking prevention. Mac-only, Gmail-only, no AI features. Roadmap (per FAQ): Microsoft 365 / Outlook, IMAP, and JMAP; a native iOS app is in a TestFlight beta (see iOS app note below).
...
| Tracking Prevention | Blocks common tracking pixels (best-effort, per official docs) + can block remote images from auto-loading | UI-only |
...
| Group | $49.99/seat/yr or $4.99/seat/mo | All Individual features + centralized billing and seat management, up to 50 seats |
| 50+ seats | Contact sales | "Need more than 50 seats? Contact sales" — no separate Enterprise tier or pricing published |
...
### iOS app status (as of 2026-06)

There is still no public iPhone/iPad app on the App Store. A native iOS app is now in a **TestFlight beta** that **requires iOS 26 or later** and has **limited capacity**. Invites have so far gone to customers who have been **subscribers for at least 36 months**, with access expanding to the longest-tenured customers first. Eligible users see a "Join TestFlight" banner after logging into their Mimestream account. No public App Store release date has been announced. (Source: mimestream.com/ios-beta)
...
- **Tracking Prevention**: "Prevent tracking when viewing messages" blocks common trackers that may notify the sender when you view their message. Official docs state this is best-effort and not a guarantee that all trackers are blocked. A separate "Prevent remote images from loading automatically" setting blocks remote images (including tracking pixels) in the viewing pane.
