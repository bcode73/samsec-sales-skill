...
- **Gmail-only.** Mimestream only works with Gmail and Google Workspace. No Outlook, iCloud, Yahoo, or IMAP support today. Per the official FAQ, the roadmap names Microsoft 365 / Outlook, IMAP, and JMAP — but no ETA.
- **Mac-only — no public iOS app yet.** There is no App Store iPhone/iPad app. A native iOS app is now in a TestFlight beta (requires iOS 26 or later, capacity limited); as of mid-2026 invites have gone to customers who've been subscribers for at least 36 months, with access expanding to longest-tenured customers first. No public App Store release date announced. You still need a separate email app for mobile in the meantime.
