<!-- Source: https://sdk-docs.krisp.ai/, https://krisp.ai/developers/, https://krisp.ai/developers/voice-translation-api/, https://help.krisp.ai/hc/en-us/articles/24514911804316-Webhook-API, https://www.postman.com/krispai/krisp-team-management-api-s-public-workspace/ -->
<!-- Note: Primary API docs are JS-rendered or behind auth (help.krisp.ai 403s). Content below is assembled from multiple accessible sources. Re-verified 2026-06-13. -->

# Krisp API Reference

## 1. Webhook API (Meeting Assistant)

The Webhook API lets you automatically send Krisp meeting outputs (transcripts, notes, outlines) to another tool or internal system via a secure URL endpoint.

**Plan gate (re-verified 2026-06-13 — sources conflict):** The current krisp.ai/pricing page lists webhook capability starting at **Core** ("Unlimited Integrations & webhook"), with **Advanced** adding *team-level* webhooks. However, Krisp's launch announcement described the Webhook API as a "Business" tier feature. The Webhook API help article (help.krisp.ai/.../Webhook-API) returned HTTP 403 on re-verification, so the exact gate could not be confirmed first-hand — verify in-app for your workspace.
...
API keys are generated at the developer dashboard **https://developers.krisp.ai** (the SDK docs portal is https://sdk-docs.krisp.ai). The long-lived API key is exchanged at runtime for **short-lived session keys** — temporary scoped tokens with a configurable TTL (5 minutes to 24 hours). Keys are workspace-scoped. (Verified 2026-06-13 via krisp.ai/developers.)

---

## 2b. Voice Translation API (self-serve, developers)

Self-serve speech-to-speech translation API for developers, launched as part of the Krisp developer platform.

| Property | Value |
|---|---|
| Languages | 61 languages, any-to-any pair (locale variants incl. US Spanish, French Canadian, Egyptian Arabic, Catalan, Basque, Galician) |
| Accuracy | 96% on real calls (live enterprise calls with accents/background noise) |
| Free credit | 60 minutes of translation credit on every new account |
| Access | Self-serve — sign up at developers.krisp.ai, no sales call required |
| SDKs | Python, JavaScript (with sample code). C++ coming soon. |
| Auth | API key from developers.krisp.ai → short-lived session key minted via `mintVtSessionKey()` |
| Features | Custom vocabulary/dictionary, developer dashboard + playground |

Docs: https://krisp.ai/developers/voice-translation-api/ (Verified 2026-06-13.)
