...
**For integration questions:** Webhooks are the primary programmatic integration path — the current pricing page shows webhook capability starting at Core, with Advanced adding team-level webhooks (an older announcement framed the Webhook API as Business/Advanced-only; verify in-app). Krisp fires events when transcripts, notes, or outlines are generated. For CRM sync, HubSpot is available on Core+, Salesforce on Advanced+. Krisp also offers MCP integration (Claude/ChatGPT/Cursor) on Core+.

**For SDK questions:** Point to sdk-docs.krisp.ai. The JS SDK (WASM) is the most documented. VIVA SDK is for voice AI agents (server-side); there's also a self-serve Voice Translation API (61 languages, 60 min free credit). Auth is an API key from the **developers.krisp.ai** dashboard, exchanged at runtime for a short-lived session key.
...
- **Accent conversion** officially covers Indian, Filipino, African, Latin American English. Both speaker-side and listener-side (real-time, launched March 2026) are offered; more accents roll out over time.
- **Transcription now covers up to 95 languages** via Multilingual Meeting Transcriptions (in beta as of 2026-06). English is on-device; other languages are server-side.
- **Webhook capability:** per the current pricing page, webhooks start at Core ("Unlimited Integrations & webhook"), with Advanced adding team-level webhooks. (An earlier Krisp announcement framed the Webhook API as a "Business"/Advanced feature — sources conflict; verify the gate in-app.)
...
**Skill does**: Checks common causes — Krisp microphone not selected as input (recordings captured silence), meetings under minimum duration threshold, storage limit hit (Core = 10 GB, Advanced = 60 GB). Recommends checking Settings → Recordings for failed entries, verifying audio input device in each conferencing app.
