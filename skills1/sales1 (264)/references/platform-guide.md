...
### Dialers
Per the official Supported Dialers help article (verified 2026-06-13): 8x8, Aircall, Apollo (requires Salesforce CRM), Close.io (requires Close CRM), Cloud Call, CloudTalk, ConnectAndSell, Dialpad, Five9 (custom integration), HubSpot (requires HubSpot CRM), InsideSales/Xant (requires Salesforce CRM), JustCall, Kixie (requires HubSpot CRM), Nooks (requires HubSpot CRM), Outreach, Orum (requires HubSpot CRM), RingCentral, Salesloft, Talkdesk, TwilioFlex (custom integration), Vonage, Zoom Phone.

- Some dialers are CRM-gated (e.g. Apollo/InsideSales need Salesforce; HubSpot/Kixie/Nooks/Orum need HubSpot) — confirm your CRM pairing before relying on a dialer.
- Jiminny updates call data from connected dialers every **15 minutes or sooner**, depending on the provider.

### Video conferencing
- Google Meet (Notetaker), Microsoft Teams (Notetaker), and Zoom (Zoom Notetaker)
- The Zoom Notetaker uses the Zoom Meeting SDK and requires the host's "Record to computer files" permission; it can also record Zoom Webinars and Zoom meetings with registration.
...
- **Playback statistics / call statistics**: listen count, coaching activity, automated call scoring
- **Coaching frameworks**: ratings, coach and coachee information
- **Action items** and **teams/topics**
