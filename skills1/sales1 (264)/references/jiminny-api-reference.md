...
- **Playback summaries / call statistics**: listen count, coaching activity, automated call scoring
- **Action items**
- **Teams / topics**
...
Jiminny maintains open-source OAuth2 client libraries (PHP League OAuth 2.0) for dialer/integration providers. Public repos confirmed in the org on 2026-06-13:
- `jiminny/oauth2-dialpad` — Dialpad OAuth 2.0
...
- `jiminny/oauth2-ringcentral` — RingCentral OAuth 2.0
- `jiminny/oauth2-zoom` — Zoom OAuth 2.0
- `jiminny/hubspot-php` — HubSpot PHP client

GitHub org: https://github.com/jiminny

> Note: the `customer-api-docs` repo backing the Swagger UI at jiminny.github.io/customer-api-docs is not present in the org's public repo list (the GitHub Pages site is published, but the source repo is private or renamed). The Swagger UI itself is JS-rendered and could not be statically captured.
