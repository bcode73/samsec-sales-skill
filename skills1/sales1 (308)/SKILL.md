...
- **Automation pattern**: Zapier "Letterly Actions" trigger (fires when you tap an action inside a note — not a passive new-note trigger) or webhook → route to Notion, Google Docs, Slack, email, or any connected app (text only: title, text, date, tags — audio is not sent)
...
- **90-minute recording limit** — each recording session caps at 90 minutes (raised from 15 min in May 2025). For longer sessions, split into multiple recordings. The number of recordings is unlimited.
- **Audio/video file upload IS supported** — as of Aug 2025 you can upload existing audio/video files for transcription, not just record in-app. (Older reviews saying "record-only" are stale.)
- **No search feature** — as notes accumulate, finding older recordings is difficult. Use tags aggressively to organize. This is a frequently-requested missing feature. (The MCP server lets an AI client query your notes, but there is no in-app search UI.)
- **AppSumo LTD is stackable-code pricing** — current LTD is $89/$178/$267/$356 for 1–4 stacked codes (stacking raises feature limits); older "1/2/4-device license" framing is no longer the live offer. Refundable up to 60 days.
...
- **Zapier trigger is "Letterly Actions", not automatic** — the Zapier trigger fires only when you tap an action inside a note (it is NOT a passive "new note created" trigger). Only text (title, text, date, tags) is sent — the audio file is not. Some users also report intermittent trigger reliability; test before relying on it.
- **No public REST API, but an MCP server exists** — there are no REST endpoints/SDKs, but Letterly added an MCP server (April 2026) so Claude/ChatGPT/Cursor can query and rewrite notes. Programmatic paths are MCP, Zapier, and webhooks.
...
**User**: "I need to record a 45-minute brainstorm session — does Letterly handle that?"
**Skill does**: Confirms Letterly now supports up to 90 minutes per recording (raised from 15 min in May 2025), so a 45-minute session fits in a single recording. For sessions over 90 minutes, split into chunks and tag all segments with the same tag, then combine the rewritten outputs. Notes that AudioPen still caps at 15 min, while TalkNotes Pro allows 2 hours.
