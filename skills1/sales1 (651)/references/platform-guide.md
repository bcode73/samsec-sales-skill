...
- **Rate limit**: 1000 requests/minute using a **token bucket**. Write requests (POST/PUT/PATCH/DELETE) may count as more than one request against the budget. Headers: `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`.
- **Pagination**: Cursor-based, `limit` (max/default 100) + `from` (cursor string), `next` returns null at end
- **Sorting**: `sortBy` parameter accepts `createdAt` or `updatedAt` (default `updatedAt`)
- **HTML fields**: Supports subset of HTML tags (a, img, p, div, br, ul, ol, nl, li, b, u, i, strong, em, code)
