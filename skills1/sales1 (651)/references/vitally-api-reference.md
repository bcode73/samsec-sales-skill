...
<!-- Fetched: 2026-04-14; re-verified 2026-06-13 -->
...
- **Rate limit**: 1000 req/min using a **token bucket**. Write requests (POST, PUT, PATCH, DELETE) may count as **more than one request** against that budget, so a write-heavy workload exhausts the limit faster than the raw call count suggests.
- **Rate limit headers**: `RateLimit-Limit` (e.g. `1000, 1000;window=60`), `RateLimit-Remaining`, `RateLimit-Reset` (seconds until budget refills)
- **Pagination**: Cursor-based — `limit` (max/default 100), `from` (cursor), `sortBy` (createdAt|updatedAt, default `updatedAt`)
- **Response**: `{ "results": [...], "next": "cursor_or_null" }`
- **Error codes**: 400 (bad request), 401 (unauthorized), 429 (too many requests)
- **HTML support**: `<a>`, `<img>`, `<p>`, `<div>`, `<br>`, `<ul>`, `<ol>`, `<nl>`, `<li>`, `<b>`, `<u>`, `<i>`, `<strong>`, `<em>`, `<code>` (`<a>` allows href/name/target; `<img>` allows src/alt/width/height)
...
| POST | `/resources/projects/:projectId/tasks/:taskId` | Link an existing task to a project |
| DELETE | `/resources/projects/:projectId/tasks/:taskId` | Unlink a task from a project |
