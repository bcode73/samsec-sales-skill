...
Tokens support four feature types:
- **Access resources** — manage Core API resources (conversations, contacts, inboxes)
- **Auto-provisioning** — manage SCIM provisioning resources (requires contacting Front)
- **Application triggers** — process external service events (OAuth tokens excluded)
- **MCP server** — connect Front data to MCP clients like Claude and ChatGPT
...
Integrations installed by a customer do NOT count against the customer's rate limit — the only exception is the Zapier integration, which does count against it.

**Server connection limit**: Calls Front makes to your configured app servers (e.g. application webhooks / connectors) are limited to 600 rpm.
...
Webhooks fire for conversation, message, tag, comment, link, and assignee events. Mass actions (batch status changes, inbox migrations, historical imports) are excluded.

Application webhooks support these documented event types (~14+): `inbound_received`, `outbound_sent`, `conversation_moved`, `message_delivery_failed`, `conversation_archived`, `conversation_reopened`, `conversation_deleted`, `conversation_restored`, `conversation_snoozed`, `conversation_snooze_expired`, `new_comment_added`, `assignee_changed`, `tag_added`, `tag_removed`, `link_added`, `link_removed`.

### Signature verification (documented)

**Application webhooks** — header `x-front-signature` carries a base64 HMAC-**SHA256** digest. Compute it by concatenating the `x-front-request-timestamp` header value + a colon (`:`) + the raw request body, then HMAC-SHA256 with your app's signing secret (provided during webhook configuration). Use a constant-time comparison.

**Rule webhooks** — header `X-Front-Signature` carries a base64 HMAC-**SHA1** digest of just the raw request body, using your API secret as the key (Settings → Company → App store → Webhooks app). Note the different algorithm vs application webhooks.

### Delivery & retries (documented)

Webhooks are POST requests with a JSON event body; your endpoint must respond within a 5-second window.

- **Application webhooks** retry up to 3 times when Front receives an error / 429 / 500 / 408 / timeout. If all retries fail, Front **disables the webhook** and stops sending events — an admin must re-enable it via the webhook settings (published apps get manual review first).
- **Rule webhooks** do NOT retry on failure. Recover missed data by polling the List events API (`GET /events`).

Implement idempotency using event IDs regardless.
...
- Webhook payload schemas vary by event type and aren't fully enumerated in public docs — use OpenAPI specs or test with rule webhooks to capture exact payloads
- Webhook signing and retry behavior ARE documented (see Webhooks section above): app webhooks = HMAC-SHA256 with timestamp; rule webhooks = HMAC-SHA1, no retries
