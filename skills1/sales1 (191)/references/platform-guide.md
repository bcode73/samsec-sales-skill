...
**Autopilot pricing**: Usage-based, starting at $0.05 per conversation (not a per-seat add-on like Copilot/QA/CSAT).
...
app = Flask(__name__)
SIGNING_SECRET = b"YOUR_APP_SIGNING_SECRET"  # from webhook configuration

def verify_app_webhook(req) -> bool:
    # Application webhooks: HMAC-SHA256 over timestamp + ":" + raw body, base64
    ts = req.headers.get("x-front-request-timestamp", "")
    raw = req.get_data()  # raw bytes, before JSON parsing
    base = ts.encode() + b":" + raw
    expected = base64.b64encode(
        hmac.new(SIGNING_SECRET, base, hashlib.sha256).digest()
    ).decode()
    return hmac.compare_digest(expected, req.headers.get("x-front-signature", ""))

@app.route("/front-webhook", methods=["POST"])
def handle_webhook():
    if not verify_app_webhook(request):
        return jsonify({"error": "bad signature"}), 401

    event = request.get_json()
    # Application webhooks wrap details under "type" + "payload"
    event_type = event.get("type")

    if event_type in ("inbound_received", "outbound_sent"):
        payload = event.get("payload", {})
        print(f"{event_type}: {str(payload)[:100]}")
        # Process: sync to CRM, trigger alert, create ticket in external system

    return jsonify({"status": "ok"}), 200
```

**Gotchas**: Application webhooks require your server to be ready at configuration time and must respond within 5 seconds — Front retries up to 3x then disables the webhook (admin must re-enable). Rule webhooks are easier for testing (event previews you fetch via API) but do NOT retry, and they sign with HMAC-**SHA1** not SHA256. Mass actions (bulk status changes, inbox migrations) don't trigger webhooks.
...
- **Application webhooks**: Recommended for production — deliver full event payloads, shared inbox scope. Verify with `x-front-signature` (HMAC-SHA256 over `x-front-request-timestamp` + `:` + raw body)
- **Rule webhooks**: Better for testing — can send event preview (lighter payload) with option to fetch full event via API. Verify with `X-Front-Signature` (HMAC-SHA1 base64 of the raw body) — note the different algorithm
- **Retry behavior**: App webhooks retry up to 3x (on error/429/500/408/timeout, 5s window), then Front disables the webhook until an admin re-enables it. Rule webhooks do NOT retry — poll `GET /events` to recover missed data. Implement idempotency with event IDs
