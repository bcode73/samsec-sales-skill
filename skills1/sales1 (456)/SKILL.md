...
- **Free plan is company-level only**: The Free plan (150 resolutions) does NOT include person-level identification — and as of January 2026 it no longer returns any person-level contact data (no names, emails, or LinkedIn URLs), only company-level ID. You need a paid plan (Starter or above) for person-level ID. Note Starter returns LinkedIn URLs only (no email addresses); business email addresses start at Pro.
- **API is a separate product (APIs V2)**: The Identity Resolution API has its own account at ui.api.rb2b.com and its own credit-based pricing, completely separate from the pixel product. Base URL is `https://api.rb2b.com/api/v1`, all calls are **POST with a JSON body** and an `Api-Key` header (not GET / not x-api-key), credit cost is **per endpoint (1–4)** not a flat 1, and the rate limit is 50 requests/second per endpoint. There's an official MCP server (`@rb2b/rb2b-apis-mcp`). Don't confuse pixel plan limits with API credits.
- **Credit-per-visitor billing (pixel)**: As of Jan 2026 each unique visitor identified within a 30-day window consumes one credit no matter how many times they return; credits don't roll over. Going over the monthly limit incurs overage (~$0.45/resolution on Starter, ~$0.25 on Pro/Pro+).
