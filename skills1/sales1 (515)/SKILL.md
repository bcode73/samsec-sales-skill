...
- **Standard SEO/Projects API is a Business add-on.** The core SEO + Projects API is "available as an add-on to a Business subscription" ($499.95/mo) and you buy API units separately. But not every API is gated this way: the **Listing Management API** is available on Local Pro + Business, and the **Map Rank Tracker API** is free for all Semrush users (no API units, no special subscription). The Zapier Site Audit connector also works on all plans without API keys — use it for basic automations.
