...
- **Auth**: API key as `key` query param (found in Subscription info > API units); OAuth 2.0 also supported for Projects + Map Rank Tracker; Listing Management uses an `Authorization: Apikey` header
- **Pricing**: Unit-based — each request/line consumes API units, costs vary by endpoint (per **line returned**, not flat per request)
- **Access** (verified 2026-06-13): Standard **SEO + Projects API** is "available as an add-on to a Business subscription." Other API products have different access: **Listing Management API** = Local Pro + Business; **Map Rank Tracker API** = all users, no API units; **Trends API** = separate Basic/Premium tiers (own 10,000 req/mo limit)
- **Types**: SEO/Analytics API (domain/keyword/backlink reports), Projects API (position tracking, site audit), Trends API, Local API (Listing Management + Map Rank Tracker)
- **Response format**: SEO/Analytics API returns CSV with **semicolon** separators; newer v4 REST endpoints (`apis/v4/keywords/v1/metrics`, `apis/v4/backlinks/v1/*`) support JSON + CSV
- **MCP Server**: Official app in Claude, Claude Code, ChatGPT, Cursor, VS Code, Perplexity, Gemini; available across Semrush One, SEO, and Trends API plans; consumes API units
