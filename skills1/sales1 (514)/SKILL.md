...
1. **For automation/webhook questions** — Sembly uses outbound automations (HTTPS POST to your endpoint). Configure in My Automations → Custom tab. Supports transcripts, meeting notes, and tasks. Filter by all meetings, title keywords, conversation type, meeting owner, or team. Use the Test button to confirm your endpoint parses the payload. Zapier triggers: New Meeting Notes, New Tasks, New Transcription.
...
3. **For AI features** — Semblian AI chat (multi-meeting on Pro+) uses AI credits. AI-generated documents (proposals, briefs, decks) capped at 5/mo per user on Pro, 40/mo per user on MAX (AI insights same). MCP access requires Pro+ and a workspace admin to enable Sembly MCP.
...
- **MCP access requires Pro plan + admin enable.** Basic cannot use MCP. On Pro+, Sembly MCP is **disabled by default** in workspace settings — a workspace admin must enable it first. The MCP token grants read-only access to all your meetings.
- **No public REST/GraphQL API.** Sembly's integration model is webhook-first (outbound automations) for push. For programmatic *read* access, use the **Sembly MCP** server (Pro+, admin-enabled in workspace settings, token-based, read-only) — supported by Claude, Cursor, and n8n. No dedicated n8n node exists; n8n connects via MCP or generic HTTP/Webhook nodes.
