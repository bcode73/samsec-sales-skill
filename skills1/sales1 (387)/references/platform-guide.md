...
- **SMS credits**: As of **May 4, 2026** SMS is a volume-based add-on (from ~$0.007/SMS) on both Standard and Pro — Pro **no longer** bundles SMS credits equal to the subscription price (legacy benefit retired). Free plan includes only 60 trial SMS. Per-message cost varies by destination country. (See Pricing section.)
...
- **Free plan**: capped at **500 web push notifications/mo** (re-verified 2026-06-13). **Unlimited** web push is on paid plans (Standard, Pro)
...
**Quick reference** (re-verified 2026-06-13): Base URL `https://api.omnisend.com` (resource paths under `/api/`), API key auth via `Authorization: Omnisend-API-Key YOUR-API-KEY` (the legacy `X-API-KEY` header is no longer documented), header-based versioning (`Omnisend-Version: 2026-03-15`), cursor-based pagination (`paging.cursors` + `paging.hasMore`). Per-endpoint rate limits: 100/min lists, 15/min writes, 10/min + 55/day analytics. Webhooks are outbound automation action blocks (paid plans). An **MCP Server** is available at `https://mcp.omnisend.com/v2/mcp`. Key capabilities: contact management (POST is upsert; no individual delete), campaign CRUD, product sync, event tracking, segment CRUD, analytics.
...
## Pricing (re-verified 2026-06-13 against the official pricing page + Help Center "Pricing Plans 2026")
- **Free** — $0/mo: 250 contacts, 500 emails/mo, **500 web push notifications** (not unlimited), all core features except advanced reporting, 60 trial SMS
- **Standard** — from $16/mo: 500 contacts (scales by billable contact count), ~6,000 emails/mo (roughly contacts × 12 per cycle), unlimited web push, SMS as a **paid add-on**
- **Pro** — from $59/mo: 2,500 contacts (scales), **unlimited emails**, unlimited web push, advanced reporting, SMS as a paid add-on
- Scales by contact count. 30% off the first 3 months when paying upfront on the first subscription (e.g. Standard ~$11.20/mo, Pro ~$41.30/mo during the intro period).

### SMS pricing changed 4 May 2026 (IMPORTANT)
As of **May 4, 2026**, Omnisend moved to a **volume-based pay-as-you-go SMS model** starting at **~$0.007/SMS** (tiered, e.g. $0.009 → $0.007).
- **Pro no longer bundles SMS credits equal to the subscription price** — that legacy "credits = your bill" benefit ended. Pro and Standard both now buy SMS as a usage-priced add-on.
- Legacy/grandfathered accounts created before May 4, 2026 may still see the old add-on/credit model. Always confirm the user's account against the live pricing page.
