...
- **Don't assume SMS is included on all plans.** The Free plan has only 60 trial SMS. As of **May 4, 2026** SMS is a volume-based add-on (from ~$0.007/SMS) on both Standard and Pro — Pro **no longer** bundles SMS credits equal to the subscription price (that legacy benefit was retired). Per-message cost is destination-country-dependent. Legacy/grandfathered accounts may still see the old model. Always check the user's plan and SMS billing before recommending SMS workflows.
...
3. Covers authentication (`Authorization: Omnisend-API-Key YOUR-API-KEY` header) and versioning (`Omnisend-Version: 2026-03-15` header)
