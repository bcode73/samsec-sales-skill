...
- **`manuscript/`** holds everything that is part of the continuous, linear read: front matter, the four parts, appendices, and references. This keeps the reading order visible directly from the file tree and lets the whole book be concatenated for export in one pass.
...
4. The Humanizer Pass (`BOOK_BIBLE.md` §13): reviewed against the root-level `SKILL (1).md` skill so the prose reads as a person wrote it, not a model. No em dashes, no en dashes, no AI-vocabulary clusters, no inflated significance, no promotional language.
